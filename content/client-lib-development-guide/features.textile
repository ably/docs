...
** @(RTC4a)@ Unlike the stateless REST client library, the @Auth#clientId@ is populated when the connection is established.  The @CONNECTED@ @ProtocolMessage@ contains the confirmed @clientId@ for this connected client. See "@RSA7b@":#RSA7b and "@RSA12@":#RSA12 for futher info
...
**** @(RTL6g1b)@ Ably will not assign a @clientId@ upon receiving the @Message@. A test should assert that the @clientId@ value is @null@ for the @Message@ when received
...
** @(RTP8f)@ Raises an exception if the client library is not authenticated (i.e. assigned a wildcard @'*'@ or using Basic Auth without a @clientId@)
