...
The REST client library is available in "most popular languages":https://ably.io/download/rest-client-libraries with a consistent API between languages.  You can view this documentation in each supported language using the language selector navigation.
...
The Rest library may be instanced multiple times with different @options@ in any given context. Except where specified otherwise, instances operate independently of one another.
...
- log := Parameters to control the log output of the library. The supplied value must be an object that may contain one or both of the following entries: @level@: a number controlling the verbosity of the output from 1 (minimum, errors only) to 4
