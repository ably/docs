...
CAVEAT-INPUT
  ...
  Clients must be "identified":/auth/identified-clients by having a @ClientId@ in order to be present on a channel. They also require the @presence@ "capability":/auth/capabilities to enter the presence set, and the @subscribe@ capability to subscribe to presence events.
...
The number of clients that can be simultaneously present on a channel is "limited":/general/limits. This ensures the rate of presence messages remains supportable, as it is common for all members on a channel to change state at a similar time.
...
Messages are streamed to clients as soon as they "attach":/channels#attach to a channel, as long as they have the "subscribe capability":/auth/capabilities.
...
The REST interface of an SDK enables you to retrieve the presence state of multiple channels in a single request. The presence state contains details about the clients in the presence set, such as their @clientId@, member data and "presence action":/api/realtime-sdk/types#presence-action.
