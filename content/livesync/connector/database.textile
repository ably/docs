...
The Ably Database Connector requires two tables to exist in your database, an "outbox table":#outbox and a "nodes table":#nodes.

h2(#create). Create tables

The required tables can be created using the "CLI":#cli or created "automatically":#auto on startup of the Database Connector.

h3(#cli). Using the CLI

Use the @create tables@ command to automatically create the required tables in your database. This requires the application to use a PostgreSQL user that has been granted @CREATE@ permissions on the desired schemas.
...
To apply migrations to your database, use the following DDL:
...
* Channels provide a mechanism for data access control via channel "capabilities":/auth/capabilities.
...
| @mutation_id@ | String | Yes | @680f3f78-21ec-49a0-be99-25f89a84f232@ | TThe ID of the mutation, used for correlating the outbox event with an optimistic event applied locally on the client. |
