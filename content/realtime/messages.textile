...
Message Interactions are currently only supported by the "Ably JavaScript SDK":https://github.com/ably/ably-js.
...
blang[javascript].
  ...
  * @timeSerial@ (String) - the timeSerial of the message being referenced.
  ...
  6. Select the *Message references enabled* checkbox then click *Create channel rule*.

  <a href="/images/realtime/dmessage-interactions.png" target="_blank">
  ...
  *Note*: for @type@ constants Ably has reserved strings beginning: @com.ably.@.
  ...
  For example, if you wanted to send an emoji reaction to the previous message above you could send something like:

  bc[jsall]. const sendReaction = (emoji) => {
  ...
  * @isRef@ (Boolean) - filter for messages that only do or do not reference another message.
  ...
  h3(#interactions-unsubscribe). Unsubscribing to Interactions
  ...
  *Note*: Unsubscribing with a filter requires the *exact* same filter object to be passed in.
