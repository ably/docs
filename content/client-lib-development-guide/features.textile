...
** @(RSA8a)@ Method signature is @revokeTokens(TokenRevocationRequest)@.
** @(RSA18b)@ If the @TokenRevocationRequest#key@ attribute is not provided, then the function must throw an error with a @"Key is not provided"@ message.
** @(RSA18c)@ If the @TokenRevocationRequest#targets@ attribute is null or invalid (as per @TF3@ ), then the function must throw an error with a @"targets are invalid"@ message.
** @(RSA18d)@ If the @TokenRevocationRequest#key@ is provided and @TokenRevocationRequest#targets@ valid, then the function must create the request and execute it.
** @(RSA18e)@ Function returns a @TokenRevocationResponse@ object that contains the result of the request if request is successful.
** @(RSA18f)@ Function throws an appropriate error/exception  if the request fails either for the reasons of invalid request, or as  a result of request failing.
** @(RSA18g)@ Tests in this section must exist in client library.
*** @(RSA18g1)@ Given a null @TokenRevocationRequest#key@ attribute, verify that the function throws an error with a @"Key is not provided"@ message.
*** @(RSA18g2)@ Given a null @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets are invalid"@ message.
*** @(RSA18g3)@ Given an empty @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets are invalid"@ message.
*** @(RSA18g4)@ Given a non-null @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function creates the request and executes it.
*** @(RSA18g5)@ Given a non-null but invalid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that client library throws an API error.
*** @(RSA18g6)@ Given a valid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function executes the request and returns a @TokenRevocationResponse@ with targets matching with the original targets in request.
*** @(RSA18g7)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the future for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
*** @(RSA18g8)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the past > 1 hour for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
