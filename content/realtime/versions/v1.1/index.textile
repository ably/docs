...
The Realtime library, when instanced, establishes a "WebSocket":/concepts/websockets connection to Ably. This "WebSocket":/concepts/websockets connection is both multiplexed, allowing multiple channels to share a single connection, and bi-directional, allowing realtime messages to be sent both to and from the Ably service without having to fall back to inefficient techniques like "HTTP polling":/concepts/long-polling.
