...
When you update the data in your database, you record a change event transactionally in a dedicated "outbox table":/livesync/connector/database#outbox. The Database Connector detects new changes in the outbox and publishes them as messages on Ably Channels. Using the Models SDK, your client applications subscribe to these change events and update their local state with the new data.

The Database Connector is available as a "Docker":/https://www.docker.com/ container image that can be deployed on any cloud infrastructure platform that supports running Docker container images.
...
<p>Please "reach out":https://docs.google.com/forms/d/e/1FAIpQLSd00n1uxgXWPGvMjKwMVL1UDhFKMeh3bSrP52j9AfXifoU-Pg/viewform if you would rather use an Ably-hosted version.</p>
</aside>

h2(#docker). Deploy using Docker Compose

Use "Docker Compose":https://docs.docker.com/compose/ to run the Database Connector locally for development purposes. This will create and run a PostgreSQL database and an instance of the Database Connector on your local machine.
...
There are three "options":#options to configure the Database Connector. The following examples will use environment variables.
...
Open a terminal and subscribe to an Ably "channel":/channels named @foo@ using "Server-Sent Events":/protocols/sse
...
The Database Connector will detect and publish the newly inserted record as a message to the Ably channel specified in the record. In the example the channel was named @foo@. You will receive a response similar to the following over your SSE connection to Ably, indicating that you have successfully received the event:
...
"Railway":https://railway.app/ is an infrastructure platform that you can use to deploy the Database Connector to in order to test its functionality in the cloud.
...
Railway requires a Dockerfile that defines the application you want to deploy. Use the following command to create a new file called Dockerfile which creates a new image using the Database Connector as the base:

```[sh]
echo "FROM ghcr.io/ably-labs/adbc:latest" > Dockerfile
```

Deploy your application using @railway up@. This process will build the Database Connector container image from the Dockerfile located in the project root and deploy it to your Railway project:
...
After completing the initial setup the Database Connector instance has been created in your Railway project, however it will crash on startup. This is because it needs to be configured.
...
<aside data-type='note'>
<p>At least an Ably API key and the connection details of your database must be supplied to correctly configure the connector.</p>
</aside>
...
| @ADBC_POSTGRES_PASSWORD@ | your Postgres database user password as an alternative to providing the @CONNECTION_URI@. |
...
The following is an example of setting your Ably API key using an environment variable:
...
The following is an example of setting your Ably API key using a YAML file:
...
The following is an example of setting your Ably API key using command line flags:
