...
Using configurable rules, you can stream various "payload types":#payloads, including messages, presence events, occupancy, and channel lifecycle events. 

Unlike our "channels":/realtime/channels, which follow a "pub/sub pattern":https://en.wikipedia.org/wiki/Publish%E2%80%93subscribe_pattern, where each message is delivered to any number of subscribers, Firehose operates such that each message is delivered once to your streaming or queueing server.
...
Find out why we think streams and message queues help solve many of the challenges associated with consuming pub/sub data server-side in our article: "Message queues — the right way to process and work with realtime data on your servers":https://ably.com/blog/message-queues-the-right-way.

Please note that if you want to consume realtime data from a queue, you should take a look at "Ably Queues":/general/queues. They provide a simple and robust way to consume realtime from your worker servers without having to worry about queueing infrastructure.
...
Pulsar and Kafka rules are currently in Preview development status.
...
2. AWS SQS
3. AMQP
4. "Apache Kafka":/general/firehose/kafka-rule
5. Apache Pulsar

h2(#aws-authentication). AWS Authentication

For details of authenticating AWS services with Ably, see our "AWS authentication":/general/aws-authentication/ documentation.
