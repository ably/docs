...
| Property | Description | Type |
| --- | --- | --- |
| unique | Reactions counted with the "unique" strategy (one per client per message). | `Map<string, number>` |
| distinct | Reactions counted with the "distinct" strategy (one of each type per client). | `Map<string, number>` |
| multiple | Reactions counted with the "multiple" strategy (unlimited per client). | `Map<string, number>` |
...
room.messages.subscribe((event) => {
  ...
  const distinctReactions = message.reactions.distinct;
  distinctReactions.forEach((count, type) => {
