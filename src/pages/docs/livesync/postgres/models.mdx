...
When using the event buffer, change events are de-duplicated and ordered according to their [`message.id`](/docs/pub-sub/api/javascript/realtime/message#properties), which corresponds to the change event's [`sequenceId`](#sequence).
