...
If occupancy is returned as part of a "REST request":/metadata-stats/metadata/rest, then @metrics@ are within a "@ChannelDetails@":/docs/api/realtime-sdk/channel-metadata#channel-details object. The following is an example of a @ChannelDetails@ object:
...
"Integration rules":/docs/general/integrations can be configured with @channel.occupancy@ set as a rule source. Occupancy events that occur on the selected channels will be pushed to external targets, such as AWS Lambda, or custom webhook endpoints.