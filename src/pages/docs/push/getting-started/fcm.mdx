...
Create a new Android project in Android Studio with an **Empty Views Activity** template using Kotlin, and set the minimum SDK to API 21 (Android 5.0) or higher.
...
object AblyHelper {
    ...
    fun getInstance(): AblyRealtime {
        return instance ?: synchronized(this) {
            ...
            AblyRealtime(options).also { instance = it }
        }
    }
}
```
...
fun subscribeToChannel(channelName: String) {
    ...
    channel.push.subscribeDevice(object : CompletionListener {
...
fun unsubscribeFromChannel(channelName: String) {
    val channel = realtime.channels.get(channelName)
    channel.push.unsubscribeDevice(object : CompletionListener {
        override fun onSuccess() {
...
class MainActivity : AppCompatActivity() {
    ...
    fun subscribeToChannel(channelName: String) {
        realtime.channels.get(channelName).push.subscribeDevice(object : CompletionListener {
            override fun onSuccess() {
                updateStatus("Subscribed to push on channel: $channelName")
            }
            override fun onError(reason: ErrorInfo?) {
    ...
    fun unsubscribeFromChannel(channelName: String) {
        realtime.channels.get(channelName).push.unsubscribeDevice(object : CompletionListener {
            override fun onSuccess() {
                updateStatus("Unsubscribed from push on channel: $channelName")
            }
            override fun onError(reason: ErrorInfo?) {
...
fun sendPushToDevice() {
    ...
    realtime.push.admin.publish(recipient, payload, object : CompletionListener {
        override fun onSuccess() {
...
Sending push directly to a `deviceId` or `clientId` requires the `push-admin` capability on your API key. In production, use token authentication and send pushes from your backend server rather than embedding admin credentials in your app.
</Aside>
