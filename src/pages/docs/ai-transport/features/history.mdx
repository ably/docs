...
The client transport loads history using `view.loadOlder()` with `untilAttach` for gapless continuity. This ensures no gap between historical messages and live messages arriving through the realtime subscription.
