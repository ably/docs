...
```[rest_go]
  client, err := ably.NewREST(ably.AuthWithURL("/auth"))
```
...
async def get_token_request(*args, **kwargs):
    tokenRequestResponse = await createTokenRequest() // Make a network request to your server

    if tokenRequestResponse.status_code == 200:
        token_request = tokenRequestResponse.json()

        return token_request
    else:
        raise Exception(f"Failed to retrieve token request: {tokenRequestResponse.status_code} - {tokenRequestResponse.text}")
```
...
```[rest_python]
realtime = AblyRest(auth_url='/auth',
                    auth_method="GET',
                    auth_headers={'h1': 'v1'},
                    auth_params={'param1': 'param2'})
```
