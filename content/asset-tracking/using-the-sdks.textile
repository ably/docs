...
jump_to:
  Help with:
    ...
    - Using the Publisher SDK#publisher-sdk
    - Using the Subscriber SDK#subscriber-sdk
    - See also#see-also
---
...
You can set these values in your @~/.gradle/gradle.properties@ file.
...
h2(#publisher-sdk). Using the publisher SDK

Common operations you will need to carry out on the publisher include:
...
- context := Called to provide the Android runtime context (on Android only).
...
You can start tracking an asset by calling the @track@ method of the publisher. You need to supply the tracking identifier of the asset to be tracked, and any "resolution constraints":/asset-tracking#resolution-constraints that need to be applied.
