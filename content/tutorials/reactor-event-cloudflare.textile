...
excerpt: Learn how to use the Cloudflare Workers and Integrations by building
  a 2D game in React
group: sdk
index: 103
languages:
...
The "Ably Integrations":https://ably.com/integrations provide a way to trigger events and to stream data from Ably's pub/sub channels. This tutorial will go over how to use Ably Integrations with "Cloudflare workers":https://developers.cloudflare.com/workers/quickstart/.
...
We'll be building a postal service delivery game using React and "Functions":/general/events. Players use their arrow keys to drive a delivery van around a map to each of the postboxes that appear. Integration events will be triggered every time the player 'delivers' a parcel to a postbox, this will call a Cloudflare worker which will notify a separate 'admin page' that the delivery has been made.
...
Your event will now be created, and you'll be able to continue developing the game.
