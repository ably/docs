...
<Aside data-type="note">
Namespaces are not required to refer to a group of channels within a [capability](/docs/platform/auth/capabilities). A resource specifier, such as `foo:*`, a glob expression, will match a channel named `foo:bar`, even without a `foo` namespace.
</Aside>
