...
# Subscribe one or more devices to one or more channels. You can either subscribe "by device ID":https://ably.com/documentation/general/push/activate-subscribe#subscribing-device-id to subscribe one device, or subscribe "by client ID":https://ably.com/documentation/general/push/activate-subscribe#subscribing-client-id to subscribe all devices with that client ID.
...
* Devices identified by their native recipient attributes such as their unique @registrationToken@ in the case of FCM, @deviceToken@ in the case of APNS, or @targetUrl@ and @encryptionKey@ in the case of a Web device (*experimental*). This is particularly useful when migrating to Ably with existing push notification target devices.
