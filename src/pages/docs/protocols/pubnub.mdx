...
PubNub uses different API keys for publishing and subscribing. Ably does not use different keys and instead manages access through API key and token [capabilities](/docs/platform/auth/capabilities). The adapter will use the value passed to `subscribe_key` to create a connection to Ably. If that API key doesn't have publish capabilities, the client won't be able to publish.
</Aside>
