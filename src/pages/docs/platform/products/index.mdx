...
2. **Which features**: [Rules](/docs/channels#rules) for per-namespace configuration, [push notifications](/docs/push) for offline and native push delivery
...
[Pub/Sub](/docs/basics) is Ably's core product. It provides flexible, low-level APIs for publishing and subscribing to messages on channels, giving you full control over your message architecture.
...
Pub/Sub gives you direct access to Ably's underlying messaging primitives, including [channels](/docs/channels), [presence](/docs/presence), and [history](/docs/storage-history/history), enabling you to design your own message architecture.
...
Pub/Sub channels, without LiveObjects, can also be used as a durable store of channel state, and for client hydration, but in this case the state is simply the content of messages sent on the channel, retrieved using features such as [history](/docs/storage-history/history) and [rewind](/docs/channels/options/rewind).
...
* Get started with [Chat](/docs/chat/setup), [Spaces](/docs/spaces/setup), [LiveObjects](/docs/liveobjects), [LiveSync](/docs/livesync), or [AI Transport](/docs/ai-transport).
* Explore [examples](https://ably.com/examples) to see working implementations of common use cases.
* Understand [authentication](/docs/platform/auth) before going to production.
