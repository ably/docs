...
Now we'll implement the @enterLobby@ function. It has one parameter, @onGamePending@, which is a callback function provided in the @lobby.vue@ file that we implemented earlier. See the "Ably documentation on Presence":https://ably.com/docs/realtime/presence for full details on how presence works. In the code below, we subscribe to the lobby's @presence@ channel. The documentation for the @presence.subscribe()@ method "explains this in more detail":https://ably.com/docs/realtime/presence#subscribe-listener.
...
export function enterGame(gameId, onGameStateUpdated) {
  ...
  gameChannel.history({ limit: 1000 }, (error, page) => {
    ...
    // See https://ably.com/docs/storage-history/history#paginated-result for details.
