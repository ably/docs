...
```java
implementation 'io.ably:ably-java:1.7.3'
runtimeOnly 'io.ably:liveobjects:1.7.3'
```

For `Android` platform, use `ably-android` instead of `ably-java`

```java
implementation 'io.ably:ably-android:1.7.3'
runtimeOnly 'io.ably:liveobjects:1.7.3'
```
...
<Aside data-type='important'>
`.join()` blocks the calling thread until the future completes. It is fine for scripts, tests, and learning, but blocking a UI thread (Android's main thread) causes an [ANR](https://developer.android.com/topic/performance/vitals/anr) and blocking inside an Ably event listener callback can deadlock; in production code, compose the future with `thenApply`/`thenCompose`/`thenAccept`/`exceptionally` as shown above.
</Aside>
...
myObject.set("reactions", LiveMapValue.of(reactions)).join();
