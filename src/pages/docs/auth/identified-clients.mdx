...
You can use [token authentication](/docs/auth/token) to set an explicit `clientId` when creating or issuing an Ably Token. Clients using that token are restricted to operations for only that `clientId`, and all operations will implicitly contain that `clientId`.
...
See [JWT authentication scenarios](/docs/auth/jwt#scenario-standard) for complete server and client examples.
