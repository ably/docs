...
- When a channel is no longer needed, call <If lang="javascript,nodejs">[`detach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel)</If><If lang="go,ruby">[`detach()`](/docs/api/realtime-sdk/channels)</If> to avoid unnecessary outbound messages and channel minutes usage.
- Always call <If lang="javascript,nodejs">[`close()`](/docs/pub-sub/api/javascript/realtime/connection)</If><If lang="go,ruby">[`close()`](/docs/api/realtime-sdk/connection)</If> on connections when finished to avoid billing for the [2-minute connection timeout](/docs/connect/states#connection-state-recovery).
