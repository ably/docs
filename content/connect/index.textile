...
The Ably Realtime SDK establishes and maintains a connection to the Ably service, using the most efficient transport available, typically "WebSockets":https://ably.com/topic/websockets. The Ably realtime protocol operates and multiplexes all channel traffic over that connection. "Channels":/channels are separate from connections and are used to logically separate messages into different topics.

CAVEAT-INPUT

  This section applies to the realtime interface of the Ably SDK, see the "how to use ably section":/basics/use-ably for more information.

h2(#getting-started). Create a connection
...
This example relies on the default auto-connect behavior of the SDK, checking for when the connection state is "@connected@":/connect/states#connection-states event:
...
CAVEAT-INPUT
  ...
  Browser security models ensure that each tab is effectively sandboxed from every other so it is not possible to share connections between tabs. As such, if a user has three tabs open, each with a connection to Ably, that will count as three connections.

  The Ably dashboard contains a developer console. In the developer console you can view connection events. See the "developer console section":/account/LINK-ME for more information.
...
Metachannels are a namespace of channels beginning with the [meta] qualifier, separating them from regular channels. For connections there is a specific @[meta]connection.lifecycle@ channel that publishes messages about the lifecycle of realtime connections.

Learn more about "metachannels here":/LINK-ME.
...
You can use "@ping@":/api/realtime-sdk/connection#ping to get a measure of the true round-trip latency to the connected Ably server.
...
A connection to Ably can be closed once it is no longer needed to keep the number of your peak connections down. There is a 2 minute delay if a connection is not explicitly closed which needs to be considered to make the most of your "account limits":/account/LINK-ME.

You can read more about the `close()` method in the "API Reference":/api/realtime-sdk/connection#close. The following code sample explicitly closes the connection to Ably and prints the message @Closed the connection to Ably@:
