...
* **Persisted messages** - If enabled, all messages within this namespace will be persisted. You can access stored messages via the History API. The number of hours a message is stored is configurable, "find out more about message persistence":https://knowledge.ably.com/how-long-are-messages-stored-for
* **Require authentication** - if enabled, only "authorized clients":/realtime/authentication#authorize with a clientId will be permitted to subscribe to matching channels. Anonymous clients will not be permitted to join the channel.
