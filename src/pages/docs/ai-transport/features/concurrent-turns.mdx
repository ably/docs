...
<Code>
```javascript
const run1 = await view.sendMessage({
  id: crypto.randomUUID(),
  role: 'user', parts: [{ type: 'text', text: 'Summarize the report' }],
});
const run2 = await view.sendMessage({
  id: crypto.randomUUID(),
  role: 'user', parts: [{ type: 'text', text: 'What are the key risks?' }],
});
...
<Code>
```javascript
await session.cancel(run1.runId);
...
Each `ActiveRun` exposes `started: Promise<void>` (resolves when the agent's `ai-run-start` is observed). To wait for a Run to terminate, subscribe to the session tree's `run` events and resolve on a matching `end`:
...
const run1 = await view.sendMessage({ id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text: 'Hello' }] });
await whenRunEnds(session, run1.runId);
```
</Code>
...
const newRun = await view.sendMessage({
  id: crypto.randomUUID(),
  role: 'user', parts: [{ type: 'text', text: 'Actually, focus on the budget instead' }],
});
```
</Code>
...
<Code>
```javascript
const runA = await viewA.sendMessage({
  id: crypto.randomUUID(),
  role: 'user', parts: [{ type: 'text', text: 'What does section 3 mean?' }],
});

const runB = await viewB.sendMessage({
  id: crypto.randomUUID(),
  role: 'user', parts: [{ type: 'text', text: 'Summarize section 5' }],
});
```
</Code>
...
The session tracks Run identity internally. Hold onto the `ActiveRun` returned by `view.sendMessage()` only if you need to cancel it specifically or wait for it specifically.
