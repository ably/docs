...
2. Choose **Webhook**.
3. Choose **AWS Lambda**.
4. Configure the AWS Lambda [settings](#settings).
5. Click **Create**.

</Tab>
<Tab value="control-api" label="Control API">

Create an AWS Lambda rule using the Control API by sending a `POST` request to [`/apps/{app_id}/rules`](/docs/api/control-api):
