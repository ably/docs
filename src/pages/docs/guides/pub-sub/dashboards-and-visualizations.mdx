...
The key architectural decision is understanding which category your dashboard falls into. With Ably, you are not limited by technology only by the user experience you want to deliver. Realtime dashboards and charts typically fall into two categories, each with distinct requirements:

### Fan engagement: Streaming to millions

Fan engagement dashboards and charts prioritize broadcasting data to massive audiences where the experience is shared. Common examples include:
...
* Eventual consistency is acceptable and intermediate values can be discarded to reduce outbound messages
...
Critical monitoring dashboards and charts prioritize guaranteed delivery, data integrity, and low latency for operational decisions where lives or significant assets may depend on the data. Common examples include:
...
This pattern enables bandwidth optimization because you don't stream detailed telemetry for every vehicle until the user actually needs it. You can also apply different update frequencies to different levels of the hierarchy, because overview data might update every 5 seconds while detailed vehicle data updates every 100ms.
...
For state-based dashboards using delta compression, enabling [message persistence](/docs/storage-history/storage) allows clients to retrieve message history when they connect or reconnect. This is particularly valuable for dashboards where users need to see recent updates or catch up on missed data.

[Persistence](/docs/storage-history/storage#persistence) stores all messages published to a channel for 24 hours by default, and can be extended up to 30 days. New clients can attach with `rewind=1` to immediately receive the most recent message, or query the full history within the retention window.

Longer retention periods are possible for some packages, please [contact](https://ably.com/support) customer support to discuss your requirements)
...
For scenarios requiring retention beyond 30 days but where only the final state matters, consider [persist last message](/docs/storage-history/storage#persist-last-message) instead. This stores only the most recent message for 365 days, which can be useful for accessing final statistics after an event concludes (for example, viewing match results days or weeks later).

Enable persistence via [channel rules](/docs/channels#rules) in your Ably dashboard. For state-based dashboards, publish complete state snapshots as single messages rather than partial updates to ensure clients can reconstruct the full state from a single persisted message.
...
* You can also enable server-side batching on presence events via [rules](/docs/channels#rules) to reduce outbound message counts during high churn periods, helping to keep costs manageable. Server-side batching also enables much higher effective presence event rate limits, beyond the default 50 messages/second.
