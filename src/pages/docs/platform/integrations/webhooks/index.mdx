...
Multiple requests can be in-flight at once, however be aware there is a [limit on concurrency](/docs/platform/pricing/limits#integration). If it is exceeded then new messages are placed in a short 10 message queue. If that is exceeded then further messages are rejected.
...
<Aside data-type="note">
Batched requests support [request signing](#security) for enhanced security, enabling you to verify webhook requests from Ably.
</Aside>
...
The Ably SDK automatically decodes messages sent over the Realtime service into [`message`](/docs/pub-sub/api/javascript/realtime/message) objects. However, batched, enveloped webhook payloads require explicit decoding using:

* [`Message.fromEncoded`](/docs/pub-sub/api/javascript/realtime/message#from-encoded) for an array of messages.
* [`Message`](/docs/pub-sub/api/javascript/realtime/message) for a single message.
...
Presence messages sent [over the realtime service](/docs/channels) are automatically decoded into [`PresenceMessage`](/docs/pub-sub/api/javascript/realtime/presence-message) objects by the Ably client library. For webhooks, you need to do this manually using [`PresenceMessage.fromEncodedArray`](/docs/pub-sub/api/javascript/realtime/presence-message#from-encoded-array) on the `data.presence` array, or [`PresenceMessage.fromEncoded`](/docs/pub-sub/api/javascript/realtime/presence-message#from-encoded) on an individual entry. These methods convert the encoded values into [`PresenceMessage`](/docs/pub-sub/api/javascript/realtime/presence-message) objects—either as an array or a single instance.

This allows you to decode the numerical action into a [`Presence action`](/docs/pub-sub/api/javascript/realtime/presence-message) string (such as `enter`, `update`, or `leave`), fully decode the `data` (using the `encoding`) back into its original datatype or an equivalent in the client library, and, if you're using [encryption](/docs/channels/options/encryption), pass your encryption key to decrypt the `data`.
...
| `data.status` | A [`ChannelStatus`](/docs/pub-sub/api/javascript/realtime/channel-details#ChannelDetails) object that describes the channel's current state. |
...
The payload will contain the [data](/docs/pub-sub/api/javascript/realtime/presence-message) of the `Presence` message.

The following example demonstrates a non-enveloped [enter](/docs/pub-sub/api/javascript/realtime/realtime-presence#enter) presence event:
