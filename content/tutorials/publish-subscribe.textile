...
blang[javascript].
  ...
  #_ Add your "Ably API key":https://faqs.ably.com/setting-up-and-managing-api-keys to @example.html@ by replacing @YOUR_ABLY_API_KEY@ with your own API key.
  # In a new browser tab, open your browser's developer tools.
  # Open the @example.html@ file in the browser tab and inspect the console output.
...
blang[javascript].
  First, create a blank HTML page called @example.html@, to host your web app:
  ...
  To start using Ably in your web app, you first need to include the Ably client library. In this example, we recommend that you include the latest client library from our CDN using a simple @<script>@ tag. The client library must be instantiated with the API key you copied in Step 1.
...
blang[javascript].
  First, create a variable to store a reference to the channel:
  ...
  Then, ensure that you are connected to the Ably platform, by monitoring the @connected@ event:
  ...
  Within the @connected@ event handler, use @realtime.channels.get@ to get a reference to the "sport" channel and then subscribe to the channel:
  ...
  This client now receives any messages sent to the "sport" channel and log them to the browser console.
...
blang[javascript].
  Add the call to @channel.publish@ to the last line of code within the @connected@ event handler:
  ...
  Then, refresh your browser tab and watch the message payload being displayed in the browser console.
