...
Clients must be [identified](/docs/platform/auth/identified-clients) by having a `ClientId` in order to be present on a channel. They also require the `presence` [capability](/docs/platform/auth/capabilities) to enter the presence set, and the `subscribe` capability to subscribe to presence events.
</Aside>
