...
To generate a P12 certificate, you'd need an AppID from your paid "Apple developer account":https://developer.apple.com. You can use one of your existing AppIDs, if any, otherwise just follow along to see how to create one from scratch.
...
You should now see that your credentials are automatically added into the fields in that section, as shown below. Just try refreshing the page in case you don't see it.
...
Next, we'll add a function to initialize Ably's Realtime library via Basic auth strategy. Below the variables we just declared inside the @AppDelegate@ class, go ahead and add the following function to initialize the Ably Realtime library or get an existing instance if it was already initialized:
...
If this registration is successful, APNs would return a unique @deviceToken@ for each device. We'll use that @deviceToken@ received to share with Ably. 
...
Publishing push notifications direclty using the clientId as the identifier can be done as follows:
...
Publishing push notifications direclty using the deviceToken as the identifier can be done as follows:
