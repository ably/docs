...
const sidebarDataFromPageFurniture = (data) =>
  data.map(({ label, link, level = ROOT_LEVEL, contentString = false, contentArray = false }) => {
    const result = {
      label,
      link,
      level,
...
const sidebarDataFromDocumentPaths = (data) => {
  const objects = data.map(({ node: { id, label, level, link, parent = { id: false } } }) => ({
    id,
    label,
    level,
    link,
  ...
  range(ROOT_LEVEL, MAX_LEVEL).forEach((currentLevel) => {
    objectsByLevel[currentLevel] &&
      objectsByLevel[currentLevel].map((object) => {
        object.content = objectsByLevel[currentLevel + 1]
          ? [{ ...object, label: 'Overview' }].concat(
              objectsByLevel[currentLevel + 1].filter(({ parent: { id } }) => id === object.id),
            )
          : null;
        return object;
      });
