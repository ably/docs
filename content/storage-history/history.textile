...
History is available for both realtime and REST interfaces, enabling you to retrieve previously sent messages and presence event history for channels. By default two minutes of message history is available to retrieve, but this can be extended by configuring the "storage options":/message-storage/storage for an application. It is also possible to persist messages to external storage services using "integrations":/integrations.

h2(#history-rewind). History vs rewind

You can retrieve previously published messages using the history feature or using the "rewind channel option":/channels/options/rewind. There are several differences between the two features that are important to be aware of when deciding how to retrieve older messages:

* The number of messages that are returned varies between history and rewind. History returns up to 1000 messages. Rewind returns up to 100 messages.
* History can be called using either the realtime or REST interface of an SDK. Rewind can only be used with the realtime interface of an SDK.
* You can define a custom start or end time period for history. Rewind can return a specified number of messages, or a specified number of seconds/minutes, that immediately precede the moment of attachment.
* History can be called repeatedly to return additional messages or timeframes. Rewind can be called on channel attachment.
* Only history can return presence events. Rewind cannot return presence events.
* If "persist last message is enabled":/storage-history/storage#persist-last-message then rewind can also return the last event published to a channel persisted for 365 days.

There are also some similarities between the two:

* If using the realtime interface you can use both history or rewind with @untilAttach@ to return "continuous history":#continuous-history.
* Both history and rewind are limited by the "persistence":/storage-history/storage settings enabled for a channel.
...
The following example retrieves the latest message sent in a channel:
...
<span lang="ruby">Note that all examples on this page assume you are running them within an EventMachine reactor. Find out more in our "realtime usage documentation":/LINK-ME.</span>
...
Retrieve "presence":/presence-occupancy/presence history using the "@history()@":/api/realtime-sdk/presence#history method. This enables a client to retrieve historical presence events from the channel.
...
h3(#presence-parameters). Presence parameters
