...
API keys and tokens have a set of capabilities assigned to them that specify which operations, such as @subscribe@ or @publish@ can be performed on which resources. To use the Spaces SDK, the API key requires the following "capabilities":/docs/auth/capabilities#58:
...
<aside data-type='note'>
If you are using the CDN version of the Ably client, replace the call to `new Realtime.Promise` in the next section with `new Ably.Realtime.Promise`.
</aside>
