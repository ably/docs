...
Service worker is a key part of the push notifications workflow for web browsers. It's a JavaScript file that runs in the background and can receive push notifications even when the page is not open.
...
Back in your `index.html`, add a listener to receive messages from the service worker. This lets you update the page UI when a push notification is received or clicked. You can add it anywhere in the `<script>` section (for example, before the `getStarted()` call):
...
Click the "Activate Push" button and wait until the log message with the `deviceId` is displayed.
...
Add the new buttons to your HTML, then add the following to the `<script>` section. Place the button references after the channel name declaration, and the event listeners before the `getStarted()` call:
...
<Code>
```javascript
const sendPushToDeviceBtn = document.getElementById('sendPushToDeviceBtn');
const sendPushToClientBtn = document.getElementById('sendPushToClientBtn');
const sendPushToChannelBtn = document.getElementById('sendPushToChannelBtn');

sendPushToDeviceBtn.addEventListener('click', sendPushToDevice);
sendPushToClientBtn.addEventListener('click', sendPushToClient);
...
![Screenshot of the web push tutorial application showing push controls for sending notifications](../../../../images/content/screenshots/getting-started/web-push-getting-started-guide-2.png)
