...
- Only small portions change between updates (a player's ranking shifts, a score increments, some game metric updates)
...
- There's high similarity between successive messages, as more unchanged data yields greater compression ratios
- Structured data where only specific fields change frequently
- The payload naturally grows during a session, making each full transmission increasingly expensive
- Bandwidth constraints like mobile networks, high-volume scenarios, or regions with expensive data transfer amplify the benefits
- Many consumers multiply the bandwidth savings across all subscribers
...
For applications with independent data streams (different telemetry sources, separate instrument feeds) or very high throughput, consider using multiple channels. Benefits of this approach:
...
- Always call `close()` on connections when finished to avoid billing for the [2-minute connection timeout](/docs/connect/states#connection-state-recovery)

Choose the appropriate client type based on your publisher characteristics. Use the Realtime SDK when:
- Publishing messages at very high volume
- You need the lowest possible latency
- You need bidirectional communication (publish and subscribe)
- Ordering of published messages is critical

Use the REST API when:
- You have stateless publishers (serverless functions, for example)
- Publishing from environments where maintaining persistent connections is impractical
- Publishing from an authoritative backend server or on behalf of multiple users
...
Read the [Deltas documentation](/docs/channels/options/deltas) for complete implementation details, the [Conflation documentation](/docs/messages#conflation) for configuration options, and the [Server-side batching documentation](/docs/messages/batch#server-side) for advanced usage. Explore [Pub/Sub basics](/docs/pub-sub) to understand fundamental concepts, learn about [channel configuration](/docs/channels) and namespaces, and review [message concepts](/docs/messages) for deeper understanding. [Contact us](https://ably.com/contact?cta=enterprise_package) for enterprise-scale requirements and custom solutions.
