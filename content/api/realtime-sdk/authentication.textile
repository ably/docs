...
blang[jsall].
  ...
  - specifiers := an array of "TokenRevocationTargetSpecifier":#token-revocation-target-specifier objects<br>__Type: "@TokenRevocationTargetSpecifier[]@":#token-revocation-target-specifier__

  - options := an optional set of options which are used to modify the revocation request<br>__Type: "@TokenRevocationOptions@":#token-revocation-options__
