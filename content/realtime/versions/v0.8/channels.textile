...
History provides access to instantaneous "live" history as well as the longer term persisted history for attached channels. If "persisted history":/realtime/history#persisted-history is enabled for the channel, then messages will "typically be stored for 24 - 72 hours":https://knowledge.ably.com/how-long-are-messages-stored-for-1. If persisted history is not enabled, Ably retains the last two minutes of message history in memory.
...
Gets a "paginated":#paginated-result set of historical messages for this channel. If the "channel is configured to persist messages to disk":https://knowledge.ably.com/how-long-are-messages-stored-for-1, then message history will "typically be available for 24 - 72 hours":https://knowledge.ably.com/how-long-are-messages-stored-for-1. If not, messages are only retained in memory by the Ably service for two minutes.
