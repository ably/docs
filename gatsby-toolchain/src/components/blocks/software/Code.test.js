...
const codeRootElement = <Code data={[{ data: "var useVars = false;", type: "text" }]} attribs={{ lang: 'javascript' }} />;
...
describe('Different langs provided to Code elements results in different rendering', () => {
    ...
    act(() => {
        codeRootElementRenderer.update(<Code data={[{ data: "This is pseudocode, honest", type: "text" }]} attribs={{ lang: null }} />)
    });
    ...
    it('Does not contain any non-code children', () => {
        expect(codeRootElementRenderer.root.findAllByType('code').length).toBe(1);
        expect(codeRootElementRenderer.root.findAll(elem => { elem.type !== 'code' }).length).toBe(0);
    });
});