...
- Check the server logs for an error on publish after the first token. The error references appends being rejected.
...
## Turn never ends <a id="turn-never-ends"/>
...
- See [turns](/docs/ai-transport/concepts/turns#lifecycle) for the full lifecycle contract.
...
- Before editing or regenerating, cancel the in-flight turn explicitly. AI Transport does not auto-cancel an active turn on edit; see [conversation branching](/docs/ai-transport/features/branching#edit) for the recommended pattern.
