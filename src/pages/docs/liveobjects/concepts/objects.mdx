...
<If lang="java">
In the Java SDK the class names `LiveMap` and `LiveCounter` refer to **creation value types** (immutable blueprints), not live handles; live access goes through [`PathObject`](/docs/liveobjects/concepts/path-object?lang=java)s and [`Instance`](/docs/liveobjects/concepts/instance?lang=java)s.
</If>
...
<If lang="javascript">Access the channel object using `channel.object.get()`, which returns a [PathObject](/docs/liveobjects/concepts/path-object) that resolves to the root `LiveMap`:</If><If lang="java">Access the channel object using `channel.object.get()`, which returns a future that completes with a [`LiveMapPathObject`](/docs/liveobjects/concepts/path-object?lang=java) resolving to the root `LiveMap`:</If>
...
const theme = rootObject.get('settings').get('theme').instance();
...
if (instance != null) {
    instance.asLiveCounter().increment(5).join();
}

Instance settings = rootObject.get("settings").instance();
if (settings != null) {
    settings.asLiveMap().set("notifications", LiveMapValue.of(true)).join();
}
```
</Code>
...
// Assign the profile map to the channel object - the whole tree publishes as a single operation
...
<If lang="java">
<Aside data-type='note'>
In the Java SDK each assignment creates a distinct object: `LiveMapValue` accepts only new-object blueprints, not references to existing live objects, so aliasing one object under multiple keys (or building reference cycles) is not expressible. Reading such structures created by other SDKs or the [REST API](/docs/liveobjects/rest-api-usage) works fine; `compactJson()` renders cycles as `{"objectId": …}` markers.
</Aside>
</If>
...
if (instance != null) {
    System.out.println(instance.asLiveCounter().getId()); // e.g. "counter:J7x…@1734567890123"
}
```
</Code>
</If>
