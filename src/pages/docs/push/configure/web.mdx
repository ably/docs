...
Activating a browser for push notifications is typically done directly on the browser itself. Use the [`push.activate()`](/docs/pub-sub/api/javascript/realtime/push#activate) method to activate push notifications from a browser. The following steps describe the direct activation process:
...
Note that once activated, the browser remains registered even after the application is closed until the [`push.deactivate()`](/docs/pub-sub/api/javascript/realtime/push#deactivate) method is called. Calling activate again has no effect if the browser is already activated.
...
* Use the Ably [dashboard](https://ably.com/accounts) or [API](/docs/pub-sub/api/javascript/realtime/push-admin) to send a test push notification to a registered browser.
...
<Aside data-type='note'>
Utilize the [push notifications admin API](/docs/pub-sub/api/javascript/realtime/push-admin) on your server to register browsers.
</Aside>
...
When the [`push.activate()`](/docs/pub-sub/api/javascript/realtime/push#activate) and [`push.deactivate()`](/docs/pub-sub/api/javascript/realtime/push#deactivate) methods are called, the browser registers Web Push.
