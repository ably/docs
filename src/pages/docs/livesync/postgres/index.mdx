...
intro: "Postgres database connector distributes changes from databases to clients at scale using the outbox pattern. This managed integration consumes outbox table records and publishes them to Ably channels with automatic failover."
meta_description: "Ably Postgres database connector: distribute database changes to clients via outbox pattern with managed integration and automatic failover."
redirect_from:
