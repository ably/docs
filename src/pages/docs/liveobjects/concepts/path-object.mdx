...
Java expresses typing through a fixed view hierarchy instead of user-supplied type parameters. On the path layer, the `as*` casts (`asLiveMap()`, `asLiveCounter()`, `asString()` and so on) re-wrap the same path without validating it and **never throw**; a wrong cast surfaces on use: reads return `null`/empty, and the write's returned `CompletableFuture` completes exceptionally with an `AblyException`. Discriminate with `getType()` (returns a `ValueType`, or `null` when nothing resolves).
...
See the [Typed views documentation](/docs/liveobjects/typing?lang=java#typed-views) for the full contract, including how the instance-layer casts differ.
</If>
...
<If lang="java">
<Aside data-type='note'>
`at()` is only available on `LiveMapPathObject` and returns the base `PathObject`; finish with an `as*` cast to obtain a typed view.
</Aside>
</If>
...
<If lang="javascript">If the entry at the path does not exist, or the entry does not contain a `LiveMap` or `LiveCounter` object, `instance()` returns `undefined`:</If><If lang="java">If the entry at the path does not exist, or the entry does not contain a `LiveMap` or `LiveCounter` object, `instance()` returns `null`:</If>
...
```java
// The 'username' entry contains a primitive, not an object
Instance username = rootObject.get("username").instance();
...
for (Map.Entry<String, PathObject> e : rootObject.get("visits").asLiveMap().entries()) {
    // never executes
}
```
</Code>
...
<If lang="java">
<Aside data-type='important'>
In Java you cannot call a wrong-typed method without an explicit cast, and the cast itself never throws. Instead:

- Wrong-typed *reads* return `null`/empty.
- A *write* returns normally, but its `CompletableFuture` completes exceptionally with an `AblyException`: code `92007` when the resolved object's type doesn't match the typed view, or `92005` when the path cannot be resolved to any object at the time the method is called.
- When blocking with `.join()`, the failure surfaces as a `CompletionException` whose cause is the `AblyException`.

See the [Typed views documentation](/docs/liveobjects/typing?lang=java#typed-views) for the full cast contract.
</Aside>
<Aside data-type='note'>
Mutation methods return a `CompletableFuture<Void>`. These examples call `.join()` for brevity, which blocks the calling thread. In production code, especially on a UI thread or inside an Ably callback, compose the future with `thenCompose`/`thenAccept`/`exceptionally` rather than blocking on it. See the [quickstart](/docs/liveobjects/quickstart/java#step-4) for details.
</Aside>
</If>
