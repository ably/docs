...
A pre-bound version of the core [`createClientSession`](/docs/ai-transport/api/javascript/core/client-session#constructor) with `codec: UIMessageCodec` already supplied and `api` defaulting to `/api/chat` (matching the Vercel AI SDK's `DefaultChatTransport` default).
...
| runStartDeadlineMs | optional | How long a send waits for the agent's `ai-run-start` event before rejecting with `RunStartDeadlineExceeded`. Defaults to 30000. | Number |
...
<MethodSignature>{`function createAgentSession(options: VercelAgentSessionOptions): AgentSession<VercelInput, VercelOutput, VercelProjection, UIMessage>`}</MethodSignature>
...
`AgentSession<VercelInput, VercelOutput, VercelProjection, UIMessage>`. An agent session whose codec is pre-bound to `UIMessageCodec`.
...
Wrap a `ClientSession` in a `ChatTransport` adapter that satisfies Vercel AI SDK's `useChat` `transport` option. The adapter maps `useChat`'s `sendMessages` and `reconnectToStream` contract onto the session's `view.sendInput`, `view.regenerate`, and tool-resolution paths.
...
| --- | --- | --- |
| prepareSendMessagesRequest | Customise the POST body and headers before sending. Called by `sendMessages` with the conversation <Table id='SendMessagesRequestContext'/>. | `(context: SendMessagesRequestContext) => { body?, headers? }` |
...
| Property | Description | Type |
| --- | --- | --- |
| chatId | Chat session ID (from `useChat`'s id). | String |
