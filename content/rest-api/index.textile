...
bc[sh]. curl -X DELETE https://rest.ably.io/push/deviceRegistrations/01ARZ3NDEKTSV4RRFFQ69G5FAV \
 -u "{{API_KEY}}"
...
h3(#delete-channel-subscription). Unsubscribe from push notifications from channel

h6. DELETE rest.ably.io/push/channelSubscriptions

Stop receiving push notifications when push messages arrive to the specified channel.
...
- channel := string, required when deleting for specific @deviceId@
- deviceId := Filter to restrict to subscriptions for that @devideId@. Cannot be set when @clientId@ is set.
- clientId := Filter to restrict to subscriptions associated with that @clientId@. Cannot be set when @deviceId@ is set.
