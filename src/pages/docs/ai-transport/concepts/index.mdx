...
```text
                       ...
                       └─┬──────┬───────┬──┘
                         │      │       │
                  ┌──────▼──┐ ┌─▼────┐ ┌▼─────────────┐
                  │   Tree   │ │ Runs │ │ Connections   │
                  │ (branchy │ │ (per │ │ (ClientSession│
                  │ history) │ │ turn)│ │  / AgentSession)│
                  └──────────┘ └──┬───┘ └───────────────┘
                                  │
                          ┌───────▼────────┐
                          │  Invocations   │
                  ...
                  ┌─────────────┐       ┌──────────────────┐
                  │   Codecs    │       │ Authentication    │
                  │ (event ↔   │       │ (Ably token + POST│
                  │  message)   │       │  auth + cancel)   │
                  └─────────────┘       └──────────────────┘
```
...
An [Invocation](/docs/ai-transport/concepts/invocations) is the trigger a client posts to the agent endpoint to start or continue a Run. It carries the `runId`, the `inputEventId`, and the session name.

The same Run can be triggered by many Invocations: tool results, regenerates, retries. The agent mints an `invocationId` per HTTP request and returns it on the response so the caller can observe it.
