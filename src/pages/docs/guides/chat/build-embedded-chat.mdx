...
meta_description: "Architecting embedded chat with Ably for 1:1 and small group conversations linked to a specific context. Performance, reliability, and cost optimization for support chats, deliveries, gaming sessions, and more."
meta_keywords: "embedded chat, 1:1 chat, small group chat, realtime messaging, support chats, delivery tracking, gaming chat, Ably Chat, chat SDK, dependability, cost optimization"
---
...
Building with Ably means that you no longer need to worry about managing websocket servers, handling failover, network disruptions, or keeping latency low. Ably handles all of this for you, leaving you free to focus on creating engaging experiences. If you need to deliver seamless embedded chat experiences without sacrificing performance, reliability, or costs, then Ably is the proven choice.

This guide explains the architectural decisions, technical challenges, and unique benefits of building embedded chat experiences with Ably, focusing on 1:1 and small group chats. It will help you design efficient embedded chat solutions for scenarios such as:
...
And more - all with performance, user experience, and cost optimization in mind.
...
Despite the challenges of delivering these guarantees, Ably is designed to keep costs predictable. You can choose between [MAU-based](/docs/platform/pricing#mau) (more predictable for a known number of users) or [per-minute pricing](/docs/platform/pricing#per-minute) (consumption-based, pay for what you use), depending on your needs. Both pricing models are designed to be cost-effective and scalable. Features such as efficient connection management are available to help you reduce costs.
...
**Room architecture:** The optimal approach for embedded chats is one chat room per context or task, where all users join to communicate within that specific context. Rooms are:
...
Each room includes all relevant users: customers and agents in support, buyers and sellers in marketplaces, all players in games, team members in projects. Participants can join and leave for the duration of the chat. Access to message history and other features can be granted to all or select users via capabilities.

Key considerations for embedded chat rooms:

* How will rooms link to your application, and when will users join? Create one room per task and have users join when it begins. For example:
  - Return room details with a new marketplace enquiry.
  - Allocate a room per game session or location.
  - Map a delivery to a chat room.
* Will users be active continuously, or come and go? Detach during inactivity and use push notifications to bring them back. This saves costs and resources.
* Do you need long-term archiving? If you need history beyond 30 days, see the guide for [exporting chat data](/docs/guides/chat/export-chat).
...
## Message history: essential context <a id="history"/>
