...
Client libraries currently do not support this API, but it is usable via the generic "request API":/api/rest-sdk/index#request.

It is worth noting that "occupancy":/rest/channel-status#occupancy is currently only available to our "enterprise":https://ably.com/pricing#enterprise customers. However, you can still try and test out the occupancy events using your free account.