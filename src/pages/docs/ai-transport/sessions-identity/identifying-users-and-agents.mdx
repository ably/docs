...
function getJWTClaims(userId) {
  ...
  // The exp and iat claims are added automatically by jwt.sign().
  return {};
}
...
This is a top-level server program. The full code is verified as a standalone Vapor application.
*/}
```swift
// Server code
import Vapor
...
struct AblyJWTPayload: JWTPayload {
    ...
    // Required by the JWTPayload protocol. Left empty because this server
    // only signs tokens; verification is performed by Ably's servers.
...
// Mock authentication middleware.
// This should be replaced with your actual authentication logic.
struct AuthenticateUser: AsyncMiddleware {
    func respond(to request: Request, chainingTo next: any AsyncResponder) async throws -> Response {
        // Assign a mock user ID for demonstration
        request.storage[UserIDKey.self] = "user123"
        return try await next.respond(to: request)
    }
}

// Return the claims payload to embed in the signed JWT.
func getJWTClaims(userID: String) -> AblyJWTPayload {
    // Returns a payload with only the standard exp and iat claims, so the
    // token inherits the capabilities of the signing key.
    let now = Date()
    return AblyJWTPayload(
...
let keyParts = "{{API_KEY}}".split(separator: ":")
let keyName = String(keyParts[0])
let keySecret = String(keyParts[1])

// Register the HMAC-SHA256 signing key.
await app.jwt.keys.add(
    hmac: HMACKey(from: keySecret),
    digestAlgorithm: .sha256,
    kid: JWKIdentifier(string: keyName)
)

// Define an auth endpoint used by the client to obtain a signed JWT
// which it can use to authenticate with the Ably service.
app.grouped(AuthenticateUser()).get("api", "auth", "token") { req async throws -> Response in
    let userID = req.storage[UserIDKey.self]!

    // Sign a JWT using the secret part of the Ably API key.
    let token = try await req.jwt.sign(
        getJWTClaims(userID: userID),
        kid: JWKIdentifier(string: keyName)
    )

    let response = Response(status: .ok, body: .init(string: token))
    response.headers.contentType = .init(type: "application", subType: "jwt")
    return response
}
...
{/*
Swift test harness:
func identifyingUsersAndAgents2() {
...
options.authCallback = { params, callback in
    // Fetch a JWT from your auth server
    var request = URLRequest(url: URL(string: "http://localhost:3001/api/auth/token")!)
...
ably.connection.on(.connected) { stateChange in
    print("Connected to Ably")
}
```
...
{/*
Swift test harness:
func identifyingUsersAndAgents3() {
    // insert example code here
}
*/}
```swift
...
// Includes the `x-ably-capabilities` claim, which controls
// which operations the user can perform on which channels.
private static Map<String, Object> getJWTClaims(String userId) {
...
struct AblyJWTPayload: JWTPayload {
    var expiration: ExpirationClaim
    var issuedAt: IssuedAtClaim
    var ablyCapability: String

    enum CodingKeys: String, CodingKey {
        case expiration = "exp"
        case issuedAt = "iat"
        case ablyCapability = "x-ably-capability"
    }
...
func getJWTClaims(userID: String) -> AblyJWTPayload {
    let orgID = "acme" // Mock organization ID for demonstration
    ...
    return AblyJWTPayload(
        expiration: .init(value: now.addingTimeInterval(3600)),
        issuedAt: .init(value: now),
        ablyCapability: String(data: try! JSONSerialization.data(withJSONObject: capabilities), encoding: .utf8)!
    )
}
```
</Code>
...
{/*
Swift test harness:
func identifyingUsersAndAgents5(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
// Client code
let acmeChannel = ably.channels.get("org:acme:{{RANDOM_CHANNEL_NAME}}")
acmeChannel.publish("prompt", data: "What is the weather like today?") // succeeds

let foobarChannel = ably.channels.get("org:foobar:{{RANDOM_CHANNEL_NAME}}")
foobarChannel.publish("prompt", data: "What is the weather like today?") // fails

let announcementsChannel = ably.channels.get("announcements")
announcementsChannel.publish("prompt", data: "What is the weather like today?") // fails
...
{/*
Swift test harness:
func identifyingUsersAndAgents7(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
// Agent code
let weatherChannel = ably.channels.get("org:acme:weather:{{RANDOM_CHANNEL_NAME}}")
weatherChannel.subscribe { message in
    print(message)
} // succeeds
weatherChannel.publish("update", data: "It's raining in London") // succeeds

let otherChannel = ably.channels.get("org:acme:other:{{RANDOM_CHANNEL_NAME}}")
otherChannel.subscribe { message in
    print(message)
} // fails
otherChannel.publish("update", data: "It's raining in London") // fails
```
...
struct AblyJWTPayload: JWTPayload {
    ...
    enum CodingKeys: String, CodingKey {
        case expiration = "exp"
        case issuedAt = "iat"
        case ablyClientID = "x-ably-clientId"
    }
...
{/*
Swift test harness:
func identifyingUsersAndAgents9(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
// Publish a message - the clientId is automatically attached
channel.publish("prompt", data: "What is the weather like today?")
```
...
{/*
Swift test harness:
func identifyingUsersAndAgents10(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
channel.subscribe("prompt") { message in
    ...
    let prompt = message.data as? String
...
{/*
Swift test harness:
func identifyingUsersAndAgents11() {
    // insert example code here
}
*/}
```swift
...
{/*
Swift test harness:
func identifyingUsersAndAgents12(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
channel.subscribe { message in
    if message.clientId == "weather-agent" {
        print("Weather agent response: \(message.data ?? "" as Any)")
    }
}
```
...
struct AblyJWTPayload: JWTPayload {
    var expiration: ExpirationClaim
    var issuedAt: IssuedAtClaim
    var channelOrgAcme: String
    var channelWildcard: String

    enum CodingKeys: String, CodingKey {
        case expiration = "exp"
        case issuedAt = "iat"
        case channelOrgAcme = "ably.channel.org:acme:*"
        case channelWildcard = "ably.channel.*"
    }
...
func getJWTClaims(userID: String) -> AblyJWTPayload {
    ...
    return AblyJWTPayload(
        ...
        issuedAt: .init(value: now),
        // The user is an editor on all acme channels.
        channelOrgAcme: "editor",
        // The user is a guest on all other channels.
        channelWildcard: "guest"
    )
}
```
</Code>
...
{/*
Swift test harness:
func identifyingUsersAndAgents13(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
// Agent code
let channel = ably.channels.get("org:acme:{{RANDOM_CHANNEL_NAME}}")

// Subscribe to user prompts
channel.subscribe("prompt") { message in
    // Access the user's role from the user claim in message extras
    let extras = (try? message.extras?.toJSON()) as? [String: Any]
    let role = extras?["userClaim"] as? String

    print("Message from user with role: \(role ?? "")")
}
```
...
{/*
Swift test harness:
func identifyingUsersAndAgents14() {
    // insert example code here
}
*/}
```swift
...
let extras: NSDictionary = [
    "headers": [
        "model": "gpt-4"
    ]
]

channel.publish("update", data: "It's raining in London", extras: extras as ARTJsonCompatible)
```
...
{/*
Swift test harness:
func identifyingUsersAndAgents15(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
channel.subscribe { message in
    if message.clientId == "weather-agent" {
        ...
        print("Response from weather agent using \(model ?? ""): \(message.data ?? "" as Any)")
    }
}
```
