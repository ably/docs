...
func example(ably: ARTRealtime) async throws {
...
```swift
// Query presence to see both devices
let members = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[ARTPresenceMessage], any Error>) in
...
for member in members {
    guard let clientID = member.clientId else { continue }
    let data = member.data ?? ""
    print(clientID, member.connectionId, data)
}
// Example output:
// user-123 hd67s4!abcdef-0 { device = desktop; }
// user-123 hd67s4!ghijkl-1 { device = mobile; }
...
// Subscribe to leave events to see why members left
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.presence.subscribe(.leave, onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        }
    }, callback: { presenceMessage in
...
options.transportParams = [
    "remainPresentFor": ARTStringifiable.withString("30000")
]
...
// Subscribe only to enter events
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.presence.subscribe(.enter, onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
func example(ably: ARTRealtime, targetUserID: String) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.presence.subscribe(attachCallback: { error in
        ...
        channel.presence.get { members, error in
            ...
            // Check if a specific client is offline
            if !members.map({ $0.clientId }).contains(targetUserID) {
