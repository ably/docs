...
* "Messages":/realtime/messages - messages trigger function calls as soon as they are published on a channel
* "Presence events":/realtime/presence#presence-states - presence events trigger function calls when clients enter, update their data, or leave channels
* "Channel lifecycle events":/realtime/channel-metadata#lifecycle-events – get notified when a channel is created (following the first client attaching to this channel) or discarded (when there are no more clients attached to the channel). Currently this is *only* available for "batched":#batching messages
