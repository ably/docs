...
- Connection state management := Actively managing the "WebSocket":https://ably.com/topic/websockets connection by reconnecting "automatically to an available datacenter":https://knowledge.ably.com/routing-around-network-and-dns-issues when a connection drops and restoring the connection state. Find out more about "the connection object and its state":/realtime/connection and "connection state recovery which provides message continuity over changing network conditions":https://knowledge.ably.com/connection-state-recovery.
- Channel management := Providing "publish and subscribe":/realtime/channels capabilities over channels and actively managing them by queueing published messages when disconnected and "retrieving missed messages":https://knowledge.ably.com/connection-state-recovery once reconnected. The library proactively reattaches channels that become suspended due to long periods of disconnectedness.
...
* If you want to consume realtime data from one or more of your servers, then we recommend you consider using our "Reactor Queues":/general/queues or "Reactor Firehose":/general/firehose. With the Reactor, you can consume realtime data in a robust, resilient and scalable way across multiple support protocols. Find out more about "Ably Integrations":https://ably.com/integrations.
