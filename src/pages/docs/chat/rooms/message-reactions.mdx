...
```swift
// Add a 👍 reaction using the default type
await room.messages.reactions.send(to: message.serial, params: .init(reaction: "👍"))

// The reaction can be anything, not just UTF-8 emojis:
await room.messages.reactions.send(to: message.serial, params: .init(reaction: ":like:"))
await room.messages.reactions.send(to: message.serial, params: .init(reaction: "+1"))
