...
h2(#send-message). Step 7 - Updating the settings

Now we need to update the settings options so that their respective dropdowns reflect the value selected in the setting above.
...
Lastly we need to listen for errors to the @messaging@ module since that helps in communication between the companion and app.
Add the following code in your @index.js@ file which is inside the @companion@ folder: 
...
We have added the styles in the gui files and we are also updating them in the @index.js@ of the app depending on the values. 

"See this step in Github":https://github.com/ably/tutorials/commit/55d2b927547b6b7520fe4bde2dec0fa0948ae152

h2(#send-message). Step 9 - Updating the text on the watch with dynamic values

Now when we get the data that we need using the @messaging@ module to the @app@ from the @companion@.
...
If you're outside the fitbit shell then first get into it and you can directly run @bi@ which is short for build and install:
