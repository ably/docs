...
Ably Chat is purpose-built for livestream chat at scale. If you need to deliver a seamless, high-throughput chat experience to thousands or millions of user, without sacrificing performance, reliability, or costs then Ably is the proven choice. By building chat with Ably, you no longer need to worry about scaling websocket servers, handling failover or keeping latency low. Ably handles all of this for you, leaving you free to focus on the experience for your users and what matters to you. This guide explains the architectural decisions, technical challenges, and unique benefits of building livestream chat with Ably, so you can deliver the best possible user experience to your end-users.

## Why Ably for livestream chat?

Ably is trusted by organisations delivering chat to millions of users in realtime. Our platform is engineered around the four pillars of dependability:
...
Delivering chat messages in realtime is key to an engaging experience. Ably's serverless architecture eliminates the need for you to manage websocket servers. It also automatically scales to handle millions of concurrent connections without provisioning or maintenance. Ably handles all of the edge-cases around delivery, failover and scaling - so that you can focus on what's important to you and your end-users. For more information on how Ably achieves these guarantees, and more, see the [Ably Architecture Guide](/docs/platform/architecture).

Despite the challenges of delivering these guarantees, Ably is designed to **keep costs predictable, no matter your scale**. Features including server-side batching, efficient connection management, and flexible pricing keep costs predictable—even as your audience grows.
...
- How Ably's platform solves common real-world challenges including scale and reliability
- What key chat features the Ably Chat SDK's provide to help you build an engaging livestream chat experience
...
The key architectural decision is how best to structure your chat rooms. With Ably, you are not limited by technology - only by the user experience you want to deliver. There are two main options when using Ably Chat:
...
- Should everyone see the same messages, or do you need to segment by language, region, friend group or topic?
  - If you're running livestreams over multiple languages or geographical regions, you may want to consider separating users similarly so that everybody understands the chat. This can be achieved by using a logged-in users provided location or using GeoIP to determine the location of the user.
...
- Do you want an environment where everyone has time to read every message, or the chat to go wild with hype when something exciting happens?
  - At anything more than 5-10 messages per second, the chat becomes unreadable for the audience. This may be ok in the moment when a goal is scored, for example, but may degrade the experience if it's for the entire game.
...
- **Proven at scale:** Ably delivers over 500 million messages per day for customers, with individual rooms supporting over 1 million concurrent users. See how we achieve this our [platform scalability guide](/docs/platform/architecture/platform-scalability).
- **Server-side batching:** Reduce costs and network overhead by grouping messages before delivery. For example, batching messages over a 500ms interval can reduce the number of outbound messages by up to 8x in high-traffic streams, as we demonstrate [later in this guide](#server-side-batching).
...
PLACEHOLDER: Visual image showing server-side batching in action
...
- What balance do you want to strike between real-time delivery and cost efficiency?
  ...
  - For smaller groups and 1:1 chats, you may find that batching does not make a material difference to costs, but does affect the latency of the chat. In this situation, you may wish to disable batching.
...
Ably supports moderating messages before they've been published to the livestream, and retroactively after they've been published. This makes it easy to integrate with AI-powered moderation tools.
...
Ably's integration with moderation providers enables you to customise the moderation policy on a per-chat basis, enabling you to implement different rules and sensitivity levels for different types of content and communities. You can even configure what happens if the moderation provider can't be reached - for example, publishing the message anyway. Get up and running with **out-of-the box integrations in minutes** and try out many providers for free. Ably takes care of all of the complex bits of invoking the moderation provider, handling the results and failure conditions.

Of course, you may already have built moderation in other parts of your organisation and want to re-use it for your livestream chat. Ably's "bring your own" moderation rules enable you to connect your chat rooms to your own infrastructure via popular transports including serverless functions, queues and webhooks. By implementing simple APIs, you can integrate your own moderation solution seamlessly with Ably.
...
PLACEHOLDER: Image showing how occupancy metrics can be used to hype up a stream

## Reactions and typing indicators

Enhance engagement with room reactions and typing indicators. Celebrate the winning goal or protest the controversial decision by enabling users to express sentiment en-masse.

- **Room reactions:** Broadcast sentiment (e.g., 👍, ❤️) to all users.
- **Typing indicators:** Show when users are composing messages.

Typing indicators are less common in large livestream chat, where a large number of users are typing at once. However, in more intimate experieces such as supporting 1:1 and group chats, they can be a great way to enhance engagement. So a key design decision is whether or not user engagement will benefit from them.

Room reactions are a great way to enhance engagement in a livestream and enable users to quickly express sentiment to key moments in the stream. They are not persisted, and are optimised for scale. [Ably's architecture](/docs/platform/architecture/platform-scalability) is designed to support the spikey traffic of livestream chat, ensuring that you don't encounter any performance issues in the pivotal moments.

Both of these features are presented as simple APIs in the Ably Chat SDKs, enabling you to add them to your livestream with minimal effort.

CODE HERE: Example of sending a room reaction

## Message history and context

Some users may join your livestream late, or miss important information. Ably stores chat history for 30 days by default, with options to extend. You can use this to retrieve previous messages to provide context for users joining mid-stream.
...
Every SDK instance keeps track of where it's at in the message stream. If the connection is lost, the library will automatically attempt to reconnect to the servers and in doing so, resume its position in the stream. This enables the chat to continue as if the user never left. After extended periods of disconnection, the client can make use of the chat history API to backfill missing messages.
...
Read more about how Ably SDKs and infrastructure handle network disruption in the [connection recovery overview](/docs/platform/architecture/connection-recovery).

## Priced for scale

Livestreams can be a high-churn affair, with many users coming and going throughout. In Monthly Active Users (MAU) pricing models, this often leads to bills that far outweigh the actual usage. Ably offers a competitive pricing model that is based on consumption, with discounted rates for higher usage. So you won't be billed for users that come and go, only for whilst they are engaging with your experience.

If you have an audience that is relatively consistent, for example a number of people sign up for an event and then stay for the duration, Ably now offers a competitive MAU pricing model in addition to the traditional consumption-based model for these scenarios.
...
In large livestream chats, the majority of the cost comes from delivering messages to all of the participants. It also limits how many messages per second can be processed.
...
PLACEHOLDER: Code for heartbeat interval
...
Ably's flexible integration capabilities make it easy to connect your chat system with AI services. Through Ably's integrations, you can automatically route messages to:

- Serverless functions (AWS Lambda, Azure Functions, etc.)
- Webhooks
- Custom endpoints

We're working hard on bringing a more native Chat experience to getting these integrations set up, but this can be easily achieved using the existing [Ably Pub/Sub integrations](/docs/integrations).
