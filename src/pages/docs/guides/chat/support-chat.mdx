...
Support chat has unique scaling requirements. You might have thousands of concurrent conversations, each with its own room, while agents handle multiple tickets simultaneously. Ably's architecture scales automatically to handle this load without provisioning or re-engineering as you grow.
...
* How do customers start a new conversation?
...
Building a polished chat widget requires handling message delivery guarantees, reconnection logic, presence synchronization, and typing indicators. Ably Chat abstracts away these challenges, so you can focus on the user experience rather than the infrastructure. Customers interact with one conversation at a time, typically through an embedded widget.

### Designing conversation flow <a id="conversation-flow"/>

How conversations begin and progress through your system affects customer experience, agent efficiency, and operational costs.

**Starting a new conversation:** When a customer initiates support, you need to connect them to a chat room and track the conversation in your system. The room name typically encodes the relationship—for example, `support:ticket-123`—making it straightforward to look up context. Your server generates this room name when creating the ticket, returns it along with an authentication token, and the customer attaches to that room.
...
**Handling returning customers:** Since message history persists in Ably Chat, customers can return to existing conversations and see the full history. When a customer returns to a ticket you've marked as resolved, you can reopen it in your system and notify agents again. The chat room is already there with all history intact—your ticket management logic decides what happens.
...
* **Improve flow:** Better conversation pacing.
...
Message history is crucial for chat, ensuring continuity and context even when participants join mid-conversation or return after interruptions.
...
### Presence: see when agents are online <a id="presence"/>
...
The pattern is a dedicated Pub/Sub channel where your server publishes notifications when tickets need agent attention. Agents subscribe to this single channel to learn about all tickets requiring action, without attaching to each room individually. This approach scales efficiently because agents receive lightweight notifications rather than subscribing to every active conversation.
...
- A new conversation starts and needs agent attention (after automation).
...
The agent dashboard is where support chat demands careful architectural thinking. Agents need to see and respond to multiple conversations, know when new tickets need attention, coordinate with other agents, and access context and AI assistance.
...
Agents handle multiple conversations, but attaching to hundreds of rooms simultaneously isn't practical. The pattern is to attach only to rooms the agent is currently viewing or working on, and detach when switching away.
...
AI chatbots are revolutionizing the way we interact with digital platforms. Integrating an AI agent (such as an LLM) into your support chat flow can reduce the workload for your agents and provide faster responses to customers. AI can also help support agents be more effective by providing contextual information and suggested responses. Additionally, AI can help you analyze past conversations for training, monitoring, and building knowledge bases.
...
3. An integration routes each message to an AI service, which responds:
   ...
   - Or escalates to a human agent with the full context of the conversation by pushing the ticket to the agent's inbox channel.
...
- Summarizing the context of the conversation, such as previous tickets from this customer or recent purchase history.
...
## Ticket resolution and archiving <a id="closing-conversation"/>

When a ticket is resolved, update its status in your system, optionally collect customer feedback, and export the conversation for long-term storage.
...
**Exporting chat history:** Ably Chat excels at realtime messaging, but your support system likely needs features Ably doesn't provide: full-text search across all tickets, long-term storage spanning years, analytics, and compliance reporting. This means exporting conversations from Ably into your own systems.
...
If the ticket is reopened and more messages are exchanged, re-export the full history when it closes again. The complete conversation is always available via Ably's history API.
...
* **Scale planning:** Confirm you are on the right Ably package for your expected volume of concurrent conversations.
