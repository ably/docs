...
blang[swift].
  Subscribe to users' presence status by creating a subscription. Presence events are emitted whenever a member enters or leaves the presence set, or updates their user data. Use the @presence.subscribe()@ method in a room to receive updates:
...
blang[javascript].
  Use the "@presence.update()@":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat_js.Presence.html#update method when a user wants to update their data, such as an update to their status, or to indicate that they're raising their hand. Updates will send a presence event to all users subscribed to presence:
...
```[swift]
try await room.presence.leave(data: .init(userCustomData: ["status": .string("Away")]))
```
