...
The time taken before a @leave@ event is sent in the case of an abrupt disconnect can be reduced to a minimum of 1 second by setting a value for @dormantAfter@, in milliseconds. This property is set within the @transportParams@ property of the "@clientOptions@":/realtime/usage#client-options object.
...
The following example code demonstrates establishing a connection to Ably with @dormantAfter@ set to 1 second:

bc[jsall]. const ably = new Ably.Realtime(
  {
    key: '{{API_KEY}}',
    transportParams: { dormantAfter: 1000 }
  }
);
