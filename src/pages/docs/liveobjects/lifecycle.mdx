...
if (counterInstance) {
  counterInstance.subscribe(({ object, message }) => {
    if (message?.operation.action === 'object.delete') {
      ...
      // The subscription is automatically unsubscribed after this notification
...
The [root object](/docs/liveobjects/concepts/objects#root-object) cannot be deleted, so deletion notifications will not occur for the root object itself.
</Aside>
</If>
