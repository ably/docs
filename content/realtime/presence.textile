...
[channel.presence history:query callback:^(ARTPaginatedResult<ARTPresenceMessage *> *resultPage, ARTErrorInfo *error) {
...
h2(#properties).
  ...
  dotnet: IO.Ably.Realtime.Presence Properties
...
###########TODO: Currently not fully supported in dotnet
...
bq(definition#enter-none).
  ...
  dotnet:   public Task EnterAsync(object clientData)
...
- <div lang="dotnet">data</div> := data payload for the current present member. The supported payload types are strings, plain object convertable to json, binary data as byte arrays, and null.<br>__Type: @Object@__
...
bq(definition#leave-data).
  ...
  dotnet:  public Task LeaveAsync(object clientData)
...
- <div lang="jsall">data</div> := data payload for the current present member. The supported payload types are Strings, JSON objects and arrays, buffers containing arbitrary binary data, and null.<br>__Type: @Object@__
- <div lang="java">data</div> := data payload for the current present member. The supported payload types are Strings, JsonObject, binary data as byte arrays, and null.<br>__Type: @Object@__
- <div lang="dotnet">data</div> := data payload for the current present member. The supported payload types are strings, plain objects convertable to json, binary data as byte arrays, and null.<br>__Type: @Object@__
- <div lang="ruby">data</div> := data payload for the current present member. The supported payload types are Strings, Hash or Array objects that can be serialised to JSON using @to_json@, binary data as @ASCII-8BIT@ byte arrays, and null.<br>__Type: @Object@__
- <div lang="objc,swift">data</div> := data payload for the message. The supported payload types are <span lang="objc">@NS@</span>@String@, <span lang="objc">@NS@</span>@Dictionary@ or <span lang="objc">@NS@</span>@Array@ objects that can be serialised to JSON, binary data as @NSData@, and @nil@.<br>__Type: @Object@__
...
bq(definition#update-data).
  ...
  dotnet:  public Task UpdateAsync(object clientData)
...
h4. <span lang="default">@options@ parameters</span><span lang="objc,swift">@ARTRealtimePresenceQuery@ properties</span><span lang="dotnet">method parameters</span>
...
bq(definition).
  ...
  java:    public "PaginatedResult":#paginated-result<"PresenceMessage":#presence-message> history("Param":#param[] options)
  ...
  dotnet:  public Task<PaginatedResult<PresenceMessage>> HistoryAsync(DataRequestQuery query, bool untilAttached = false)
...
bq(definition#subscribe-event).
  ...
  dotnet:  public void Subscribe("PresenceAction":#presence-action action, Action<PresenceMessage> handler)
...
- <div lang="dotnet">handler</div> := called with each matching "@presence message@":#presence-message when received on the channel
...
bq(definition#unsubscribe-event).
  ...
  dotnet:  public bool Unsubscribe("PresenceAction":#presence-action presenceAction, Action<PresenceMessage> handler) 
...
bq(definition#unsubscribe-listener).
  ...
  dotnet:  public bool Unsubscribe(Action<PresenceMessage> handler)
...
bq(definition#unsubscribe-all).
  ...
  dotnet: public bool Unsubscribe()
...
- <div lang="dotnet">handler</div> := previously registered handler
...
bq(definition#enter-client-none).
  ...
  dotnet:   public Task EnterClientAsync(string clientId, object clientData)
...
- <div lang="dotnet">data</div> := data payload for the member. The supported payload types are strings, plain object convertable to json, binary data as byte arrays, and null.<br>__Type: @Object@__
...
There are two overloaded versions of this method. With both versions, a <span lang="default">callback</span><span lang="ruby">block</span><span lang="java">completion listener</span><span lang="dotnet">handler</span> can optionally be passed in to be notified of success or failure to leave.
...
bq(definition#leave-client-none).
  ...
  dotnet:   public Task LeaveClientAsync(string clientId, object data) 
...
bq(definition#update-client-data).
  ...
  dotnet: public Task UpdateClientAsync(string clientId, object data)
...
h3(#presence-message).
  ...
  dotnet:  IO.Ably.PresenceMessage
