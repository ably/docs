...
The View concatenates each visible node's messages from `codec.getMessages(node.projection)`, then applies regenerate substitution: when the visible chain contains both an original reply and its regenerate sibling, the regenerate's content replaces the original in the rendered message list. The result is a linear list of `TMessage` representing the currently selected conversation.
