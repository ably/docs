...
One powerful use case for token authentication is dynamically changing the channels a user has access to without disconnecting. This can be achieved by issuing a re-authentication request from the client using the [`authorize()`](/docs/api/realtime-sdk/authentication#authorize) method.

### How it works

When you call `client.auth.authorize()` with new token parameters:

1. New token acquisition: The client obtains a new token with different [capabilities](/docs/auth/capabilities) from your authentication server
2. Connection upgrade: The new token is sent to Ably using the existing realtime connection 
3. Automatic application: The capabilities (permissions) specified in the new token are automatically applied without disconnecting

### Use cases

Insufficient permissions scenario: A client might receive an `insufficient permissions` error (code [40160](/docs/platform/errors/codes#40160)) when attempting to attach to a channel. Your application can detect this error and trigger re-authentication to obtain the necessary permissions.

Permission changes: You can proactively change a client's permissions by instructing them to re-authenticate. This can be done:
- Via Ably channels: Send a message over an Ably channel that the client is listening on
- Out-of-band communication: Use any other communication method your application supports

### Example: Re-authentication for new permissions
...
### Handling non-compliant clients

For security purposes, if you need to handle potentially "rogue clients" that ignore re-authentication instructions:

1. Token TTL reduction: You can reduce the token TTL (time-to-live) from the default of one hour to force more frequent re-authentication. However, avoid setting this shorter than 10 minutes to prevent excessive load on your authentication server.

2. Token revocation: Use Ably's [token revocation API](/docs/auth/revocation) to immediately invalidate tokens for non-compliant clients. This provides a more scalable solution than relying solely on short TTLs.
