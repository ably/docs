...
A `Room` object represents a chat room. Access it via `ChatClient.Rooms.get(name)`.
...
| -------- | ----------- | ---- |
| `get name()` | The unique name of the room. | String |
| `get status()` | The current status of the room. | [`RoomStatus`](#roomstatus) |
| `get error()` | The current error, if any, that caused the room to enter the current status. | [`ErrorInfo`](LINK) \| `undefined` |
| `get channel()` | The underlying Ably realtime channel used for the room. | [`RealtimeChannel`](LINK) |
| `get messages()` | Allows you to send, subscribe to, and query messages in the room. | [`Messages`](LINK) |
| `get presence()` | Interact with presence events in the room. | [`Presence`](LINK) |
| `get occupancy()` | Interact with occupancy metrics for the room. | [`Occupancy`](LINK) |
| `get reactions()` | Interact with room-level reactions. | [`RoomReactions`](LINK) |
| `get typing()` | Interact with typing events in the room. | [`Typing`](LINK) |
...
| --------- | ----------- | ---- |
| `listener` | Function called when the room status changes. | `RoomStatusListener` |
...
Returns a `RoomOptions` object containing the configuration options used to create the room.
