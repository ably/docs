...
func requestHumanApproval(toolCall: ToolCall) async throws {
...
func example(
    ...
    displayApprovalUI: @escaping @MainActor (_ request: Any?, _ toolCallID: String?) -> Void
) {
    // --- example code starts here ---
*/}
```swift
...
func reject(toolCallID: String) async throws {
    let message = ARTMessage(name: "approval-response", data: [
        "decision": "rejected"
    ])
    message.extras = [
...
func example(
    ...
    getUserPermissions: @escaping @Sendable (String) async -> (canApprove: @Sendable (String) -> Bool, Void),
    executeToolCall: @escaping @Sendable (ToolCall) async -> Any
) {
    // --- example code starts here ---
*/}
```swift
...
channel.subscribe("approval-response") { message in
    Task { @MainActor in
        ...
        // The clientId is the trusted approver identity
        guard let approverID = message.clientId else { return }
        ...
        if (response?["decision"] as? String) == "approved" {
            let result = await executeToolCall(pending)
...
channel.subscribe("approval-response") { message in
    Task { @MainActor in
        ...
        guard let policy = approvalPolicies[pending.name] else { return }

        // Get the trusted role from the JWT claim
        guard let approverRole = extras?["userClaim"] as? String else { return }
        ...
        if (response?["decision"] as? String) == "approved" {
            _ = await executeToolCall(pending)
