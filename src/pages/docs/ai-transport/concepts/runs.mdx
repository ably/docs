...
- `messages`: the Run's whole turn, the triggering input message followed by its streamed output. This is the unit to persist; see [database hydration](/docs/ai-transport/features/database-hydration).

Each side then adds its own verbs. The client's `ClientRun` adds `started` and `cancel()`; the agent's `AgentRun` adds `located` (resolves once the triggering input folds into the tree) and the lifecycle methods `start()`, `pipe()`, `suspend()`, and `end()`.
