...
"Capabilities":/auth/capabilities for metachannels work almost identically to how they do for regular channels. The only difference is that the resource name in the capability must explicitly include the @[meta]@ qualifier.

Note that it is never possible to publish or be present on a metachannel, however you can query their "history":/LINKME.
...
The @data@ property of all events is a "@ChannelDetails@":/api/realtime-sdk/channel-metadata#channel-details object. The "@ChannelDetails.ChannelStatus@":/api/realtime-sdk/channel-metadata#channel-status which includes "occupancy":LINKME details varies depending on the event. If the event is specific to a region, such as @channel.region.active@ then the occupancy metrics will only be for that region. For other events such as @channel.opened@, the occupancy metrics will be global.
...
The @[meta]log@ and @[meta]log:push@ metachannels publish events that aren't otherwise available be received by a client. For example, if Ably receives an error response when delivering a webhook to your endpoint, Ably sends an event to @[meta]log@ with information about the error.
...
Note that this is an Enterprise-only feature and must be enabled for each app you would like to sample.
