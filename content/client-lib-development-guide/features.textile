...
** @(RSC7b)@ (Please note this clause and the associated header has now been superseded by "RCS7d":#RSC7d) The header @X-Ably-Lib: [lib][.optional variant]?-[version]@ should be included in all REST requests to the Ably endpoint where @[lib]@ is the name of the library such as @js@ for @ably-js@, @[.optional variant]@ is an optional library variant, such as @laravel@ for the @php@ library, which is always delimited with a period such as @php.laravel@, and where @[version]@ is the full client library version using "Semver":http://semver.org/ such as @1.0.2@. For example, the 1.0.0 version of the JavaScript library would use the header @X-Ably-Lib: js-1.0.0@.
...
** @(RSC7d)@ Where possible, the header @Ably-Agent@ should be included in all REST requests to the Ably endpoint and contain a series of products and associated versions in the format described by "RFC 7321 Section 5.5.3":https://tools.ietf.org/html/rfc7231#section-5.5.3 (ie a list of <product>/<product-version> joined by spaces). The products described in this header should include the SDK itself and any relevant runtime platforms, for example ably-js version 1.2.8 running on NodeJS 14.16.0 should send "ably-js/1.2.8 NodeJS/14.16.0".
*** @(RSC7d1)@ When it is not possible to send the @Ably-Agent@ header, such as for @JSONP@ requests, the library version should be sent as a query param such as @agent=ably-js/1.2.8 NodeJS/14.16.9@ (in practice this query param should also be URL escaped).
