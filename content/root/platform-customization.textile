...
- @rest.ably.io@ - this endpoint is used for all REST-based requests and is optimized for all standard HTTP-based request types including all "REST requests":/api/rest-api, "authentication requests":/general/authentication, and "Comet and JSONP fallback":/general/authentication when WebSockets cannot be used.
