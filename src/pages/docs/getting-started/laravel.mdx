...
6. Now run the two dev servers (PHP and Vite) in separate Terminal windows:
...
<Aside data-type='note'>
This example uses an API key for simplicity. Laravel Echo handles authentication through token-based authentication automatically. In production, you should configure Laravel's [broadcasting authentication](https://laravel.com/docs/broadcasting#authorizing-channels) for private and presence channels. For public channels shown here, the API key is exposed to the client but scoped by the broadcaster configuration.
</Aside>
