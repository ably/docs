...
Agent code runs in a trusted environment, so it can directly specify the [`clientId`](/docs/auth/identified-clients#setting) when instantiating the agent client to establish its identity. This identity appears as the [`clientId`](/docs/messages#properties) in all messages the agent publishes, allowing subscribers to identify the agent which published a message.
