...
In the current Ably service the backend supports the Realtime protocol over a "WebSocket":/concepts/websockets transport and over a Comet transport. Other transports are possible and this document aims to define the protocol in a transport-independent manner.
