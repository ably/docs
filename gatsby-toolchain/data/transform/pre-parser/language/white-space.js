 ...
 */
const trimWhitespaceBetweenSpans = (content) =>
  content.replace(/<\/span>\s+<span\slang=["']([^"']+)["']>/g, '</span><span lang="$1">');

const ensureNewlineBetweenDivs = (content) =>
  content.replace(/<\/div>\s*<div\slang=["']([^"']+)["']>/g, '</div>\n<div lang="$1">');

const enforceWhiteSpaceLevelsBetweenLanguageElements = (content) => {
  return trimWhitespaceBetweenSpans(ensureNewlineBetweenDivs(content));
};
