...
"Reactor Firehose":/general/firehose enables "Enterprise customers":/pricing#enterprise to send messages published by the Ably platform directly to a "third party streaming or queuing service":/general/firehose#services. 
...
In this tutorial you will learn how to use the Reactor Firehose integration with Amazon Kinesis. You will publish messages representing random sensor data from a configurable number of simulated IoT devices to a channel and create an integration that automatically pushes these messages into a "Amazon Kinesis Data Stream":https://aws.amazon.com/kinesis/data-streams/.
...
Make a note of the @StreamARN@ key's value. You will need it in the next and later steps:
...
# Name your app @firehose-kinesis-tutorial@, select the SSL Required checkbox, then click Create app.
...
# From the list of rule types, select Reactor Firehose and, from the list of services, select Amazon Kinesis.
# Configure your reactor rule as follows:
...
@NUM_SENSORS@ is the number of IoT devices your application will simulate, each of which will generate a random temperature and humidity reading. Set this value to five initially.
...
This code imports all the required dependencies, instantiates the "Ably REST API client":/documentation/rest and serves the @index.html@ web page and supporting files. Test it by running @node server.js@ and visiting @http://localhost:5000@ in your browser. If everything is working correctly, you should see a web page with a button inviting you to generate sensor readings. The button doesn't do anything yet: you will write the code for this in the next step. 

h2. Step 5: Generate random sensor data

In this step, you will use the Node.js @random-number@ module to generate random sensor readings that contain temperature and humidity values and then publish them to the @raw-sensor-data@ channel. Because you created a Reactor Firehose integration rule on that channel, the transactions you created will be sent to your data stream.
...
Now that you can send messages to your stream, you want to be able to do something with them. In this tutorial you will create a "serverless" function using "AWS Lambda":https://docs.aws.amazon.com/lambda/latest/dg/welcome.html to calculate running averages of the stream sensor data.
...
In the final step you will subscribe to the @processed-sensor-data@ channel and display the messages that your lambda publishes to that channel.
...
Then, in @client.js@, add the following code under the @generateSensorData()@ function. This code requests the API key, instantiates the Ably client, and subscribes to the @processed-sensor-data@ channel:
...
# Run your server by executing @node server.js@ in the root of your project directory.
# Visit @https://localhost:5000@ in your browser.
# Click the Generate Readings button.
# If everything is working correctly, the web page will display your sensor data averages.
# Continue to click the Generate Readings button and watch the data accumulate.
