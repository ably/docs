...
h4(#jwt-claims). Using JWT for per connection publish rate limits

It is possible for JWTs to contain publish rate limits for a user on particular channels. These limits can be used to prevent any individual user from sending excessive messages.

An example use case is in a large live chat where you may wish to limit users to posting messages once every 10s.

To set the limits you need to include @ably.limits.*@ in your JWT authentication payload, for example:
