...
func example(realtime: ARTRealtime, stream: [StreamEvent]) {
...
channel.publish([.init(name: "response", data: "")]) { result, error in
    ...
    for event in stream {
        if event.type == "token" {
            ...
            channel.append(appendMessage, operation: .init(clientId: nil, descriptionText: nil, metadata: ["phase": "streaming"]), params: nil, callback: nil)
        }
    }
    ...
    channel.append(doneMessage, operation: .init(clientId: nil, descriptionText: nil, metadata: ["phase": "done"]), params: nil, callback: nil)
}
```
</Code>
...
message.extras = ["headers": ["responseId": "resp_abc123"]] as NSDictionary

channel.publish([message], callback: nil)
```
</Code>
...
// Listen for cancellation requests
channel.subscribe("cancel") { message in
    let extras = message.extras as? NSDictionary
    ...
    confirmMessage.extras = ["headers": ["responseId": responseID ?? ""]] as NSDictionary

    channel.publish([confirmMessage], callback: nil)
}
```
</Code>
