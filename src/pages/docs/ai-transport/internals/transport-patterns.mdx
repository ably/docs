...
Ably channel
      │
      ▼
Channel subscription (ai-input, ai-output, ai-run-start, ai-run-end, ai-cancel)
      │
      ▼
Decoder ── DecodedMessage<TInput, TOutput>
      │
      ▼
Tree ── fold per Run (TProjection) ──► RunNode update
      │
      ▼
Stream router ── per-Run ReadableStream<TOutput>
      │
      ▼
ActiveRun.stream consumed by application code

View ── walks Tree, emits 'update' on visible changes
```
...
The router maintains state keyed by `runId`. When the client calls a write method on the View (`sendMessage`, `sendInput`, `regenerate`, `edit`), the View mints a `runId` and registers the optimistic Run in the Tree. The returned `ActiveRun` carries the `runId` and a `started: Promise<void>` that resolves when the agent's `ai-run-start` for this Run lands.

When inbound `ai-output` messages decode to `TOutput` events, the router folds them into the matching Run's per-Run projection. When `ai-run-end` arrives for the Run, the router marks the Run terminal. The View's `update` event fires; consumers re-read `getMessages()` / `runs()`.
...
2. Client POSTs InvocationData (runId, inputEventId=E1, sessionName) to the agent.
...
7. run.start() resolves, the agent fetches conversation history and pipes
   the LLM stream. The agent route returns { invocationId: run.invocationId }
...
1. The client calls `session.cancel(runId)` or `activeRun.cancel()`. The session publishes an `ai-cancel` message with `run-id: runId` under `extras.ai.transport`.
2. The agent session receives the cancel message through its channel subscription.
3. The agent matches the `run-id` against its registered Runs.
