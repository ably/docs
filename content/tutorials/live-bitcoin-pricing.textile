...
In this tutorial, we'll show you how you can build an Angular app to get a realtime data stream of changing bitcoin prices in USD using the "Ably Hub":https://www.ably.io/hub, (a marketplace for open streaming data sources) and display it conveniently in a chart using Kendo UI's Angular components. There are many other free-to-use open streaming data sources on the Ably Hub to choose from and so many other components offered by Kendo UI that you can use to visually plot the realtime data easily. So, while this article is a good primer on how to get started, feel free to explore other data sources and "UI components":https://www.telerik.com/kendo-angular-ui/components/. As a side node, you can also "publish your own realtime data using Ably":/realtime and share it with others and even host it publicly on the Ably Hub.
...
* Kendo UI's Angular chart component: Only needing the chart component for now, we’ll run this command to just install the chart goodies: @ng add @progress/kendo-angular-charts@.
...
The Angular CLI offers a very handy command @ng serve@, which will serve up your code locally in your browser and refreshes the page when changes to your code is made.

h2(#adding-chart-component). Step 2 - Creating a chart using Kendo UI's Angular chart component

First we'll need to import the @ChartModule@ into our app!
...
* A basic "Publish/Subscribe tutorial":/tutorials/publish-subscribe#lang-javascript
* Ably's "API Streamer documentation":/api-streamer
