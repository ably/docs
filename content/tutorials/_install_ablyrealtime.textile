<%
  client_id = 'jamie'
  # default to showing all languages
  all_languages = !@languages
%>
h2.
  default: Step 2 - Install Ably
<% if all_languages || @languages.include?(:swift) %>
  swift: Step 2 - Setup an Xcode project and install Ably
<% end %>

<% if all_languages || @languages.include?(:java) %>
blang[java].
...
<% if all_languages || @languages.include?(:js) %>
blang[javascript].
...
<% if all_languages || @languages.include?(:ruby) %>
blang[ruby].
...
<% if all_languages || @languages.include?(:php) %>
blang[php].
...
<% if all_languages || @languages.include?(:python) %>
blang[python].
