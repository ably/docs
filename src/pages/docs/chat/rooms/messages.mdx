...
| version.clientId | The client identifier of the user associated with the action. Only set for `message.update` and `message.delete` actions. | String or undefined |
| version.description | Optional description for the action. Only set for `message.update` and `message.delete` actions. | String or undefined |
| version.metadata | Optional additional metadata about the action. Only set for `message.update` and `message.delete` actions. | Object or undefined |
...
<If lang="javascript">
Use the [`messages.get()`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.Messages.html#get) method to get a message in a chat room.
</If>
...
const {unsubscribe} = room.messages.subscribe((event) => {
  switch (event.type) {
    ...
    case ChatMessageEventType.Updated:
      ...
      if (existing && event.message.isOlderVersionOf(existing)) {
...
| version | Contains additional fields compared to `message.create` action: |
| version.serial | Set to the serial of the update action. |
| version.timestamp | Set to the time the message was updated. |
| version.clientId | The client identifier of the user who performed the update. |
| version.description | Optional description provided in the update request. |
| version.metadata | Optional metadata provided in the update request. |
...
| version | Contains additional fields compared to `message.create` action: |
| version.serial | Set to the serial of the deletion action. |
| version.timestamp | Set to the time the message was deleted. |
| version.clientId | The client identifier of the user who performed the deletion. |
| version.description | Optional description provided in the delete request. |
| version.metadata | Optional metadata provided in the delete request. |
...
The [`Message`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.Message.html) object has a method [`with`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.Message.html#with) that takes a [`MessageEvent`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.MessageEvent.html), automatically compares version serials, and returns the newest `Message` instance. For updates and deletes, if `with` is called with an event that has an older `version.serial` than the message, the message is returned. If it is called with a newer event (greater `version.serial`), the message from the event is returned. For message reaction events, the reactions will be correctly applied to the returned message.
