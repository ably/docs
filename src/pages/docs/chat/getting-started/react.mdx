...
The above code creates a room with the ID `my-first-room` and sets up a listener to monitor the room status. It also displays the room name and current status in the UI.
...
Update the listener provided to the `useMessages()` hook to handle the `MessageEventType.Updated` event:
