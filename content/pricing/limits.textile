...
This highlights the potential for 80,400 messages to be sent in a very short space of time on a single channel. Note that there are options for "mitigating":/presence-occupancy/presence#present-no-subscribe this such as utilizing "server-side batching":/channels#server-side-batching.
...
Limits will come into force either because there isn't any buffer on them, or because you have exceeded the buffer. The effect of limits depends on their type, for example:

* the message publish rate on an individual channel is 50 messages per second. Any message publish attempts in excess of the 50 messages per second will be rejected and an error code will be returned to the publisher.
* if the limit for publishing messages into a queue is exceeded by twice the rate, then each message each message will have a 50% chance of being rejected. The suppression probability is continuously updated based on the publishing rate.
