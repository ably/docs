...
** The process begins with your app (client) or server, which publishes a push notification. This server could be any backend service that needs to send notifications to users' devices.
** Your server sends the notification data to Ably.
...
** Ably then forwards the packaged message to the appropriate push notification service _FCM_, _APNs_, or in the case of Web Push, the browser vendor's notification service.
* Deliver push notification:
** The selected push notification service takes responsibility for delivering the notification to the target devices.
** The push notification finally arrives at the end-user's devices or browsers, displaying the notification on their screen.
