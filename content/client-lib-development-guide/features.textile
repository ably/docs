...
*** @(RSA7a3)@ @Auth#clientId@ attribute returns a string value for the authenticated client's @clientId@. If the client is not authenticated and a @clientId@ value was not provided when instanced with @ClientOptions@, then @Auth#clientId@ is @null@
...
** @(RSL1f)@ Unauthenticated clients using basic auth without a @clientId@ (i.e. any @clientId@ is permitted):
...
*** @(RSL1g4)@ When publishing a message with an explicit @clientId@ that is incompatible with the @clientId@ (either inferred or explicitly configured in the token or @ClientOptions@), the library will reject the message immediately and raise an exception.
...
*** @(RTL6g3)@ When publishing a @Message@ with a different @clientId@ attribute value to the authenticated @clientId@, the client library should reject publishing immediately. The message should not be sent to Ably and the errr callback should be called. The connection and channel must remain available for further publishing and other operations
