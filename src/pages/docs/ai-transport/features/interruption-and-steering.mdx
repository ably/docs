...
- `outcome` resolves to `{ consumed: boolean, runTerminalReason?: RunEndReason }` once the agent consumes the steering message, or once the Run ends without consuming it:
  - `consumed: true`: the agent saw the steering message before it sent its response. The `pipe()` that records the message resolves `outcome`, so a consumed message resolves even when the Run later suspends rather than ends.
