...
* @(TX2)@ @TokenRevocationResponse#revokedTokens@ is a collection of @RevokedTarget@s representing the result of the token revocation request.

h4. RevokedTarget
* @(TY1)@ @RevokedTarget@ is an object contain a single @TokenRevocationTarget@ that has been revoked with some additional information.
* @(TY2)@ @RevokedTarget#target@ is a @TokenRevocationTarget@ that has been revoked.
* @(TY3)@ @RevokedTarget#issuedBefore@ is a timestamp (in milliseconds since the epoch) representing the time for revoked target, that was issued before this time.
* @(TY4)@ @RevokedTarget#appliesAt@ is a timestamp (in milliseconds since the epoch) representing the time the revocation takes effect for this target
