...
Encryption is supported for the @data@ attribute, or payload, of published "messages":LINKME and "presence messages":LINKME on a channel, using both the REST and realtime interfaces. Decryption is supported for message and presence message subscriptions in the realtime interface, and for both the REST and realtime interfaces when using "history":LINKME.
