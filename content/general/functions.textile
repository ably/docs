...
@Enveloped@ - when the Reactor rule is configured to use an "enveloper":#enveloped, the @HTTP message body@ will contain a bunch of extra attributes, which are the following:
...
* "Follow one of our Reactor Function step-by-step tutorials":/tutorials#functions