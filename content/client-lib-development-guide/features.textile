...
* @(TF2)@ @TokenRevocationRequest#targets@ is an array of @TokenRevocationTarget@s to revoke. For request to be valid, it must contain at least one target. This array of token revocation targets must be used to construct the individual @TokenRevocationTarget@s in the form of "@TokenRevocationTarget#key@:@TokenRevocationTarget#value@" for the "targets" array in the request body.
