...
If subscribing to a channel in delta mode using MQTT or one of the protocol adaptors such as "SSE":/protocols/sse, then you will need to decode any received delta messages yourself. There are decoder libraries available to do this for several platforms; see the "download":LINK-ME section for details.

When subscribing without an Ably library, the channel @delta@ parameter must be specified using a "qualified channel name":/channels/options. 
