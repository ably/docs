...
Stub functions provided by harness (not shown in documentation):
  func loadChannelName() async -> String
  func loadLastProcessedTimestamp() async -> Date
  func processMessage(_ message: ARTMessage)
  func saveLastProcessedTimestamp(_ timestamp: Date)

The translated code is wrapped in `func resumingSessions1() async throws`.
*/}
```swift
// Agent code
import Ably

let ably = ARTRealtime(options: {
...
// Load persisted session state
let channelName = await loadChannelName()
let lastProcessedTimestamp = await loadLastProcessedTimestamp()
...
// Fetch history from the attachment point back to the last checkpoint
var page: ARTPaginatedResult<ARTMessage>? = try await withCheckedThrowingContinuation { continuation in
    do {
        try channel.history(
            {
...
// Paginate through all missed messages, storing in oldest-to-newest order
var missedMessages: [ARTMessage] = []
while let currentPage = page {
    missedMessages.insert(contentsOf: currentPage.items.reversed(), at: 0)

    // Move to next page if available
    if currentPage.hasNext {
        page = await withCheckedContinuation { continuation in
            currentPage.next { result, _ in
                continuation.resume(returning: result)
            }
        }
    } else {
