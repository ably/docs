...
meta_description: "Build a streaming AI chat app whose agent side runs as a Vercel Workflow inside your Next.js app. Each model call and tool is its own retryable WDK step; a retry supersedes the failed attempt on the session, and the user's stream never breaks."
...
intro: "Build a Next.js chat app whose agent side runs as a Vercel Workflow in the same deployment. The workflow opens the run and runs each model call as its own durable WDK step; a retry supersedes the failed attempt on the session, and the user's stream never breaks."
---
...
- The workflow opens the run in one step, then runs each model call and each tool as its own step. Every model response and tool result is published as an AI Transport [step](/docs/ai-transport/concepts/runs#steps), identified by the WDK step id.
...
You do not need a separate workflow server or CLI install. The workflow runtime ships in the `workflow` package and runs inside `next dev`.
...
1. `sendMessage({ text })` publishes the user input on the session and POSTs an [invocation](/docs/ai-transport/concepts/runs#invocations) to `/api/chat`, which starts a WDK workflow.
...
4. Cancels arrive on the session rather than through a workflow API. The in-flight step's own session routes them to [`run.abortSignal`](/docs/ai-transport/api/javascript/core/agent-session#run), which flows into the LLM call.

This guide's happy path publishes each run terminal inside the step that produced it. When a step exhausts its retries instead, the workflow's catch schedules `failRun` to [close the run once](/docs/ai-transport/features/durable-execution#close-once) with `run.end({ reason: 'error' })` so every observer's UI unsticks; `maxRetries = 0` keeps that final write to a single attempt. The catch holds the run's ids only once `openRun` has returned them, so a turn whose `openRun` fails every attempt opened no run to strand.

## Understand the architecture <a id="architecture"/>

Vercel WDK owns execution durability: process crashes, step retries, workflow progress. AI Transport owns conversation state: what appears on the session, how retries reconcile, how clients observe. The [Vercel WDK composition model](/docs/ai-transport/frameworks/vercel-wdk) shows how the two fit together, and [durable execution](/docs/ai-transport/features/durable-execution) generalises the pattern to any workflow engine.
