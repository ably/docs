...
Note: In Swift, responses is captured by the closure and must be declared before the subscribe call.
The ToolCallInfo and ResponseInfo types are helper types defined elsewhere.
...
channel.subscribe { message in
    ...
    case "tool_call":
        if let toolCallId = headers["toolCallId"] as? String,
           let data = message.data as? [String: Any] {
            response.toolCalls[toolCallId] = ToolCallInfo(
    ...
    case "tool_result":
        if let toolCallId = headers["toolCallId"] as? String,
           let data = message.data as? [String: Any] {
            response.toolCalls[toolCallId]?.result = data["result"] as? String
        }
    default:
        break
    }
...
Note: renderWeatherCard is a stub function that would render UI components in your app.
...
Note: getGeolocationPosition() is a stub async function that would use CoreLocation in a real app.
...
Note: PendingToolCall is a helper struct with a responseId property.
processResult is a stub function that would pass the result back to the AI model.
