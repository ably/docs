...
Establishment of a "WebSocket":https://ably.com/topic/websockets transport requires that the client makes a "WebSocket":https://ably.com/topic/websockets or secure "WebSocket":https://ably.com/topic/websockets connection to the Ably "WebSocket":https://ably.com/topic/websockets host (realtime.ably.io or one of the "WebSocket fallback hosts":/client-lib-development-guide#realtime-transports). The client library must allow the client to specify a non-default host or environment, at least to be able to connect to the sandbox environment at sandbox-realtime.ably.io.
