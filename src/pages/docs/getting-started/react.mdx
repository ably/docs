...
![Screenshot of the completed React Pub/Sub application showing a web interface with connection status, a message input field, real-time message display, and a presence indicator showing online users. The interface demonstrates the key features you'll build including publishing messages, subscribing to receive messages in real-time, and tracking which clients are currently present in the channel.](../../../images/content/screenshots/getting-started/pub-sub-react-getting-started-guide.png)
...
![Ably CLI demonstrating Pub/Sub functionality. The demonstration shows multiple terminal windows: one subscribing to a channel to receive messages, another publishing messages to the same channel in real-time, and two additional terminals demonstrating presence functionality where clients enter and leave the channel, with presence events being broadcast to all subscribers.](../../../images/content/screenshots/getting-started/cli-pub-sub.gif)
...
You can also explore the [Ably CLI](https://www.npmjs.com/package/@ably/cli) further, or visit the Pub/Sub [API references](/docs/api/realtime-sdk?lang=react).
