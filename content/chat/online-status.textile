...
Along with determining whether a member is actively online or not, another key piece of functionality in a chat application is to determine when the member was last seen as online. So long as the member calls @enter@ on the presence set for that channel, you'll have a historical record of that entry and a record of when the member left the presence set. Whether it was specifically calling @leave@ or they timed out due to an unstable connection.

To retrieve the channel's presence history, call the channel's presence followed by its history function. Calling @history@ will retrieve up to 1000 records, starting from the latest presence entries. To find the @leave@ events for a specific member, loop through each entry, first filtering out any records by the @clientId@ of your desired member, and the action, which is @leave@. You then need to sort the records, finding the latest timestamp first. Below is an example of how you would implement this functionality:
...
channel.attach(() => {
  ...
  presence.history({}, (err, resultPage) => {
      ...
      if (err) {
        ...
        memberStatus = resultPage.items.filter(
          item => item.clientId === 'MEMBER_CLIENT_ID' && item.action === 'leave'
        );

        memberStatus.sort((recordA, recordB) => {
