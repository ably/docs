...
* @(G4)@ All REST requests and "WebSocket":/concepts/websockets connections to Ably must include the current API version @0.8@. Should any new API version with breaking changes be released, the client library will continue to use the API version explicitly requested
...
* @(RTN1)@ @Connection@ connects to the Ably service using a "websocket":/concepts/websockets connection. The "ably-js library":https://github.ably.io/ably/ably-js supports additional transports such as Comet and XHR streaming; however non-browser client libraries typically use only a "websocket transport":/concepts/websockets
* @(RTN2)@ The default host used for realtime "websocket connections":/concepts/websockets is @realtime.ably.io@, and the following query string params should be used when opening a new connection:
