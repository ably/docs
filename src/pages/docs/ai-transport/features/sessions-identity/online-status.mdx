...
In modern AI applications, agents need to know when users are online, when they've fully disconnected, and how to handle users connected across multiple devices. This allows agents to decide when to continue processing, when to wait for user input, and when to clean up resources. Ably's [Presence](/docs/presence-occupancy/presence) feature provides realtime online status with automatic lifecycle management.

This page shows you how to detect which users and agents are currently connected to a session, distinguish between a single device disconnecting and a user going completely offline, and build responsive online/offline indicators in your applications.
...
Use the [`enter()`](/docs/presence-occupancy/presence#enter) method to signal that a user or agent is online. You can optionally include data when entering presence to communicate additional context.
...
Clients require the [`presence`](/docs/auth/capabilities) capability on the channel to enter presence.
</Aside>
...
<Aside data-type="note">
Clients require the [`presence`](/docs/auth/capabilities) capability on the channel to leave presence.
</Aside>
...
By default, clients remain present for 15 seconds after an abrupt disconnection. This prevents excessive enter/leave events during brief network interruptions. If the client reconnects within this window, they remain in the presence set without triggering leave events.
...
<Aside data-type="note">
For more information, see [Handle unstable connections and failures](https://ably.com/docs/presence-occupancy/presence?lang=javascript#unstable-connections).
</Aside>

For example, if implementing resumable agents using techniques like durable execution, configure a longer `remainPresentFor` period to allow time for the new agent instance to come online and resume processing before the previous instance appears as offline. This provides a seamless handoff:
...
<Aside data-type="note">
Clients require the `subscribe` or `presence-subscribe` [capability](/docs/auth/capabilities) on the channel to retrieve the presence set and subscribe to presence events.
</Aside>

### Retrieving current presence members <a id="get-members"/>

Use [`presence.get()`](/docs/api/realtime-sdk/presence#get) to retrieve the current list of users and agents in the session. This is useful for showing who is currently available or checking if a specific participant is online before taking action.
