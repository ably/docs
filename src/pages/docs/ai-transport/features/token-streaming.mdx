...
The message keeps the content it had at the time of the cancel and its `status` header (under `extras.ai.codec`) transitions to `aborted`. Use this to distinguish a partial response from a complete one.
