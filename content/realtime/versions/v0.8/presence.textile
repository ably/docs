...
In order to be present on a channel, a client must "be identified by having a client ID":https://knowledge.ably.com/authenticated-and-identified-clients-1, "have permission to be present":https://knowledge.ably.com/using-capabilities-to-manage-client-access-privileges-on-channels-1, and be attached to the channel. For simplicity, the library will implicitly attach to a channel when entering or subscribing to presence events. Clients are assigned a <span lang="default">"@clientId@":/realtime/usage#client-id</span><span lang="ruby">"@client_id@":/realtime/usage#client-id</span><span lang="csharp">"@ClientId@":/realtime/usage#client-id</span> when using token authentication, "find out more about token authentication":/core-features/authentication#token-authentication.
...
History provides access to instantaneous "live" history as well as the longer term persisted history for presence channels. If "persisted history":/realtime/history#persisted-history is enabled for the channel, then presence events will "typically be stored for 24 - 72 hours":https://knowledge.ably.com/how-long-are-messages-stored-for-1. If persisted history is not enabled, Ably retains the last two minutes of presence event history in memory.
...
In order to be able to publish presence changes for arbitrary client IDs, the client library must have been instanced either with an "API key":https://knowledge.ably.com/what-is-an-app-api-key-1, or with a "token bound to a wildcard client ID":https://knowledge.ably.com/can-a-client-emulate-any-client-id-i.e.-authenticate-using-a-wildcard-client-id-1.
...
In order to enter and be present on a channel, the client must "be identified by having a client ID":https://knowledge.ably.com/authenticated-and-identified-clients-1, "have permission to be present":https://knowledge.ably.com/using-capabilities-to-manage-client-access-privileges-on-channels-1, and be attached to the channel. For simplicity, the library will implicitly attach to a channel when entering. Entering when already entered is treated as an "update":#update.
...
Gets a "paginated":#paginated-result set of historical presence message events for this channel. If the "channel is configured to persist messages to disk":https://knowledge.ably.com/how-long-are-messages-stored-for-1, then the presence message event history will "typically be available for 24 - 72 hours":https://knowledge.ably.com/how-long-are-messages-stored-for-1. If not, presence message events are only retained in memory by the Ably service for two minutes.
