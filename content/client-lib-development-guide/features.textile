...
** @(RSA17b)@ key to build the request must be retrieved from stored @Auth#AuthOptions#key@
...
** @(RSA17d)@ If the @TokenRevocationRequest#targets@  is valid, then the function must create the request and execute it.
...
*** @(RSA17g1)@ Given a null or empty @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets cannot be null or empty"@ message.
*** @(RSA17g2)@ Given a valid @TokenRevocationRequest#targets@ attribute, verify that the function creates the request and executes it.
*** @(RSA17g3)@ Given a valid @TokenRevocationRequest#targets@ attribute, verify that the function executes the request and returns a @TokenRevocationResponse@ with targets matching with the original targets in request.
...
* @(TF2)@ @TokenRevocationRequest#targets@ is an array of @TokenRevocationTarget@s to revoke. For request to be valid, it must contain at least one target. A test must exist to ensure there is at least one target in the request before it is sent.
* @(TF3)@ @TokenRevocationRequest#issuedBefore@ is an optional timestamp (in milliseconds since the epoch) to revoke tokens issued before this time.
* @(TF4)@ @TokenRevocationRequest#allowReauthMargin@ is an optional boolean that permits a token renewal cycle to take place without needing established connections to be dropped. Default for this value is false if not provided.
