...
Retry behavior varies by platform. For example, AWS Lambda automatically retries failed invocations up to two times with delays of 1 minute, then 2 minutes. Refer to the [webhooks documentation](/docs/platform/integrations/webhooks) for details on retry behavior for your chosen integration. Use the [`[meta]log` channel](/docs/metadata-stats/metadata/subscribe#log) to detect failures (see [monitoring section](#monitoring-performance)).
