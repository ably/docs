...
Add a new state variable `messageIDs` and update the `setupMessages` function in your `ContentView.swift` to retrieve previous messages:
        ...
        let subscription = room.messages.subscribe { event in
            withAnimation {
                ...
                case .created:
                    ...
                    if !messageIDs.contains(event.message.id) {
                        messages.append(event.message)
                        messageIDs.insert(event.message.id)
                    }
                case .updated:
                    if let index = messages.firstIndex(where: { $0.id == event.message.id }) {
                        messages[index] = event.message
