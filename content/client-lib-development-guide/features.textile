...
* @(RSL7)@ @Channel#setOptions@ takes a @ChannelOptions@ object and sets or updates the stored channel options, then indicates success. (For libraries that are not REST-only, completion indication should be done in the same way as @RealtimeChannel#setOptions@, for uniformity; for example, if @RealtimeChannel#setOptions@ takes a callback, @Channel#setOptions@ should as well)
...
** @(RTL4j)@ If the attach is not a clean attach (where a 'clean attach' is an attach attempt where the channel has either not previously been attached or has been explicitly detached since the last time it was attached); for example an automatic reattach triggered by "@RTN15c3@":#RTN15c3 or "@RTL13a@":#RTL13a (non-exhaustive); the library should set the "@ATTACH_RESUME@":#TR3f flag in the @ATTACH@ message
...
** @(TB2d)@ @modes@ (for realtime client libraries only) a typed array of modes, which are the subset of flags defined in "@TR3@":#TR3 whose value is ≥16
