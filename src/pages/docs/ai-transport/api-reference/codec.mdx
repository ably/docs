...
intro: "The codec interface bridges your AI framework and Ably messages. Implement it to integrate any framework with AI Transport."
---
...
2. Decoder. Parse inbound Ably messages back into your event type. Handle the message name conventions used by your encoder. `decode` returns `DecoderOutput<TEvent, TMessage>[]`; wrap your event in an array.
3. Accumulator. Maintain a list of messages, appending tokens to the current message as events arrive and finalising when a terminal event is processed. `createAccumulator()` must return an accumulator object.
...
const myCodec: Codec<MyEvent, MyMessage> = {
  ...
  createDecoder() {
    return {
      decode(message) {
        // Return an array; wrap a single event.
        return [{ event: message.data, messageId: message.id }];
      },
    };
  },
  createAccumulator() {
    ...
    return {
      ...
      seedMessages(seeded) {
        for (const { message } of seeded) messages.push(message);
      },
      completeSeeded(messageId) {
        // Mark a seeded message complete.
      },
      get messages() { return messages; },
      get completedMessages() { return messages; },
      get hasActiveStream() { return false; },
    };
  },
  isTerminal(event) {
