This returns a "ChannelDetails":/realtime/channel-metadata#channel-details for the given channel, indicating global "occupancy":/api/rest-sdk/channel-status#occupancy. A side-effect of this request, in the current version of this API, is that it will cause the channel in question to become activated; therefore it is primarily intended to be used in conjunction with the "enumeration API":#enumeration-rest or in situations where the application has another means to know whether or not a given channel is active.
