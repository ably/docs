...
* a "@token@":/realtime/types#client-options or "@tokenDetails@":/realtime/types#client-options is provided;
* an "@authUrl@":/realtime/types#client-options or "@authCallback@":/realtime/types#client-options is provided that returns a token or token request;
* a "@clientId@":/realtime/types#client-options is provided;
* "@useTokenAuth@":/realtime/types#client-options is true
...
Using our client libraries, a "signed token request is generated":/realtime/auth#create-token-request and handed to client libraries. Our client libraries then use that "signed token request":/realtime/types#token-request to "request a token":/realtime/auth#request-token from Ably and then authenticate with that token. This is the recommended approach for authentication as: a "signed token request":/realtime/types#token-request can be generated securely by your servers without communicating with Ably; your secret API key is never shared with Ably or your clients; signed token requests cannot be tampered with, must be used soon after creation and can only be used once. This process is depicted in the following diagram:
...
Then Ably will be unable to issue a token because the requested capabilities do not intersect with the API key's capabilities i.e. they are entirely incompatible. In the example above, @requestToken@ will call the callback with an error when a token cannot be issued.

h3(#tokens). Tokens

In the documentation, references to tokens typically refer to both "@TokenDetails@":/realtime/types#token-details object that contain the token string or the token string itself. @TokenDetails@ objects are obtained when "requesting tokens":/realtime/auth#request-token from the Ably service and contain not only the token string in the @token@ attribute, but also contain attributes describing the properties of the token.
...
See "understanding capabilities and token security":#capabilities-explained above to get a more thorough overview of how capabilities can be used to secure your application along with working examples.
