...
<Aside data-type='usp'>
Responsive React updates

Ably guarantees [sub-100ms latency](/docs/platform/architecture/latency#how-latency-is-measured) for 99% of published messages globally, so state changes from hooks like `useMembers` and `useCursors` reach your components almost instantly.
</Aside>
