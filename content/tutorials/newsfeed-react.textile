...
In this tutorial, we will see how to use the "Ably Realtime client library":/basics/use-ably to build a realtime news feed with React. Ably enables realtime data sharing using Pub/Sub messaging architecture via a concept called "channels":/channels.
...
We have passed a few "client options":/api/realtime-sdk#client-options when instantiating Ably:
* @key@ is the API key
...
"Channels":/channels are the medium through which messages are distributed; clients attach to channels to subscribe to messages, and every message published to that channel is broadcast by Ably to all its subscribers.

"Presence":/presence-occupancy/presence enables clients to be aware of other clients that are currently “present” on a channel, i.e, connected or online.
