---
title: "Getting started: Chat, React & Vite"
...
* Installing Vite on macOS or Linux.
* Initialising a React + TypeScript project.
* Installing the Ably Chat SDK.
* Setting up tailwind for styling.
* Setting up the Ably and Chat React providers.
...
* Usage of the Ably CLI and custom Chat React hooks.
...
h3(#prerequisites-ably). Ably-CLI
...
Run the following command to install the Ably CLI:

```[sh]
  npm install -g @ably/cli
```

Once installed, you will need to authenticate the CLI with your Ably account, run the following command:
...
<aside data-type='note'>
<p>You will be prompted to enter your "Ably API key":/docs/auth#create. You can find this on your "Ably dashboard":https://ably.com/dashboard.</p>
</aside>

You will then need to set the default app to use with the CLI, run the following command:

```[sh]
  ably apps switch
```

Finally, to select the API key to use with the CLI, run the following command:

```[sh]
  ably auth keys switch
```

h3(#prerequisites-node). Node

Ensure you have recent versions of Node installed if using macOS or Linux.
To do this on macOS (Homebrew), run the following command:

```[sh]
    brew install node
```

Or for Linux (Debian/Ubuntu) use:
...
h3(#prerequisites-vite). Create a new Vite project

Create a new React + TypeScript project with Vite. On macOS or Linux, run:

```[sh]
  npm create vite@latest ably-chat-demo -- --template react-ts
```

or, if you prefer Yarn use:

```[sh]
  yarn create vite ably-chat-demo --template react-ts
```
...
<aside data-type='note'>
<p>The tsconfig files should be set up for you, there is no need to change them.</p>
</aside>

h3(#prerequisites-chat). Chat SDK

Install the Ably Chat SDK in the new project. This will also install the Ably Pub/Sub SDK as it is a dependency:

```[sh]
  npm install @ably/chat
```

Add React (and React DOM) if they are not already installed:

```[sh]
  npm install react react-dom
```

h3(#prerequisites-tailwind). Tailwind

The project will also use tailwind for styling. To install tailwind, run the following command:

```[sh]
  npm install tailwindcss @tailwindcss/vite
```

Now in the @vite.config.ts@ file, make sure you import the tailwindcss plugin and add it to the plugins array:
...
Then, in your @src/App.css@ file, add the following to the top of the file:

```[css]
@import "tailwindcss";
```

Finally, in your @src/App.tsx@ file, ensure you import the CSS file:
...
This enables them to send and receive messages in realtime across channels.
You will need to create a new component component
that uses the "useChatConnection":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useChatConnection.html hook,
exposed by the Ably Chat SDK, to get the current connection status of the Ably Chat client. This hook must be nested within a @ChatClientProvider@, so the component must be placed inside the @ChatClientProvider@ in your app.
...
Now that you have a connection to Ably, you can create a room.
...
* In your project, open @src/App.tsx@, and add the following additional imports, along with a new component called @RoomStatus@:

```[react]
// App.tsx
import { ChatRoomProvider, useRoom, AllFeaturesEnabled } from '@ably/chat';
...
Ably Chat exposes a @useMessages()@ hook to interact with the "messages":./messages feature of the Chat SDK.

After the room provider is set up, you can use the "useMessages()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useMessages.html hook to send and receive messages in the room.

* In your project, open @src/App.tsx@, and add the following imports to those already in place:

```[react]
// App.tsx
import { useState } from 'react';
import { ChatRoomProvider, useMessages } from '@ably/chat';
import { MessageEvent, Message } from '@ably/chat';
```

* Then add a new component called @ChatBox@, like so:
...
To do this, you can extend the functionality of the @ChatBox@ component to allow updating of messages.

The @useMessages()@ hook exposes the "update()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.Messages.html#update method of the Chat SDK "Messages":./messages feature.
...
Ably Chat allows retrieving previously sent messages in a room, this is useful for providing conversational context when a user first joins a room, or when they subsequently rejoin it later on.

The @useMessages()@ hook exposes the "getPreviousMessages()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-js.Messages.html#getPreviousMessages method to enable this functionality. This method returns a paginated response, which can be queried further to retrieve the next set of messages.
...
* In your @src/app.tsx@ file, ensure you add the @useEffect@ and @useState@ imports to your existing imports, then add the following @useEffect@ to your @ChatBox@ component:
...
2. Start your app and you should see the messages appear in the chat box UI.

h2(#step-6). Step 6: Show who is typing a message

The Chat SDK also provides a typing indicator feature that allows you to show when users are typing a message.
The @currentlyTyping@ array from "useTyping()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useTyping.html hook tells you which clients are currently typing, allowing you to render them in the UI. The hook also exposes @start()@ and @stop()@ methods to start and stop typing.

* In your @src/app.tsx@ file, ensure you add the @useTyping()@ hook to your imports:

```[react]
import { useTyping } from '@ably/chat';
```

* You will need to update the existing @ChatBox@ component to include the typing indicator hook, a function to handle text input and then update the @handleSend@ method so typing stops on message send:
...
The Chat SDK also provides a presence feature that allows you to see who is currently present in the room.
You can show a list of everyone who’s currently present in the same room by using the "usePresenceListener()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.usePresenceListener.html hook. You can also @enter@ and @update@ your own presence status using the "usePresence()":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.usePresence.html hook.

* In your @src/app.tsx@ file, ensure you add the hooks to your current imports:
...
* Create a new component called @PresenceStatus@ like so:
...
* In your @src/app.tsx@ file, ensure you import the @useReactions()@ hook:
...
* Then add a new component called @ReactionComponent@, like so:
