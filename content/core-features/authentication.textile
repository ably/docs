...
h3(#development-status). Development status

The revocable tokens feature is currently in Beta.

h3. Revocable tokens

Tokens are revocable if they are created from an API key that has a @revocableTokens@ attribute set to @true@. 

Tokens created from a key with the @revocableTokens@ attribute set to @true@ are restricted to a TTL of 1 hour; an attempt to create, or present, a token that is revocable, but has an TTL of longer than 1 hr, is rejected with an error.
...
Do not expect to be able to enable token revocation on an app, or the @revocableTokens@ attribute of a key, and then revoke a token that's been used since before you made that change. If you want to be able to revoke a token, revocation should have been enabled at the time that the token was issued and first used.
...
```[javascript]
const ablyRest = new Ably.Rest({ key: '{{API_KEY}}' })
const requestBody = {"targets": [ "revocationKey:client1@example.com" ]}                                                                                                                                                                                       
