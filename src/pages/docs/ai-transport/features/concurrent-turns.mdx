...
- A multi-agent setup must let each sub-agent's `createRun` mint its own `runId`; do not pass a shared `RunRuntime.runId` across sub-agents or their messages will collide on the channel. The default minting path (one `crypto.randomUUID()` per `createRun`) is already unique.
