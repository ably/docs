...
When continuity is lost on a Pub/Sub channel, the client receives an `ATTACHED` or [`UPDATE`](/docs/channels/states#update) event with the `resumed` flag set to `false`. This flag is part of the [`ChannelStateChange`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on) object.
