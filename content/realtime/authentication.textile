...
There are several "auth options":/realtime/authentication#auth-options you can specify along with @authUrl@ and @authCallback@:
...
```[jsall]
const ably = new Ably.Realtime({ authUrl: "/auth", authMethod: "POST", authParams: { p1: param1, b: param2}, authHeaders: {h1: header1, h2: header2} });
```
...
const options = {
  ...
  method: "POST",
  headers: {
    "User-Agent": "Realtime Client v0.2",
