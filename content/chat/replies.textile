...
Reply to a message directly with a reference to the original message, or create threads on a message to separate the new branch of messages from the main path of messages. Replies enable users to quickly and directly respond to an existing message. Threads enable users to create a thread for specific a specific focused conversation between two or more chat participants within the channel.
