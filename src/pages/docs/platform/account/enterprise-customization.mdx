...
<Aside data-type='note'>
If you are using an SDK that does not yet support the `endpoint` client option, or are on an older version of an SDK that does, use the [legacy options](#legacy-options) instead.
</Aside>
