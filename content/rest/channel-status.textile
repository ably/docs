...
The @Channel Status API@ is part of the "Channel Metadata API":/realtime/channel-metadata, and provides REST the capability to access information about channels, such as the current state changes of a channel, or the changes in its occupants. This is done in the REST client through two mechanisms:

- "Requesting":/rest/usage#request the "lifecycle events":#lifecycle-events of a single channel.
- "Enumerating":#enumeration-rest all currently active channels, obtaining each channel's current state information.

Using our "Realtime libraries":/realtime, you can subscribe to "channel lifecycle events":#lifecycle-events (such as being created or closed), or subscribe to periodic "occupancy":#occupancy updates for all active channels (such as how many people are subscribed to a channel). Our recommendation is to use the realtime library where possible.
...
Having access to channel status events can provide numerous benefits. In a scenario where the number of subscribers of a channel goes well beyond a hundred, usage of other options such as "presence":/core-features/presence become less effective. You could instead make use of our "channel metadata":#metachannels to check the number of active subscribers, getting accurate values easily.

Equally, you may want to publish your data only if there is a subscriber for that data. Therefore, "channel lifecycle events":/realtime/channel-metadata#lifecycle-events can notify you when a channel becomes active, and can notify you when the last subscriber leaves the channel.
