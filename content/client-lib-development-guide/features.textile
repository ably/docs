...
*** @(RSC7d1)@ In general, the @Agent@ library identifier is composed of a series of @key/value@ entries joined by spaces as described in described by "RFC 7321 Section 5.5.3":https://tools.ietf.org/html/rfc7231#section-5.5.3. This must include at least the @<library name>/<library version>@ entry followed optionally, in dependency order, the @name/version@ entry for underlying library layer or platform layers. An example would be @ably-flutter/1.2.0 ably-java 1.2.1 android/24@. The documentation for each library must define the content and meaning of the @Agent@ identifiers advertised by that library.
