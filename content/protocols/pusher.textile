...
<p>Normally, it is not possible to connect to Ably without a complete API key, or a "token":/docs/auth/tokens derived from one. When the Pusher Adapter is enabled, you can connect to Pusher public channels with only the Ably API "key name":/docs/auth#format. The key name is a combination of the app ID and the key ID.</p>
</aside>
