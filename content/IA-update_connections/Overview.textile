...
what are connections, mention states exist
(repeating box) realtime only
Create a connection
(caveat box) connection is implicit with instantiation but this section cover explicit call to connect + usecase for when to use this option
connect method + code example
Monitor connections to an app
From FAQ
Connection metachannel and limitations of it. This will also cover dev console.
Connection IDs
(caveat box) Connections across multiple browser tabs
Heartbeats
explanation
Code example + ping
Close a connection
explanation, incl 2min delay if not explicitly closed = peak rate + code example
---
...
h3(#getting-started). Getting started: creating a connection

p(caveat box). Connection is implicit with instantiation but this section covers explicit calls to connect and use-cases for when to use this option.
...
p(note). Connections can be monitored from your Ably app dashboard. See the "Ably FAQs":/https://faqs.ably.com/how-do-i-access-my-app-dashboard for more information.
...
p(note). There is a 2 minute delay if a connection not explicitly closed which could impact peak connections and thus costs.
