...
### We should clarify this is only on initialisation
* @(RSC3)@ The log level can be changed
### We should clarify this is via ClientOptions (see e.g. how RSC12 does it)
...
### What's the need for mentioning this API default? Is it for the purposes of documentation? (Applies throughout)
...
### fallbackRetryTimeout missing from IDL
...
### realtimeRequestTimeout is not in IDL
...
### So, shouldn't this have been removed already? Should it go in IDL?
...
### I think should be called RestChannel to match IDL
...
### This should be called RestPresence
...
### The `batch` property isn't described (but it's in the IDL) - and is there a RestBatch and RealtimeBatch?
...
### BatchSpec isn't described - should be done alongside BatchPublishResponse etc
...
### BatchResponse isn't described anywhere
...
### I think should be called RealtimeChannel to match IDL
...
### Should it not conform to EventEmitter? (Possibly not - I think that having a subscribe doesn't necessarily mean you have to conform, see wording of RTE2)
...
### The usage of "no arguments" is a bit confusing, because it's actually referring to a single listener argument
...
### But is this mean to be part of the public interface? Because, if it's intended for internal use, then a. it shouldn't be in the IDL, and b. the spec doesn't explain how it's meant to be used internally.
...
### What is this "and optionally a JSON-encodable object"?
...
### This uses the RestChannel / RealtimeChannel naming I said earlier would be a good idea
...
### I feel like AuthOptions probably only has a few valid combinations of properties - perhaps a Swift enum-like type would be good
h4. AuthOptions
...
```
// This is misnamed - should be RestClient
class Rest:
  // RSC1a describes a _single_ String constructor that’s responsible for deciding if a key or token
  // RSC1b describes this constructor as being able to throw - should that be mentioned here?
...
// Is it consistent with RSH8 that this be IO (in particular, that it be able to fail)? Ditto for Realtime. There's nothing in e.g. iOS that reports errors, that's handled inside the library
  device() => io LocalDevice
  channels: Channels<RestChannel> // RSN1
  request(
// RSC19a says it should be a "valid HTTP verb, must support GET, POST, PUT" etc
    String method,
    String path,
    Dict<String, String> params?,
    JsonObject | JsonArray body?,
// Optional in RSC19a (and correctly written in the RestClient proxy in IDL)
    Dict<String, String> headers
  ) => io HttpPaginatedResponse // RSC19
  // RSC6
  stats(
    // does api-default imply optional? I think we're not explicit about that, we should address
    // the default is not mentioned in feature spec
...
// Should be RealtimeClient? RTC2 etc
class Realtime:
  constructor(keyStr: String) // RSC1
  constructor(tokenStr: String) // RSC1
  constructor(ClientOptions) // RSC1
...
// This should be a function and have a signature and be io
  stats: // Same as Rest.stats, RTC5a
  close() // proxy for RTN12
  connect() // proxy for RTN11
  time() => io Time // RTC6a

// Is the implication that all "Options" types should be user-instantiable?

class ClientOptions:
// the spec doesn't explicitly say "embeds AuthOptions" - and the list of "auth option attributes" in TO3j is longer than the list of AuthOptions
  embeds AuthOptions // TO3j
// should the deprecated logExceptionReportingUrl (TO3m) be here?
  autoConnect: Bool default true // RTC1b, TO3e
// e.g. why does this one not have a default and others have things like "default nil"?
  clientId: String? // RSC17, RSA4, RSA15, TO3a
// this is listed under "auth option attributes" but isn’t in AuthOptions
  defaultTokenParams: TokenParams? // TO3j11
  echoMessages: Bool default true // RTC1a, TO3h
  environment: String? // RSC11b, RSC15b, TO3k1
...
// should this not just be nullable?
  restHost: String default "rest.ably.io" // RSC11a, RSC12, TO3k2
// Shouldn't this be nullable? e.g. because T03K3 says you can't provide this plus `environment`
  realtimeHost: String default "realtime.ably.io" // RTC1d, TO3k3
// RSC15g1 implies it can not be "set" But T03k6 says you disable fallback hosts by providing an empty array 
  fallbackHosts: String[] default nil // RSC15b, RSC15a, TO3k6
// fallbackHostsUseDefault should be here until removed from spec, although deprecated
  recover: String? // RTC1c, TO3i
  tls: Bool default true // RSC18, TO3d
  tlsPort: Int default 443 // TO3k5
  useBinaryProtocol: Bool default true // RSC8b, TO3f
  transportParams: [String: Stringifiable]? // RTC1f
  addRequestIds: Bool default false // TO3p, RSC7c
  ...
  // There's nothing to imply `String?`
...
// RSA8c3 implies these should be in ClientOptions too
// I don't see why authHeaders or authParams need to be nullable
...
// A02h just says "token string"
  token: String? | TokenDetails? | TokenRequest? // RSA4a, RSA4, TO3j2
  tokenDetails: TokenDetails? // RSA4a, RSA4, TO3j3
// What does the "is unspecified" in RSA4 mean? How does that translate to API design?
  useTokenAuth: Bool? // RSA4, RSA14, TO3j4

// So, this class needs a constructor, right? Because you need to be able to create one to use as defaultTokenParams.
class TokenParams: // RSAA8e
// the api-default isn't mentioned anywhere in the feature spec
// TK2b says "when omitted, Ably will default to the capabilities of the underlying key"
  capability: String api-default '{"*":["*"]}' // RSA9f, TK2b
  clientId: String? // TK2c
  nonce: String? // RSA9c, Tk2d
// is this nullable because of what TK2d says about "not validly a member of any default token params"?
  timestamp: Time? // RSA9d, Tk2d
// shouldn't this be optional? (I don't know how languages that don't have a concept of a nullable duration – e.g. Objective-C – would then have to handle this)
  ttl: Duration api-default 60min // RSA9e, TK2a

class Auth:
// Is this meant to be readonly? RSA12 implies it to me. We seem to have a readonly modifier used a couple of places here, but I assume it should be more widespread
...
class RestChannel:
  ...
  history(
    start: Time, // RSL2b1
// api-default not mentioned in spec, also nothing in spec to suggest it's optional
...
// TODO where's params come from? Ah, RTC1f
  publish(Message, params?: Dict<String, Stringifiable>) => io // RSL1
  publish([Message], params?: Dict<String, Stringifiable>) => io // RSL1
// type of `data` not specified in spec
...
// The feature spec (BO1) doesn't mention 
class BatchOperations:
// These 3 should be IO
// We don't have a consistent way of representing overloads, sometimes we use | and sometimes we have multiple signatures
...
// Not clear to me if this should be optional
  responses: []T? // BPA2a
...
// As with PushChannelSubscriptions#list I think we can do better with params here. Is the idea here that we don't allow them to specify name / device ID / client ID?
...
enum ChannelEvent:
// This word "embeds" needs to be clarified throughout the IDL
  embeds ChannelState
  UPDATE // RTL2g
...
// this Params isn't defined anywhere - see description in TB2b2. applies to getDefaultParams arg too
  cipher: (CipherParams | Params)? // RSL5a, TB2b
// according to the below-mentioned spec points, these should only be for realtime
  params?: Dict<String, String> // TB2c
  modes?: [ChannelMode] // TB2d

// CHD1 says that it has the name too?
...
// this can also be a string (base64) - see RSE1c. but TZ2d doesn't mention it
  key: Binary // TZ2d
  keyLength: Int // TZ2b
  mode: String default "CBC" // TZ2c

class Crypto:
// This should be more specifically described according to RSE1b
...
// Spec doesn't mention either of these 2 as being optional.
...
class RealtimePresence:
  ...
  history(
    start: Time, // RTP12a
// this doesn't have api-default, not consistent with RestChannel
    end: Time, // RTP12a
...
// From what I can see (CD1) this is an internal thing and shouldn’t be in the IDL. So, I will ignore it for now
...
class Message:
// These constructors aren't specified in the Message section in the spec (TM*), nor in the linked Ruby documentation. And there isn’t a PresenceMessage one here
...
// These are all readonly in the linked Ruby documentation
// It's not clear where the nullability of any of these has come from. Also seems to not match that of PresenceMessage
...
// Same as above re readonly in Ruby
...
// I don't think this needs to be in the IDL then if it’s just internal? Since it's not describing an interface but really a DTO from the backend. Skipping.
...
// If these flags are part of the public API (Not sure) These should be brought into the IDL so can be documented
...
// I don't think the spec is explicit that it implements EventEmitter, which for contrast it is about RealtimeChannel - see RTL2
class Connection:
...
// RTN13a says that it should indicate the response time (i.e. a Duration)
  ping() => io // RTN13
...
// PCD5 suggests this is [String:String]
  metadata: JsonObject
...
// There's nothing in the spec to say that LocalDevice extends DeviceDetails
class LocalDevice extends DeviceDetails:
// RSH8c implies this should be nullable
...
// The signature of Push#activate/deactivate is only alluded to in the spec, by instructions to call these callbacks. There's no mention of how to use the ErrorInfo param of registerCallback.

// Also, I don't think activate and deactivate are meant to be => io. Ditto I don't understand why the callbacks are - I believe they’re just meant to be informative, there’s no details of how to behave if they error
  activate(
...
class PushDeviceRegistrations:
  // This (with DeviceDetails arg) not mentioned in RSH1b1
  get(DeviceDetails) => io DeviceDetails // RSH1b1
  get(deviceId: String) => io DeviceDetails // RSH1b1
  // I think we can be more specific here with the params
  list(params: Dict<String, String>) => io PaginatedResult<DeviceDetails> // RSH1b2
// so a DeviceDetails is something you're meant to be able to manipulate yourself?
  save(DeviceDetails) => io DeviceDetails // RSH1b3
  // as with `get`, this DeviceDetails variant not mentioned in RSH1b4
  remove(DeviceDetails) => io // RSH1b4
  remove(deviceId: String) => io // RSH1b4
  // as with `list`, I think we can be more specific here with the params
  removeWhere(params: Dict<String, String>) => io // RSH1b5

class PushChannelSubscriptions:
  // as above, can be more specific with params
  list(params: Dict<String, String>) => io PaginatedResult<PushChannelSubscription> // RSH1c1
  // params should probably be consistent with the above `list`, including being more restrictive and also the nullability
  listChannels(params: Dict<String, String>?) => io PaginatedResult<String> // RSH1c2
// as with DeviceDetails, this means a PushChannelSubscription should be user-manipulable?
  save(PushChannelSubscription) => io PushChannelSubscription // RSH1c3
  remove(PushChannelSubscription) => io // RSH1c4
// again, I think we can be more specific with the params
...
// what's with the ... in all these? I don't think should be there
// overload or optional arg? see this question elsewhere here
  on((Data...) ->) // RTE4
  on(Event, (Data...) ->) // RTE4
  once((Data...) ->) // RTE4
...
class VCDiffDecoder
// This interface isn’t properly defined anywhere, the closest is PC3 which alludes to it and describes the `base` argument
