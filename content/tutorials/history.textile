...
bq. You can now persist the last realtime message on a channel for 365 days. *Persist last message* allows you to instantly retrieve realtime messages sent in the past, helping with long-term state synchronization. Read how to enable "persist last message":/channels/channel-parameters/rewind#persist-last-message.
...
blang[ruby].
  ...
  As we do not need asynchronous access to the realtime API for this tutorial, we'll be using the simpler "REST client library":/basics/use-ably. However, in some cases where users want to subscribe to events asynchronously from Ruby, we recommend the use of the "Realtime client library":/basics/use-ably.
...
1. If you would like to find out more about how to use the history API from your devices and apps, see the "Realtime history documentation":/storage-history/history. Typically on servers customers prefer to use the "REST history API":/storage-history/history
