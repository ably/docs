...
The realtime interface is implemented using an Ably-defined protocol, primarily over "WebSockets":https://ably.com/topic/websockets. It enables clients to establish and maintain a persistent connection to Ably. The realtime interface is most commonly used client-side. It is used to:
...
The REST interface communicates with the Ably service using the HTTP protocol and is effectively stateless. The REST interface provides a convenient way to access the "REST HTTP API":#rest-api and is intended to be used by clients that don't require realtime updates. It is more commonly used server-side. Uses include: 

* publishing messages on behalf of other clients
* issuing tokens on behalf of other realtime clients
* retrieving persisted messages, presence history and application usage statistics
...
Ably provides protocol adapters for "Pusher":/tutorials/pusher-adapter and "PubNub":/tutorials/pubnub-adapter. These protocol adapters only require you to change your API key. This enables you to migrate from Pusher or PubNub incrementally, or all at once.
