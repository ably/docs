...
|-------------|-------------------|---------|
| Client-side (browsers, mobile apps) | [Token authentication](/docs/auth/token) | Your server issues tokens ([JWT](/docs/auth/token/jwt) recommended); clients use `authCallback` to fetch them |
...
1. Click the **API Keys** section in your [dashboard](https://ably.com/accounts/any/apps/any/app_keys).
...
|----------|-------------|---------|
| Client-side (browsers, mobile) | JWT | Use [JWT authentication](/docs/auth/token/jwt). No Ably SDK needed on your server |
...
| Channel-scoped user claims | JWT only | Embed trusted metadata via [channel-scoped claims](/docs/auth/token/jwt#channel-claims) |
| Per-connection rate limits | JWT only | Restrict publish rates via [rate limit claims](/docs/auth/token/jwt#rate-limits) |
| Large capability list or confidential capabilities | Ably Token | JWTs have size limits and can be decoded by clients. Use [Ably Tokens](/docs/auth/token/ably-tokens) |
