...
intro: "Internal mechanics of the conversation tree: serial ordering, sibling groups, fork chains, and the flatten algorithm that projects a branch into a list."
---
...
A conversation with an edit looks like:

```
root
  user "What is the budget?"  (M1)
    assistant "The budget is..."  (M2)
      user "Now focus on Q4"  (M3)
        assistant "Q4 specifically..."  (M4)
      user "Now focus on Q4 only"  (M3', fork-of M3)
        assistant "Q4 focus..."  (M4')
```

The view selects one of the sibling pairs at the M3 fork point and flattens the chosen branch into a linear list for rendering.
...
- [Conversation branching](/docs/ai-transport/features/branching) - the user-facing edit, regenerate, and branch-navigation feature.
