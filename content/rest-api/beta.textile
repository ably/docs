...
where:
* @<channel names>@ is a @String@ of channel names seperated by commas, or a @String@ of channel names seperated by your specified @channelSeparator@.

Below is an example of a curl request which uses a @channelSeperator@:

```[json](code-editor:rest/batch-presence)
curl -X GET https://rest.ably.io/presence?channel,1!channel,2=&channelSeperator=! \
    -u "{{API_KEY}}"
```

h4(#batch-presence-response). Batch reponses
