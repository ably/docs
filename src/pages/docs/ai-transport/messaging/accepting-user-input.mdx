...
func example(
    ...
    processAndRespond: @escaping (ARTRealtimeChannel, String, String, String) -> Void
) {
    // --- example code starts here ---
*/}
```swift
...
func example(
    channel: ARTRealtimeChannel,
    processAndRespond: @escaping (ARTRealtimeChannel, String, String) -> Void
) {
    // --- example code starts here ---
*/}
```swift
channel.subscribe("user-input") { message in
    let role = (message.extras as? [String: Any])?["userClaim"] as? String
    // promptId is a user-generated UUID for correlating responses
...
func example(
    ...
    prompt: String,
    promptId: String,
    generateAIResponse: @escaping (String, @escaping (String) -> Void) -> Void
) {
    // --- example code starts here ---
*/}
```swift
// Generate the response (e.g., call your AI model)
generateAIResponse(prompt) { response in
    // Publish the response with the promptId for correlation
    ...
    channel.publish([message]) { error in
        if let error = error {
            print("Error publishing response: \(error.message)")
        }
    }
}
```
...
*/}
```swift
var pendingPrompts: [String: [String: Any]] = [:]

// Send a prompt and track it
func sendPrompt(text: String) -> String {
    let promptId = UUID().uuidString
    pendingPrompts[promptId] = ["text": text]
    channel.publish("user-input", data: ["promptId": promptId, "text": text])
    return promptId
}

// Handle responses
channel.subscribe("agent-response") { message in
    let promptId = ((message.extras as? [String: Any])?["headers"] as? [String: Any])?["promptId"] as? String

    if let promptId = promptId, let originalPrompt = pendingPrompts[promptId] {
        let text = originalPrompt["text"] as? String ?? ""
        print("Response for \"\(text)\": \(message.data ?? "")")
...
func example(
    ...
    prompt: String,
    promptId: String,
    generateTokens: any AsyncSequence<String, Never> & Sendable
) {
    // --- example code starts here ---
*/}
```swift
...
channel.publish([initialMessage]) { result, error in
    if let error = error {
        print("Error publishing initial message: \(error.message)")
        return
    }

    guard let serial = result?.serials.first?.value else {
    ...
    Task {
        // Stream tokens by appending to the message
        for await token in generateTokens {
            let appendMessage = ARTMessage(name: nil, data: token)
            appendMessage.serial = serial
            ...
            channel.append(appendMessage, operation: nil, params: nil) { _, error in
                if let error = error {
                    print("Error appending token: \(error.message)")
                }
            }
        }
    }
...
func example(
    channel: ARTRealtimeChannel,
    streamResponse: @Sendable @escaping (ARTRealtimeChannel, String, String) async -> Void
) {
    // Agent handling multiple concurrent prompts
    actor ActiveRequests {
        var requests: [String: [String: Any]] = [:]

        func set(_ promptId: String, _ value: [String: Any]) {
            requests[promptId] = value
        }
...
channel.subscribe("user-input") { message in
    ...
    Task {
        // Track active request
        await activeRequests.set(promptId, [
            "userId": userId,
            "text": text
        ])
        ...
        defer {
            Task {
                await activeRequests.remove(promptId)
            }
        }
