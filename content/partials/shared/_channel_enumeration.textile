...
- by := _value_ optionally specifies whether to return just channel names (@by=id@) or "ChannelDetails":/api/realtime-sdk/channel-metadata#channel-details (@by=value@ is the default, but if you do not actually need the extra information you get with that, we strongly recommend you use @by=id@, which will be faster)
