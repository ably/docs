...
* @(RSC15)@ Host Fallback
** @(RSC15a)@ All HTTP requests sent to Ably are fault-tolerant and will fallback to an alternative host name if the default host endpoint is unreachable or indicates that it is unserviceable with a 500 to 504 status code. This ensures that a client library is able to work around routing or other problems with the user's closest data centre. For example, if a @POST@ request to @rest.ably.io@ fails because the default endpoint is unreachable or unserviceable, then the @POST@ request should be retried again against the fallback hosts in attempt to find an alternate healthy data centre to service the request. The fallback hosts are @[a-e].ably-realtime.com@ and should be attempted in random order, with a maximum number of fallback attempts being the configured @httpMaxRetryCount@ (see "@TO3l5@":#TO3l5)
** @(RSC15b)@ This fallback behaviour only applies to the default @rest.ably.io@ endpoint; if either @host@ or @environment@ options are set resulting in the endpoint not being @rest.ably.io@, no fallback host is used
** @(RSC15d)@ Automatic retries to fallback hosts will not occur for any HTTP status codes not in the range 500 to 504. For example, if the credentials are invalid (a 401 status code) or the maximum message length has been exceeded, then resending the request to an alternative data centre will not fix the problem and will only increase the load on other data centres. See the "complete list of Ably error codes":https://github.com/ably/ably-common/blob/master/protocol/errors.json#L11
** @(RSC15e)@ Every new HTTP operation is always first attempted to the primary host, and then, only upon "an appropriate request failure":#RSC15a, attempted against the fallback hosts when "RSC15b":#RSC15b applies. This ensures that should the closest data centre become available at a later time, the client library will automatically resume using the primary data centre at @rest.ably.io@
...
* @(RTN17)@ Host Fallback
** @(RTN17a)@ All requests sent to Ably are fault-tolerant and will fallback to an alternative host if the default host is unreachable or indicates that it is unserviceable with a 500 to 504 status code. This ensures that a client library is able to work around routing or other problems with the user's closest data centre. For example, if a connection attempt to @realtime.ably.io@ fails because the default endpoint is unreachable or unserviceable, then the @Connection@ manager should check if an internet connection is available by sending a request to our CDN @internet-up.ably-realtime.com/is-the-internet-up.txt@ and looking for the text "yes" in the response.  If the text "yes" is included in the response, then the client library can assume it has a viable internet connection and should then immediately retry the connection against the fallback hosts in attempt to find an alternate healthy data centre to connect to. The fallback hosts are @[a-e].ably-realtime.com@ and should be attempted in random order
** @(RTN17b)@ The fallback behaviour only applies to the default @realtime.ably.io@ endpoint; if either @host@ or @environment@ options are set resulting in the host endpoint not being @realtime.ably.io@, no fallback host is used
** @(RTN17c)@ If the realtime client is currently connected to a fallback host endpoint, then for the duration that the transport is open to that host, all HTTP requests, such as history or token requests, should be first attempted to the same data centre the realtime connection is established with i.e. the same fallback host must be used as the default host. If however the HTTP request against that fallback host fails, then the normal fallback host behaviour should be followed attempting the request against another fallback host as described in "RSC15":#RSC15
