...
You can send, update, and delete messages in a chat room using the Ably REST API. This is useful when you need to send messages from a server or other backend service, or when operating in an environment where the Ably SDKs are not available.

h2(#common-rest-params). Common REST parameters

The following parameters are common to all REST requests:
