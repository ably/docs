...
Returns a promise. The promise is fulfilled with the new or existing [`Room`](/docs/chat/api/javascript/room) instance, or rejected with an `ErrorInfo` object. The promise is also rejected if the room already exists with different options, or if [`release()`](#release) is called before `get()` resolves.
