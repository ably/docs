...
When multiple members change location simultaneously, all subscribers observe those updates in the [same order](/docs/platform/architecture/message-ordering#practical-implications-of-dual-ordering), so everyone has a consistent view of where each member is.
</Aside>
