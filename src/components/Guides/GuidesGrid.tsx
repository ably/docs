...
const GuidesGrid = ({
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,_minmax(260px,_1fr))] gap-x-5 gap-y-8">
      {guides.map(({ id, name, description, link, products, aiProvider }, key) => (
        <a
          href={link}
        ...
        >
          ...
          <div className="z-10 pt-4">
            ...
            <div className="mt-4 flex gap-x-1">
              ...
              {aiProvider && aiProviders[aiProvider] ? (
                <Badge key={aiProvider} className="uppercase text-neutral-800">
                  {aiProviders[aiProvider].label}
                </Badge>
              ) : null}
            </div>
          </div>
