---
title: "Zapier: Build an IoT realtime attendance tracker system for Slack"
alt_title: "Reactor Events Tutorial: IoT based realtime attendance system for Slack using Zapier"
...
The Ably Reactor provides a way to trigger events and to stream data from Ably's pub/sub channels. In this tutorial, we'll see how to build an IoT based realtime attendance system that sends a message to a Slack channel upon detecting an access card. We'll use "Ably's MQTT adapter":/mqtt to communicate with the IoT device and Zapier to send messages to Slack.
