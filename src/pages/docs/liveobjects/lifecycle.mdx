...
<If lang="java">
`on(...)` returns a `Subscription`; keep it and call `subscription.unsubscribe()` to stop listening (calling it more than once is a no-op), or `channel.object.offAll()` to remove all listeners. (`ObjectStateEvent` lives in `io.ably.lib.liveobjects.state`.)
</If>
...
if (instance != null) {
    LiveCounterInstance counterInstance = instance.asLiveCounter();
