...
The Realtime library, when instanced, establishes a "WebSocket":https://www.ably.io/topic/websockets connection to Ably. This "WebSocket":https://www.ably.io/topic/websockets connection is both multiplexed, allowing multiple channels to share a single connection, and bi-directional, allowing realtime messages to be sent both to and from the Ably service without having to fall back to inefficient techniques like "HTTP polling":https://www.ably.io/topic/long-polling.
...
- Connection state management := Actively managing the "WebSocket":https://www.ably.io/topic/websockets connection by reconnecting "automatically to an available datacenter":https://knowledge.ably.com/routing-around-network-and-dns-issues-1 when a connection drops and restoring the connection state. Find out more about "the connection object and its state":/realtime/connection and "connection state recovery which provides message continuity over changing network conditions":https://knowledge.ably.com/connection-state-recovery-1.
- Channel management := Providing "publish and subscribe":/realtime/channels capabilities over channels and actively managing them by queueing published messages when disconnected and "retrieving missed messages":https://knowledge.ably.com/connection-state-recovery-1 once reconnected. The library proactively reattaches channels that become suspended due to long periods of disconnectedness.
