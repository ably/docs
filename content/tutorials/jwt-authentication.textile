...
last_updated: '2022-07-25T17:24:15+00:00'
languages:
- javascript
- nodejs
level: medium
...
With Ably, you must send the time-to-live (@expiresIn@) as well as the @keyId@ as part of the @jwtOptions@ parameter. You can optionally configure capabilities for the client using this token or a client ID, these will be part of the payload. Add the following code:
...
In the front-end HTML we call the authUrl with a querysting variable, `/auth?clientId={username}:{password}`, which we access with the Express Request object. The parameters of the GET query are exposed by the @Request.query@ collection. In our example the users input will be available as @req.query.clientId@
