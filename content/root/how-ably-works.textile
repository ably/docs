...
Ably's global realtime service enables Internet enabled devices, such as a browser, phone, server or IoT sensor, to stream data in realtime between to any other Internet connected device in milliseconds. The Ably platform brings enterprise scale messaging to developers by delivering a "highly-available service":http://support.ably.io/solution/articles/3000029531, "message delivery guarantees":http://support.ably.io/solution/articles/3000044640 and "low-latencies globally":http://support.ably.io/solution/articles/3000044625 (typically less than 60ms).
...
* On disk in three locations for 24 - 72 hours if the "channel is configured to persist messages":https://support.ably.io/support/solutions/articles/3000030057
...
- "Connection state recovery":https://support.ably.io/support/solutions/articles/3000044639 := ensures that clients disconnected abruptly from Ably can resume their connection
- "Proactive health checked DNS":https://support.ably.io/support/solutions/articles/3000044636 := our DNS TTL is kept very low allowing us to route traffic away from unhealthy datacenters in 60s once our monitoring systems detect an issue
- "Secondary domain endpoints":https://support.ably.io/solution/articles/3000044636#fallback := we operate a completely isolated backup domain that is used by our client libraries if the primary @ably.io@ endpoint is failing
- "Host fallback":https://support.ably.io/solution/articles/3000044636#fallback := clients that are unable to connect to their closest datacenter using the primary @ably.io@ domain will automatically fallback to an alternative datacenter using the secondary domain
- "Auto scaling":https://support.ably.io/solution/articles/3000045691:= as load on the system increases, more servers are automatically provisioned in every part of the system that requires additional capacity
- "Load balancers":https://support.ably.io/solution/articles/3000045691 := our load balancers are elastic and scale to meet demand, but are also responsible for intelligently routing traffic to existing and new frontends that are coming online
...
- "Self-healing cluster":https://support.ably.io/solution/articles/3000045692 := as problems are detected in the system, they are isolated or remedied by our automated health servers
