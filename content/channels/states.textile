...
Normally, errors in attaching to a channel are communicated through the "@attach()@":/api/realtime-sdk/channels#attach callback. For implicit attaches, where a client only calls @subscribe()@, there is no callback.

In these instances, if you want to know what happens, you'll need to listen for "channel state changes":#listen. This is also true in other cases where a channel is attached or re-attached automatically, for example, following the library reconnecting to Ably after a period of time in the @suspended@ state.
