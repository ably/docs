...
```[javascript](code-editor:realtime/channel-publish)
  ...
  channel.publish('example', 'message published successfully');
```
...
bc[jsall]. var options = { encrypted: true, cipherParams: { key: 'privateKey' } };
var channel = realtime.channels.get('channelName', options);

bc[java]. import io.ably.lib.realtime.Channel;
import io.ably.lib.types.ChannelOptions;
import io.ably.lib.util.Crypto.CipherParams;
CipherParams params = Crypto.getDefaultParams("privateKey");
ChannelOptions options = new ChannelOptions();
options.encrypted = true;
options.cipherParams = params;
Channel channel = realtime.channels.get("channelName", channelOpts);

bc[ruby]. options = { encrypted: true, cipherParams: { key: 'privateKey' } }
...
Key or token capabilities can also specify access rights based on channel namespace, find out more about "authentication":/general/authentication
...
bc[java]. import io.ably.lib.realtime.CompletionListener;
...
Channels expose a @history@ method whereby a client can obtain messages previously sent on the channel. It provides access to instantaneous "live" history as well as the persisted history for attached channels.

This live history provides the ability to obtain continuous message history up to the point that a realtime channel was attached and also supports recent history of up to around 2 minutes for channels that do not have persistence enabled. The persisted history allows clients to retrieve older messages that are no longer stored in memory. Persisted history is "typically kept for 24 - 72 hours":https://support.ably.io/solution/articles/3000030059-how-long-are-messages-stored-for.

The following example retrieves all historical messages published up until the point the channel was attached from the paginated history.
  ...
  channel.history({ untilAttach: true }, function(err, resultPage) {
    if(err) {
      ...
      if(resultPage.hasNext()) {
        resultPage.next(function(err, resultPage) { ... });
      }
  });
});

bc[java]. import io.ably.lib.types.Message;
import io.ably.lib.types.PaginatedResult;
PaginatedResult<Message> resultPage = channel.history(null);
System.out.println(messages.items().length + " messages received in first page");
if(messages.hasNext()) {
  resultPage = resultPage.next();
  System.out.println(...);
}
...
The current <span lang="java">"@io.ably.lib.realtime.ChannelState@":#channel-state @state@</span><span lang="ruby">"@Ably::Realtime::Channel::STATE@":#channel-state @state@</span><span lang="default">"@ChannelState@":#channel-state</span> of this @Channel@. See the supported "channel states":#channel-states for more information.
...
When a channel failure occurs this <span lang="default,ruby">attribute</span><span lang="java">member</span><span lang="jsall">property</span> contains the <span lang="java,ruby">"@AblyException@":/realtime/types#ably-exception</span><span lang="jsall">"@ErrorInfo@":/realtime/types#error-info</span>.
...
- <div lang="default">String name</div> := event name for the published message

- <div lang="jsall">Object data</div> := data payload for the message. The supported payload types are Strings, JSON objects and arrays, buffers containing arbitrary binary data, and null.
- <div lang="java">Object data</div> := data payload for the message. The supported payload types are Strings, JsonObject, binary data as byte arrays, and null.
- <div lang="ruby">Object data</div> := data payload for the message. The supported payload types are Strings, Hash or Array objects that can be serialised to JSON using @to_json@, binary data as @ASCII-8BIT@ byte arrays, and null.

- <span lang="default">"Message":#message[] messages</span> := An array of message objects to publish

- <div lang="jsall">callback</div> := is a function of the form: @callback(err)@
...
blang[ruby].
  ...
  On successful publish of the message, the registered success callbacks for the "@Deferrable@":/realtime/types#deferrable and any block provided to the method yield. On failure to publish the message, the @errback@ callbacks of the "@Deferrable@":/realtime/types#deferrable are called with an "@ErrorInfo@":#error-info argument describing the failure reason.
...
bq(definition#subscribe-listener).
  default:  subscribe(callback("Message":#message))
...
bq(definition#subscribe-event).
  default: subscribe(String name, callback("Message":#message))
...
bq(definition#subscribe-event-array).
  default: subscribe(String[] names, callback("Message":#message))
...
- <div lang="jsall">callback</div> := is a function of the form @callback(message)@ to be called for each message
...
- <div lang="java">"MessageListener":#message-listener listener</div> := previously registered listener
- <div lang="ruby">&listener_block</div> := previously registered listener block

h6(#history). history

bq(definition).
  default: history(Object params, callback("ErrorInfo":/realtime/types#error-info err, "PaginatedResult":#paginated-result<"Message":#message> resultPage))
  ruby:    "Deferrable":/realtime/types#deferrable history(Hash options) -> yields "PaginatedResult":#paginated-result<"Message":#message>
  java:    public "PaginatedResult":#paginated-result<"Message":#message> history(Param[] params)

Gets a paginated set of historical messages for this channel. If the "channel is configured to persist messags to disk":https://support.ably.io/support/solutions/articles/3000030059-how-long-are-messages-stored-for-, then message history will "typically go back 24 - 72 hours":https://support.ably.io/solution/articles/3000030059-how-long-are-messages-stored-for. If not, messages are only retained in memory for two minutes.

h4. Parameters

- <div lang="default">params</div><span lang="java">Param[] </span> := an optional object containing the query parameters, as specified in the "history API documentation":/realtime/history.
- <div lang="ruby">options</div> := an optional set of key value pairs containing the query parameters, as specified in the "history API documentation":/realtime/history.
...
Further details of the supported query params are in the "history API documentation":/realtime/history.
...
blang[ruby].
  ...
  A "@Deferrable@":/realtime/types#deferrable object is returned from the message method.

  On success, the registered success callbacks for the "@Deferrable@":/realtime/types#deferrable and any block provided to the method yields a "PaginatedResult":#paginated-result that encapsulates an array of "@Message@":#message objects corresponding to the current page of results. "@PaginatedResult@":#paginated-result supports pagination using "@next()@":#paginated-result and "@first()@":#paginated-result methods.
...
bq(definition).
  ...
  java:     public void attach()
  ruby:     "Deferrable":/realtime/types#deferrable attach -> yields

Attach to this channel ensuring the channel is created in the Ably system and all messages published on the channel will be received by any channel listeners registered using @subscribe()@. Any resulting channel state change will be emitted to any listeners registered using the "@on@":#on or "@once@":once methods.

As a convenience, @attach()@ will be called implicitly if either "@publish()@":#publish or "@subscribe()@":#subscribe for the @Channel@ is called, or "@enter@":/realtime/presence#enter or "@subscribe@":/realtime/presence#subscribe is called on the "@Presence@":/realtime/presence for this @Channel@.

h4.
  jsall: Parameters
  ruby:  Parameters

- <div lang="jsall">callback</div> := is a function of the form @callback(err)@ and is called once the channel attach succeeds or fails
...
p(tip). Once all clients globally have detached from the channel, the channel will be released in the Ably service within two minutes.

Detach from this channel. Any resulting channel state change will be emitted to any listeners registered using the "@on@":#on or "@once@":once methods.

h4.
  jsall: Parameters
  ruby:  Parameters

- <div lang="jsall">callback</div> := is a function of the form @callback(err)@ and is called once the channel detach succeeds or fails
...
blang[ruby].
  ...
  On successful detachment, the success callbacks for the "@Deferrable@":/realtime/types#deferrable are called and any block provided to the method is yielded. Failure to detach will trigger the @errback@ callbacks of the "@Deferrable@":/realtime/types#deferrable with an "@ErrorInfo@":#error-info object with the failure reason.
...
bq(definition#on-state-listener).
  ...
  java:    public void on("ChannelState":#channel-state state, "ChannelStateListener":#channel-state-listener listener)

Register the given listener <span lang="ruby">block</span> for the specified "ChannelState":#channel-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span> changes on the @Channel@.
<span class="default">The listener is passed the current state<span lang="jsall"> as the @event@ property of @this@</span>.</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span>.

bq(definition#on-listener).
  default: on(listener())
  ruby:    on -> yields "ChannelStateChange":#channel-state-change
  java:    public void on("ChannelStateListener":#channel-state-listener listener)

Register the given listener <span lang="ruby">block</span> for all "ChannelState":#channel-state changes on the @Channel@.
<span class="default">The listener is passed the current state<span lang="jsall"> as the @event@ property of @this@</span>.</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span>.

h4. Parameters

- <div lang="jsall">String state</div> := the channel state to subscribe to, see "ChannelState":#channel-state for supported channel states
- <div lang="java">"ChannelState":#channel-state state</div> := the channel state to subscribe to
- <div lang="ruby">"ChannelState":#channel-state state</div> := the channel state as a Symbol such as @:attached@ or a @ChannelState@ object to subscribe to

- <div lang="jsall">listener</div> := is a function of the form @listener()@ to be notified for matching state changes. The current state is available as @this.event@
- <div lang="java">"ChannelStateListener":#channel-state-listener listener</div> := listener to be notified for matching state changes
...
bq(definition#once-state-listener).
  ...
  java:    public void once("ChannelState":#channel-state state, "ChannelStateListener":#channel-state-listener listener)

Register the given listener <span lang="ruby">block</span> for a single occurence of the specified "ChannelState":#channel-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span> change on the @Channel@. Once the listener has been called, it is removed as a registered listener and will not be called again.
<span class="default">The listener is passed the current state<span lang="jsall"> as the @event@ property of @this@</span>.</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span>.

bq(definition#once-listener).
  default: once(listener())
  ruby:    once -> yields "ChannelStateChange":#channel-state-change
  java:    public void once("ChannelStateListener":#channel-state-listener listener)

Register the given listener <span lang="ruby">block</span> for a single occurence of any "ChannelState":#channel-state change on the @Channel@. Once the listener has been called, it is removed as a registered listener and will not be called again.
<span class="default">The listener is passed the current state<span lang="jsall"> as the @event@ property of @this@</span>.</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span>.
...
- <div lang="jsall">listener</div> := is a function of the form @listener()@ to be notified for a single occurence of a matching state change. The current state is available as @this.event@
- <div lang="java">"ChannelStateListener":#channel-state-listener listener</div> := listener to be notified for a single occurence of a matching state change
...
bq(definition#off-state-listener).
  ...
  java:    public void off("ChannelState":#channel-state state, "ChannelStateListener":#channel-state-listener listener)

Remove the given listener <span lang="ruby">block</span> for the "ChannelState":#channel-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span>.

bq(definition#off-listener).
  default: off(listener)
  ruby:    off(&block)
  java:    public void off("ChannelStateListener":#channel-state-listener listener)

Remove the given listener <span lang="ruby">block</span> for all "ChannelState":#channel-state states.

h4. Parameters

- <div lang="jsall">String state</div> := the channel state to unsubscribe from
- <div lang="java">"ChannelState":#channel-state state</div> := the channel state to unsubscribe from
- <div lang="ruby">"ChannelState":#channel-state state</div> := the channel state as a Symbol such as @:detached@ or a @ChannelState@ object to unsubscribe from

- <div lang="jsall">listener</div> := is the listener function to be removed
- <div lang="java">"ChannelStateListener":#channel-state-listener listener</div> := is the listener to be removed
