...
Apple provides a sandbox endpoint for testing push notifications. This ensures device IDs in that environment do not work in production.
...
However, [publishing a push notification via a channel](/docs/push/publish#via-channels) activates that channel. Therefore, your concurrent (peak) channel count includes the number of channels you simultaneously publish to.
...
If you're unable to [publish push notifications to channels](/docs/push/publish#via-channels), this is typically due to configuration or permission issues. The following are possible reasons for this issue:
