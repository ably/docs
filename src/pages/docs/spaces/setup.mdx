...
If a client reconnects within two minutes, Ably guarantees full [connection recovery with exactly-once delivery](/docs/platform/architecture/idempotency#connection-recovery-and-exactly-once-delivery), so no space events are lost or duplicated during brief disconnections.
</Aside>
