...
* @(RSA17)@ @Auth#revokeTokens@ function: This function is intended to be used to revoke tokens using provided @TokenRevocationRequest@ details. It internally creates request for  "REST revokeTokens endpoint":/rest-api/#revokeTokens and handles the response.
** @(RSA17a)@ Method signature is @revokeTokens(TokenRevocationRequest)@.
** @(RSA17b)@ If the @TokenRevocationRequest#key@ attribute is not provided, then the function must throw an error with a @"Key is not provided"@ message.
** @(RSA17c)@ If the @TokenRevocationRequest#targets@ attribute is null or invalid (as per @TF3@ ), then the function must throw an error with a @"targets are invalid"@ message.
** @(RSA17d)@ If the @TokenRevocationRequest#key@ is provided and @TokenRevocationRequest#targets@ valid, then the function must create the request and execute it.
** @(RSA17e)@ Function returns a @TokenRevocationResponse@ object that contains the result of the request if request is successful.
** @(RSA17f)@ Function throws an appropriate error/exception  if the request fails either for the reasons of invalid request, or as  a result of request failing.
** @(RSA17g)@ Tests in this section must exist in client library.
*** @(RSA17g1)@ Given a null @TokenRevocationRequest#key@ attribute, verify that the function throws an error with a @"Key is not provided"@ message.
*** @(RSA17g2)@ Given a null @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets are invalid"@ message.
*** @(RSA17g3)@ Given an empty @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets are invalid"@ message.
*** @(RSA17g4)@ Given a non-null @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function creates the request and executes it.
*** @(RSA17g5)@ Given a non-null but invalid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that client library throws an API error.
*** @(RSA17g6)@ Given a valid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function executes the request and returns a @TokenRevocationResponse@ with targets matching with the original targets in request.
*** @(RSA17g7)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the future for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
*** @(RSA17g8)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the past > 1 hour for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
...
* @(TF2)@ @TokenRevocationRequest#key@ attribute is a string representing key to revoke (appId.Key).
* @(TF3)@ @TokenRevocationRequest#targets@ is an array of @TokenRevocationTarget@s to revoke. For request to be valid, it must contain at least one target. A test must exist to ensure there is at least one target in the request before it is sent.
* @(TF4)@ @TokenRevocationRequest#issuedBefore@ is an optional timestamp (in milliseconds since the epoch) to revoke tokens issued before this time.
* @(TF5)@ @TokenRevocationRequest#allowReauthMargin@ is an optional boolean that permits a token renewal cycle to take place without needing established connections to be dropped. Default for this value is false if not provided.

h4. TokenRevocationTarget

* @(TL1)@ @TokenRevocationTarget@ is a type representing a target for "REST revokeTokens endpoint":/rest-api/#revokeTokens.
* @(TL2)@ @TokenRevocationTarget#key@ is a string containing the target key. Target key can be "clientId","revocationKey" or "channel". Where possible, clients should predefine those strings (enums for example) to prevent customers passing an invalid value.
* @(TL3)@ @TokenRevocationTarget#value is a string representing the value of specified for target key. This value cannot be empty or null. 

h4. TokenRevocationResponse
* @(TX1)@ @TokenRevocationResponse@ is a type representing a target for "REST revokeTokens endpoint":/rest-api/#revokeTokens.
* @(TX2)@ @TokenRevocationResponse#revokedTokens@ is a collection of @RevokedToken@s representing the result of the token revocation request.

h4. RevokedToken
* @(TY1)@ @RevokedToken@ is an object representing a single token that is revoked. 
* @(TY2)@ @RevokedToken#target@ is a @TokenRevocationTarget@ representing the target of revoked token
* @(TY3)@ @RevokedToken#issuedBefore@ is a timestamp (in milliseconds since the epoch) representing the time the the revoked token issued before.
* @(TY4)@ @RevokedToken#appliesAt@ is a timestamp (in milliseconds since the epoch) representing the time the revocation takes effect for this token.
...
* @(TN3)@ A token string (referred to in this specification as a "JWT token string") may be a "JSON Web Token":https://tools.ietf.org/html/rfc7519 satisfying "the Ably requirements for JWTs":https://ably.com/documentation/core-features/authentication#ably-jwt.
