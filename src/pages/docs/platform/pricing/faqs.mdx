...
### Are there any volume discounts? <a id="volume"/>
...
### Why are my concurrent connections higher than expected? <a id="high-connections"/>

If your app dashboard shows higher than expected concurrent connections, consider these common causes:

#### Using Realtime library server-side

Using a Realtime library from your servers when you should use the REST library can cause higher connection counts. Every time the Realtime library is instantiated, a connection is established to Ably and will not be closed unless you explicitly close it. Server-side code typically doesn't need realtime features like subscriptions or presence.

Instead, use the [REST interface](/docs/basics#rest) for server-side operations like:
- Publishing messages
- Issuing tokens for client authentication  
- Retrieving message history
- Publishing on behalf of other clients

Only use the [Realtime interface](/docs/basics#realtime) server-side if you specifically need:
- Persistent connections for high-frequency publishing
- Server-side subscriptions to messages
- Server-side presence functionality

#### Multiple SDK instances

Problem: Accidentally instantiating the Realtime client library multiple times in one page or app.

Each instance, unaware of the others, maintains its own connection to Ably. You should only use a single instance of the Realtime library per page/app.

Solution: Ensure you only instantiate the library once and reuse that instance throughout your application.

#### Browser tabs and mobile apps

Problem: Not accounting for multiple browser tabs or app instances.

Browser tabs are completely sandboxed from each other for security reasons, so [connections cannot be shared between tabs](/docs/connect#connection-monitoring). If a user has three tabs open, each with a connection to Ably, this counts as three separate connections.

Similarly, each mobile app instance (even from the same user) maintains its own connection.

Expected behavior: This is normal - each tab/app instance requires its own connection.

#### Troubleshooting steps

If none of the above resolves your high connection count:

1. Monitor connections in real-time: Use the [dev console](https://ably.com/accounts/any/apps/any/console) in your Ably dashboard to monitor connections being opened in real-time. This helps identify the source of new connections.

2. Review your code: Check for:
   - Multiple SDK instantiations in the same scope
   - Missing `close()` calls in server-side code
   - Realtime libraries used where REST would suffice

3. Check connection lifecycle: Ensure connections are properly closed when no longer needed, especially in server-side applications.

<Aside data-type="important">
If you significantly exceed your paid package limits or exceed your free package limits, Ably may restrict your account. See [what happens when you exceed limits](/docs/platform/pricing/limits#hitting) for more details.
</Aside>
