...
Ably guarantees that all subscribers on a channel receive messages in the [same order](/docs/platform/architecture/message-ordering), so batched operations are not only applied atomically but arrive in a consistent sequence across all clients.
</Aside>
