...
The Hive (Model Only) rule is a rule applied to chat rooms in Ably Chat which allows you to use Hive's text moderation models to detect and handle inappropriate content before it is published to other users.
...
h2(#chunking). Content chunking

Hive's models accept content with a maximum length of 1024 characters. If sending a message larger than this, Ably will automatically chunk it into smaller requests, with crossover between segments to ensure context is preserved.

For more information on Hive Text moderation, please refer to the "Hive Text Moderation documentation":https://docs.thehive.ai/docs/classification-text.
