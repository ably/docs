...
Use the [Control API](/docs/platform/account/control-api) to create a channel rule with message conflation by sending a `POST` request to `/apps/{app_id}/namespaces`:
...
Use the [Ably CLI](/docs/platform/tools/cli) to create a channel rule with message conflation:
