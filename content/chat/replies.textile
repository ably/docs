...
Reply to a message directly with a reference to the original message, or create threads on a message to separate the new branch of messages from the main path of messages. Replies enable members to quickly and directly respond to an existing message. Threads enable members to create a thread for a specifically focused conversation between two or more members within the channel.
...
Namespaces enable your chat to be grouped based on a prefix included as part of the channel name. Defining namespaces helps manage channels, such as defining the speed at which members can post messages or whether members can interact with messages such as reactions, replies, or threads.

The namespace for your replies and threads will need to be prefixed within this "channel rule":/channels#rules.
...
With message interactions enabled, each message now automatically has the @timeSerial@ attached, enabling the message to be referenced in replies or threads. To reference a previous message, include the @ref@ object inside the @extras@ object of your payload.
...
h2(#example). Example of message replies
...
// Join or create the channel 'general' in the 'public' namespace
var channel = client.channels.get('public:general');

// Replying to a message
// message is the message string containing the reply
// timeSerial is the unique ID of the message the reply is added to
function publishReply(message, timeserial) {
