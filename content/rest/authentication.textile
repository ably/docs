...
The REST client library provides the ability to issue tokens using the "@requestToken@ API endpoint":/rest-api#request-token, and also provides a number of helper functions to simplify the use of the authentication API.

The "@requestToken@":#request-token endpoint may itself be used without explicit authentication, relying on the implicit authentication of submitting a validly signed token request.
...
- <span lang="java">public String</span> keyName := The name part of the key against which this request is made.
