...
redirect_from:
  - /rest/channel-status
  - /rest/versions/v1.1/channel-status
  - /rest/versions/v1.0/channel-status
---

Metadata can be retrieved by REST request for a single channel, or for all active channels in an app. The information returned includes the current state of a channel and its "occupancy":/presence-occupancy/occupancy.

Since the metadata of channels often changes frequently, Ably recommends subscribing to "realtime events":/metadata-stats/metadata/subscribe rather than polling for status updates via REST. This is because data is likely to become stale shortly after it has been received.

h2(#single). Retrieve metadata of a single channel

Requesting the metadata for a single channel returns a "@ChannelDetails@":/api/realtime-sdk/channel-metadata#channel-details object for the given channel. This provides details about the channel including its "occupancy":/presence-occupancy/occupancy. A side effect of this request is that it will cause the channel to be activated. It is intended to be used in conjunction with the API to "enumerate":#enumerate a list of active channels in an app.
...
- by := _value_ optionally specifies what to return. Use @by=id@ to return just channel names. Use @by=value@ to return "@ChannelDetails@":/api/realtime-sdk/channel-metadata#channel-details. The default has changed: it used to be @by=value@, but for for protocol v3 or newer (the @version@ parameter passed to @rest#request@ is ≥ 3), it is @by=id@. **You are strongly recommended to use @by=id@ unless you really need the occupancy for every channel**. For one thing it will be much faster; for another, making very regular @by=value@ enumeration queries can prevent channels from closing down from inactivity.
...
<aside data-type='important'>
<p>The @channel-metadata@ "capability":/auth/capabilities for the wildcard resource @'*'@ is required to enumerate a list of active channels.</p>
</aside>

h3(#limitations). Known limitations

@by=value@ (which until recent protocol versions was the default) has unintuitive behaviour, see the supported parameters table for more details. Please use @by=id@ where possible.
