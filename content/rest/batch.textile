...
section: rest
index: 110
meta_description: "The Batch API is a REST API that enables you to bulk publish messages and instigate bulk presence operations."
meta_keywords: "Ably, Batch API, Bulk API, Bulk Presence."
api_separator:
jump_to:
  Help with:
    - Overview#overview
    - Generic operation#batch-operation
...
h2(#batch-operation). Generic operation

A batch mode request translates a single set of request details (that is the request body, params and headers) into an array of requests for the underlying API. Each of those requests is then performed in parallel.
...
This is the bulk publish API which is implicitly a batch-mode API. This performs a publish in parallel of messages to channels specified in the @request@ body. Check out the code example below on how to do this with the "@request@":/rest/usage#request method in the REST client library.
