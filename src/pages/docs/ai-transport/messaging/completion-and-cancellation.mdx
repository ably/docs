...
func example(realtime: ARTRealtime, stream: any AsyncSequence<StreamEvent, Never> & Sendable) async throws {
...
for await event in stream {
    if event.type == "token" {
        let appendMessage = ARTMessage(name: nil, data: event.text)
        appendMessage.serial = msgSerial

        Task {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                channel.append(appendMessage, operation: .init(clientId: nil, descriptionText: nil, metadata: ["phase": "streaming"]), params: nil) { _, error in
                    if let error {
                        continuation.resume(throwing: error)
                    } else {
                        continuation.resume()
...
// Signal content-part completion with an empty append
let doneMessage = ARTMessage(name: nil, data: "")
doneMessage.serial = msgSerial

Task {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
        channel.append(doneMessage, operation: .init(clientId: nil, descriptionText: nil, metadata: ["phase": "done"]), params: nil) { _, error in
            if let error {
                continuation.resume(throwing: error)
            } else {
                continuation.resume()
...
```swift
// After all content parts are done, signal response-level completion
let message = ARTMessage(name: "response-end", data: "")
message.extras = ["headers": ["responseId": "resp_abc123"]] as ARTJsonCompatible
...
func processPage(_ page: ARTPaginatedResult<ARTMessage>) {
    for message in page.items {
        let phase = message.version?.metadata?["phase"]
...
do {
    try channel.history(query) { page, error in
        if let error {
            print("Failed to get history: \(error.message)")
            return
        }
    ...
    print("History query error: \(error)")
}
```
</Code>
...
// Send cancellation request for a specific response
let message = ARTMessage(name: "cancel", data: "")
message.extras = ["headers": ["responseId": "resp_abc123"]] as ARTJsonCompatible
...
{/*
Swift test harness:
func stopGeneration(_ responseID: String?) {}
...
// Track pending appends for flushing
var pendingAppends: [Task<Void, any Error>] = []

// Listen for cancellation requests
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.subscribe("cancel", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        ...
        MainActor.assumeIsolated {
            guard let extras = try? message.extras?.toJSON() as? [String: Any],
                  let headers = extras["headers"] as? [String: String] else { return }
            ...
            // Flush any pending appends before confirming
            let appendsToAwait = pendingAppends
            pendingAppends.removeAll()

            Task {
                for task in appendsToAwait {
                    _ = try? await task.value
                }

                // Optionally confirm cancellation to all subscribers
                let confirmMessage = ARTMessage(name: "cancelled", data: "")
                confirmMessage.extras = ["headers": ["responseId": responseID ?? ""]] as ARTJsonCompatible

                try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                    channel.publish([confirmMessage]) { error in
                        if let error {
                            continuation.resume(throwing: error)
                        } else {
                            continuation.resume()
