...
Channels are the core functionality for a chat application or an application with chat features. Channels allow you to separate chats and their messages into categories. They provide a way to implement the Publish/Subscribe (Pub/Sub) architectural pattern. By using Pub/Sub, your application can have any number of users "join" a chat by subscribing to this channel, receiving any messages that these subscribed rooms have published to them. The users are able to "send" messages by publishing them to each chat they're subscribed to. The ability to publish and subscribe is completely decoupled from eachother.
