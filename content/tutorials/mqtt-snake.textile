...
With the libraries now available, it's time to set up MQTT. When using MQTT with Ably, there are a few requirements with regards to your setup. You'll need to connect to 'mqtt.ably.io' on port 8883, which requires the use of SSL/TLS in your connection. If you have a device which cannot support SSL, you'll need to connect via port 1883 instead, but this will come with a number of "restrictions":https://knowledge.ably.com/using-the-mqtt-protocol-adapter-1.
...
blang[javascript].
  ...
  If you have any issues with this, please check out the "Ably MQTT usage notes":https://knowledge.ably.com/using-the-mqtt-protocol-adapter-1.
...
Now that you've set up your controller, it's time to create something to run the game of Snake, and receive the data from Ably that you sent from the controller. You'll be using the "JavaScript client library SDK":https://github.com/ably/ably-js for this part. This is possible due to the fact Ably is "Protocol Agnostic":https://knowledge.ably.com/do-you-support-mqtt-are-you-protocol-agnostic-1, allowing protocols to be used interchangeably.
...
And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys-1 to @snake.js@ and your controller.
