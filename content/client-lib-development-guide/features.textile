...
* @(RSH1)@ @Push#publish(recipient, data)@ performs a HTTP request to "/push/publish":/rest-api/#push-publish
...
**** @(RSH3a2a)@ If the local device has @id@ and @updateToken@, transitions to @WaitingForNewPushDeviceDetails@.
**** @(RSH3a2b)@ If the local device has the necessary push details (registration token, etc.), transitions to @GotPushDeviceDetails@.
**** @(RSH3a2c)@ Otherwise, transitions to @WaitingForPushDeviceDetails@.
** @(RSH3b)@ State @CalledActivate@:
...
**** @(RSH3d3b)@ Otherwise, make an asynchronous PUT HTTP request to "/push/deviceRegistrations/:deviceId":/rest-api/#update-device-registration using the local @DeviceDetails@ 's push details as body.
