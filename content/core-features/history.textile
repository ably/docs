...
While, by default, messages are only stored for two minutes on Ably's servers, these messages can be retrieved in the event of a lost connection using the "History API":/storage-history/storage. There are several options available to increase this storage time. One option is to enable persistence of all messages, for typically 24 hours. These messages can also be retrieved using the "History API":/storage-history/storage. Persistence of the last message is also available, although this is retrieved using the "rewind channel option":/realtime/channels/channel-parameters/rewind. Finally, it is possible to persist messages to external storage services, using the various "integrations":/general/integrations that are available.
