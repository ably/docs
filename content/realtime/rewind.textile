...
Channels support a parameter that applies at the time of attachment that requests that an attachment start from some number of messages or point in time in the past. The @rewind@ param is specified via the "channel params":./channel-params mechanism.
