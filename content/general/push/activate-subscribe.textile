...
Every client must still be configured and register for push notifications according to their platform. Tutorials for Android and iOS can be found "here":https://ably.com/tutorials/. The Ably client libraries abstracts away this complexity and platform-specific behaviour by providing a consistent API for device activation, maintenance of the device registration, and for subscription to Ably channels for receiving push notifications.
