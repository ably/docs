...
Using configurable rules, you can stream any of messages, presence events, and channel lifecycle events. Unlike our "channels":/realtime/channels which follow a "pub/sub pattern":https://en.wikipedia.org/wiki/Publish%E2%80%93subscribe_pattern where each message is delivered to any number of subscribers, our Firehose operates on the basis that each message is delivered once to your streaming or queueing server.
