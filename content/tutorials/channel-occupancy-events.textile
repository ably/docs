...
In addition to the user-defined messages on a channel, the channel can be configured to also publish channel occupancy data, such as the number of publishers and subscribers. In this tutorial, you'll learn how to subscribe to inband channel occupancy events.
...
In the previous snippet, the code subscribes to '[meta]occupancy' inband events for the channel. Then, it displays the data returned in the text area. The structure of the occupancy metrics are defined in the "occupancy documention":/realtime/inband-occupancy.
...
1. If you would like to find out more about how channels and publishing or subscribing to messages work, see the realtime "channels":https://www.ably.io/documentation/realtime/channels and "messages":https://www.ably.io/documentation/realtime/messages documentation.
