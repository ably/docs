...
bc[sh]. curl https://rest.ably.io/channels/rest-example/messages \
 -H "Authorization: Bearer {{TOKEN_BASE_64}}"
...
This is the means by which clients obtain access tokens to use the service. The construction of a token request is described in the "Authentication":/other/authentication page. The resulting @token response@ object contains the token properties as defined in "Authentication":/other/authentication.
...
"See a complete example of a statistics response":/other/statistics.
