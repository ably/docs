...
meta_description: "Ably provides a method for obtaining application statistics. This is useful for determining usage compared to quotas."
meta_keywords: "Statistics, stats, per-application statistics"
index: 30
redirect_from:
...
Ably can be queried to obtain usage statistics for a given application. Results are  aggregated across all channels that are in use across the application, in the specified time period. Statistics are often used to track usage against account quotas. The details on how to retrieve statistics are available in "using an Ably SDK":/metadata-stats/stats and "using the REST API":/api/rest-api#stats.
