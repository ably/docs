...
The client libraries will attempt to automatically recover from non-fatal error conditions. However, you can handle them yourself if you prefer. <span lang="jsall">For explicit attaches with a callback, both fatal and nonfatal errors will result in the callback being called with an @ErrorInfo@ argument</span>
...
Note that while fatal errors won't get better on their own, they can be fixable. For example, if a channel goes into the @FAILED@ state due to the client not having the right capabilities to attach to it, that client could use "@Auth#authorize()@":/realtime/authentication#authorize to obtain a new token which does have the right capabilities, then call "@attach()@":/channels-messages#attach on the channel. While the library will not automatically reattach in the @FAILED@ state, explicit calls to "@attach()@":/realtime/channels-messages#attach will make the client try again.

h4(#nonfatal-errors). Nonfatal errors

Other types of error are nonfatal. For example, a client may have network connectivity issues, or a channel may experience a loss of strict message continuity. The library will automatically attempt to recover from these events. If channel continuity is lost in the process, the library will notify you though a @resumed@ flag in the @ATTACHED@ event, so you can decide how to handle that failure.

For every channel @ATTACHED@ event, the "@ChannelStateChange@ object":/realtime/types/#channel-state-change contains a @resumed@ attribute. When true, there has been no loss of continuity from the last time the channel was attached. When false, there has been a loss of continuity. So for example:

* The first time you attach to a channel on a fresh connection, @resumed@ will be false, as there was nothing to continue from.
* If you "resume":#connection-state-recovery a connection successfully, all your channels will get @ATTACHED@ events with @resumed@ true (and will receive any messages they missed while you were disconnected).
* If you "resume":#connection-state-recovery a connection unsuccessfully (so you get a fresh connection) — generally because you were disconnected for more than two minutes, which is how long Ably holds connection state for — continuity is lost. All your channels will still reattach automatically, and you will get @ATTACHED@ events with @resumed@ false.
* If Ably needs to signal a loss of channel continuity, you will get an @ATTACHED@ event with @resumed@ false (usually after the channel goes very briefly into @ATTACHING@).
...
bq(definition#on-state-listener).
  default: on(String state, listener("ChannelStateChange":#channel-state-change stateChange))
...
<span lang="default">The listener is passed the current state</span><span lang="jsall">The listener is passed a "ChannelStateChange":#channel-state-change object that contains the current state, previous state, a boolean indicating whether the channel was resumed, and an optional reason for the state change.</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span><span lang="objc,swift">The listener is passed an optional reason for the state change</span>.

<div lang="jsall">
bq(definition). on(String[] states, listener("ChannelStateChange":#channel-state-change stateChange))

Same as above, but registers multiple listeners, one for each state in the array.
</div>
...
<span lang="default">The listener is passed the current state</span><span lang="jsall">The listener is passed a "ChannelStateChange":#channel-state-change object that contains the current state, previous state, boolean indicating whether the channel was resumed, and an optional reason for the state change</span><span lang="ruby">The listener is passed a "ChannelState":#channel-state object that contains the current state, previous state, and an optional reason for the state change</span><span lang="objc,swift">The callback is passed an optional reason for the state change or error</span>.

h4. Parameters

- <div lang="jsall">state(s)</div> := the channel state(s) to subscribe to, see "ChannelState":#channel-state for supported channel states<br>__Type: @String@ or @String[]@__
...
bq(definition#once-state-listener).
  default: once(String state, listener("ChannelStateChange":#channel-state-change stateChange))
...
Removes the given listener from all "ChannelState":#channel-state states in the array.

bq(definition). off(String state)

Removes all listeners for a given "ChannelState":#channel-state state.

bq(definition). off(String[] states)

Removes all listeners for all "ChannelState":#channel-state states in the array.

bq(definition). off()

Removes all listeners (including both those registered against specific states and those registered without a state).
</div>

h4. Parameters

- <div lang="jsall">state(s)</div> := the channel state(s) to unsubscribe from<br>__Type: @String@ or @String[]@__
