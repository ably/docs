...
To append data to an existing message, use the `appendMessage()` method on a REST or realtime channel. The published append will have an `action` of `message.append`.
...
Unlike updates, appends are not stored in version history, since they're designed for a usecase of a frequent rate of updates from a single publisher.
