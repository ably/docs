...
intro: "Vercel AI SDK Core orchestrates LLM calls on the server. AI Transport encodes the resulting stream onto a durable session, so the same server code reaches every connected client instead of a single HTTP response."
---
...
If you are building the client side, start with [Vercel AI SDK UI](/docs/ai-transport/frameworks/vercel-ai-sdk-ui).
...
- [Agent session API reference](/docs/ai-transport/api/javascript/core/agent-session): every method and property on the agent session.
