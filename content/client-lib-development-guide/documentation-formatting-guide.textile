...
h2(#code-blocks-github-style). Code blocks Github style

As an alternative, you can use Github styled code blocks using triple backticks with the language specified after the backticks in square brackets.  The advantage of Github blocks over traditional Textile blocks is that line breaks are supported.  Please note that lower level indentation is stripped
