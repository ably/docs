...
h2. Build a Peer to Peer Static Web App with Vue.js
...
This app will use "Ably Channels":/realtime/channels and "Token Authentication":/core-features/authentication/#token-authentication.
...
h2. You now have a working peer to peer demo, but wait there's more!
...
Now we should update the client to listen for this message. Add a @case@ statement for the @word@ message to it's @onReceiveMessage@ handler.
...
Ably offers a @History API@ and a @Rewind@ setting. The History API allows you to query the history of a channel for by default, the last *two minutes* of data. If you enable @persisted history@ on your channel, that window is extended for 24-72 hours (these will count towards your message quotas - "see docs for more on this":https://support.ably.com/support/solutions/articles/3000030059). We can use this extended window to allow clients to join "mid-stream" and catch up when they connect.

The "Rewind":https://www.ably.io/documentation/realtime/channels/channel-parameters/overview#supported-parameters parameter can be enabled when you subscribe to a channel, which will immediatey then receive up to the last 100 messages in the time window you specify.
...
To enable Rewind, we need to add an additional parameter to the @channels.get@ call, to add the parameter @ { params: { rewind: '1m' } }`. This tells our client to return up to the last 100 messages, from the minute. You can provide different time periods in here in either minutes ('2m') or seconds ('15s').
