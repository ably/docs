...
Each channel lifecycle event in the @items@ array consists of a JSON-like object with the following attributes:
...
Each presence event envelope in the @items@ array consists of a JSON-like object with the following attributes:
...
Each member of @data.presence@ is a 'raw' presence event. Presence messages sent "over the realtime service":/realtime/channels-messages are automatically decoded into "@PresenceMessages@":/realtime/types#presence-message object by the Ably client library. With webhooks you need to to do this explicitly, using "@PresenceMessage.fromEncodedArray@":/realtime/presence#from-encoded-array on the @data.presence@ array, or "@PresenceMessage.fromEncoded@":/realtime/presence#from-encoded or an individual member of that array. This will transform them into an array of "@PresenceMessages@":/realtime/types#presence-message (or in the case of @fromEncoded@, an individual "@PresenceMessage@":/realtime/types#presence-message). This has several advantages, e.g.:
...
* If you are using "encryption":/general/encryption, you can pass your encryption key to the method and it will decrypt the @data@ for you
...
Each message envelope in the @items@ array consists of a JSON-like object with the following attributes:
