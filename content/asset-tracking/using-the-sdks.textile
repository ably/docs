...
The location animator receives location updates from the publisher and creates animation steps from them. These steps are then applied to an ongoing animation. 
...
Each step is animated at 60 FPS, and this applies to both the map marker and the camera position.
...
- animationStepsBetweenCameraUpdates := By default, when a step is being animation, the camera position is also updated. But this can be changed so that the camera position updates, for example, on every five location updates.  
...
This provides data you can use to move your map marker on the map, or when you want to move your trackable in the screen.
...
- locationUpdate := The location update from the publisher.
...
* "Asset Tracking Android training course":https://github.com/ably/asset-tracking-android-training
