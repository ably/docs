---
title: "Reactor Events Tutorial: Cloudflare Integration"
alt_title: "Integrating Cloudflare Workers with Ably using Reactor Events"
excerpt: "Learn how to use our Reactor Functions to consume realtime data with Cloudflare by building a 2D game in React"
section: tutorials
...
The "Ably Reactor":https://www.ably.io/reactor provides a way to trigger events and to stream data from Ably's pub/sub channels. This turorial will go over how to use the Ably Reactor with "Cloudflare workers":https://developers.cloudflare.com/workers/quickstart/. 
...
h2. Step 3 - cCreating your function
...
Add your own Ably API key inbetween the quotation marks!
...
During configuration of your Cloudflare worker, you're going edit the "./cloudflare-worker/wrangler.toml":https://github.com/ably/tutorials/blob/reactor-event-cloudflare-javascript/cloudflare-worker/wrangler.toml file to include your Cloudflare account_id. (You might be wondering where your Cloudflare API key goes - in order to deploy a Worker using Wrangler, you'll configure Wrangler using it's config subcommand so your API keys never touch the Worker code itself.)
...
Once you're done with configuration, publish your Worker using Wrangler. After publishing your Worker to your Cloudflare account, it will have a URL, you will need this to set up the Ably Reactor.
 ...
 <!--- %= partial 'tutorials/_create-reactor-rule-event.textile' % --->
...
```
The Worker is re-publishing our initial message to a channel called @cloudflare-bot@ using the "Ably REST API":/rest. "We would usually recommend":https://support.ably.io/a/solutions/articles/3000061848 that you use the Client library SDKs rather than make a direct POST to the Ably Rest API, since it is more discoverable and easier to use, however in this particular case, installing the library files in the Worker would mean bundling the entire library with your Worker, which would increase the size of your script unnecessarily. 
...
1. Take a look at the "Reactor Functions documentation":/general/functions for further details about what was described in this tutorial.
2. Find out more about "Ably Reactor features and capabilities":https://www.ably.io/reactor.
