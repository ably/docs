...
Ably sends a heartbeat to connected clients every 15 seconds. The lack of two consecutive heartbeats will indicate to Ably that a client has disconnected and the "connection state":#connection-states will be updated to @disconnected@. Note that this is only in the case of an abrupt disconnect. Closing a connection, such as by calling "@close()@":#close, is instant.
