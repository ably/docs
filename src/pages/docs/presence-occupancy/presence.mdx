...
| Update | An already present member has updated their [member data](#member-data). Being notified of member data updates can be useful, for example, it can be used to update the status of a user when they are typing a message (see also [Chat typing indicators](/docs/chat/rooms/typing) for a purpose-built solution) |
...
A single [`clientId`](/docs/api/realtime-sdk#client-id) may be present multiple times on the same channel via different client connections. Ably considers these as different members of the presence set for the channel, however they will be differentiated by their unique connectionId. For example, if a client with ID “Sarah” is connected to a [chat](/docs/chat) channel on both a desktop and a mobile device simultaneously, “Sarah” will be present twice in the presence member set with the same client ID, yet will have two unique connection IDs. A member of the presence set is therefore unique by the combination of the clientId and connectionId strings.
...
await channel.presence.subscribe('enter', (member) => {
  console.log(member.clientId + ' entered my-channel');
});

await channel.presence.enterClient('Bob'); // => Bob entered my-channel
await channel.presence.enterClient('Mary'); // => Mary entered my-channel
```
...
rest.auth.requestToken(clientId: '*') do |token|
  ...
  channel.presence.subscribe(:enter) do |member|
    puts "#{member.client_id} entered my-channel"
  end

  channel.presence.enter_client 'Bob' # => Bob entered my-channel
  channel.presence.enter_client 'Mary' # => Mary entered my-channel
end
```
...
channel.presence.subscribe(new Presence.PresenceListener() {
    @Override
    public void onPresenceMessage(PresenceMessage member) {
        System.out.println(member.clientId + " entered my-channel");
    }
});
...
channel.presence.enterClient("Bob", "message", noop); /* => Bob entered my-channel */
channel.presence.enterClient("Mary", "message", noop); /* => Mary entered my-channel */
```
...
channel.Presence.Subscribe(member => {
  Console.WriteLine(member.ClientId + " entered my-channel");
});

await channel.Presence.EnterClientAsync("Bob", null); /* => Bob entered my-channel */
await channel.Presence.EnterClientAsync("Mary", null); /* => Mary entered my-channel */
```
...
[rest.auth requestToken:tokenParams withOptions:nil callback:^(ARTTokenDetails *tokenDetails,
    ...
    ARTRealtimeChannel *channel = [realtime.channels get:@"my-channel"];

    [channel.presence subscribe:ARTPresenceEnter callback:^(ARTPresenceMessage *member) {
        NSLog(@"%@ entered my-channel", member.clientId);
    }];

    [channel.presence enterClient:@"Bob" data:nil]; // prints 'Bob entered my-channel'
    [channel.presence enterClient:@"Mary" data:nil]; // prints 'Mary entered my-channel'
}];
```
...
rest.auth.requestToken(ARTTokenParams(clientId: "*"), withOptions: nil) { tokenDetails, error in
    ...
    channel.presence.subscribe(.enter) { member in
        print("\(member.clientId) entered my-channel")
    }
...
```rest_go
	...
	_, err = channel.Presence.Subscribe(context.Background(), ably.PresenceActionEnter, func(msg *ably.PresenceMessage) {
		log.Printf("%s entered my-channel", msg.ClientID)
	})
	if err != nil {
		log.Fatal(err)
	}
...
final channel = realtime.channels.get('my-channel');

channel
    .presence
    .subscribe(action: PresenceAction.enter)
    .listen((message) {
      print('${message.clientId} entered my-channel');
});
...
A second approach is to split capabilities across two tokens for the same channel. A publisher-role token grants only `publish` and `presence`. Clients using it enter the presence set and publish messages but receive nothing back, since they lack `subscribe`. A subscriber-role token grants `subscribe` (and `history` if needed) so other clients can read the channel.
...
The `subscribe` capability covers both regular messages and presence state-change messages, so it cannot be used to receive one without the other. Use [channel mode flags](/docs/channels/options#modes) when a client must subscribe to regular messages and remain present without receiving presence events.
