...
<a href="/images/tutorials/queue-amqp-neutrino.png" target="_blank">
  <img src="/images/tutorials/queue-amqp-neutrino.png" style="width: 100%" alt="Reactor Queues and Neutrino diagram">
</a>
...
First add the Ably client library loader from our CDN to the HTML file "@public/index.html@":https://github.com/ably/tutorials/blob/cf84468563234364e2beb7269a0dc1616207ad81/public/index.html
...
Then add the following to your "@public/app.js@":https://github.com/ably/tutorials/blob/cf84468563234364e2beb7269a0dc1616207ad81/public/app.js Javascript file:
...
<a href="/images/tutorials/queue-amqp-neutrino-setup.png" target="_blank">
  <img src="/images/tutorials/queue-amqp-neutrino-setup.png" style="max-width: 100%" alt="Provision a Neutrino queue">
</a>
...
# Click on the "Queues" tab
# Click the "Create a new Queue rule" button (this appears below all the provisioned queues). When prompted, choose the @neutrino@ queue you just provisioned, and enter @^neutrino:raw@ in the channel filter field to ensure that only unfilered text published by clients is republished to the queue. The unfiltered text will then sit in the queue waiting for one or more workers (consumers) to process the text. See screenshot example below to set up a queue rule:

<a href="/images/tutorials/queue-amqp-neutrino-rule.png" target="_blank">
  <img src="/images/tutorials/queue-amqp-neutrino-rule.png" style="max-width: 100%" alt="Create a queue rule">
</a>
...
<a href="/images/tutorials/queue-amqp-neutrino-provisioned-test.png" target="_blank">
  <img src="/images/tutorials/queue-amqp-neutrino-provisioned-test.png" style="max-width: 100%" alt="Check that queue has updated">
</a>

h2. Step 6 - Add queue worker

We're now ready to start consuming messages from the queue. There are "plenty of AMQP compatible libraries":https://www.ably.io/download available, however for this tutorial we will us the popular @amqplib@ library. First let's add the @amqplib@ library to the "@package.json file@":https://github.com/ably/tutorials/blob/48c3f21b821304ebef190b84355161e5736309cc/package.json:
...
Now create a "@worker.js@ file":https://github.com/ably/tutorials/blob/48c3f21b821304ebef190b84355161e5736309cc/worker.js that will consume from the AMQP queue, process messages in real time and post a placeholder response back to the web client using REST:
...
<a href="/images/tutorials/neutrino-signup.png" target="_blank">
  <img src="/images/tutorials/neutrino-signup.png" style="max-width: 100%" alt="Neutrino sign up">
</a>
...
<a href="/images/tutorials/neutrino-api-keys.png" target="_blank">
  <img src="/images/tutorials/neutrino-api-keys.png" style="max-width: 100%" alt="Neutrino API keys">
</a>

Now that you have an API key you can use to communicate with the Neutrino API, you'll need an HTTP library for the worker. @request@ provides a straightforward API for HTTP requests, so add it to "@package.json file@":https://github.com/ably/tutorials/blob/eb8418f094ddea3919c527cf8e0f4212053a14c8/package.json:
...
Now add the following to "@worker.js@":https://github.com/ably/tutorials/blob/eb8418f094ddea3919c527cf8e0f4212053a14c8/worker.js so that consumed text is sent to the Neutrino Bad Word Filter API, and filtered text from the API is published back to the web client through an Ably pub/sub channel:
...
bc[sh]. git checkout queue-neutrino-profanity-nodejs
