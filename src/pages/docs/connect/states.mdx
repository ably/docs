...
When a browser client library first connects to Ably to establish a realtime connection, by default the library initiates connection using Comet (long polling). It then attempts to upgrade the Comet connection to a WebSocket connection, typically within a few seconds.

This approach ensures compatibility with environments where WebSockets may be blocked or subtly broken, such as some corporate proxies. Using Comet transport first provides a connection that works for everyone as quickly as possible, after which the upgrade to WebSockets happens automatically.
...
In v2 of the JavaScript library, the default transport behavior has changed. See the [v2 migration guide](/docs/sdk/js/v2-migration) for details about changes affecting environments where WebSocket connections may be blocked.
</Aside>
