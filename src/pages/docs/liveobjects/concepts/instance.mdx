...
<If lang="java">
In the Java SDK the base `Instance` type is type-agnostic; capabilities (`getId()`, `get()`, `value()`, `set()`, `subscribe()`) live on the typed instances — `LiveMapInstance`, `LiveCounterInstance` and the primitive instances — reached via throwing `as*` casts.
</If>
...
Instance instance = myObject.get("visits").instance(); // null if nothing at the path
if (instance != null) {
...
<If lang="javascript">The `instance()` method returns `undefined` if no object exists at that path.</If><If lang="java">The `instance()` method returns `null` if no object exists at that path.</If>
...
```java
// Obtain an Instance for a LiveCounter
Instance visits = myObject.get("visits").instance();
...
An `Instance` wraps an already-resolved value of known type, so its `as*` casts **throw** an `IllegalStateException` on mismatch — the opposite contract to the never-throwing `PathObject` casts. Check `getType()` first when the type isn't known; `getType()` on an `Instance` is non-null.

<Code>
```java
Instance instance = myObject.get("visits").instance();
...
<If lang="java">
<Aside data-type="note">
Unlike `PathObject`, the `get(key)` method on a `LiveMapInstance` returns `null` if the entry doesn't exist or the object at that location has been deleted. "Not a `LiveMap`" cannot happen — `get()` only exists on `LiveMapInstance`.
</Aside>
</If>
...
```java
Instance instance = myObject.get("settings").instance();
if (instance != null) {
...
```java
// Get an instance
Instance userInstance = myObject.get("user").instance();
...
```java
Instance user = myObject.get("user").instance();
if (user != null) {
    // Get an entry that doesn't exist on the instance
...
<Code>
```java
Instance settings = myObject.get("settings").instance(); // a LiveMap
if (settings != null) {
...
```java
Instance settings = myObject.get("settings").instance();
if (settings != null) {
...
LiveMapInstance root = myObject.instance().asLiveMap();

// Store binary data in the LiveMap
root.set("hello", LiveMapValue.of("world".getBytes(StandardCharsets.UTF_8))).join();

// compactJson() converts binary data to base64 strings;
// LiveMapInstance narrows the return type to JsonObject
JsonObject json = root.compactJson();
...
<If lang="java">
<Aside data-type='important'>
In Java a type mismatch is either a compile error (the method doesn't exist on the typed instance) or a fast failure in the cast — `asLiveCounter()` on a `LiveMap` instance throws an `IllegalStateException`. Check `getType()` first when the type isn't known.
</Aside>
</If>
...
<If lang="java">
- `getObject()` returns an `Instance` representing the same object instance you called `subscribe()` on — it is the base `Instance` type, so cast it with the matching `as*` view
