...
The @off()@ method for a "channel":/realtime/channels#off or "connection":/api/realtime-sdk/connection#off removes @ChannelEvent@ or @ConnectionEvent@ listeners that are listening for state changes on a channel or for a connection.
...
Monitoring presence events can be quite complex. For this reason, the @Presence@ object in the realtime library exposes a @get@ "method":/api/realtime-sdk/presence#get allowing a client to retrieve an array of all members present on the channel. The Ably client is responsible for keeping track of the presence set from the time that the channel is attached. An up-to-date presence set is pushed to the client following attach and this set is updated on each subsequent presence event. Thus @get@ returns the already-known presence set retained in memory and does not trigger a new request to the Ably service. You can simply use this method as shown below, instead of venturing into the complexity of building one of your own.
