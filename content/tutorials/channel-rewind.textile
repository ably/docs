...
However, in some applications it is useful to have access also to messages that occurred in the past. Ably supports a "History API":/storage-history/storage whereby, for channels that are persisted, it is possible to query explicitly for historical messages on a channel within any specified time bounds.
