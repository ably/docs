...
```[php]
  ...
  echo("Last published data: " . $resultPage->items->first->data);
```
...
```[objc]
  ...
  [channel history:^(ARTPaginatedResult<ARTMessage *> *resultPage, NSError *error) {
...
bc[objc]. [channel history:^(ARTPaginatedResult<ARTMessage *> *resultPage, NSError *error) {
...
Channels expose a @presence@ member which a client can use to obtain present members and presence event history for the channel itself. See the "REST presence":/rest/presence documentation for details.
...
bq(definition#publish-data).
  ...
  java:     public void publish(String name, Object data)
...
bq(definition#publish-msg-array).
  ...
  ruby,php: publish(Message[] messages)
  java:     public void publish(Message[] messages)
...
- <div lang="php">data</div> := data payload for the message. The supported payload types are Strings, Associative Array or Array objects that can be serialised to JSON, binary data as byte arrays, and null.<br>__Type: Object__
...
- <span lang="default">options</span><span lang="objc,swift">query</span><span lang="java">"Param":#param[] options</span> := <span lang="default">an optional object containing the query parameters</span><span lang="ruby">an optional set of key value pairs containing the query parameters</span><span lang="ruby">an optional Associative Array containing the query parameters</span>, as specified in the "message history API documentation":/realtime/history#channel-history.
...
Further details of the supported @options@ params, see "message history API documentation":/rest/history#channel-history.
