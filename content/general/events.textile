...
You can "configure events":https://knowledge.ably.com/how-to-set-up-a-reactor-rule-1 from the "Reactor tab in your app":https://knowledge.ably.com/how-to-set-up-a-reactor-rule-1 on a per app basis which can apply to one or more channels in that app. Reactor Events can be filtered by channel naming using a regular expression, for example @^click_.*_mouse$@. This would match the string @click_@ followed by a string followed by @_mouse@, for example, @click_left_mouse@.
...
Webhooks are configured from the Reactor tab in your "app dashboard":https://knowledge.ably.com/how-do-i-access-my-app-dashboard-1. The following fields are shared between each webhook:
...
# Trigger an event using the "Dev Console":https://knowledge.ably.com/do-you-have-a-debugging-or-development-console-for-testing-1 in your app dashboard which will generate a webhook. You should then confirm that the webhook has been received in your RequestBin
