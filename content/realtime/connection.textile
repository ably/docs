...
Within the Ably system and protocol, all connection state is temporary; the protocol provides continuity of message delivery between the client and the service, and continuity of other connection state, provided that a dropped connection is reestablished within a limited interval (typically around 1 minute). Beyond that, the connection state is stale and the system will not attempt to make a transparent connection recovery. The lifecycle
of a connection, and the strategy for reconnecting on failure, reflect the transient nature of the connection state.
...
<pre><code lang="javascript,nodejs">realtime.connection.on('connected', function(stateChange) {
...
<pre><code lang="javascript,nodejs">realtime.connection.on(function(stateChange) {
...
<pre><code lang="javascript,nodejs">/* remove a listener registered for a single event */
...
<pre><code lang="java">realtime.connection.listeners.add(new ConnectionStateListener() {
	@Override
...
<pre><code lang="java">/* remove a single listener */
...
h4(#connection-state-recovery). Connection state recovery
...
h5(#connection-recovery-options). Connection recovery options
