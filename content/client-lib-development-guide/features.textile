...
* @(RTP18)@ The realtime system reserves the right to initiate a sync of the presence members at any point once a channel is attached and presence is in sync. The server initiated sync allows it to send a complete list of members present on the channel at that point in time. A sync is started by the server when it sends a @SYNC@ @ProtocolMessage@ to the client containing a @channel@ attribute <br>**TBC - how does the client lib know definitely it's starting a @SYNC@ and not continuing one, and should the client lib continue the @SYNC@ if disconnected & reconnected usingt the cursor?**.
...
*** @(RTP5c1)@ If the "@resumed@ flag":#TH4 is true and no @SYNC@ is initiated as part of the attach, do nothing as there is no loss of continuity on the channel and no change to presence. The @PresenceMap@ is not effected and no members need to be re-entered
*** @(RTP5c2)@ If the "@resumed@ flag":#TH4 is true and a @SYNC@ is initiated as part of the attach, then once the @SYNC@ is complete, all members not present in the
@PresenceMap@ but present in the "internal @PresenceMap@":#RTP17 must be re-entered automatically by the client using the @clientId@ and @data@ attributes from each. The members re-entered automatically must be removed from the "internal @PresenceMap@":#RTP17 ensuring that members present on the channel are constructed from presence events sent from Ably since the channel became @ATTACHED@
*** @(RTP5c3)@ If the "@resumed@ flag":#TH4 is false, then the @PresenceMap@ should be cleared, and once the @SYNC@ is complete, all members in the "internal @PresenceMap@":#RTP17 must be re-entered automatically by the client using the @clientId@ and @data@ attributes from each. The "internal @PresenceMap@":#RTP17 must then be cleared to ensure that members present on the channel are constructed from presence events sent from Ably since the channel became @ATTACHED@
*** @(RTP5c4)@ If any of the automatic @ENTER@ presence messages published in "RTP5c2":#RTP5c2 or "RTP5c3":#RTP5c3 fail, then the error message received from Ably should be emitted as an @ERROR@ event on the @Channel@ with Ably error code @91201@
