...
Client-side devices should generally be considered untrusted, and as such, it is important that you minimize the impact of any credentials being compromised on those devices. Therefore, the recommended strategy for authenticating client-side devices is to issue them with time-limited tokens. It is therefore the responsibility of the developer to identify each device before issuing a token. Both "Ably Tokens":#ably-tokens and "JWT":#ably-jwt are supported.
...
It is recommended you make use of the "Auth object":#auth-object for obtaining and distributing tokens as it will deal with renewal of tokens for you.
...
h3(#token-request-process)(minimise). Ably TokenRequest is created by your servers and passed to clients
...
h3(#token-process)(minimise). Ably Token is issued by your servers and passed to clients
...
h3(#ably-jwt-process)(minimise). Ably JWT is created by your servers and passed to clients
...
h3(#jwt-embed-process)(minimise). Ably-compatible token is embedded in a External JWT from your server and passed to clients
...
The table below indicates which feature is best given a certain requirement:

|_. Scenario |_. "Basic":#basic-authentication |_. "Token":#token-authentication |_. Description |
| Your scripts may be exposed | <div style="text-align: center;"><img src="/images/table-images/cross.svg"></div> | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | If the script, program or system holding the key is exposed, for example on a user's device, you should not embed an "API key":https://support.ably.io/support/solutions/articles/3000030054-what-is-an-app-api-key and instead use "Token Authentication":#token-authentication. If the script is on a secure environment such as your own server, an "API key":https://support.ably.io/support/solutions/articles/3000030054-what-is-an-app-api-key with "Basic Authentication":#basic-authentication is fine. |
| Your connection may be insecure | <div style="text-align: center;"><img src="/images/table-images/cross.svg"></div> | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | If there is a risk of exposure of the client’s credentials, either directly or over an insecure, or insecurely proxied, connection, "Token Authentication":#token-authentication should be used. If you are sure the connection is secure and unmediated, "Basic Authentication":#basic-authentication is acceptable. |
| You have no server to control access | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | <div style="text-align: center;"><img src="/images/table-images/cross.svg"></div>  | If you do not have your own server to perform authentication and provide "tokens":#tokens to users, you'll need to use "Basic Authentication":#basic-authentication. |
| You require fine-grained access control | <div style="text-align: center;"><img src="/images/table-images/cross.svg"></div> | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | If you need to provide "privileges":#capabilities-explained on a user-by-user basis, you'd be better using "Token Authentication":#token-authentication. If you only need a few access control groups, "Basic Authentication":#basic-authentication is reasonable. |
| Users need restricted periods of access | <div style="text-align: center;"><img src="/images/table-images/cross.svg"></div> | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | If you need users to only have access for a certain period of time, or the ability to revoke access, Token Authentication is needed. If users are able to always have access, Basic Authentication is acceptable. |
| Users need to identify themselves | <div style="text-align: center;"><img src="/images/table-images/exclamation.png"></div>  | <div style="text-align: center;"><img src="/images/table-images/tick.svg"></div> | If the user can be trusted to "identify":#identified-clients itself, "Basic Authentication":#basic-authentication is fine. If the user cannot be trusted however, "Token Authentication":#token-authentication is better as it allows for a trusted token distributor to identify the user instead. |

*Note:* Many applications will most naturally use a mixed strategy: one or more trusted application servers will use basic authentication to access the service and issue tokens over HTTPS, whereas remote browsers and devices will use individually issued tokens.
...
h4(#capabilities-explained-all)(minimise). Ably Token request without capabilities example
...
h4(#capabilities-explained-intersection)(minimise). Ably Token is requested with intersection of capabilities example
...
h4(#capabilities-explained-error)(minimise). Ably Token is requested with incompatible capabilities
