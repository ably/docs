...
If the POST request fails, the optimistic message stays in the conversation tree and the session emits an `error` event. The message is not removed automatically; the agent-wake POST happens in the background after `view.send()` returns its `ActiveRun`:
