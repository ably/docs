...
```[objc]
  ARTRealtime *realtime = [[ARTRealtime alloc] initWithKey:@"xVLyHw.fXSk9A:EYAP95RZblK3d9gw"];
  ARTTokenParams *tokenParams = [[ARTTokenParams alloc] initWithClientId:@"Bob"];
  [realtime.auth createTokenRequest:tokenParams options:nil callback:^(ARTTokenRequest *tokenRequest, NSError *error) {
      // ... issue the TokenRequest to a client ...
  }];
```

```[swift]
  let realtime = ARTRealtime(key: "xVLyHw.fXSk9A:EYAP95RZblK3d9gw")
...
h2(#properties).
  ...
  dotnet: IO.Ably.AblyAuth Properties
...
h2(#methods).
  ...
  dotnet: IO.Ably.AblyAuth Methods
...
bq(definition).
  ...
  dotnet:  public "Task<TokenDetails>:#token-details AuthoriseAsync("TokenParams:#token-params?, "AuthOptions":#auth-options?)
...
h6(#create-token-request).
  ...
  dotnet:  CreateTokenRequestAsync
...
bq(definition).
  ...
  dotnet:  public Task<"TokenRequest":#token-request> CreateTokenRequestAsync("TokenParams":#token-params tokenParams, "AuthOptions":#auth-options authOptions)
...
bq(definition).
  ...
  dotnet:  public async Task<"TokenDetails":#token-details> RequestTokenAsync("TokenParams":#token-params? tokenParams, "AuthOptions":#auth-options? options)
...
Both <span lang="ruby">@auth_options@ and @token_params@</span><span lang="default">@authOptions@ and @tokenParams@</span> are optional. When they are omitted or @null@, the default token parameters and authentication options for the client library are used, as specified in the @ClientOptions@ when the client library was instanced, or later updated with an explicit "@authorise@":#authorise request.
...
h3(#token-details).
  ...
  dotnet:  IO.Ably.Auth.TokenDetails
