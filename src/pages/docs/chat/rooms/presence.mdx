...
| `member.userClaim` | A server-signed [user claim](/docs/chat/setup#user-claims) attached to this presence event, derived from the user's [JWT](/docs/chat/setup#set-user-claims). | `String` (optional) |

<Aside data-type="usp">
Reliable presence recovery.

Ably automatically [recovers connections](/docs/platform/architecture/idempotency#connection-recovery-and-exactly-once-delivery) and prevents message loss or duplication when clients reconnect within two minutes, keeping your presence sets accurate.
</Aside>
...
| userClaim | A server-signed [user claim](/docs/chat/setup#user-claims) attached to this presence member, derived from the user's [JWT](/docs/chat/setup#set-user-claims). | String (optional) |
