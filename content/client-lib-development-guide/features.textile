...
** @(RSC7d)@ Where possible, the header @X-Ably-Platform-Version: [lib][.optional variant]?-[version]@ should be included in all REST requests to the Ably endpoint where @[lib]@ is the name of the library such as @js@ for @ably-js@, @[.optional variant]@ is an optional library variant, such as @laravel@ for the @php@ library, which is always delimited with a period such as @php.laravel@, and where @[version]@ is a string representing the version of the library's runtime environment. For example, in NodeJS this would be the output of @process.version@.
