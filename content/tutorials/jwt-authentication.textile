...
The @ttlSeconds@ variable is the duration (time-to-live) in seconds for which the JWT will be valid for. After this time the token has expired and can no longer be used for authentication, and new one must be created.
