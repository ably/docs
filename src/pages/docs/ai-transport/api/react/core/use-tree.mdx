...
Use this hook when you need to inspect tree structure outside the visible branch (for example, to count edit or regenerate siblings on a node before showing navigation arrows). [`useView`](/docs/ai-transport/api/react/core/use-view) covers branch navigation for the currently visible chain.
