...
**The problem:** One Ably customer operates a platform that streams race car telemetry to fans during live events. Each message contains hundreds of datapoints - speed, RPM, temperature, tire pressure, fuel levels, and more - sent multiple times per second to populate realtime dashboards. Between consecutive messages, most datapoints changed only slightly or not at all, and structural metadata like key names remained constant. Transmitting complete state repeatedly to every consumer would waste massive bandwidth on redundant information.

**What they needed to solve:** The customer needed to maintain their high update frequency for responsive dashboards while reducing bandwidth consumption, and required a solution that wouldn't force them to redesign their publishing logic either.
...
**The problem:** Another Ably customer operating a cryptocurrency trading platform faced a challenge with their price update distribution. During volatile market periods, individual financial instruments could update 10+ times per second, generating large volumes of price changes. However, their consumer applications only refreshed user displays every six seconds, meaning users never saw most of the intermediate values. The platform was consuming high bandwidth and generating many outbound messages that would be immediately discarded, while simultaneously risking overwhelming mobile devices and browsers with unnecessary processing and rendering work.

**What they needed to solve:** The customer needed to maintain high-frequency publishing from their price feeds without modification, while reducing outbound delivery to match what consumers actually needed. They required a way to ensure clients always received the most current price without processing every intermediate update, reducing both infrastructure costs and client-side load. The solution had to handle multiple instruments independently on shared channels while maintaining the flexibility for producers to publish at different rates.
...
**The problem:** An Ably customer providing live event chat for major sporting events faced large traffic spikes during pivotal moments. When a goal was scored, thousands of users would react simultaneously within seconds. In a 10,000-user room, just 10,000 reactions would generate 100 million outbound messages. These burst patterns created multiple risks - unpredictable cost spikes from message volume, potential rate limit violations that could degrade service, and the possibility of overwhelming client applications with processing demands.

**What they needed to solve:** The customer needed to preserve the experience of shared reactions during high-intensity moments while ensuring sustainable cost and resource usage at scale. They required a solution that would smooth traffic spikes without losing any user contributions, protect against rate limit issues during bursts, and prevent client applications from being overwhelmed. The optimization had to work transparently without requiring changes to how users published reactions or how applications subscribed to them.
