...
- **Database schema**: Design your schema to allow you to easily build the features you need, keeping in mind scale and reliability.
  ...
  - **Indexing for search**: If you're implementing search, you may want to index only the latest version of each message, or have a way to indicate which search results represent current versions. Your schema must support this efficiently.
...
Do you need all versions of a message or just the latest version?
  ...
  - When performing a message search or lookup, you may want to return only the latest version of each message, even if you store the full version history.
...
2. Do you have a requirement to store the list of clientIds who reacted to a message, or just the totals?
  ...
  - If you predict that your reaction summaries will get clipped and you need to store the list of clientIds who reacted, you will need to consider storing individual reaction events (called _"raw reactions"_ in Chat).
...
- **Ordering**: Messages can arrive out-of-order. You can sort them using their `serial` and `version.serial` properties. However, if you're storing reaction summaries, out-of-order delivery may result in an older summary overwriting a newer one, causing temporary inconsistencies.
...
- Storing message reactions can be difficult since you will not have access to the aggregate (summaries) Ably provides. If they are required, you can add them via the other methods presented in this guide.
