    ...
    if event.type == "tool_call" {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
            channel.publish(
                ...
                ] as NSDictionary as ARTJsonCompatible
            ) { error in
                if let error {
...
@MainActor
func toolCalls2(realtime: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
func toolCalls3(realtime: ARTRealtime) {
    // insert example code here
}
*/}
```swift
    ...
    if message.name == "tool_call", let data = message.data as? [String: String], data["name"] == "get_weather" {
        if let argsJSON = data["args"]?.data(using: .utf8),
           let args = try? JSONSerialization.jsonObject(with: argsJSON) as? [String: Any] {
            renderWeatherCard(["location": args["location"] as Any, "loading": true])
        }
    }
...
func toolCalls4(realtime: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
@MainActor
func toolCalls5(channel: ARTRealtimeChannel) {
    // insert example code here
}
*/}
```swift
...
channel.subscribe("tool_result") { message in
    ...
    MainActor.assumeIsolated {
        ...
        // Pass result back to the AI model to continue the conversation
        processResult(pending.responseID, toolCallID, result as Any)
...
@MainActor
func toolCalls7(realtime: ARTRealtime) {
    // insert example code here
}
*/}
```swift
        ...
        case "tool_result":
            if let toolCallID {
                ...
                if let data = message.data as? [String: Any] {
                    renderCompleted(toolCallID, data["result"] as Any)
                }
            }
