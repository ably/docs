...
** The process begins with your server, which publishes a push notification. This server could be any backend service that needs to send notifications to users' devices.
** Your server sends the notification data to Ably, a realtime messaging platform.
* Handle push notification:
** After receiving the push notification from your server, Ably processes and packages the notification with any additional data payload. This payload may include data such as message content, metadata, or any other relevant information needed for the notification.
** Ably then forwards the packaged message to the appropriate push notification service _FCM, APNs, or Web Push_.
* Deliver push notification:
** The selected push notification service _FCM, APNs, or Web Push_ takes responsibility for delivering the notification to the target devices.
** The push notification finally arrives at the end-use's devices or browsers, displaying the notification on their screen.
