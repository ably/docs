...
Using a token obtained via the REST API "@requestToken@":/rest-api/#request-token endpoint, tokens provide a means to authenticate with Ably without sharing a private API key. Tokens have  a short lifetime and therefore they may more readily be distributed to clients where there is a risk of compromise. Tokens may also be issued with a particular scope - such as a limited set of "access rights or capabilities)":/rest-api/#capabilities-explained or being limited to use by a specific "@clientId@ identity":/general/authentication#identified-clients - and therefore token-based authentication provides the flexibility to implement trusted identities and access control policies in the application. See "Authentication":/general/authentication for more details.
