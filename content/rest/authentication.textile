...
Typically, in a browser environment, the <span lang="default">"@authUrl@":#auth-options</span><span lang="csharp">"@AuthUrl@":#auth-options</span> provided would be a relative URL to a local endpoint that is used to issue tokens to trusted clients. Client requests can, for example, be trusted based on session cookies.
...
There are several "auth options":/rest/authentication#auth-options you can specify along with @authUrl@ and @authCallback@:
...
```[jsall]
const ably = new Ably.Rest({ authUrl: "/auth", authMethod: "POST", authParams: { p1: param1, b: param2}, authHeaders: {h1: header1, h2: header2} });
```
...
const options = {
  ...
  method: "POST",
  headers: {
    "User-Agent": "Rest Client v0.2",
    username: process.env.USERNAME,
    password: process.env.PASSWORD,
  },
};

const ably = new require("ably").Rest({
