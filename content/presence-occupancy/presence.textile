...
<p>Clients must be "identified":/docs/auth/identified-clients by having a @ClientId@ in order to be present on a channel. They also require the @presence@ "capability":/docs/auth/capabilities to enter the presence set, and the @subscribe@ capability to subscribe to presence events.</p>
</aside>
...
The number of clients that can be simultaneously present on a channel is "limited":/docs/pricing/limits#channel. This ensures the rate of presence messages remains supportable, as it is common for all members on a channel to change state at a similar time.
...
<aside data-type='new'>
<p>"Server-side batching":/docs/messages/batch#server-side enables you to support large-scale presence sets in your applications.</p>
</aside>
