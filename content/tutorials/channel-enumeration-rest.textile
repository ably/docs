...
"Ably's global platform":https://ably.com/platform organizes all message traffic within an application into named "channels":/channels. Clients attach to the channels they are interested in. They can then publish messages to those channels or subscribe to receive messages from them.

Often, its useful for developers to be able to access channel metadata. Ably provides three APIs for this:

* The "Channel Metadata API":/metadata-stats/metadata enables you to subscribe to "channel lifecycle events":/metadata-stats/metadata or to "channel occupancy events":/presence-occupancy/occupancy.

* The "Channel Status API":/api/rest-sdk/channel-status enables you to query the status of a specific channel or perform "channel enumeration":/api/rest-api#enumeration-rest: that is, to list __all__ active channels within an application that are associated with a particular API key and optionally retrieve the status of each channel at the same time.
...
Channel enumeration is part of the Channel Status API, and is currently only available via the "REST API":/api/rest-api. Ably provides a "REST client library":/basics/use-ably to make it straightforward to access the REST API and you will be using the client library in this tutorial.
...
For simplicity, this tutorial will use "basic authentication":/auth/basic. In production, "Ably advises that you use token authentication":/auth/token in client-side applications to "protect your API key":/best-practice-guide#using-api-key-client-side.
...
Channel enumeration is not currently integrated into Ably's REST client libraries as a dedicated method. However, you can still use the "REST client libraries":/getting-started/sdks to access the REST API it via the "request() method":/api/rest-sdk#request, which can perform arbitrary requests to Ably's REST API endpoints.
