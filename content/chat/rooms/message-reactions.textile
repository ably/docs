...
The reactions can be any strings. Summaries are aggregated based on unique strings. UTF-8 emojis are a common use case, but any string can be used as long as they are consistent across all front-ends of your app. Examples of common reactions are @👍@, @❤️@, @:like:@, @like@, @+1@, and so on. How those are presented to the user is entirely up to the app.
...
```[javascript]
    ...
    await room.messages.reactions.add(message, { reaction: '👍' });

    // The reaction can be anything, not just UTF-8 emojis:
    await room.messages.reactions.add(message, { reaction: ':like:' });
    await room.messages.reactions.add(message, { reaction: '+1' });
