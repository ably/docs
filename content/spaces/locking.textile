...
<p>Component locking subscription listeners only trigger on events related to lock status updates. Each event only contains the payload of the member that triggered it. Alternatively, "space state":/spaces/space can be subscribed to which returns an array of all members with their latest state every time any event is triggered.</p>
</aside>
...
space.locks.subscribe('update', async (lock) => {
  ...
  // Determine which user has locks for which UI components
  if (lock.request.status === LockStatus.LOCKED && self.connectionId === lock.member.connectionId) {
    ...
    // Allow edits if the lock was successfully acquired by the current user
