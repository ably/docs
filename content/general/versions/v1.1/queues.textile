...
_Please note that the @messages@ attribute is an @Array@ so that future envelope options may allow messages to be bundled into a single envelope ("Webhooks":/general/events can batch messages). However, with the current queue rule design, an envelope will only ever contain one message._
...
_Please note that the @presence@ attribute is an @Array@ so that future envelope options may allow presence messages to be bundled into a single envelope ("Webhooks":/general/events can batch messages). However, with the current queue rule design, an envelope will only ever contain one presence message._
