...
Message reactions enable clients to add reaction emojis to existing sent messages within a chat application.

Message reactions are possible through message interactions, which need to be enabled for channels. Once a channel has message interactions enabled, messages received on that channel will contain a unique @timeSerial@ that can be referenced by later messages. These message interactions also add a @ref@ object to message payloads that enables you to indicate what an interaction represents, and which message it is referring to. The @ref@ object contains two fields:
