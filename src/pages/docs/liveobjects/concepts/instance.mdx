...
<If lang="java">
In the Java SDK, the base `Instance` type has no type-specific methods. To read or update an instance, cast it to its typed form using the `as*` methods: `asLiveMap()` returns a `LiveMapInstance`, `asLiveCounter()` returns a `LiveCounterInstance`, and so on. These casts throw an `IllegalStateException` if the instance is not of the expected type. Primitive instances (for example `StringInstance`, `NumberInstance`) are read-only and anonymous: they expose only `value()`, with no `getId()`, no mutation methods and no `subscribe()`.
</If>
...
// Get the specific Instance of a LiveCounter located at the 'visits' key
Instance instance = rootObject.get("visits").instance(); // null only if nothing exists at the path
if (instance != null) {
...
<If lang="java">
In JavaScript, calling a method that doesn't match the underlying type returns an `undefined` value, an empty iterator or an `undefined` size. In Java this cannot happen: the method simply doesn't exist on the wrong typed instance, and casting to the wrong view throws an exception. Check `getType()` when the type isn't known:
