...
h2(#understanding). Quick intro to Ably Authentication
...
Token authentication is the recommended authentication scheme for client devices as tokens are short-lived and may more readily be distributed to clients where there is a risk of compromise. Tokens may also be issued with a particular scope - such as a limited set of "access rights or capabilities":#capabilities-explained or be limited to a "specific identity (<span lang="default">@clientId@</span><span lang="csharp">@ClientId@</span>)":/realtime/authentication#identified-clients. Tokens are most commonly issued server-side and may be signed using a private API key which guarantees to Ably that the token request is authentic and issued by a trusted issuer.
...
Then Ably will be unable to issue a token because the requested channel scope is greater than the API key's capabilities i.e. @chat:*@ is requested yet the API key only has acccess to the narrower @chat:team:*@ channel scope. In the example above, @requestToken@ will call the callback with an error.
