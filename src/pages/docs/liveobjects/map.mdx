...
// Create an empty map
myObject.set("settings", LiveMapValue.of(LiveMap.create())).join();
...
myObject.set("user", LiveMapValue.of(user)).join();
...
myObject.set("config", LiveMapValue.of(config)).join();
```
</Code>
...
// Each assignment creates a different object
myObject.set("map1", LiveMapValue.of(mapValue)).join();
myObject.set("map2", LiveMapValue.of(mapValue)).join();

// map1 and map2 are different objects with different IDs
Instance map1 = myObject.get("map1").instance();
Instance map2 = myObject.get("map2").instance();
...
<If lang="java">
<Aside data-type="note">
Typed reads on a `PathObject` return `null` when the path is missing or holds a value of a different type. On an `Instance` the typed reads are non-null — the throwing `as*` cast catches mismatches earlier. Learn more about reading values on a [`PathObject`](/docs/liveobjects/concepts/path-object#read-values) and [`Instance`](/docs/liveobjects/concepts/instance#read-values).
</Aside>
</If>
...
```java
// Get a PathObject to a LiveMap stored in 'settings'
System.out.println(myObject.get("settings").compactJson()); // JsonElement, null if unresolved
...
// Set a counter
settings.set("visits", LiveMapValue.of(LiveCounter.create(0))).join();
...
```java
// PathObject: remove key at path
myObject.get("settings").asLiveMap().remove("oldSetting").join();
...
Long size = myObject.get("settings").asLiveMap().size();
