...
When using the default recovery behavior with multiple ably-js SDK instances on the same origin (e.g., in different iframes or different contexts within the same page), sessionStorage key collisions can occur since the recovery key is stored under a fixed sessionStorage key name.
