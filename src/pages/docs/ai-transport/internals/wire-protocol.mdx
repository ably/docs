...
intro: "Wire-format detail for AI Transport over Ably channels. Headers, lifecycle events, content messages, and the turn sequence."
---
...
A typical turn produces the following sequence on the channel:

```
Client publish: turn-start (x-ably-turn-id=T1, x-ably-from=client)
Client publish: user message (x-ably-msg-id=M1, x-ably-parent=...)
Server publish: assistant message start (x-ably-msg-id=M2, x-ably-parent=M1)
Server append:  text delta on M2
Server append:  text delta on M2
Server append:  ...
Server update:  assistant message close (x-ably-msg-id=M2, status=finished)
Server publish: turn-end (x-ably-turn-id=T1, reason=completed)
```

Cancel inserts a cancel message between deltas; abort sets the message status to `aborted` instead of `finished`.
