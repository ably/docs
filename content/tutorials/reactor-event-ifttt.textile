...
```
Without this format, it is still possible to trigger an event, but not to send customied messages. In the case of this game, we'll be sending only `value1`, which will contain the move command.
