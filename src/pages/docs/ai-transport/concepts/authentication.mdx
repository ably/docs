...
On the core flow, the application owns the POST and sets credentials in its `fetch` call. On the Vercel flow, [`ChatTransportProvider`](/docs/ai-transport/api/react/vercel/chat-transport-provider) accepts a `credentials` prop and a `chatOptions.prepareSendMessagesRequest` hook that returns `{ body?, headers? }` per request, which the transport merges into every invocation POST.
