...
* @(RTE3)@ @EventEmitter#on@ allows a listener to be registered for either all events when no event argument is provided, or for only a single type of event when an event argument is provided. If @on@ is called more than once with the exact same arguments, the same listener is added multiple times to the array of registered listeners. Therefore, assuming the same listener is registered twice using @on@, and an event is emitted once, the listener would be invoked twice
* @(RTE4)@ @EventEmitter#once@ registers a listener that is called only once when the first of the applicable events occurs (i.e. the specified event if given as an argument, or the first event if it is registered for all events). As with @on@, it is perfectly valid to register the same listener more than once which would result in that listener being invoked multiple times once for that event. Therefore, assuming the same listener is registered twice using @once@, and an event is emitted once, the listener would be invoked twice. However, all subsequent events emitted would not invoke the listener as it was registered to be fired only once
* @(RTE5)@ @EventEmitter#off@ deregisters every matching listener for the specified event if given, else it is deregisters every matching listener for any registrations i.e. registrations for any event and registrations for a specific event. If @off@ is called with no arguments, all listeners for all and any events are removed
