...
Short disconnections (under two minutes) are resolved by replaying missed [operations](/docs/liveobjects/concepts/operations) via [automatic connection recovery](/docs/platform/architecture/connection-recovery), while longer disconnections trigger a full resync from [durable storage](#default-persistence). Either way, clients always converge to the correct state automatically.
</Aside>
