...
* @(RTP1)@ When a channel @ATTACHED@ @ProtocolMessage@ is received, the @ProtocolMessage@ may contain a @HAS_PRESENCE@ or @HAS_LOCAL_PRESENCE@ flag indicating that there are currently members present on the channel, see "TR3":#TR3 . If either flag is set, the server will shortly perform a @SYNC@ operation as described in "RTP18":#RTP18 . If that flag is unset or there is no @flags@ field, the presence map should be considered in sync immediately with no members present on the channel
