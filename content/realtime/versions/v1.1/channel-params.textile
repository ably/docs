...
If you have enabled the "**Persist Last** channel namespace rule,":/general/channel-rules-namespaces/ on the channel in question, then attaching with @rewind=1@ will retrieve that last message. Note that you specifically need @rewind=1@ to reach into long-term storage: @rewind=2@ will not work. Only messages can be persisted to long-term storage, presence messages will not be.
