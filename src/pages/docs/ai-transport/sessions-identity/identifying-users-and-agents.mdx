...
{/*
Swift test harness:
func identifyingUsersAndAgents5(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    announcementsChannel.subscribe(attachCallback: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
{/*
Swift test harness:
func identifyingUsersAndAgents9(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
{/*
Swift test harness:
func identifyingUsersAndAgents10(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
{/*
Swift test harness:
func identifyingUsersAndAgents14() async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.publish("update", data: "It's raining in London", extras: extras) { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
{/*
Swift test harness:
func identifyingUsersAndAgents15(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.subscribe(attachCallback: { error in
        ...
        if message.clientId == "weather-agent" {
            ...
            print("Response from weather agent using \(model ?? ""): \(data)")
        }
    })
}
```
