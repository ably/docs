...
* @(BO3)@ @Batch#getPresence@ function:
** @(BO3a)@ Sends a @GET@ request to @<restHost>/messages@ with the supplied with the supplied channel names joined by a comma as the @channel@ query param where @<restHost>@ represents the current rest host as described by "@RSC11@":#RSC11
** @(BO3b)@ Takes an array of strings as an argument, representing the channel names to include in the batch request
** @(BO3c)@ Returns the response body as a @BatchResult<BatchPublishResponse>@ object or an @ErrorInfo@ if a failure response is received
