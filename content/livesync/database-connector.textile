...
| Outbox table schema | Schema for the "outbox table":/livesync/outbox-node-tables#schema in your database which allows for the reliable publication of an ordered sequence of change event messages over Ably. |
| Outbox table name | Name for the "outbox table":/livesync/outbox-node-tables#outbox-table. |
| Nodes table schema | Schema for the "nodes table":/livesync/outbox-node-tables#schema in your database to allow for operation as a cluster to provide fault tolerance. |
| Nodes table name | Name for the "nodes table":/livesync/outbox-node-tables#nodes-table. |
| SSL mode | Determines the level of protection provided by the SSL connection. Options are: @refer@, @require@, @verify-ca@, @verify-full@; default value is @prefer@. |
...
There are three "options":#apply to configure the Database Connector. The following example solely demonstrates "environment variables":#variables.
