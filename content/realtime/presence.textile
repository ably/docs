...
A client must be attached to a channel, and must have the relevant permission, in order to enter. Although, the library implicitly calls <span lang="ruby">#attach</span><span lang="jsall">attach()</span> if the client is not attached at the time <span lang="jsall">enter()</span> is called.

A given <span lang="jsall">clientId</span><span lang="ruby">client_id</span> might be present multiple times, via different connections. As far as Ably is concerned these are different members of the presence set. A member of the presence set is, therefore, the combination of a <span lang="jsall">clientId</span><span lang="ruby">client_id</span> string and a <span lang="jsall">connectionId</span><span lang="ruby">connection_id</span> string unique to a connection.
...
In addition to the <span lang="jsall">clientId</span><span lang="ruby">client_id</span> for members on a channel, it is also possible to include a data payload. Clients can "update":#update their data at any point whilst present on a channel.
...
bq(definition).
  jsall: presence.history(params, callback)
