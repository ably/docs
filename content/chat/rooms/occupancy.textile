...
h2(#rest-api). Fetching occupancy with the REST API

You can retrieve the occupancy of a room using the REST API. This can be useful if you are building a server-side application or if you are working with an environment not yet supported by the Ably SDKs.

h3(#rest-api-example). Example REST API request

Here is an example of how to fetch the occupancy of a room using the REST API:

```bash
  curl 'https://rest.ably.io/chat/v1/rooms/<roomId>/occupancy&envelope=json' \
...
The following parameters are required to fetch the occupancy of a room using the REST API:
...
| ably-version | The version of the Ably API protocol to use. |
...
If the request is successful, the response should provide a @200@ status code with the occupancy metrics for the room. The response will be in the following format:
