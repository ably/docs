...
intro: "Internal transport components: StreamRouter, TurnManager, pipeStream, and the cancel routing pipeline. How the SDK coordinates between codec, channel, and application code."
---
...
```
Ably channel -> Subscription -> StreamDecoder -> StreamRouter
                                                     |
                                  +------------------+
                                  v
...
Outbound data flow on the server:

```
Application code -> Turn (via TurnManager) -> StreamEncoder -> ChannelWriter -> Ably channel
                          |
                          +---> abortSignal (cancel routing)
```
