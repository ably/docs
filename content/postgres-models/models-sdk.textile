...
The Models SDK is a typescript client library for use in your frontend applications. It helps manage the state updates based on events streamed by the Postgres Database connector via Ably channels. It is a standalone SDK built on "Ably's JavaScript SDK":/getting-started/setup with full TypeScript support.
...
A @model@ is a data model representing a specific part of the frontend application. Each frontend client can have multiple @models@ within the same application.
...
- the @merge()@ function is used by the SDK to merge state change events published by the Postgres Connector with the existing frontend state in the model.
