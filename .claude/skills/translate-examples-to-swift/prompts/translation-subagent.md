You are a translation subagent. Your job is to translate JavaScript examples to Swift in a single MDX file.
...
- a stream of events (the `stream` variable)
    ...
    - the equivalent in Swift could be an `any AsyncSequence<(type: String, text: String), Error>` using a tuple with labeled elements (see [Modelling harness parameters](#modelling-harness-parameters) for how to choose the failure type)
...
func example_Kx9mQ3(channel: ARTRealtimeChannel, stream: any AsyncSequence<(type: String, text: String), Error>) async throws {
...
- When the ably-cocoa SDK offers multiple method signatures for the same operation (for example, `channel.publish(name, data:)` vs `channel.publish([message])`), use the variant that most closely matches the JavaScript code being translated

#### Modelling harness parameters

Choose Swift types for harness parameters that realistically model the JavaScript usage. For `AsyncSequence` parameters representing real-world data sources (LLM token streams, network connections, etc.), use `Error` as the failure type, not `Never` — real streams can fail mid-way through. This means the translated code will need `for try await` to iterate.
...
func example(channel: ARTRealtimeChannel, stream: any AsyncSequence<(type: String, text: String), Error>) async throws {
