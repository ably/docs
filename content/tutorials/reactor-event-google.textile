...
Ably's "Webhooks":https://ably.com/integrations feature provides a way to define custom criteria that will automatically trigger different kinds of actions in response to channel messages, "channel lifecycle events":/realtime/metachannels#lifecycle-events (such as a channel being created) and presence events (such as users entering or leaving a channel), and is how we'll integrate with your server functions without exposing them to your users.

h2. What we'll be building

For this tutorial, we'll be using "Webhooks":/general/events events in response to incoming messages in order to *automatically invoke your own Google functions*. We're going to build a simple pizza ordering system with a virtual pizza assistant chat bot. When a new customer hits your website, they'll be assigned a random ID, and their messages will be published to a private channel created just for that customer. A webhook triggered by those messages will call your Google functions to invoke the pizza chat bot to take their order. Using the Ably REST API, it will post its responses back on another channel that the web application subscribes to.
...
1. Take a look at the "Webhook documentation":/general/events for further details about what was described in this tutorial.
...
4. Gain a good technical "overview of how the Ably realtime platform works":https://ably.com/documentation/key-concepts
