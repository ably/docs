...
Before running your server, make sure to replace the API Key in 'server.js' file with your own. The DEMO would not work otherwise.
...
Let's start by creating a node.js file and call it server.js. Now, let's require the "jsonwebtoken":https://www.npmjs.com/package/jsonwebtoken npm library. Before doing this, make sure that you have node and npm installed on your system.
...
<a href="/images/tutorials/api-key-parts.jpg" target="_blank">
    <img src="/images/tutorials/api-key-parts.jpg" style="width: 100%" alt="API key parts">
</a>

As you see, the first part before the dot forms the @appId@, the second part before the colon forms the @keyId@ and the remaining part after the colon forms the @keySecret@. We'll make use of these parts for our JWT token request as you'll see further. Let's go ahead and add it in our file:
...
```
Next, it's time to set up the server. We'll use "express":https://www.npmjs.com/package/express to build a simple server to serve files on our localhost:
...
Let's now set up the '/auth' route in this server that will infact be responsible to create a JSON Web Token and send this token back to the client, confirming it's authentication.
...
```
As you can observe, we begin by instantiating Ably's Realtime client library, then we pass the '/auth' path of our auth server within the @authUrl@ parameter. If you remember from above, the '/auth' path of our auth server is responsible to sign a JWT using the private API key and return back the TokenID, in other words, the JSON Web Token itself.

When this is successful, we send an alert to let the client know that the connection was successful. One last caveat is that because browsers often cache the token which in our tutorial is not automatically renewed, we might end up trying to use an expired (but cached) token. In order to prevent this, add a manifest file called 'manifest.appcache' in the root folder
...
*
```
then add the manifest attribute to your html tag:

```[html]
<html manifest="manifest.appcache">
```
This will prevent your HTML file from being cached by the browsers. Both the server and client-side code contains helpful log messages so you can understand easily the step-by-step execution of the complete tutorial.

That's it, simple as that!

h2. Step 4 - Checking the output

Now that we're all set with all of our files in place,let's go ahead and test it out. Fire up your command line/terminal. Most of the code editors come with a handy integrated terminal. After making sure you are in the right folder, start your server by running:

```[sh]
node server.js
```

Now open up a browser and visit 'localhost:3000' since our express server is listening on this port. You should see the login form. Keep the browser console open if you wish to see the logged messages. Enter some dummy data into the username and password fields and hit login button.

If all went well, you should see an alert with the message 'Client successfully connected Ably using JWT auth'.

h2. Next steps

TODO-ADD-JWT-AUTH-LINK
