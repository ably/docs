...
Configure conflation through [rules](/docs/channels#rules) in your Ably dashboard. You specify a conflation interval and a conflation key pattern that determines which messages are related. Messages with the same conflation key within the time window are then conflated together automatically, no code changes are needed.
...
4. Set a batching interval (e.g., 100ms for responsive dashboards), determining how frequently batches are sent to subscribers.
5. For conflation, configure the conflation key pattern (referenced in message `extras.ref.params.key`) to determine which messages are related and should be conflated together.

Using namespace patterns means you can apply different rules to different parts of your application. For example, you might enable conflation on high-frequency sensor channels (`^sensors:.*`) where only the latest reading matters, while using server-side batching on event feeds (`^events:.*`) where complete event history is required. You can also combine both strategies—server-side batching with conflation enabled will first conflate messages by key, then batch the conflated messages together for efficient delivery.
