...
| clientId | The client ID configured on the underlying Ably Realtime client. Used to identify the current user. May be `null` until authenticated with a token. | String |
...
| message | The exception message, or `null`. | String |
| cause | The underlying cause of the exception, if any, or `null`. | Throwable |
...
| href | A URL providing more information about the error, or `null`. | String |
| cause | The underlying cause of the error, or `null`. | ErrorInfo |
| requestId | The request ID if the error originated from an API request, or `null`. | String |
