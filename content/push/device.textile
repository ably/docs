...
h3. Configure Web Push for web browsers

* Web push is supported using the "W3C Push API":https://www.w3.org/TR/push-api/ using an app-specific VAPID key pair which is created when you first activate a web browser.
* In order to subscribe to push messages, you must provide a service worker which is used to process inbound notifications.
