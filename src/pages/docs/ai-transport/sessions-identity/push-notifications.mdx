...
{/*
Swift test harness:
func example(getUserSettings: @escaping (String) async -> UserSettings) {
    // insert example code here
}
*/}
```swift
// Agent code
func shouldNotifyUser(withID userID: String) async -> Bool {
    // Check the user's notification preference from your application's settings
    let userSettings = await getUserSettings(userID)
    return userSettings.pushNotificationsEnabled
}
```
</Code>
...
func example(
    ...
    userID: String,
    result: [String: Any],
    sendPushNotification: @escaping (String, String, [String: Any]) -> Void
) {
    // insert example code here
}
*/}
```swift
// Agent code
func onTaskComplete(channel: ARTRealtimeChannel, userID: String, result: [String: Any]) {
    // Always publish results to the channel for history and connected clients
    channel.publish("result", data: result)
    ...
    channel.presence.get { members, error in
        ...
        let userIsOnline = members.contains { $0.clientId == userID }
...
{/*
Swift test harness:
func example(channel: ARTRealtimeChannel, userID: String) {
    // insert example code here
}
*/}
```swift
// Agent code
func shouldSendPush(channel: ARTRealtimeChannel, userID: String) async -> Bool {
    await withCheckedContinuation { continuation in
        channel.presence.get { members, error in
            let userDevices = (members ?? []).filter { $0.clientId == userID }
...
{/*
Swift test harness:
func example(ably: ARTRealtime, userID: String, channel: ARTRealtimeChannel) {
    // insert example code here
}
*/}
```swift
...
{/*
Swift test harness:
struct ExamplePushNotification {
    let data: [String: String]
}

func example(navigateToSession: @escaping (String, String) -> Void) {
    // insert example code here
}
*/}
```swift
// Client code - handling a push notification tap
func onNotificationTap(_ notification: ExamplePushNotification) {
    guard let channelName = notification.data["channelName"],
          let sessionID = notification.data["sessionId"] else { return }
    // Navigate to the session and reattach to the channel
    navigateToSession(sessionID, channelName)
}
```
...
func example(
    ...
    sessionID: String,
    result: [String: Any],
    generateSummary: @escaping ([String: Any]) -> String
) {
    // insert example code here
}
*/}
