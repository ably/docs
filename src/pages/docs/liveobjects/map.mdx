...
<If lang="java">
Unlike JavaScript, Java wraps every value passed to `set()`, blueprints included, in `LiveMapValue.of(...)`.
</If>
...
<If lang="java">
<Aside data-type="note">
Typed reads on a `PathObject` return `null` when the path is missing or holds a value of a different type. On an `Instance` the typed reads are non-null, because the throwing `as*` cast catches mismatches earlier. Learn more about reading values on a [`PathObject`](/docs/liveobjects/concepts/path-object?lang=java#read-values) and [`Instance`](/docs/liveobjects/concepts/instance?lang=java#read-values).
</Aside>
</If>
