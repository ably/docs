...
- "Channel lifecycle events":#lifecycle-events provide information on when a channel changes its "lifecycle state":#lifecycle-events. 
- "Occupancy":#occupancy events are triggered whenever there is a change in the number occupants in a channel, indicating how many of each type of occupant there are.
- The "Channel Status API":/rest/channel-status allows for users to "enumerate":/rest/channel-status#enumeration-rest through all active channels, along with providing "REST":/rest-api alternatives to the above two points for individual channels. Our recommendation is to make use of the realtime library where possible.
...
At present, REST "requests":/rest/usage#request for the "channel status API":/rest/channel-status  will not count towards your message limit. If you are making use of "Reactor":/reactor however, then each message sent over Reactor *will* count towards your message limits.
