...
| "Messages":#messages (per hour) | 83,300 | 100,000 | Quota / 72 | 2.5x quota / 72 | Quota / 72 | 5x quota / 72 |\2. Custom |
| "Message rate":#messages (per second) | - | 70 | - | Hourly * 2.5 | - | Hourly * 2.5 |\2. Custom |
| "Total bandwidth":#bandwidth (GiB) | 12 | 14.4 | Quota | 2.5x quota | Quota | 5x quota |\2. Custom |
| "Bandwidth":#bandwidth (per hour in MiB) | 166 | 200 | Quota / 72 | 2.5x quota / 72 | Quota / 72 | 5x quota / 72 |\2. Custom |
| "Bandwidth rate":#bandwidth (per second in KiB ) | - | 110 | - | Hourly * 2.5 | - | Hourly * 2.5 |\2. Custom |
...
| "Firehose message rate":#firehose-messages (per second) | 50 | - | - | Custom |
...
|_/2=. Limit |_\2=. Free |_\2=. Self-service |_\2=. Business (legacy) |_\2=. Enterprise |
|_. Soft |_. Hard |_. Soft |_. Hard |_. Soft |_. Hard |_. Soft |_. Hard |
| API requests (per hour) | 8,333 | 10,000 | 0.1 * hourly message soft limit | 0.1 * hourly message hard limit| 0.1 * hourly message soft limit | 0.1 * hourly message hard limit |\2. Custom |
| API request rate (per second) | - | 27 | - | 50 minimum | - | 50 minimum |\2. Custom |
