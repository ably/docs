...
Values in a map can contain <If lang="javascript,java">numbers, strings, booleans and buffers</If>numbers, strings, booleans, `Data`, JSON objects and JSON arrays<If lang="swift"></If>, as well as other `LiveMap` and `LiveCounter` objects.
...
```java
// Initial subscription
ObjectsSubscription subscription = map.on("deleted", () ->
...
```java
// Initial subscription
Runnable listener = () -> System.out.println("Map deleted");
map.on("deleted", listener);
// To remove the listener
map.off("deleted", listener);
```
