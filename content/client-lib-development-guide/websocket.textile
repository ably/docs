...
If the library has been initialized with a clientId then the authorisation params must include a "client_id" param with the clientId value. In the case of token auth, the token must either have been issued against that clientId or against the wildcard clientId.
