...
* @(RTE3)@ @EventEmitter#on@ allows a listener to be registered for either all events when no event argument is provided, or for only a single type of event when an event argument is provided. If @on@ is called twice with the exact same arguments, two listeners are added to the array of registered listeners. Therefore, it is perfectly valid to register the same listener multiple times and expect the listener to be called multiple times for each event emitted
* @(RTE4)@ @EventEmitter#once@ registers a listener that is called only once when the first of the applicable events occurs (i.e. the specified event if given, or the first event if it is registered for all events). As with @on@, it is valid to register the same listener more than once which will result in the listener being called once for each registration
