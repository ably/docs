...
In the current Ably service the backend supports the Realtime protocol over a "WebSocket transport":https://www.ably.io/concepts/websockets and over a Comet transport. Other transports are possible and this document aims to define the protocol in a transport-independent manner.
...
Separate sections provide more detailed information on the "WebSocket":https://www.ably.io/concepts/websockets and Comet transports.
...
- CONNECT (3) := Unused. This is a placeholder for transports that might need to pass connection request parameters in a Protocol Message instead of in URI params as happens presently for the "WebSocket":https://www.ably.io/concepts/websockets and Comet transports.
...
In the "WebSocket":https://www.ably.io/concepts/websockets transport, the service indicates success or failure to the client with the message acknowledgement protocol. This is a simple series of @ACK@ or @NACK@ responses, each addressing a contiguous sequence (by @msgSerial@) of messages.
