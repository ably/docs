...
jump_to:
  ...
  Push API:
    ...
    - list registered devices#get-device-registrations
    - list channel subscriptions#get-channel-subscriptions
...
The request body contains device details and is an object of the form:

bc[json]. {
  id: <string>,
  clientId: <optional client identifier to associate with registration>,
  platform: <ios, android, browser>,
  formFactor: <mobile, tablet, desktop, other>,
  metadata: <object with additional data>,
  push: {
    transportType: <gcm, fcm, apns, web>,
    metadata: <object with additional data, like transport-specific recipient address>
  }
}

The recipient address is necessary and varies by underlying transport service.
...
- Content-Type := @application/json@, @application/x-msgpack@ or @application/x-www-form-urlencoded@
- Accept := @application/json@ by default, or @application/x-msgpack@, @text/html@
- Auth required := yes ("basic":#basic-authentication or "token":#token-authentication with @push-subscribe@ capability; when registering with a client ID, the token must be associated with it)

A successful request returns the just-registered device details. It includes a @updateToken@, which is a token that can be used to "authenticate":#token-authentication afterwards to "update an existing device registration":#update-device-registration (for example, for updating a GCM or FCM registration token).
...
bc[json]. {
  deviceId: <optional, string, must be set when clientId is empty, cannot be set when clientId is set>,
  clientId: <optional, string, must be set when deviceId is empty, cannot be set when deviceId is set>,
  channel: <string>
}
...
- Content-Type := @application/json@, @application/x-msgpack@ or @application/x-www-form-urlencoded@
- Accept := @application/json@ by default, or @application/x-msgpack@, @text/html@
- Auth required := yes ("basic":#basic-authentication or "token":#token-authentication with @push-subscribe@ capability; when subscribing for a client ID, the token must be associated with it)

A successful request returns the subscription details.

A unsuccessful request returns an error.

h3(#delete-device-registration). Unregister devices for push notifications

h6. DELETE rest.ably.io/push/deviceRegistrations

Unregister devices. All their subscriptions for receiving push notifications through channels will be removed.

Example request:

bc[sh]. curl -X DELETE https://rest.ably.io/push/deviceRegistrations?deviceId=01ARZ3NDEKTSV4RRFFQ69G5FAV \
 -u "{{API_KEY}}"

h5. Parameters

- deviceId := Filter to restrict to subscriptions for that @devideId@. Cannot be set when @clientId@ is set.
- clientId := Filter to restrict to subscriptions associated with that @clientId@. Cannot be set when @deviceId@ is set.

h5. Options

- Content-Type := not applicable
- Auth required := yes ("basic":#basic-authentication or "token":#token-authentication with @push-subscribe@ capability; when deleting by client ID, the token must be associated with it)

h5. Returns

A successful request returns nothing.
...
Stop receiving push notifications when push messages arrive to channels.

Example request:

bc[sh]. curl -X DELETE https://rest.ably.io/push/channelSubscriptions?deviceId=01ARZ3NDEKTSV4RRFFQ69G5FAV \
 -u "{{API_KEY}}"

h5. Parameters

- channel := string, optional. If not set, all subscriptions for all channels will be deleted.
- deviceId := Filter to restrict to subscriptions for that @devideId@. Cannot be set when @clientId@ is set.
- clientId := Filter to restrict to subscriptions associated with that @clientId@. Cannot be set when @deviceId@ is set.
...
h3(#update-device-registration). Update a registered device

h6. PUT rest.ably.io/push/deviceRegistrations/@<device id>@

The body must have the same shape as when "registering the device":#post-device-registration, except only fields to be changed should be provided.

Only @metadata@ and @push.metadata@ can be updated. If non-null, the provided new values will be merged with their old values.

Example request:

bc[sh]. curl -X PUT https://rest.ably.io/push/deviceRegistrations/01ARZ3NDEKTSV4RRFFQ69G5FAV \
 -H "Authorization: Bearer `echo -n 'myUpdateToken' | base64`" \
 -H "Content-Type: application/json" \
 --data \
 '
 {
...
h3(#get-device-registrations). List registered devices
...
h3(#get-channel-subscriptions). List channel subscriptions

h6. GET rest.ably.io/push/channelSubscriptions

Obtain push notification subscriptions to channels.
...
h3(#get-channels). List all channels with at least one subscribed device
...
Convenience endpoint to deliver a push notification payload to a single device.

Note that this bypasses. If you want to send a push notification to multiple devices, or use a more flexible publish-subscribe architecture so that you don't need to know about recipient devices's details, you should look at "registering devices for push":#post-device-registration, then "subscribe them to channels":#post-channel-subscription, and then "send messages to the channels with push payloads":#message-extras-push.
...
bc[json]. {
  recipient: <object>
  <... rest of the fields just like a normal push-enabled message ...>
}
...
The rest of the fields are as "in a normal push-enabled Ably message":#message-extras-push.
