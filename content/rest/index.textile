...
h1(#overview). REST API Overview

The Ably REST API provides a way for a wide range of server and client devices to communication with the Ably service. The REST API does not support real-time connections, but in all other respects is a simple subset of the full Real-time messaging API.
...
The API is defined at the REST level of in terms of the available routes, supported parameters and expected request and response data types; these are set out below. Ably also provides "client libraries for a range of languages":https://ably.io/download/rest-client-libraries including "Java":https://ably.io/download/rest-client-libraries, "Python":https://ably.io/download/rest-client-libraries, "Ruby":https://ably.io/download/rest-client-libraries, ".NET":https://ably.io/download/rest-client-libraries.

Note also that since the REST API is also a direct subset of the Real-time API, all "Real-time client libraries":https://ably.io/download/realtime-client-libraries also provide access to the REST API.

h2. Common API behaviour
...
The REST API may be accessed using either Basic or Token Authentication. Additionally the "@requestToken@":#requesttoken endpoint may be used without explicit authentication, relying on the implicit authentication of submitting a validly signed token request.

Both authentication methods use the @Authorization@ header when using the REST API, as follows.

h3(#basic). Basic authentication
...
h3(#history). GET /apps/@<app id>@/history

h4. Retrieve message history for an application.
...
h3(#stats). GET /apps/@<app id>@/stats

h4. Retrieve usage statistics for an application.
...
h3(#publish). POST /apps/@<app id>@/channels/@<channel id>@/publish

h4. Publish one or more events on a channel
...
h3(#channel-history). GET /apps/@<app id>@/channels/@<channel id>@/history

h4. Retrieve message history for a channel
...
h3(#channel-stats). GET /apps/@<app id>@/channels/@<channel id>@/stats

h4. Retrieve usage statistics for a channel.
...
- Alternative paths := <pre>GET /channels/@<channel id>@/stats</pre>
- Request body := not applicable
- Content-Type := not applicable
- Accept := @application/json@ by default, or @text/html@, @text/xml@
- Auth required := yes ("basic":#basic or "token":#token)
...
h3(#presence). GET /apps/@<app id>@/channels/@<channel id>@/presence

h4. Retrieve instantaneous presence status for a channel
...
h3(#presence-history). GET /apps/@<app id>@/channels/@<channel id>@/presence/history

h4. Retrieve presence history for a channel
...
h3(#requesttoken). POST apps/@<appId>@/requestToken

h4. Request an access token
