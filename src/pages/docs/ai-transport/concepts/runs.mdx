...
console.log('Run started:', activeRun.runId);
...
On the agent side, the symmetric primitive is [`AgentSession.createRun(invocation, runtime?)`](/docs/ai-transport/api/javascript/core/agent-session#create-run). The Invocation carries the same `runId`, so both sides converge on the same Run identity.
