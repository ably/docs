---
title: Building a Clockface app for Fitbit using Ably
section: tutorials
index: 95
---
...
* You do not need to install Fitbit SDK separately as we'll be using NPX (that comes bundled with NPM) for doing this.
...
We'll be using the Fitbit command line interface for creating the clockface. You find the guide "here":https://dev.fitbit.com/build/guides/command-line-interface/ .

We'll start by creating a new clockface. To do this, we'll be using the @create-fitbit-app@ command that allows you to generate the minimal scaffolding required for either an app or clock face.
...
So since we are making a clockface for the London Tube schedule, we ask user for three inputs the @Line@, @Station@ and @Towards@ .
Each input is an autocomplete, hence the user can just type and select the option he/she desires. 
...
```
You can refer the Settings guide "here":https://dev.fitbit.com/build/guides/settings/ .
...
It contains two arrays, one is used for @StationsIDMapping@ (line & station combination - @stationid@) and other is used for @towardsMapping@ (line & origin combination to get @towards@ options) . 
...
Subscribe to the product Ably hub for the TFL London tube schedule "here":https://www.ably.io/hub/products/10 .

The @locationSuccess@ gets triggered everytime the location changes and then we get the new @latitude@ & @longitude@. 
Now using that location we send it as @origin@ & the @station@ as @destination@ to the Google API.
Then we get the @walking time@ as response from the Google API, from current location to your selected station. 
...
The new data to the hub is published automatically for the given input and we just subscribe to the channel to recieve that.
...
The data the consists of three times - walking time, time of next train & leave time(that the person uses to decide when to leave the current location to catch the train) . 
Add the following code in your @index.js@ file which is inside the @companion@ folder: 
     ...
     trainChannel.subscribe(msg => {
       ...
       const trains = msg.data.filter(train => {
         return (
           ...
           new Date(train.ExpectedArrival) > new Date()
         );
       });
...
h2(#send-message). Step 7 - Update the settings options onchange of a setting

Now we need to update the settings options(dropdown) on change of any setting since all the settings depend on the previous above settings value.
We also need to call the @sendMessage@ function on updating the settings or on starting up with all settings values selected already.
Lastly we need to hear on errors to the @messaging@ module since that helps in communication between the companion and app.
Add the following code in your @index.js@ file which is inside the @companion@ folder: 
...
h2(#send-message). Step 8 - Update the gui files of app
...
Empty the file @styles.css@ which is inside the @resources@ folder. We have added the styles in the gui files and we are also updating them in the @index.js@ of the app depending on the values. 

"See this step in Github":https://github.com/ably/tutorials/commit/55d2b927547b6b7520fe4bde2dec0fa0948ae152

h2(#send-message). Step 9 - Updating the text on the watch with dynamic values

Now we when we get the data that we need using the @messaging@ module to the @app@ from the @companion@.
Using those values we decide the color of the text eg: Red when time is less . We also set the time to be displayed on the watch.
...
Go ahead and open the simulator, and run the app using the commands which we used before.
If you're outside the fitbit shell then first get into it or you can directly run @bi@:
