...
### Connection timeouts in React Native <a id="connection-timeouts"/>

If you're experiencing immediate connection timeouts in React Native, where connections go from `connecting` to `disconnected` state immediately with a reason of "Connection to server temporarily unavailable", this may be caused by incorrect device time settings.

#### The problem

This issue is commonly seen in:

- **Debug mode**: React Native's debug environment can interfere with JavaScript timers
- **Emulators**: Virtual devices may have incorrect system time
- **Devices with wrong time**: Devices with incorrect date/time settings

The root cause is that JavaScript's `setTimeout` and related timer functions are not working correctly, causing Ably's connection timeouts to expire immediately.

#### Diagnosis

You can diagnose this issue by testing JavaScript timers directly:

```javascript
// This should print "done" after 10 seconds
// If it prints immediately, you have a timer issue
setTimeout(() => console.log("done"), 10000);
```

If "done" appears immediately instead of after 10 seconds, your React Native environment has a timer problem.

#### Solutions

**For emulators:**
1. **Android emulator**: Ensure the emulator's date and time are set correctly in the device settings
2. **iOS simulator**: Check that the simulator's time matches your host system

**For debug mode:**
1. **Try release mode**: Test your app in release mode instead of debug mode to see if the issue persists
2. **Restart Metro bundler**: Stop and restart your React Native development server

**For physical devices:**
1. **Check device time**: Ensure the device's date, time, and timezone are correct
2. **Network time sync**: Enable automatic date/time synchronization in device settings

**Alternative debugging approach:**
If timer issues persist, you can also test Ably connections using a REST client instead of realtime to isolate whether the problem is specific to persistent connections or affects all Ably operations.

<Aside data-type="important">
This timer issue affects all JavaScript timeout-based functionality in React Native, not just Ably connections. Fixing the underlying timer problem will resolve connection issues and improve overall app performance.
</Aside>

For more React Native-specific guidance, see the [React Native debugging documentation](https://reactnative.dev/docs/debugging) and the [StackOverflow discussion](https://stackoverflow.com/questions/51163349/javascript-settimeout-immediately-runs-in-react-native) referenced for this specific timer issue.
