...
Lists push subscriptions on a channel specified by it's channel name (@channel@). These subscriptions can be either client (@clientId@) subscriptions, device (@deviceId@) subscriptions, or if @concatFilters@ is set to @true@, both. This method requires clients to have the "Push Admin capability":/general/push/admin. For more information, take a look at the @GET rest.ably.io/push/channelSubscriptions@ "Rest API":/rest-api.
