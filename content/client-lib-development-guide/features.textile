...
* @(TS4)@ @Stats.ConnectionTypes@ - contains a breakdown of summary stats data for different (TLS vs non-TLS) connection types. Has the following properties:
...
* @(TS5)@ @Stats.MessageCount@ - contains aggregate counts for messages and data transferred. Has the following properties:
** @(TS5a)@ @count@ - integer. The count of all messages.
** @(TS5b)@ @data@ - integer. The total number of bytes transferred for all messages.
* @(TS6)@ @Stats.MessageTypes@ - contains a breakdown of summary stats data for different (channel vs presence) message types. Has the following properties:
...
* @(TS7)@ @Stats.MessageTraffic@ - contains a breakdown of summary stats data for traffic over various transport types. Has the following properties:
...
* @(TS8)@ @Stats.RequestCount@ - contains aggregate counts for requests made. Has the following properties:
...
* @(TS9)@ @Stats.ResourceCount@ - contains aggregate data for usage of a resource in a specific scope.
...
### Doesn't even exist in the Ruby codebase (so this link is a non-starter), nor JS, but exists in iOS and Java (`PushedMessages`) - these two platforms differing in those types
### touch for commit message
* @(TS10)@ @Stats.PushStats@ - see the "Ruby PushStats documentation":https://www.rubydoc.info/gems/ably/Ably/Models/Stats/PushStats
### Doesn't even exist in the Ruby codebase (so this link is a non-starter), nor in iOS nor JS nor Java
...
### Wait, no, the above both exist in JS and Rust, see
### https://github.com/search?q=org%3Aably+xchgmessages&type=code
### https://github.com/ably/ably-js/blob/3386abae8f35f5d41f016c0feeb7406bb5537cb2/src/common/lib/types/stats.ts
### https://github.com/ably/ably-rust/blob/main/src/stats.rs
### but in JS it's not in ably.d.ts, just the implementation
### Where is this type in the Ruby codebase? I want to check it and the below properties
