...
You must also update the listener provided to `DisposableEffect` to handle the `MessageEventType.Updated` event:
...
Implement a `replaceFirstWith` function:
