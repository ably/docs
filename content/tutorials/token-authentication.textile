...
blang[python].
  "See this step in Github":https://github.com/ably/tutorials/commit/2627e67

blang[php].
  "See this step in Github":https://github.com/ably/tutorials/commit/9bdec36
...
blang[swift].
  ...
  First we will add a Login button and a textfield so that user can provide their choosen username. Go to your @ExampleViewController@ and add a @UIButton@ from @Object library@. Name the action "loginAction".
  ...
  To define the flow between the @ExampleViewController@ and the @LogoutViewController@ we will use segues. If you are not familliar with them, please refer to "Apple developer guide":https://developer.apple.com/library/content/featuredarticles/ViewControllerPGforiPhoneOS/UsingSegues.html on how they work.
  ...
  Last thing we need to do is perform a logout action. We didn't connect the logout button to an action because we will connect it to a segue right now. In your @Storyboard@ hold ctrl and right-click the button and drag it to @ExampleViewController@ just like the erlier segue. Name it "toLoginSegue". When user taps the logout button, before the segue is performed we will close the connection to ably and nullify the instance. All the actions that should be done before performing the segue can be done in @prepareForSegue@ method in your @LogoutViewController@:
...
blang[android].
  <a href="/images/tutorials/token-authentication-android-anonymous.png" target="_blank">
...
blang[python].
  "See this step in Github":https://github.com/ably/tutorials/commit/911640d

blang[php].
  "See this step in Github":https://github.com/ably/tutorials/commit/fbf5c45
