...
| Type | Origin | Handle |
| --- | --- | --- |
| Own Run | This client sent the HTTP POST that created the Run. | An `ActiveRun` returned from `view.sendMessage`, with a `stream` and a `cancel()` method. |
| Observer Run | Another client or agent created the Run. | Lifecycle events and streamed messages only; no direct stream handle. |

Both types appear in the conversation tree and the UI. The difference is internal routing: own Runs have a dedicated stream, observer Runs accumulate from channel messages. `RunInfo.clientId` on the view tells you which client started any given Run.
...
The `clientId` is used throughout: turn ownership, cancel scoping (`{ own: true }` filters by the sender's client ID), and active turn tracking. See [Set up authentication](/docs/ai-transport/getting-started/authentication) for the full setup.
