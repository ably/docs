...
** @(RTC5c)@ Proxy to @RestClient#time@ presented with an async or threaded interface as appropriate
...
** @(RTN12b)@ Sends a @CLOSE@ @ProtocolMessage@ to the server, sets the state to @CLOSING@ and waits for a @CLOSED@ @ProtocolMessage@ to be received
** @(RTN12c)@ If the @CLOSED@ @ProtocolMessage@ is not received within the "configured close timeout":https://github.com/ably/ably-ruby/blob/master/lib/ably/realtime/connection/connection_manager.rb, the transport will be disconnected and the connection will automatically move to the @CLOSED@ state
** @(RTN12d)@ If the transport is abruptly closed following a @CLOSE@ @ProtocolMessage@ being sent, then the connection will automatically move to the @CLOSED@ state
...
** @(RTN5c)@ If the connection state is @CLOSED@, @CLOSING@, @SUSPENDED@ or @FAILED@, an exception should be thrown immediately
...
** @(RTN6c)@ Channel state conditions:
...
** @(RTP8a)@ Enters this client into this channel
** @(RTP8b)@ Optionally a callback can be provided that is called for both success or failure to enter
** @(RTP8c)@ A @PRESENCE ProtocolMessage@ with a @PresenceMessage@ with the action @ENTER@ is sent to the Ably service
...
** @(RTP8f)@ Raises an exception of the client library has not @clientId@ configured
** @(RTP8g)@ Raises an exception if the channel is @DETACHED@ or @FAILED@
** @(RTP8h)@ Fails and emits an error if the client does not have required presence permissions
...
** @(RTP9d)@ A @PRESENCE ProtocolMessage@ with a @PresenceMessage@ with the action @UPDATE@ is sent to the Ably service
** @(RTP9e)@ In all other ways, this method is identical to @Presence#enter@ and should have matching tests
* @(RTP10a)@ @Presence#leave@ function:
** @(RTP10b)@ Leaves this client from the channel and the data will be updated with the value provided. If the language permits the data argument to be omitted, then the previously set data value will be omitted as a convenience
** @(RTP10c)@ Optionally a callback can be provided that is called for both success or failure to leave
** @(RTP10d)@ A @PRESENCE ProtocolMessage@ with a @PresenceMessage@ with the action @LEAVE@ is sent to the Ably service
** @(RTP10e)@ If the client is not currently @ENTERED@ then an exception is raised
** @(RTP10f)@ In all other ways, this method is identical to @Presence#enter@ and should have matching tests
...
** @(RTP14d)@ A test should exist that registers a number of members each with different @clientId@s on a presence channel, and then a @Presence#get@ should be used to verify that all members are present as expected
