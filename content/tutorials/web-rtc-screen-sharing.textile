...
First, we need to make an @Enter@ subscription, so that we are notified every time a new member joins our channel. We do so by using "@AblyRealtime.presence@":/presence-occupancy/presence. In the callback, we get a list of all current members and add it to our member’s list. After this, we call on the render members method.
