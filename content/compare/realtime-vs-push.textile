...
| *Message history* | Partial. Up to 128 messages can be retained for up to 60 seconds. Although this is disabled by default. | Can clients or servers retrieve messages previously published on a channel? | Partial. A fixed number of messages can be retained for a small amount of time. A NoSQL cloud database with built-in real-time notifications keeps data synchronized between users. |
