...
The body must have the same shape as when "registering the device":#post-device-registration, except only fields to be changed should be provided. The @PUT@ operation is in fact a @PATCH@ operation i.e. only fields provided are updated.

Only @metadata@ and @push.recipient@ can be updated. If non-null, the provided new values will be merged with their old values.
