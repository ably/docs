---
title: Less code, more efficient screen sharing with Ably
section: tutorials
index: 107
---
h2. Introduction

Screen sharing software allows people from a variety of locations view another computer screen in real-time. When you share your screen, your guests can see exactly what’s happening on your computer desktop at the time it is happening. 

Both Chrome and Firefox currently have built-in support for screen capturing in the desktop versions.
Chrome has built in support for screen capture, however to gain permission to use this functionality, an application must use a Chrome Extension.
For Firefox, the situation is simpler, as Firefox allows us to request for the screen capture directly.

In this lesson, we will learn how to share our screen using WebRTC. 
...
Open up a new file called @index.html@ and add the following code:
...
# Required the temasys  @adapter.js@ to iron out cross platform issues for WebRTC
# Required the temasys  @adapter.screenshare.js@ which handles connection to Chrome extensions and getting screen permissions.
# Required the Ably JavaScript library
# Required the simple peer JavaScript library.
# Required a JavaScript file named @ably-screenshare.js@ (which we will create soon).
...
Let us move ahead by understanding the methods defined in the class:
...
Create a file named @ably-screenshare.js@ and add:
...
Lastly, we have our @renderMembers@ functions, which loops through the @membersList@, and append them as list items to the `ul` tag with the id of @memberList@. 

h2.
  default: Step 6 – Sending and receiving screen calls
...
Before jumping into the code, it would be nice to notify users when a call is coming in, and when a call is declined. Let’s get started doing that.
...
Now, let’s go ahead to make, receive and end the call. 
...
Here, we defined the @initiateCall@ method, which waits for the @adapter.js@  function to be ready, then gets the stream from the user via @navigator.mediaDevices.getUserMedia@, then passes it to the @Connection@ class, to initiate a connection to the other peer. You will also notice a helper function @attachMediaStream@ which is used to render the stream to the video element. This function is also exposed by @adapter.js@
...
Next, you would see the @recieveStream@ function. This function was called in step 4, under the @_onStream@. What it does, is to load the remote stream into the second video tag in our markup. 
...
Looking at the function above, we notice that we receive an optional argument of @client_id@. Why is this argument optional?

Remember in the @Connection@ class, the @_onClose()@ method calls this function, passing in the client id of the other peer whose connection was closed/dropped. We use this parameter, to know which connection to delete from our @connections@ object. This function is also called when you click the @end call@  button, but here, no parameter is passed, as we have a variable called @currentCall@, which holds the value of the call you must be ending. 

In this function, we destroy the peer connection, as well as deleting the key from our connection objects. We go ahead to stop the Video and Audio streams coming from the webcam, then hide the video inputs.

h2. Conclusion

In this tutorial, we have seen how to use WebRTC in conjunction with Ably to create real time screen share app. Have any thoughts on how we could make this lesson better? Let us know.
