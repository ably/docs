...
## Debugging WebHooks <a id="debugging"/>

If your WebHooks aren't being received, use `RequestBin` to help identify where the issue is occurring. `RequestBin` is a public service that provides an HTTP endpoint and logs all incoming requests.

Follow these steps to debug a WebHook:

1. Create a new `RequestBin` at https://www.requestbin.com and copy the provided URL.

2. In your app dashboard, navigate to the Integrations tab and create a new WebHook rule using your `RequestBin` URL.

3. Click the *Test rule* button to verify that Ably can successfully send messages to your `RequestBin` endpoint.

4. Open the Dev Console, attach to a channel, and publish a test message.

5. Refresh your `RequestBin` page to confirm the WebHook request was received.

6. Delete the test *WebHook rule* once you've finished debugging.

If the WebHook was successfully received by `RequestBin`, the issue is not with Ably's message delivery. Check the following:

1. Any channel filters that may be blocking messages.
2. Your web server logs to confirm requests are arriving.
3. Your endpoint URL is publicly accessible.

Contact [support](https://ably.com/support) if you need help identifying the root cause.
