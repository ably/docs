...
History refers to accessing previously sent messages, enabling you to retrieve both channel history and presence history. If "storage":/storage-history/storage is enabled messages will typically be stored for 24 for free accounts or 72 hours for paid accounts, otherwise Ably retains the last two minutes of message history.
...
History is ideal for when you want to retrieve any message within your "storage time frame":/storage-history/storage other than the last published message, enabling a longer, more complete context. Rewind is useful for getting an instant update of a channel, enabling an update from either a given number of messages or a previous point in time as a time interval.

History is useful for cases where you need to access messages published beyond the most recent, for example:

* Chat applications where a longer period of chat history should be available to new joiners
* Seeing the history of periodically sent data, such as weather apps that enable you to see recent weather 
* IoT devices that care about recent statuses

Whereas rewind would be useful in situations similar to the above, but with a more limited scope, for example:
...
You can retrieve channel history by using the "@history()@":/api/realtime-sdk/history#channel-history method. 

The Ably SDKs provide a straightforward API to retrieve paginated messages or presence event history. By default each page of history contains up to 100 messages and is ordered from most recent to oldest.
...
The following query parameters can be included in a call to @history()@. Note that @untilAttach@ is only available when using the realtime interface of an Ably SDK:
...
You can retrieve presence history by using the "@Presence@":/api/realtime-sdk/presence object to expose the "@history()@":/api/realtime-sdk/presence#history method, enabling a client to retrieve historical presence events on the channel.
...
The following query parameters can be included in a call to @history()@"
