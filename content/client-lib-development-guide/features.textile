...
h2(#rest-push). Push notifications

* @(RSH1)@ @Push#publish(channel, notification)@ function is a shortcut for @Rest#channels.get(channel).publish(null, null, extras: {"push": notification})@
* @(RSH2)@ Only in platforms that support receiving push notifications:
** @(RSH2a)@ If possible, @Push@ implements @EventEmitter@ and emits all push notifications received, in whatever form is native to the platform.
** @(RSH2b)@ @Push#deviceDetails@, when populated, holds data on how to identify the current device as a push notification target. Depending on the platform and transport type, the exact meaning of each field can vary slightly, but the following invariants must always hold:
*** @(RSH2b1)@ @Push#transport@ is the @PushTransportType@ selected to deliver notifications to this device.
*** @(RSH2b2)@ @Push#platform@ is the @PushPlatformType@ that best describes the software platform the library runs on.
*** @(RSH2b3)@ @Push#deviceType@ is the @PushDeviceType@ that best describes this device.
*** @(RSH2b4)@ @Push#targetId@ must uniquely identify the target (the minimum subject of a push notification delivery in the given @PushTransportType@; a device, an app, etc.) for the given @Push#transport@: no two different targets may have the same target ID for the same @PushTransportType@.
*** @(RSH2b5)@ @Push#recipientAddress@ must be the necessary information to deliver a push notification to the device identified by @Push#targetId@. As opposed to @Push#targetId@, it may change, and won't be used to identify the target.
** @(RSH2c)@ @Push#activateFromDevice(onRenewal, clientId)@ function:
*** @(RSH2c1)@ If possible for the platform, and if not already populated, populates @Push#deviceDetails@ by calling the necessary platform APIs. If not possible, it must return or callback with an @ErrorInfo@ with some appropiate error message.
*** @(RSH2c2)@ Constructs a @PushDeviceRegistration@ with @clientId@ and @Push#deviceDetails@. @PushDeviceRegistrations#save@ and calls @Push#admin.deviceRegistrations.save@ with it. If that fails, it returns or calls back with the received @ErrorInfo@.
*** @(RSH2c3)@ If "RSH2c2":#RSH2c2 succeeds, it then calls @Push#admin.generateNewRenewalSecret@ and returns or calls back with its result.
*** @(RSH2c4)@ If "RSH2c1"#RSH2c1 populated @Push#deviceDetails@, it also calls the necessary platform APIs, if any, to be notified when the registration needs to be renewed, ie. when it suffers some changes not affecting its @targetId@, and persists those changes in @Push#deviceDetails@.
*** @(RSH2c5)@ Whenever the renewal process described at "RSH2c4":#RSH2c4 happens, it reacts by following "RSH2c2":#RSH2c2 and "RSH2c3":RSH2c3 again, using @onRenewal@ (if provided) as callback instead for the generated error or renewal secret.
** @(RSH2d)@ @Push#activateExternally(registerDevice, renewRegistration)@ function:
*** @(RSH2d1)@ Does as in "RSH2c1":#RSH2c1, using @registerDevice@ as error callback.
*** @(RSH2d2)@ If "RSH2d1":#RSH2d1 succeeds, calls @registerDevice@ with @Push#deviceDetails@. This can return or callback with a renewal secret string; if it does, it is stored internally.
*** @(RSH2d3)@ If "RSH2d1"#RSH2d1 populated @Push#deviceDetails@, and "RSH2d2":#RSH2d2 stored a renewal secret, it does as in "RSH2c1":#RSH2c1.
*** @(RSH2d4)@ Whenever the renewal process described at "RSH2d3":#RSH2d3 happens, it reacts by calling @renewRegistration@ with the renewal secret previously stored as per "RSH2d2":#RSH2d2 or a previous instance of "RSH2d4":#RSH2d4, plus either with an @ErrorInfo@ or with @Push#deviceDetails@. This can return or callback with a renewal secret string; if it does, it is stored internally.
...
*** @(RTP5c3)@ If any of the automatic @ENTER@ presence messages published in "RTP5c2":#RTP5c2 fail, then an @UPDATE@ event should be emitted on the channel, with a @reason@ set to the @ErrorInfo@ received from realtime
...
** @(TM2i)@ @extras@ JSON-encodable object, used to contain any arbitrary key value pairs which may also contain other primitive JSON types, JSON-encodable objects or JSON-encodable arrays. The extras field is provided to contain message metadata and/or ancillary payloads in support of specific functionality, e.g. push. Each of these supported extensions is documented separately; for 0.9 the only supported extension is @push@, via the @extras.push@ member. The processing of any other members is undefined
...
* @(DD1)@ @transport@ string - Must be one of the strings specified in "PTT1":#PTT1
* @(DD2)@ @platform@ string - Must be one of the strings specified in "PPT1":#PPT1
* @(DD3)@ @deviceType@ string - Must be one of the strings specified in "PDT1":#PDT1
* @(DD4)@ @targetId@ string
* @(DD5)@ @recipientAddress@ string
* @(DD5)@ @metadata@ dictionary from string to string; optional

h4. PushDeviceRegistration

* @(PDR1)@ Has all fields of DeviceDetails (@DD*@).
* @(PDR2)@ @clientId@ string; optional
* @(PDR3)@ @state@ either ACTIVE or FAILED
* @(PDR4)@ @errorReason@ "ErrorInfo":#TI1; optional

h4. PushChannelSubscription

* @(PCS1)@ @transport@ string; optional - Must be one of the strings specified in "PTT1":#PTT1
* @(PCS2)@ @targetId@ string; optional
* @(PCS3)@ @clientId@ string; optional
* @(PCS4)@ @channelName@ string
* @(PCS5)@ If @transport@ is set, then @targetId@ must be set, and vice-versa.
* @(PCS6)@ If @transport@ and @targetId@ are set, then @clientId@ must not be set, and vice-versa.

h4. PushTransportType

* @(PTT1)@ One of the following strings: "fcm", "gcm", "apns", "web"
* @(PTT2)@ The client library will not check that a string acting as @PushTransportType@ complies with @(PTT1)@. Only the server will check this.

h4. PushPlatformType

* @(PPT1)@ One of the following strings: "android", "ios", "browser"
* @(PPT2)@ The client library will not check that a string acting as @PushPlatformType@ complies with @(PPT1)@. Only the server will check this.
...
class Push:
  publish(channel: String, JsonObject) => io // RSH1
  admin: PushAdmin

  // Only on platforms that support receiving notifications:

  embeds EventEmitter<.Message, JsonObject> // RSH2a
  deviceDetails: DeviceDetails? // RSH2b
  activateFromDevice(
    onRenewal: ((Error?, String?) ->)?,
    clientId: String?
  ) => io (ErrorInfo?, String?) // RSH2c
  activateExternally(
    registerDevice: (ErrorInfo?, DeviceDetails?) -> io String?,
    renewRegistration: (String, ErrorInfo?, DeviceDetails?) -> io String?
  )
  deactivateFromDevice() => io ErrorInfo?
  deactivateExternally(deactivate: (ErrorInfo?, DeviceDetails?) -> io)

class PushAdmin:
  deviceRegistrations: PushDeviceRegistrations
  channelSubscriptions: PushChannelSubscriptions
  + generateNewRenewalSecret(targetId: String)
  + generateNewRenewalSecret(PushDeviceRegistration)
...
class PushDeviceRegistrations:
  ...
  find(type: PushDeviceType, targetId: String) => io PushDeviceRegistration?
  get(targetIds: [(PushDeviceType, String)], state?: .Active | .Failed) -> io PaginatedResult<PushDeviceRegistration>
  get(clientId: String, state?: .Active | .Failed) -> io PaginatedResult<PushDeviceRegistration>
  remove(type: PushDeviceType, targetId: String) -> io
...
class PushChannelSubscriptions:
  ...
  save(PushChannelSubscription) => io
  listChannels() => io PaginatedResult<String>
  get(channelName: String) => io PaginatedResult<PushChannelSubscription>
  get(type: PushDeviceType, targetId: String, channelName: String?) => io PaginatedResult<PushChannelSubscription>
  get(clientId: String, channelName: String?) => io PaginatedResult<PushChannelSubscription>
...
class DeviceDetails:
  ...
  transport: PushTransportType // DD1, RSH2b1
  platform: PushPlatformType // DD2, RSH2b2
  deviceType: PushDeviceType // DD3, RSH2b3
  targetId: String // DD4, RSH2b4
  recipientAddress: String // DD5, RSH2b5
  metadata: Dict<String, String>? // DD6
...
class PushChannelSubscription:
  ...
  constructor(clientId: String, channelName: String) -> PushChannelSubscription
  transport: PushTransportType? // PCS1, PCS5, PCS6
  targetId: String? // PCS2, PCS5, PCS6
...
* "v0.8":/client-lib-development-guide/versions/features-0-8 (deprecated in July 2016)<br>"View 0.8 -> 0.9 changes":https://github.com/ably/docs/blob/source/content/client-lib-development-guide/versions/features-0-8__0-9.diff
