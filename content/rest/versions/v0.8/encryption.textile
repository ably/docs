...
All other attributes of "messages":/api/rest-sdk/types#messages and "presence messages":/api/rest-sdk/types#presence-message, such as event @name@ or <span lang="default">@clientId@</span><span lang="ruby,python">@client_id@</span><span lang="csharp">@ClientId@</span> remain unencrypted. All sensitive data, when using the library's symmetric encryption, must be placed in the @data@ attribute to ensure it is encrypted before it is transmitted to Ably.
