...
```
These are our standard application handlers for registering for notifications. We also tell Ably if it has been successful or not so that it can handle registration for us. We also convert the device token to a string and assign it to a variable that is shared in the AppDelegate.
...
This call back tells us we have deactivated push, there is no reason for us to do that at the moment so we reactivate push. We now need to add the following method:
...
This function is a delegate method, it tells us we have activated push and gives us an opportunity to subscribe to channels. We will not be doing this here as our server will be responsible for this. Let's add the optional delegate method that allow us to do this.
...
This function provides us the device details that we can pass to our server in order to register the device with Ably and subscribe to channels. We encode and pass the deviceId in this example however you can pass entire objects if you wish.
