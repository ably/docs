...
5. (Optional) Add custom Headers as key:value pairs to include metadata. Click **Another header** to add additional headers.
...
Use the [Control API](/docs/platform/account/control-api) to create a queue rule by sending a `POST` request to `/apps/{app_id}/rules`:
