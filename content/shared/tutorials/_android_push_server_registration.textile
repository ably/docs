...
Our server is all set to register the device and subscribe it to the push channel. Next, lets have the device send the required identifiers to the server for performing the actual registration and subscription.