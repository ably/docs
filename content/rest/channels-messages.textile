...
bc[dotnet]. Channel channel = rest.Channels.Get("channelName");
...
bc[dotnet]. CipherParams cipherParams = Crypto.GetDefaultParams(key);
...
The supported payload types are Strings, JSON objects and arrays, <span lang="dotnet">plain c# objects which are converted to json,</span> buffers containing arbitrary binary data, and Null objects. Client libraries detect the supplied message payload and encode the message appropriately.
...
bc[dotnet]. channel.PublishAsync("event", "This is my payload");
...
bc[dotnet]. PaginatedResult<Message> resultPage = await channel.HistoryAsync();
...
h2(#properties).
  ...
  dotnet: IO.Ably.Rest.RestChannel Members
...
bq(definition#publish-data).
  ...
  dotnet:   public Task PublishAsync(string name, object data, string clientId = null)
...
bq(definition#publish-msg-array).
  ...
  java:     public void publish("Message":#message[] messages)
  dotnet:   public Task PublishAsync(IEnumerable<"Message":#message> messages)
...
- <div lang="dotnet">data</div> := data payload for the message. The supported payload types are strings, plain .net objects, binary data as byte arrays, and null.<br>__Type: @Object@__
...
bq(definition).
  ...
  dotnet:  public Task<"PaginatedResult":#paginated-result<"Message":#message>> HistoryAsync(DataRequestQuery dataQuery)
