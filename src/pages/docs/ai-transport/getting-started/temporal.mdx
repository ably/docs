...
meta_description: "Build a streaming AI chat app whose agent side runs inside a Temporal workflow. Retryable steps supersede failed attempts on the session, and the user's stream never breaks."
meta_keywords: "AI Transport, Temporal, durable execution, workflow, activity, stepIdFor, adoptRun, createStep, Next.js, Ably"
intro: "Build a Next.js chat app whose agent side runs inside a Temporal workflow. Each step is one retryable activity; a retry supersedes the failed attempt on the session, and the user's stream never breaks."
---
...
- Each activity opens one AI Transport [step](/docs/ai-transport/concepts/runs#steps) with the Temporal activity id as the `stepId`.
...
<Code>
```shell
npm install @ably/ai-transport@^0.6.0 ably ai@^6 \
...
1. `sendMessage({ text })` publishes the user input on the session and POSTs an [invocation](/docs/ai-transport/concepts/runs#invocations) to `/api/chat`, which starts a Temporal workflow with `workflowId = invocationId`.
...
4. Cancels arrive on the session rather than through a Temporal signal. Each activity's own session routes them to [`run.abortSignal`](/docs/ai-transport/api/javascript/core/agent-session#run), which flows into the LLM call.

This guide's happy path ends the run inside the final inference activity. When a turn exhausts its retries with the run still open, your failure path must [close the run once](/docs/ai-transport/features/durable-execution#close-once) by adopting it and publishing `run.end({ reason: 'error' })` so every observer's UI unsticks. Splitting `openRun` from the first inference matters here: the run's ids reach the workflow before any inference runs, so your failure path always has the ids to end the run, even if the very first inference exhausts its retries.

## Understand the architecture <a id="architecture"/>

Temporal owns execution durability: worker crashes, activity retries, workflow history. AI Transport owns conversation state: what appears on the session, how retries reconcile, how clients observe. The [Temporal composition model](/docs/ai-transport/frameworks/temporal) shows how the two fit together, and [durable execution](/docs/ai-transport/features/durable-execution) generalises the pattern to any workflow engine.
...
- [Steps](/docs/ai-transport/concepts/runs#steps): the retry unit inside a run.
