...
** @(RTN23b)@ When initiating a connection, the client may send a @heartbeats@ param in the querystring, with value @true@ or @false@. If the value is @true@ or unspecified, the server will use Ably protocol messages (for example, a message with a @HEARTBEAT@ action) to satisfy the @maxIdleInterval@ requirement. If it is @false@, the server is permitted to use any transport-level mechanism (for example, websocket ping frames) to satisfy this. So for example, for websocket transports, if the client is able to observe websocket pings, then it should send @heartbeats=false@. If not, it should send @heartbeats=true@.
** @(RTN23c)@ When initiating a connection with a websocket transport, the client may send @supportsPong=true@ in the querystring to indicate that it guarantees to reply to any websocket ping frame with a pong. (Absent this parameter, the server will attempt to autodetect whether the client supports this).
