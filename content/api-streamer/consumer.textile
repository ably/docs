...
In order to use the Ably client SDKs, your client devices will need to authenticate with Ably first. You can simply use the API key which you can obtain from the app you wish to use, also called "Basic Auth mechanism":/auth/basic or set up an auth server so your client devices can use the "Token Auth mechanism":/auth/token. Tokens are short lived and hence more secure in cases where embedding API Keys on the client-side code isn't feasible. "Learn more about the various authentication mechanisms in Ably":/auth.
