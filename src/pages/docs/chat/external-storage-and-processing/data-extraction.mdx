...
Use `serial` and `version.serial` fields to determine the [correct order of messages](/docs/chat/rooms/messages#ordering-update-delete) and apply updates or deletions appropriately.
...
[Message reactions](/docs/chat/rooms/message-reaction) are delivered as separate events with `action` set to `message.summary`. These events contain aggregated reaction counts in the `annotations` field, which you can use to update stored reaction totals:
...
[Configure a webhook](/docs/platform/integrations/webhooks) integration in your Ably dashboard pointing to your endpoint. Supported webhooks include generic HTTP endpoints, AWS Lambda, Google Cloud Functions, and Cloudflare Workers.
...
Configure [streaming](/docs/platform/integrations/streaming) to your target system in the Ably dashboard. Supported integrations include Kafka, Kinesis, SQS, AMQP, and Pulsar.
...
Configure an [Ably queue](/docs/platform/integrations/queues) integration in your dashboard:
