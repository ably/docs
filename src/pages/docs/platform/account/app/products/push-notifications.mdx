...
| Field | How to Use | Purpose |
| ----- | ---------- | ------- |
| Notification title | Enter a short, clear title that captures the essence of the notification. | A title for the push notification, which will appear as the headline on the user's device. |
| Notification body | Write the key information or message that you want the user to read. | The main content of the notification to be displayed below the title. |
| Notification data | Include any extra data needed for app functionality, such as custom metadata or instructions. | Optional JSON data that the app can use for specific actions upon receiving the notification.  |
...
| Target | How to Use | Purpose |
| ------ | ---------- | ------- |
| Channel name | Enter the channel name and click push to channel to notify all devices subscribed to that channel. | Push notifications to all subscribers of a specific channel. |
| Device ID | Enter the Device ID and click push to device to target a single device with the notification. | Send a notification directly to a specific device. |
| Client ID | Enter the Client ID and click push to client to send the notification to the chosen client. | Notify a specific client registered with Ably. |
...
| Section | How to Use | Purpose |
| ------- | ---------- | ------- |
| Channel subscriptions | Click inspect channel to see detailed information about a specific channel, including the number of subscribers and recent activity. | View and inspect all channels currently subscribed to push notifications. |
| Device registrations  | Click inspect device to view detailed information about a specific device, such as its registration status, platform, and recent notifications. | Access a list of all devices registered to receive push notifications. |
| Client registrations  | Click inspect client ID to see detailed information about a specific client, including its subscriptions and recent activity. | Get an overview of all clients registered for push notifications within the Ably account. |
