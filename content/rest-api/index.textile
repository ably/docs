...
Register or update a device's details, including the information necessary to deliver push notifications to it.
...
A successful request returns the just-registered device details. It includes a @updateToken@, which is a token that can be used to "authenticate":#token-authentication afterwards to update an existing device registration (for example, for updating a GCM or FCM registration token).
