...
Whilst use of our REST API directly is fully supported, we instead recommend that customers should use our "Ably REST client libraries that support a range of platforms":https://ably.com/download including "JavaScript":https://ably.com/download, "iOS":https://ably.com/download, "Android":https://ably.com/download, "Java":https://ably.com/download, "Python":https://ably.com/download "Ruby":https://ably.com/download, ".NET":https://ably.com/download, "Go":https://ably.com/download, etc. The "client libraries are well documented":https://ably.com/documentation and provide additional features that improve performance and resilience that the REST API cannot deliver on its own, such as automatic re-routing around network problems by using alternative datacenters. You can use the "Rest#request()":/api/rest-sdk#request method to make arbitrary api calls against Ably, to use endpoints (such as features in beta) that libraries do not yet have built in support for.
...
bc[sh]. curl https://rest.ably.io/time \
 -H "X-Ably-Version: 1.1"

2. Include the version query string param @v@. Example:

bc[sh]. curl https://rest.ably.io/time?v=1.1
...
When successful, returns an object with @channel@ and @messageId@ properties, in case you want to know the message ID assigned to correlate with messages received by realtime subscribers or Ably Integrations recipients. When unsuccessful, returns an error as an "ErrorInfo":/rest/types#error-info object.
