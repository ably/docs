...
Ably "webhooks":/general/webhooks enable users to configure custom rules that react to channel events. For example, triggering an action when a message is sent, or when a user joins the channel. You can use our integration with "AWS Lambda":https://aws.amazon.com/lambda/ to create these rules.
...
The solution code for this tutorial is "available on Github":https://github.com/ably/ably-messages-s3-tutorial
