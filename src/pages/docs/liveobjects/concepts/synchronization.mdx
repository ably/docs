...
<If lang="java">
<Aside data-type='note'>
When a client publishes an operation, the operation is applied to its local objects as soon as the operation is acknowledged by the Ably system. This means that when the returned `CompletableFuture` completes, the operation has already been applied and you can immediately read the updated state.
</Aside>
</If>
<If lang="swift">
<Aside data-type='note'>
When a client publishes an operation, the operation is applied to its local objects as soon as the operation is acknowledged by the Ably system. This means that when the awaited mutation call returns, the operation has already been applied and you can immediately read the updated state.
</Aside>
</If>
