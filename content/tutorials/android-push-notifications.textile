...
Next, navigate to the Ably app's *Settings* tab, go down to the *Channel rules* section and add a new channel rule for a namespace by checking the *Push notifications enabled* option. In this tutorial, we'll be eventually publishing some Push Notifications via the @push:test_push_channel@ channel, so we'll use the "channel namespace":https://knowledge.ably.com/what-is-a-channel-namespace-and-how-can-i-use-them-1 @push@, as seen below.
