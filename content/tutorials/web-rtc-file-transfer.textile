...
First, we need to make an @enter@ subscription, so that we are notified every time a new member joins our channel. We do so by using "@AblyRealtime.presence@":https://ably.com/documentation/realtime/presence. In the callback, we get a list of all current members and add it to our member’s list. After this, we call on the render members method.
...
In this tutorial, we have seen how to use WebRTC data channels in conjunction with Ably to create realtime file share. Do you have any thoughts on making this lesson better? "Let us know":mailto:developer.relations@ably.com
