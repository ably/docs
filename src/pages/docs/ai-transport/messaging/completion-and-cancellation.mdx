...
{/*
Swift test harness:
func example(realtime: ARTRealtime, stream: any AsyncSequence<(type: String, text: String), Never> & Sendable) {
    ...
    Task {
        ...
        for await event in stream {
            if event.type == "token" {
                ...
                channel.append(messageToAppend, operation: operation, params: nil, callback: nil)
            }
        }

        // Signal content-part completion with an empty append
        let completionMessage = ARTMessage()
        completionMessage.serial = msgSerial
        completionMessage.data = ""

        let completionOperation = ARTMessageOperation(
            clientId: nil,
            descriptionText: nil,
            metadata: ["phase": "done"]
        )

        channel.append(completionMessage, operation: completionOperation, params: nil, callback: nil)
    }
}
```
</Code>
...
{/*
Swift test harness:
func example(realtime: ARTRealtime) throws {
    // insert example code here
}
*/}
```swift
...
Swift test harness:
@MainActor
func example(realtime: ARTRealtime, stopGeneration: @escaping @MainActor (String) -> Void) {
    // insert example code here
}
*/}
```swift
...
// Track pending appends for flushing
var pendingAppends: [Task<Void, Never>] = []
...
channel.subscribe("cancel") { message in
    MainActor.assumeIsolated {
        ...
        guard let responseID = headers?["responseId"] as? String else { return }

        // Stop generation for the matching response
        stopGeneration(responseID)

        // Flush any pending appends before confirming
        let appends = pendingAppends
        Task {
