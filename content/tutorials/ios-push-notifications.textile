...
In this tutorial, we'll see how to set up and send Push Notifications to your iOS devices using the "Ably's Push Notification service":/general/push. 

Before getting to writing any code, there are some setup steps involved. To enable and publish Push Notifications on iOS devices via Ably, you would need the following:
...
The Push Notifications feature in Ably comes in two different levels of "client permissions or capabilities":core-features/authentication#capabilities-explained:
...
Under your API Key settings on your Ably app dashboard, you can enabled or disable these permissions as needed.
...
h2(#step2-6). Step 3 - Enabling the Push Notifications rule for your channels
...
h2(#step3). Step 4 - Installing Cocoapods and adding Push Notifications capability to your Swift project
...
As you might have guessed, you'll need to replace the placeholders above for API Key and client Id with your own credentials. The API Key is from the apps dashboard and client Id can be anything of your choosing.
...
In the above function we've added the clientID and API Key as client options and initialized the Ably Realtime library with those.
