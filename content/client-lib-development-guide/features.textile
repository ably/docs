...
*** @(RSL1j1)@ A test should exist which publishes a @Message@ which includes a fixed @id@ twice, and checks that it is received only once
** @(RSL1k)@ If @idempotentRestPublishing@ (see "TO3n":#TO3n) is enabled, if the @Message@s do not include caller-supplied @ids@, for each @Message@ in the array the library should randomly generate get 9 bytes from a source of randomness, encode them as base64, and set it as the @Message@ @id@
*** @(RSL1k1)@ For the purposes of deciding whether the @Message@s include caller-supplied @ids@, the library may assume that the first @Message@ does not have an @id@ then none of them do
...
** @(TO3n)@ @idempotentRestPublishing@ boolean - defaults to false. If true, "RSL1k":#RSL1k is engaged
...
*** @(TO3l9)@ @maxFrameSize@ integer - default 524288 (512KiB). The maximum size of a single POST body or WebSocket frame. This is mostly only relevant for `Rest#request` (e.g. for batch publishes), since publishes will hit the @maxMessageSize@ limit before this
