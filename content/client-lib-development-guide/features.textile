...
* @(G4)@ All REST requests and WebSocket connections to Ably must include the current API version @0.8@ to ensure any future breaking changes to the API do not effect existing client libraries that will continue to request version @0.8@ of the API
...
** @(RSC7a)@ The header @Ably-Version: 0.8@ must be included in all REST requests to the Ably end-point
