...
If no capability is specified in an Ably `TokenRequest`, then the [Ably Token](/docs/platform/auth/token#tokens) will be given the full set of capabilities assigned to the issuing key.
...
$tokenDetails = $rest
    ->auth
    ->requestToken(
        ['capability' => json_encode(['other-namespace' => ['*']])]
    );
```
...
<Aside data-type='important'>
**JWT-only feature:** Channel-scoped user claims are only available when using [Ably JWT authentication](/docs/platform/auth/token#choosing-jwt). They are not available with TokenRequest authentication. See [JWT-only features](/docs/platform/auth/token#jwt-only-features) for a complete list of capabilities exclusive to JWT authentication.
</Aside>
