...
Channel enumeration is not currently integrated into Ably's REST client libraries as a dedicated method. However, you can still use the "REST client libraries":/rest to access the REST API it via the "request() method":/api/rest-sdk/index#request, which can perform arbitrary requests to Ably's REST API endpoints.
...
In order to receive channel enumeration information, you need to query the "@/channels@ endpoint":/rest-api#enumeration-rest. You will use the client library's @request()@ method to perform a @GET@ request and the result of this request will be an "HTTPPaginatedResponse":/api/rest-sdk/index#http-paginated-response object. This enables the results to be returned in pages, instead of all at once.
