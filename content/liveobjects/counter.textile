...
A @LiveCounter@ instance can be created using the @channel.objects.createCounter()@ method. It must be stored inside a @LiveMap@, such as the "root object":/docs/liveobjects/setup#root, to persist it within the LiveObjects state and share it with other clients.
