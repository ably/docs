...
Activate push notifications with [`push.activate()`](/docs/pub-sub/api/javascript/realtime/push#activate). Your app must request notification permission from the user before activation. Add the following inside `PushScreen`:
...
To subscribe your device to a channel so it can receive channel-based push notifications, use [`channel.push.subscribeDevice()`](/docs/pub-sub/api/javascript/realtime/push). The device must be activated first. Add the following functions inside `PushScreen`, after the functions from Step 2:
...
* Read more about the [Push Admin API](/docs/pub-sub/api/javascript/realtime/push-admin).

You can also explore the [Ably JavaScript SDK](https://github.com/ably/ably-js) on GitHub, or visit the [API references](/docs/pub-sub/api/javascript/realtime/realtime-client) for additional functionality.
