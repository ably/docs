...
* Store the "@deviceIdentityToken@":/general/push/activate-subscribe#local-device from the response from Ably in local storage so that subsequent requests to Ably to update push recipient details are authenticated as being from the device in question.
