...
bq(definition).
  ...
  ruby:   auth.requestToken(options) → json_result
...
bc[nodejs,javascript]. realtime.auth.requestToken(options, function(err, tokenDetails){
  if(err)
...
bc[ruby]. begin
  token = realtime.auth.request_token(options)
...
bc[java]. try {
  TokenDetails tokenDetails = realtime.auth.requestToken(AuthOptionsons, tokenParams);
...
bc[nodejs,javascript]. realtime.auth.authorise(options, function(err, tokenDetails){
  if(err)
    console.log('An error occurred; err = ' + err.message);
  else
    console.log('Success; token = ' + tokenDetails.id);
...
bc[nodejs,javascript]. realtime.auth.createTokenRequest(options, function(err, tokenRequest){
  if(err)
    console.log('An error occurred; err = ' + err.message);
  else
