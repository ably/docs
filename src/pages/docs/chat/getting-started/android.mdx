...
Note that this is for example purposes only. In production, you should use [token authentication](/docs/platform/auth/token) to avoid exposing your API keys publicly, the [`clientId`](/docs/platform/auth/identified-clients) is used to identify the client:
