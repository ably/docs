...
** @(RSL4b)@ If a message is encoded, the @encoding@ attribute represents the encoding(s) applied, separated by forwardslashes, ordered left-to-right innermost-to-outermost. E.g. "utf-8/base64" indicates that the original payload has a "utf-8" encoding and has subsequently been encoded in Base64 format
...
** @(RSL6b)@ If a given encoding (i.e. a @/@-delimited section of the @encoding@ field) is understood by the library but the library is unable to successfully apply the corresponding decoding (for example, incompatible encryption details are provided or invalid Base64 is detected in the message payload), a message should logged at @error@ level, and the message delivered with all completed decodings (that could be successfully applied) applied, with the remaining encodings in the @encoding@ field. For example, if a message had a decoding of "utf-8/cipher+aes-128-cbc/base64", and the payload was successfully Base64 decoded but the payload could not be decrypted because the @CipherParam@ details were not configured, the message would be delivered with a binary payload and an @encoding@ of "utf-8/cipher+aes-128-cbc"
** @(RSL6c)@ If a given encoding is not known to the library, @RSL6b@ still applies with the exception that the log message should be at the @info@ level not @error@ level
