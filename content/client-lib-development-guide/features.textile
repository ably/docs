...
*** @(RSC7d4)@ Product names must be conformed by the client to contain only the primary set of characters allowed in a @token@ as defined by "RFC 7230, Section 3.2.6":https://datatracker.ietf.org/doc/html/rfc7230#section-3.2.6 (that is, only: @DIGIT@, @ALPHA@ and the set @!#$%&'*+-.^_`|~@).
*** @(RSC7d5)@ Clients should conform product names using a common function, before they are packed into the agent string. This common function should always succeed, dropping any characters not in the supported set as defined by "RSC7d4":#RSC7d4. This function should be unit tested.
