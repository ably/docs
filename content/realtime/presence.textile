...
h2. Broadcast your presence & subcribe to presence events
...
<code lang="ruby">
EventMachine.run do
  # To use the channel presence feature, you need to specify a client_id
  ably_client = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'any_username')

  channel1 = ably_client.channel('channel1')

  channel1.attach do
    channel1.presence.subscribe(:enter) do |presence_message|
      puts "#{Time.now}: #{presence_message.member_key} entered channel1"
    end

    channel1.presence.subscribe(:leave) do |presence_message|
      puts "#{Time.now}: #{presence_message.member_key} left channel1"
    end
...
A client must be attached to a channel, and must have the relevant permission, in order to enter. Although, the library implicitly calls @attach()@ if the client is not attached at the time @enter()@ is called. You can therefore write:
...
EventMachine.run do
  # To use the channel presence feature, you need to specify a client_id
  ably_client1 = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'any_username')
  ably_client2 = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'any_username')
  ably_client3 = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'any_username')


  ably_client1.channel('my_channel').presence.subscribe(:enter) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} entered my_channel"
  end

  ably_client1.channel('my_channel').presence.enter
  ably_client2.channel('my_channel').presence.enter
  ably_client3.channel('my_channel').presence.enter
end

# Output:
# 2015-01-26 16:07:15 +0000: lDOxjGXKHG:any_username entered my_channel
# 2015-01-26 16:07:15 +0000: nacXXn7gNx:any_username entered my_channel
# 2015-01-26 16:07:15 +0000: shQO7T5TSK:any_username entered my_channel
</code>
</pre>

h2. Managing multiple client ID
...
<code lang="ruby">
EM.run do
  ably_client1 = Ably::Realtime::Client.new(api_key: '<%= %>')
  client_presence = ably_client1.channel('my_channel').presence

  client_presence.subscribe(:enter) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} entered my_channel"
  end

  client_presence.enter_client('bob')
  client_presence.enter_client('mary')
end
# Output:
# 2015-01-27 16:54:05 +0000: oVssCTfaIz:bob entered my_channel
# 2015-01-27 16:54:05 +0000: oVssCTfaIz:mary entered my_channel
</code>
</pre>
...
# Output:
# 2015-01-27 16:58:00 +0000: X0i3jFssWx:bob entered my_channel
</code>
</pre>

h2. Payload

In addition to reporting presence on a channel, it is also possible to include a payload in the attributes sent with a presence message. Clients can update this data whilst being members of the channel:

<pre lang="ruby">
<code lang="ruby">
EventMachine.run do
  ably_client1 = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'bob')
  ably_client2 = Ably::Realtime::Client.new(api_key: '<%= %>', client_id: 'mary')

  client1_presence = ably_client1.channel('my_channel').presence
  client2_presence = ably_client2.channel('my_channel').presence

  client1_presence.subscribe(:enter) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} says #{presence_message.data}"
  end

  client1_presence.subscribe(:update) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} says #{presence_message.data}"
  end

  client1_presence.subscribe(:leave) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} says #{presence_message.data}"
  end

  client2_presence.enter(data: 'Hi everyone!') do
    client2_presence.update(data: 'Hi 3!') do
      client2_presence.leave(data: 'bye bye!')
    end
  end
end
</code>
...
EM.run do
  ...
  ably_client1.channel('my_channel').presence.enter do
    ably_client1.channel('my_channel').presence.get do |members|
      puts "There is #{members.size} members on this channel"
    end
  end
end
</code>
...
bc[ruby]. EM.run do
  ably_client1 = Ably::Realtime::Client.new(config)

  ably_client1.channel('my_channel').presence.enter_client('mary') do
    ably_client1.channel('my_channel').presence.history(direction: :backwards) do |history|
      history.each do |presence_message|
