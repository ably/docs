---
title: Typing
meta_description: "Type LiveObjects state on a channel for type safety and code autocompletion."
product: liveobjects
languages:
  - javascript
---
...
blang[javascript].
  ...
  If you are using TypeScript in your project, you can leverage LiveObjects' built-in TypeScript support to ensure type safety and enable autocompletion when working with LiveObjects on a channel.

  h2(#global). Global @ObjectsTypes@ interface

  You can type LiveObjects on all your channels by defining a global `ObjectsTypes` interface. If you only want to type the root object for a specific channel, see the @Typing channel.objects.getRoot()@:#getroot section below.
...
blang[javascript].
  ...
  This allows TypeScript to infer the correct types when accessing and mutating LiveObjects:
...
blang[javascript].
  ...
  Typing `channel.objects.getRoot<T>()` is particularly useful when your application uses multiple channels, each with a different LiveObjects structure.
