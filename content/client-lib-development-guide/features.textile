...
** @(RSC19a)@ Method signature is @request(string method, string path, Hash params?, JsonObject body?, Hash headers?) -> Response@ with arguments: @method@ is a valid "HTTP verb":https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html such as @"GET"@; @path@ is the path component of the URL such as @"/channels"@; @params@ and @headers@ are optional arguments containing key value pairs that will include query params and HTTP headers respectively in the request (the argument types can be idiomatic for the language such as @Object@ in the case of Javascript); @body@ is an optional JSON-like object argument that can be easily serialised to MsgPack or JSON
** @(RSC19b)@ @request@ method returns a @Response@ object that inherits from the @PaginatedResult@ object to provide paging support where applicable with the following differences:
*** @(RSC19b1)@ @items@ is an array of JSON objects. However, if the response from REST request is a single @Hash@ object as opposed to an @Array@ of objects, then @items@ will be an array with just that one JSON object. If the REST response is empty, then @items@ will be an empty @Array@
*** @(RSC19b2)@ @statusCode@ is a integer attribute with the HTTP status code of the page
*** @(RSC19b3)@ @success@ is a boolean attribute which is @true@ when the HTTP status code indicates sucess i.e. @200@ <= @statusCode@ < @300@
*** @(RSC19b4)@ @errorCode@ is an integer attribute populated with the error code if the @X-Ably-Errorcode@ HTTP header is sent in the response
*** @(RSC19b5)@ @errorMessage@ is a string attribute populated with the error code if the @X-Ably-Errormessage@ HTTP header is sent in the response
*** @(RSC19b6)@ @headers@ is an Array of key value pairs for each response header
** @(RSC19c)@ If the HTTP network request fails for reasons such as a timeout (after the underlying fallback host attempts have failed where applicable, see "RSC15":#RSC15), then the @request@ method should indicate an error in an idiomatic way for the platform
...
* @(RTC9)@ @RealtimeClient#request@ is a wrapper around @RestClient#request@ (see "RSC19":#RSC19) delivered in an idiomatic way for the realtime library i.e. in the case of Ruby, with an evented async callback interface
