...
publisher = try? PublisherFactory.publishers() // get a Publisher builder
  .connection(ConnectionConfiguration(options))
```
...
```[swift]
let options = ARTClientOptions()
options.authUrl = NSURL(string: "http://my.website/jwt-auth")
...
let event as AddTrackableEvent: self?.performAddTrackableEvent(event){
...
```[swift]
subscriber.resolutionPreference(Resolution(accuracy: .low,
