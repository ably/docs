...
To enable Push Notifications on your device, it must be registered with APNS first. This can be done in two ways: a)you can have the device register itself directly with Ably or b) delegate the registration work to your app server, which would then register the device with Ably on its behalf. In this tutorial we'll implement direct device registration. You can jump to "our server registration tutorial":/tutorials/ios-push-server-registration if you'd rather have a server register the device with APNS.
...
h2(#step7). Step 7 - Testing out Push Notifications
