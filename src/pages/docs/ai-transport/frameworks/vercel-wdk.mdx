...
Vercel WDK owns the execution half of the job: it persists a workflow's progress, retries a failed WDK step, and runs the whole thing inside your Next.js app.
...
AI Transport owns the conversation half, layering a durable session over that execution:
...
Open the Step before reading the view. A retry re-enters this WDK step with the same `stepId`, so its `step.start()` re-emits `ai-step-start` and supersedes the failed attempt. Build the prompt from `run.view` only once that supersede has folded back into the session's read model. The publish ack does not guarantee that ordering on its own, so the retry waits for its own attempt to reflect before reading, otherwise the half-streamed response it is replacing trails the prompt as an assistant prefill, which real providers reject. The [getting-started guide](/docs/ai-transport/getting-started/vercel-wdk#define-steps) shows the short wait that guarantees this.
...
Because each WDK step constructs its own session and routes its own cancels, the abort signal fires in whichever step is subscribed when the cancel lands. An inference step aborts its model call and publishes the `cancelled` terminal inline, and the workflow stops scheduling further steps. A cancel that instead lands during a tool step, or in the gap between steps, is not lost: the tool finishes, the next step to adopt the Run replays the cancel from channel history when `load()` resolves, and the following inference ends the Run `cancelled`.
