...
| **action** | An [enum](/docs/pub-sub/api/javascript/realtime/message) telling you whether this is a normal ('create') message, an update to a previous message, an annotation summary, etc. |
...
Use one of the following methods to configure message conflation for a channel or channel namespace:
...
2. Enter the channel name or namespace to apply message conflation to.
3. Check **Conflation enabled**.
4. Choose a conflation interval over which to aggregate messages.
5. Enter a [conflation key](#routing) to assess messages against.
6. Click **Create rule** to save.

</Tab>
<Tab value="control-api" label="Control API">

Create a rule with message conflation using the Control API by sending a `POST` request to [`/apps/{app_id}/namespaces`](/docs/api/control-api):
...
Use the [Ably CLI](/docs/platform/tools/cli) to create a rule with message conflation:
