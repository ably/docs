...
bc[jsall]. const channel = realtime.channels.get('channelName');
...
bc[jsall]. Ably.Realtime.Crypto.generateRandomKey((err, key) => {
  const channelOptions = { cipher: { key: key } };
...
Channel mode flags enable a client to specify a subset of the "capabilities":/auth/capabilities granted by their token or API key as "channel options":#setting-channel-options. Channel mode flags offer the ability for clients to use different capabilities for different channels, however, as they are flags and not permissions they cannot be enforced by an authentication server.
...
bc[jsall]. realtime.channels.get('chatroom').attach((err) => {
...
bc[jsall]. const channel = realtime.channels.get('chatroom');
channel.subscribe('action', (message) => { // implicit attach
...
Published messages are immediately sent to clients on "attaching":/api/realtime-sdk/channels#attach to a channel as long as they have subscribe "capabilities":/auth/capabilities for that channel. Messages are sent regardless of whether or not the client has subscribed to the channel.
...
bc[jsall]. channel.on('attached', (stateChange) => {
...
bc[jsall]. const myListener = (stateChange) => {
