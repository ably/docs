...
channel.publish([.init(name: "response", data: response)]) { result, error in
    if let error = error {
    ...
    channel.annotations.publish(forMessageSerial: msgSerial, annotation: citation1) { error in
        if let error = error {
            print("Error publishing citation: \(error.message)")
        }
    }
...
channel.subscribe { message in
    if message.action == .messageSummary {
        if let summary = message.annotations?.summary,
           let citations = summary["citations:multiple.v1"] {
