...
Entering a space will register a client as a member and emit an "@enter@":/spaces/locations#subscribe event to all subscribers. Use the "@enter()@":https://sdk.ably.com/builds/ably/spaces/main/typedoc/classes/Space.html#enter method to enter a space.
...
A space is created as an Ably "channel":/channels. Members "attach":/channels/states#attach to the channel and join its "presence set":/presence-occupancy/presence when they "enter":#enter the space. Avatar stacks, member locations and component locking are all handled on this channel.
