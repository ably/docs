...
Here is the simplest example of a client connecting to ably and handling a connection state change event:

bc[javascript,nodejs](code-editor:realtime/connection). var ably = new Ably.Realtime('{{API_KEY}}');
ably.connection.on('connected', function() {
  alert('Wow, that was easy');
})

h3(#connection-state-explained). Connection state explained
