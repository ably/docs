...
channel.publishWithResult([.init(name: "message-name", data: "original-data")]) { result, error in
    if let error = error {
...
// Publish the original message
channel.publishWithResult([.init(name: "message-name", data: "Hello")]) { result, error in
    if let error = error {
        print("Failed to publish: \(error.message)")
        return
    }
