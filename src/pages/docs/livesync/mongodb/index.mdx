...
intro: "MongoDB database connector distributes document changes from collections to subscribing clients using MongoDB change streams. This managed integration publishes changes to Ably channels with automatic failover and ordering guarantees."
meta_description: "Ably MongoDB database connector: distribute document changes from collections to clients via change streams with automatic failover and ordering."
meta_keywords: "MongoDB connector, database synchronization, change streams, document changes, realtime distribution"
---
