...
intro: "REST API access enables direct LiveObjects manipulation without client SDKs for server-side integrations. This approach supports object operations through HTTP requests when realtime connectivity isn't required."
meta_description: "Ably LiveObjects REST API: direct object manipulation without client SDKs for server-side integrations via HTTP requests."
---

<Aside data-type='experimental'>
LiveObjects REST API is currently Experimental. Its features are still in development and subject to rapid change.
...
You can return a subset of the channel object by specifying the `path` query param. For example, to return only the `votes` `LiveMap` instance from the root object:
...
Ably applies the operation to these specific object instances. If the object instances located at that specified path change before Ably processes the operation, due to concurrent updates from other clients, Ably does not apply the operation to the new object instances.
</Aside>
...
Wildcards can be included at the end or in the middle of paths and match exactly one level in the channel object. For example, given the following compact view of the channel object:
...
All operations inside the array form a [batch operation](/docs/liveobjects/concepts/operations#batch-operations). Ably publishes this as a single message. Ably processes all operations in the batch as a single atomic unit.
