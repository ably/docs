...
You can "configure rules":https://faqs.ably.com/how-to-set-up-a-reactor-rule from the "Integrations tab in your app":https://faqs.ably.com/how-to-set-up-a-reactor-rule on a per app basis which can apply to one or more channels in that app. Integration rules can filter by channel naming using a regular expression, for example @^click_.*_mouse$@. This would match the string @click_@ followed by a string followed by @_mouse@, for example, @click_left_mouse@.
