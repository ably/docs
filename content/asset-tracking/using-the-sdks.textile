...
blang[kotlin,javascript].
  ...
  You can read more about JWT authentication in our "documentation":/auth/token#ably-jwt-process.

blang[swift].
  The client requires authentication in order to establish a connection with Ably. Currently, the Swift SDK only supports "basic authentication":/auth/basic, you authenticate with your Ably API key (available in "your account dashboard":https://ably.com/accounts) and can optionally "identify the client with a client ID":/realtime/authentication#identified-clients. The following example demonstrates how to achieve this:
