...
Types are provided explicitly via `channel.object.get<T>()` rather than using global type declarations. If you previously used `AblyObjectsTypes` for global type definitions, migrate to explicit type parameters as shown below.
</Aside>

## Type the channel object <a id="typing-get"/>
...
This enables TypeScript to infer the correct types when accessing and mutating objects:
...
<Code>
```javascript
import { ChannelObjectType } from './types/liveobjects';
...
## Type instances <a id="instance-typing"/>
...
// Get instances with full type information
const counterInstance = myObject.get('visits').instance();
if (counterInstance) {
  // TypeScript knows this is a LiveCounterInstance
...
if (mapInstance) {
  // TypeScript knows this is a LiveMapInstance
  await mapInstance.set('theme', 'dark');
  const theme = mapInstance.get('theme');
}
```
</Code>
