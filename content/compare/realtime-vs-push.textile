...
Message queues that provide a reliable and straightforward mechanism to consume, process, store, augment or reroute data efficiently from a realtime platform. | No | No |
| *Webhooks*

Use HTTP callbacks triggered by a Realtime event. This might be a new user connection or a message published in a pub/sub channel. | Yes | No |
|  *Serverless cloud function invocation*

Trigger serverless functions on third platforms like Amazon Lambda, Microsoft Azure or Google Function. | Partial. Recommends using a generic WebHook to deliver to an API Gateway that then triggers a function. | No |
|  *Presence*

Subscribe to events when users or devices enter or leave channels/topics. | Yes | No |
| *Announcement channels*

Stay updated on what’s happening on the client’s end, connection, disconnection etc. | Yes | Yes |
| *Message history*

Can clients or servers retrieve messages previously published on a channel? | Partial. A fixed number of messages can be retained for a small amount of time. A NoSQL cloud database with built-in real-time notifications keeps data synchronized between users. | Partial. Up to 128 messages can be retained for up to 60 seconds. Although this is disabled by default. |
| *Reliable message ordering*

Ensures messages are delivered to persistently connected subscribers in the order they were published on each channel/topic. | No | No |
| *Realtime data firehose*

Stream your realtime data published within a realtime platform directly to another streaming or queueing service such as Amazon Kinesis, Apache Storm or Kafka. | No | No |
| *Push notifications*

Send notifications and updates to users even when they’re offline. | Yes | Yes |
| *Conflation*

Ability to reduce the amount of information sent to clients by combining or discarding updates. | No | Yes |
