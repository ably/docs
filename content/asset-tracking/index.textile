...
jump_to:
  Help with:
    - Battery optimization scenarios#battery-optimization
...
Resolution constraints specify the thresholds and corresponding logical mappings for tracking @Resolution@ objects. These @Resolution@ objects are required by the default @ResolutionPolicy@, which may be adopted by @Publisher@ instances. 
