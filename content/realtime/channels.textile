...
It is possible to subscribe to a channel and obtain messages from that channel's "history":/storage-history/history with a single API request. To do so you would use the "rewind":/realtime/channels/channel-parameters/rewind feature. This will result in the specified number of messages from "history":/storage-history/history being returned as part of the subscription process.
