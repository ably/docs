...
Use the `unsubscribe()` function returned in the `subscribe()` response to remove a typing listener<If lang="jetpack">. Jetpack Compose automatically handles lifecycle and cleanup when using `collectAsCurrentlyTyping()`</If>:
</If>
