...
* Apply best practices to receive and display realtime data in a Flutter app.
...
In Flutter, we can make use of third-party packages to add extra functionality and make it easier to do many things without needing to re-invent the wheel. To use a package we just have to add the package name and version in @pubspec.yaml@, like this:
...
Let's go back to the "Cryptocurrency prices hub":https://www.ably.io/hub/ably-coindesk/crypto-pricing you subscribed to in a previous step. As you can see, each cryptocurrency has a _display name_ and a _code_. The code is used to connect to the specific channel for this currency on the Ably Hub, and display name is what the user will see.

We will make a @const List@ that will have the currently available currencies on the Hub, and if any new currency is added to the hub, we can add it by appending it to this list and the whole app will be updated as the channel names usually follow the same naming convention.
...
Let’s create the main service class @AblyService@, with a private constructor to initialize it.
...
To initialize our service, we will write a special @static@ method. We'll create and return the private instance of this class that can be used everywhere in the app. We'll also add the configuration necessary to establish a realtime connection to Ably.
...
Let’s take a moment to understand what we did. You can see that we passed the local @_realtime@ instance to the constructor of the @AblyService@ class which will set the class-level @_realTime@ property allowing other methods to use it. We will soon see how to use it to create and subscribe to channels.

Let’s now connect to the Cryptocurrency channel and subscribe to the coin prices. For this we will add a function called @getCointUpdates()@. This method will establish the connection, listen to the stream of messages coming from Ably, and map each message to the relevant Coin.
...
Let's understand the code above. We iterate over the constant @_coinTypes@ list that we created before. For each coin type in the list, we obtain and subscribe to the relevant Ably channel. Each such channel contains a Dart @Stream@ emitting new events as they are published on the channel. 

h3(#notify-ui). 3.3. Notifying the UI of New Data

To consume the data stream easily in the UI, we will create a new class that extends the ChangeNotifier. In Flutter, the simplest way to get notified when anything changes is by using ChangeNotifier.
...
We chose to transform Stream events into **ChangeNotifier** updates because they are much easier to use in the UI as they always have a valid value and don’t care if they have multiple subscriptions.
...
# So we register a listener for this message Stream, and use `where` to filter null values. <pre lang="flutter"><code>messageStream.where((event) => event.data != null).listen((message) {});</code></pre>
...
The first step is to register the service using @get_it@. We will do that asynchronously in the main method, as we want this service to be available as soon as the app is launched.
...
As this is an asynchronous registration, it won’t be available to our UI immediately. Hence, we will wait for it to become available before using it. For this we will use a @FutureBuilder@ which will show a loading spinner until @get_it@ reports that all services are ready.
...
For this we will add a new property in the AblyService called @connection@ of type @Stream@. This will report all connection status changes.

```[flutter]
Stream<ably.ConnectionStateChange> get connection =>  _realtime.connection.on();
```

Since it’s a Stream, we will use a @StreamBuilder@ to read the connection status, and then decide what to display accordingly.
...
```
Instead of using a @ListView.builder@, it’s just a @Column@, with a @for-collection@. In this case using a @Column@ is more convenient since the @ListView@ by default will dispose the states of any child that isn’t visible anymore. That's good behaviour in case a list is very long, but since we know already the number of graphs is limited, and we don’t want them to dispose and rebuild each time the user scrolls up or down, a @Column@ is totally fine.
...
Here we will use a third-party library called "Syncfusion":https://pub.dev/packages/syncfusion_flutter_charts to render the charts. Any other chart library should work well too.
...
In the previous section, we subscribed to a channel as a consumer of that public data stream. As we don't have publish rights on that data stream, we can’t add data to it. In this section, we will take a look at how we create private channels programmatically, subscribe to them as well as publish messages. 

Building the chat room with Ably realtime is fairly simple

In our @AblySevice@, we will add two methods, one to listen to the latest messages as long as a user is on the chat room screen, and the another method to send new messages.
...
Just like we did previously with prices, we will create @ChatUpdates@ as a @ChangeNotifier@ holding the most recently published message and push it to a queue in the UI. We'll call @getChatUpdates()@ on the same instance of @AblyService@ that we previously registered with @get_it@. Doing this will subscribe our app to the chat channel enabling it to receive updates whenever a new message is published on that channel.
...
To do this, we first initialize a service instance in @getTweets()@ using the hashtag that was passed from the dashboard. Then, we will call the @getTweetsQuery()@ method. As it returns a @Future@, we need to @await@ the result. When the result is ready, we update the local state of the widget using @setState@ which will call the build method to switch from displaying a loading spinner to the actual list of tweets.

We can’t do this directly inside @initState@ because this API is asynchronous, and @initState@ can’t be defined as an async function. We will use a separate async method called @getTweets()@ that we will call from @initState@. We can do this safely as we have already ensured that the page will render correctly even with no data received.
