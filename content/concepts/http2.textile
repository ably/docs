...
jump_to:
  Section:
    - HTTP/2: A Conceptual Deep Dive#http-2
    - Background - A Brief History of HTTP?#history
    - A Humble Origin#origin
    - HTTP/1.0#v10
    - HTTP/1.1#v11
    ...
    - Binary Framing Layer#framing-layer
    - Frame#frame
    - Message#message
    - Stream#stream
...
h1(#http-2). HTTP/2: A Conceptual Deep Dive

The Internet, as we know it, would not have been possible without Hypertext Transfer Protocol or HTTP. HTTP has come a long way since its inception. HTTP started as a simple one-line ASCII string protocol. HTTP/2 is a significant version upgrade over it predecessor HTTP/1.1

This article covers the limitations of HTTP/1.1 and how those limitations led to the protocol’s evolution to HTTP/2. The article delves into the technical concepts behind HTTP/2 and touches upon some of its limitations.

h2(#history). Background - A Brief History of HTTP

h2(#origin). A Humble Origin
...
HTTP/0.9 requests consist of the method “GET,” followed by the document address, an optional port address, and terminated with a carriage return and a line feed. HTTP/0.9 request is just a string of ASCII characters.

<a href="/images/concepts/http2/http-2-anatomy.png" target="_blank">
    <img src="/images/concepts/http2/http-2-anatomy.png" style="max-width:100%" alt="HTTP/2 anatomy">
</a>

HTTP/0.9 was very limited. It did not have HTTP headers, so only HTML files could be transmitted. It did not have status and error codes.
...
<a href="/images/concepts/http2/http-2-anatomy-2.png" target="_blank">
    <img src="/images/concepts/http2/http-2-anatomy.png" style="max-width:100%" alt="HTTP/2 anatomy">
</a>

h2(#http-12-problem). What is the Problem with HTTP/1.1?

Web applications are complex and growing. As per "HTTP Archive":https://httparchive.org/reports/state-of-the-web , the median size of the desktop version of websites grew from 1.51MB to 1.97MB (+30.6%) between November 2016 and November 2017. During the same period, the median size of the mobile version of the websites grew from 1.34MB to 1.78MB (+32.3%).
...
A lot of this is because of the limitations of HTTP/1.1. Let us look at some limitations and developer hacks for overcoming them.

h2(#line-blocking). Head of Line Blocking

When a client sends a request to a server on a connection, that connection is useless till the server sends back a complete response. Subsequent requests cannot use the TCP connection, and the situation is called head of line blocking.
...
Developers started using domain sharding to make their website load faster. Domain sharding increases the complexity of development and underutilizes TCP connections.

h2(#pipelining). Pipelining

HTTP/1.1 introduced a technique called pipelining. In theory, clients could send multiple requests to the server without waiting for the response. The server must respond to the requests in the order they are received. For pipelining to work, both the client and the server should support it.
...
Simplicity was a core principle of HTTP. However, implementation simplicity came at the cost of performance. HTTP/1.1 has many baked-in limitations, which we discussed in the previous section. With time a need was felt to fix HTTP.
...
Let’s take a look at how HTTP/2 works. HTTP/2 is an extension, not replacement of HTTP/1.1. HTTP/2 retains the application semantics of the previous HTTP version. Functionality, HTTP methods, status codes, URIs, and header fields remain the same. Every HTTP/2 connection starts as HTTP/1.1. The connection upgrades if the client supports HTTP/2.

HTTP/2 uses a single TCP connection between the client and the server, which remains open for the duration of the interaction.

h2(#framing-layer). Binary Framing Layer

HTTP/2 introduces a new binary protocol called the binary framing layer. The binary framing layer is responsible for all performance enhancements in HTTP/2. The binary framing layer lays down the protocol for encapsulation and transfer of messages between the client and the server.

The binary framing layer breaks the communication between the client and server into small chunks and creates an interleaved bi-directional stream of communication. Thanks to the binary framing layer, HTTP/2 uses a single TCP connection that remains open for the duration of the interaction. Let’s look at some terminology used in HTTP/2 before moving on.
...
<a href="/images/concepts/http2/http-2-frame-diagram.png" target="_blank">
    <img src="/images/concepts/http2/http-2-frame-diagram.png" style="max-width:100%" alt="Frame diagram">
</a>
...
<a href="/images/concepts/http2/http-2-message-diagram.png" target="_blank">
    <img src="/images/concepts/http2/http-2-anatomy.png" style="max-width:100%" alt="Message diagram">
</a>
...
* A single HTTP/2 connection can have multiple concurrently open streams. The client and the server can send frames from different streams from on the connection.
* Streams can be shared by both the client and the server. A stream can also be established and used by a single peer.
...
<a href="/images/concepts/http2/http-2-stream-diagram.png" target="_blank">
    <img src="/images/concepts/http2/http-2-stream-diagram.png" style="max-width:100%" alt="Stream diagram">
</a>

h2(#multiplexing). HTTP/2 Multiplexing

The binary framing layer solves the problem of head of line blocking, making HTTP/2 multiplexed fully. The binary framing layer breaks the communication between the client and server into small chunks and creates an interleaved bi-directional flow of streams.
...
<a href="/images/concepts/http2/http-2-flow-control.png" target="_blank">
    <img src="/images/concepts/http2/http-2-flow-control.png" style="max-width:100%" alt="Flow control in action">
</a>
...
HTTP requests contain headers and cookies data, which add performance overhead. HTTP/2 uses the HPACK compression format to compress request and response metadata. Transmitted header fields are encoded using the Huffman code. HTTP/2 requires the client and the server to maintain and update an indexed list of previously seen header fields. The indexed list is used as a reference to encode previously transmitted values efficiently.

h2(#server-push). Server Push

Server push is a performance feature that allows a server to send responses to a client ahead of time preemptively. This feature is useful when the server knows that the client needs the 'pushed' responses to process the original request fully.

Server Push feature has the potential to improve performance by utilizing the network, which otherwise would be idle. However, server push can be counterproductive if not implemented correctly.
...
* In HTTP/1.1, the client cannot retry a non-idempotent request when an error occurs. Some server processing may have occurred before the error, and a reattempt can have an undesirable effect.
* HTTP/2 has mechanisms for providing a guarantee that a request has not been processed.
...
HTTP/2 allows the client to send all requests concurrently over a single TCP connection. Ideally, the client should get all the resources quicker. On the flip side, concurrent requests can increase the load on the servers. With HTTP/1.1, requests are spread-out. But with HTTP/2 servers can receive requests in large batches, which can lead to requests timing out. The issue of server load spiking can be solved by inserting a load balancer or a proxy server, which can throttle the request.
...
HTTP/2 is optimized for speed, which makes for a better user experience. Using HTTP/2 also leads to developer happiness. Hacks like domain sharding, asset concatenation, and image spriting are no longer necessary, which lowers the development complexity.
