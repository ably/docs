...
import type { FlatNavPage, NavContext, NavLink, MdxNode } from './transpileMdxToMarkdown';
...
\`\`\`
  ...
  describe('flattenNavPages', () => {
      ...
      describe('buildMdxPageSet', () => {
        const makeMdxNode = (relDir: string, name: string): MdxNode => ({
          parent: { relativeDirectory: relDir, name, absolutePath: `/abs/${relDir}/${name}.mdx` },
          internal: { contentFilePath: `/abs/${relDir}/${name}.mdx` },
        });
    
        it('should return correct set of nav links', () => {
          const nodes: MdxNode[] = [
            makeMdxNode('docs/chat/rooms', 'messages'),
            makeMdxNode('docs/chat/rooms', 'history'),
            makeMdxNode('docs/chat', 'setup'),
          ];
        ...
        it('should handle index files correctly', () => {
          const nodes: MdxNode[] = [makeMdxNode('docs/chat/rooms', 'index')];
          const pageSet = buildMdxPageSet(nodes);
      ...
      describe('buildMetaDescriptionMap', () => {
        const makeMdxNode = (relDir: string, name: string, metaDesc?: string): MdxNode => ({
          parent: { relativeDirectory: relDir, name, absolutePath: `/abs/${relDir}/${name}.mdx` },
          internal: { contentFilePath: `/abs/${relDir}/${name}.mdx` },
        ...
        it('should map nav links to meta descriptions', () => {
          const nodes: MdxNode[] = [
            makeMdxNode('docs/chat/rooms', 'messages', 'Send and receive messages.'),
            makeMdxNode('docs/chat/rooms', 'history', 'Retrieve message history.'),
          ];
        ...
        it('should handle index files', () => {
          const nodes: MdxNode[] = [makeMdxNode('docs/chat/rooms', 'index', 'Room overview.')];
          const descMap = buildMetaDescriptionMap(nodes);
        ...
        it('should skip nodes without meta_description', () => {
          const nodes: MdxNode[] = [
            makeMdxNode('docs/chat/rooms', 'messages', 'Has description.'),
            makeMdxNode('docs/chat/rooms', 'history'),
          ];
          const descMap = buildMetaDescriptionMap(nodes);
      ...
      describe('generateNavigationFooter', () => {
        ...
        it('should generate full footer with all three sections', () => {
          ...
          expect(footer).toContain('- [llms.txt](https://ably.com/llms.txt)');
        });
        ...
        it('should omit Page Navigation section when both prev and next are absent', () => {
          const navContext: NavContext = {
            siblings: [
              { name: 'Sibling', url: 'https://ably.com/docs/sibling.md', description: 'A sibling.' },
            ],
          };
        ...
        it('should always include Documentation Index section', () => {
          ...
          expect(footer).toContain(
            '- [llms.txt](https://ably.com/llms.txt): Primary web search index. Use it to identify latest ably documentation URLs before fetching specific pages.',
          );
        });

        it('should start with horizontal rule separator', () => {
          const navContext: NavContext = {
            next: { name: 'Next', url: 'https://ably.com/docs/next.md', description: 'Next desc.' },
            siblings: [],
          };
        ...
        it('should use correct siteUrl for llms.txt link', () => {
          ...
          expect(footer).toContain('[llms.txt](https://custom-site.com/llms.txt)');
        });
      });
});
