...
channel.publish("response", data: response) { publishResult, error in
    guard error == nil, let msgSerial = publishResult?.serials.first?.value else {
        if let error {
    ...
    channel.annotations.publish(forMessageSerial: msgSerial, annotation: citation1) { error in
        if let error {
            print("Error publishing citation 1: \(error)")
        }
    }
    ...
    channel.annotations.publish(forMessageSerial: msgSerial, annotation: citation2) { error in
        if let error {
            print("Error publishing citation 2: \(error)")
        }
    }
}
```
...
channelOptions.modes = .annotationSubscribe
