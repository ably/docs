...
import { useUser, getApiKey } from '@/lib/user-context';
...
export function ExamplePageClient({ example }: ExamplePageClientProps) {
  ...
  const [activeLanguage, setActiveLanguage] = useState<LanguageKey>(() => {
    ...
    const langParam = searchParams.get('lang') as LanguageKey | null;
  ...
  useEffect(() => {
    // Update URL with the active language
    const newParams = new URLSearchParams(searchParams.toString());
    newParams.set('lang', activeLanguage);
    router.replace(`/examples/${example.id}?${newParams.toString()}`, { scroll: false });

    // There is a bug in Sandpack where startRoute is lost on re-renders.
    // This is a workaround to reintroduce it post-language change.
      ...
      if (isFirstRender.current) {
        ...
        setTimeout(() => {
          ...
          if (iframe) {
            ...
            if (currentSrc) {
              iframe.setAttribute('src', currentSrc + '?' + queryParams);
            }
          }
        }, 100);
      }
    }
  }, [activeLanguage, example.id, router, searchParams]);
  ...
  return (
    <>
      <div className="my-10">
        <Link href="/examples" className="flex gap-1 items-center mb-5">
