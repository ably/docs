This step will allow users to authenticate with Ably by creating a token request. This will mean we can have push-subscribe access rather than push-subscribe. We are going to authenticate and get a token from a node.js server.
