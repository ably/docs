...
function prepareReply(originalMessage: Message) {
  ...
  const replyingTo = {
    ...
    previewText: originalMessage.text.length > 140
      ? originalMessage.text.substring(0, 137) + '...'
...
const ChatComponent = () => {
  ...
  const prepareReply = (originalMessage) => {
    // Extract key information from the original message
    const replyingTo = {
      serial: originalMessage.serial,
      createdAt: originalMessage.createdAt.getTime(),
...
### Step 2: Send a reply message with metadata <a id="send-reply"/>
...
## Handling received messages with reply metadata <a id="handling-received-messages"/>
...
7. If the original message is not found, you might want to fetch it (see next section) or display a simplified version using the preview data.
...
const MessageList = () => {
  ...
  const findOriginalMessage = (replyData) => {
      ...
      {messages.map(message => (
        <div key={message.serial}>
          {message.metadata?.reply && (
            <div>
              {findOriginalMessage(message.metadata.reply) ? (
                <div>
                  <span>Replying to {findOriginalMessage(message.metadata.reply).clientId}:</span>
                  <p>{findOriginalMessage(message.metadata.reply).text}</p>
                </div>
              ) : (
                <div>
...
async function fetchOriginalMessage(replyData) {
    ...
    try {
        ...
        let originalMessage = result.items.find(msg => msg.serial === replyData.serial);

        // If message not found and more pages exist, continue searching
        while (!originalMessage && result.hasNext()) {
            // Get the next page of results
            result = await result.next();
            originalMessage = result.items.find(msg => msg.serial === replyData.serial);
        }
...
const FetchOriginalMessage = ({ replyData }) => {
    ...
    <div>
      {originalMessage && (
        <div>
          ...
          <span>Sent by: {originalMessage.clientId}</span>
        </div>
      )}
    </div>
  );
...
function findOriginalMessage(serial) {
...
function renderMessage(message) {
    ...
    if (message.metadata?.reply) {
        const replyData = message.metadata.reply;
        const originalMessage = findOriginalMessage(replyData.serial);

        const replyContext = document.createElement('div');
        replyContext.className = 'reply-context';

        if (originalMessage) {
            replyContext.textContent = `Replying to ${originalMessage.clientId}: ${originalMessage.text}`;
        } else {
...
const ChatComponent = () => {
  ...
  // Find original message for a reply
  const findOriginalMessage = (serial) => {
          ...
          >
            ...
            {message.metadata?.reply && (
              <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>
                {(() => {
                  const original = findOriginalMessage(message.metadata.reply.serial);
                  if (original) {
                    return `Replying to ${original.clientId}: ${original.text}`;
                  } else {
...
2. **Message updates**: Because messages can be updated (including the metadata), it's possible to accidentally remove the quoted message reference. Also, if providing a message preview, you will have to handle updates to the original message text, as the preview might become outdated.
...
7. **No built-in API**: You'll need to build some of your own custom components and logic to handle replies and quotes.
