...
In Ably a client can be identified with a @client ID@ in three ways:

* if the client is authenticated with a token that is issued for that @client ID@;
* if the client claims that @client ID@ and is authenticated using "Basic Authentication":#basic-authentication (using an API key);
* if the client claims that @client ID@ and is authenticated with a token that is issued for a "wildcard @client ID@":https://support.ably.io/solution/articles/3000048586-can-a-client-emulate-any-client-id-i-e-authenticate-using-a-wildcard-client-id (a special token privilege that allows any client identity to be assumed)
