...
![Diagram showing a user sending a follow-up message while the agent is still streaming a response](../../../../images/content/diagrams/ait-double-texting.png)

## Default behaviour: concurrent <a id="default-behavior"/>

On the client, sending a message while the agent is streaming starts a new concurrent turn. Both the existing response and the new turn run in parallel:
...
To prevent concurrent turns and queue messages instead, detect active turns on the client and gate the send:
...
On the client, to cancel the current run before sending the new message, use the [interruption and steering](/docs/ai-transport/features/interruption-and-steering) pattern:
...
- Concurrent turns share the connection's message rate. Many parallel streams approach the rate limit faster than one, so widen the [append rollup window](/docs/ai-transport/features/token-streaming#rollup).
- A pattern choice that varies across devices in the same session leads to inconsistent behaviour. Decide the pattern at the application layer rather than per device.
