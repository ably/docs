...
Ably stores all messages for two minutes by default. This can be increased up to a year, or longer, depending on your account package. It is also possible to persist the last message sent to a channel for a year. Ably "integrations":/docs/general/integrations can also be used to send messages outside of Ably for long-term storage.
...
"Integrations":/docs/general/integrations provide the ability to store your messages outside of Ably, for example in your own database or data warehouse.

Set up an integration rule to send messages to your own systems using "webhooks":/docs/general/webhooks or "serverless functions":/docs/general/webhooks. Integration rules can additionally filter which messages sent to a channel should trigger these events.
