...
Users can be provisioned into additional accounts through SCIM group membership. When a SCIM group is linked to an account, all members of that group are automatically provisioned into that account. See [Group roles](#group) for details.
...
When organizations and your identity provider are configured, the groups you create in the identity provider are synchronized with Ably. This allows you to manage group-based access centrally. Assign roles to these groups across one or more accounts. When a group is linked to an account, all users in that group are automatically provisioned into that account and inherit the assigned roles. Users added to the group afterwards are also provisioned into the linked accounts automatically.
