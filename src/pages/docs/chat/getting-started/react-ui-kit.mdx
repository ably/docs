...
<Aside data-type="note">
  When using ChatWindow with Sidebar, set `attach={false}` and `release={false}` on the ChatRoomProvider. This prevents conflicts since the Sidebar automatically manages room connections. This is currently required because RoomProviders do not yet share references.
</Aside>
