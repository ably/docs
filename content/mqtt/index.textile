...
The "Ably MQTT protocol adapter":https://www.ably.io/adapters is able to translate back and forth between "MQTT":http://mqtt.org/ and Ably's own protocol, allowing for seamless integration of any systems you may have. MQTT (MQ Telemetry Transport) is a "publish/subscribe":https://www.ably.io/topic/pub-sub , lightweight messaging protocol designed for constrained devices and low-bandwidth networks. One of the major uses of MQTT is with IoT (Internet of Things), where these principles are key to having effective communication between various devices.
...
* Session resumption is supported within the usual "Ably time limit of two minutes":https://knowledge.ably.com/connection-state-recovery-1
* It doesn’t support any MQTT features that aren’t normally supported by Ably, such as @WILL@ messages, the @RETAIN@ flag or "wildcard channel subscriptions":https://knowledge.ably.com/can-i-attach-and-subscribe-to-channels-using-wildcards-1
...
We support both SSL and non-SSL connections (the latter uses a different port, see above), but strongly recommend using SSL wherever possible. If you are not using SSL, note that the same restrictions apply as if you were using an Ably client without SSL. That is, "connection attempts using Basic Authentication (i.e. an API key) are disallowed":https://knowledge.ably.com/error-code-40103-invalid-use-of-basic-auth-over-non-tls-transport-1, and any "namespaces which you've enabled 'require TLS' on":https://knowledge.ably.com/what-is-a-channel-namespace-and-how-can-i-use-them-1 will be inaccessible. If this is a problem for you, please do let us know at support@ably.io and we'll see what we can do.
...
* While the adapter can be useful for devices which need to use MQTT, there are many benefits to using the Ably client libraries (e.g. "continuity guarantees":https://knowledge.ably.com/connection-state-recovery-1, "fallback host support":https://knowledge.ably.com/routing-around-network-and-dns-issues-1, "history":https://www.ably.io/documentation/realtime/history, "presence":https://www.ably.io/documentation/realtime/presence) require the use of the Ably client libraries. As a result, if a native Ably library is available for your platform, we recommend you consider using the Ably client libraries instead.
