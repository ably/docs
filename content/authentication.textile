...
Each is based on having a series of keys set up, via the dashboard, for each application. Each key is exposed by the dashboard as a single string which contains information that identifies the key as well as containing the "secret" key value.
...
* {{app id}} is the identifier for the application: this id is used in the [[[REST API]]] and is needed to instance the REST or Realtime client library if you do not have a key string.

* {{key id}} is a (public) identifier for the key in question: this uniquely identifies the key and is a system-assigned, URL-safe, identifier.

* {{key value}} is the key "secret": a secret string, system-generated, uniquely associated with this key.

+++ Token request format

A token request is made against the {{requestToken}} endpoint, with a //token request// in the request body. The token request comprises:
...
[[code]]
{
  access_token: <access token details>
}
[[/code]]

Each token details object has the form:
