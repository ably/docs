...
EventMachine.run do
  ...
  client.channel('realtime-chat').attach do |channel|
    channel.presence.subscribe(:enter) do |presence_message|
      puts "#{Time.now}: #{presence_message.member_key} entered realtime-chat"
    end
...
EventMachine.run do
  ...
  client.channel('realtime-chat').presence.enter
end
</code>
</pre>
...
<code lang="ruby">
EventMachine.run do
  client = Ably::Realtime::Client.new(api_key: '<%= %>')
  presence = client.channel('realtime-chat').presence

  presence.subscribe(:enter) do |presence_message|
    puts "#{Time.now}: #{presence_message.member_key} entered realtime-chat"
  end

  presence.enter_client('bob')
  presence.enter_client('mary')
end
# Output:
# 2015-01-27 16:54:05 +0000: oVssCTfaIz:bob entered realtime-chat
# 2015-01-27 16:54:05 +0000: oVssCTfaIz:mary entered realtime-chat
</code>
</pre>
...
# Output:
# 2015-01-27 16:58:00 +0000: X0i3jFssWx:bob entered realtime-chat
</code>
</pre>
...
EventMachine.run do
  ...
  presence.enter(client_id: 'bob', data: 'robert@example.com')
end
</code>
</pre>
...
EventMachine.run do
  ...
  client.channel('realtime-chat').presence.enter do |presence|
    presence.get do |members|
      puts "There are #{members.size} members on this channel" if members.size > 1
      puts "There is 1 member on this channel" if members.size == 1
    end
  end
end
</code>
...
bc[ruby]. EventMachine.run do
  client = Ably::Realtime::Client.new(config)

  client.channel('realtime-chat').presence.enter_client('mary') do
    client.channel('realtime-chat').presence.history(direction: :backwards) do |history|
