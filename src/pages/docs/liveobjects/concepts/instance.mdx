...
<If lang="java">
In JavaScript, calling a method that doesn't match the underlying type returns an `undefined` value, an empty iterator or an `undefined` size. In Java this cannot happen: the method simply doesn't exist on the wrong typed instance, and casting to the wrong type throws an exception. Check `getType()` when the type isn't known:
...
<If lang="java">
<Aside data-type='important'>
In Java a type mismatch is either a compile error (the method doesn't exist on the typed instance) or a fast failure in the cast: `asLiveCounter()` on a `LiveMap` instance throws an `IllegalStateException`. Check `getType()` first when the type isn't known. See the [Type inference documentation](/docs/liveobjects/typing?lang=java#type-inference) for the full cast contract.
</Aside>
</If>
