...
For partially successful requests, @response.items.batchResponse@ will contain a list of the individual channel's results, be it an error or a success.
...
ablyRest.request('GET', '/presence', content, null, {}, function(err, response) {
  if(err) {
    ...
    if(response.success) {
      ...
      if(response.items[0].batchResponse) {
        // If partial success
