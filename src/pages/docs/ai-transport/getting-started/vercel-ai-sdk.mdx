...
1. `sendMessage({ text })` calls the `ChatTransport`'s `sendMessages`. Under the hood, the AI Transport client SDK mints `runId` and `inputEventId`, stamps them on the channel publish under `extras.ai.transport`, and returns an `ActiveRun`. The `ChatTransport` then calls `activeRun.toInvocation().toJSON()` and POSTs the resulting `InvocationData` body (`{ runId, inputEventId, sessionName }`) to your agent endpoint.
2. The agent route receives the POST, calls `Invocation.fromJSON`, creates an `AgentSession`, and starts a Run. `session.createRun(invocation)` mints the `invocationId` (one per HTTP request); the agent stamps it on every event it publishes and returns it on the response. `run.start()` waits for the trigger input event on the channel (rewind plus live).
