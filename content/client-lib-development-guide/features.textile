...
* @(RSC15)@ Host Fallback
** @(RSC15a)@ All HTTP requests sent to Ably are fault-tolerant and will fallback to an alternative host name if the default host endpoint is unreachable or indicates that it is unserviceable with a 500 to 504 status code. This ensures that a client library is able to work around routing or other problems with the user's closest data center. For example, if a @POST@ request to @rest.ably.io@ fails because the default endpoint is unreachable or unserviceable, then the @POST@ request should be retried again against the fallback hosts in attempt to find an alternate healthy datacenter to service the request. The fallback hosts are @[a-e].ably-realtime.com@ and should be attempted in random order, with a maximum number of fallback attempts being the configured @httpMaxRetryCount@ (see "@TO3l5@":#TO3l5)
