...
func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
    DispatchQueue.main.async() {
        print("** registration with APNs failed with an error")
        ARTPush.didFailToRegisterForRemoteNotificationsWithError(error, realtime: self.getAblyRealtime())
    }
}
```

If this registration is successful, APNs would return a unique @deviceToken@ for each device. We'll use that @deviceToken@ received and send it to Ably. 