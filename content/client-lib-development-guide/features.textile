...
** @(BO2a)@ Sends a @POST@ request to @<restHost>/messages@ with the supplied @BatchSpec@ or @[]BatchSpec@ as the request body where @<restHost>@ represents the current rest host as described by "@RSC11@":#RSC11
** @(BO2b)@ Takes a @BatchSpec@ or @[]BatchSpec@ as an argument. In languages where function overloading is not supported an array is preferred.
