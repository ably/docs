...
CAVEAT-INPUT
  ...
  Occupancy updates are also included in events published to the "[meta]channel.lifecycle metachannel":/metadata-stats/metadata/subscribe. However, these lifecycle events are only published when channels are opened or closed.
...
"Integration rules":/integrations can be configured with @channel.occupancy@ set as a rule source. Occupancy events that occur on a subset of configured channels will be pushed to external targets, such as AWS Lambda, or custom webhook endpoints.