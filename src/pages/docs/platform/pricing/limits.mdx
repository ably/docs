...
In addition to the platform rate limits above, you can set per-connection publish rate limits using JWT claims. This enables you to restrict individual clients to a specific message rate, independent of the platform-wide limits.

This is useful for:

* Limiting each tenant's publish rate in multi-tenant platforms to prevent one tenant from consuming the entire account quota.
* Restricting message frequency per user in chat applications to control spam.
* Limiting publish rates for individual IoT devices to protect against misbehaving firmware or compromised hardware.

Per-connection rate limits are configured via JWT token claims. See [using JWT for per-connection publish rate limits](/docs/auth/jwt#rate-limits) for the claim syntax and code examples.
