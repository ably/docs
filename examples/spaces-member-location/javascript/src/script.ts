...
const locationColors = ["#9951F5", "#f1c232", "#f44336"];
const cellData = [
  ["Spring water", "Tacos"],
  ["Cola", "Noodles"],
  ["Ginger Beer", "Pizza"],
...
async function connect() {
  ...
  await space.enter({
    ...
    memberColor: locationColors[Math.floor(Math.random() * locationColors.length)]
  });
...
function buildCell(
  ...
  let className = 'cell';
  if (cellMembers.length > 0 && !selfInCell) className += ' cellMembers';
  if (!selfInCell && cellMembers.length === 0) className += ' rest';
  if (selfInCell) className += ' cellSelf';
...
async function buildSpreadsheet(otherMembers: Member[], self: Member) {
  ...
  for (const [rowIndex, row] of cellData.entries()) {
    ...
    for (const [colIndex, col] of row.entries()) {
      const cellMembers = otherMembers.filter((user) =>
