...
h3(#token-warnings). Token request warnings

There are three types of Token request limit warnings you may receive:
...
h3(#connection-warnings). Connection warnings

There are three types of Connection limit warnings you may receive:
...
h3(#channel-warnings). Channel warnings

There are three types of Channel limit warnings you may receive:
...
h3(#message-warnings). Message warnings

Message limit warnings are split into hourly and monthly warnings. 

h4(#hourly-warnings). Hourly warnings

There are three types of hourly Message limit warnings you may receive:
...
* @messages.hourly.hard@ - when the hard limit on the account has been exceeded.

h4(#monthly-warnings). Monthly warnings

There are three types of monthly Message limit warnings you may receive:

* @messages.monthly.warning@ - when you reach 80% of your pre-paid quota.
* @messages.monthly.soft@ - when your pre-paid quota has been exceeded.
* @messages.monthly.hard@ - when the hard limit on the account has been exceeded.
...
The function invocation rate limit is the maximum rate at which functions can be invoked. This includes AWS Lambda, Google Cloud and Azure functions that can be triggered by "events":https://ably.com/docs/general/events.
...
h3(#integration-warnings). Integration warnings

Warnings for integrations are sorted into integration type and then warning type:

* @reactor.httpEvent.maxRate.hard@ - when the hard limit on the account has been exceeded.
* @reactor.httpEvent.maxRate.warning@ - when you reach 80% of your pre-paid quota.
* @reactor.webhook.maxRate.hard@ - when the hard limit on the account has been exceeded.
* @reactor.webhook.maxRate.warning@ - when you reach 80% of your pre-paid quota.
* @reactor.externalQueue.maxRate.hard@ - when the hard limit on the account has been exceeded.
* @reactor.externalQueue.maxRate.warning@ - when you reach 80% of your pre-paid quota.
* @reactor.amqp.maxRate.hard@ - when the hard limit on the account has been exceeded.
* @reactor.amqp.maxRate.warning@ - when you reach 80% of your pre-paid quota.
...
h3(#api-warnings). API warnings

There are three types of apiRequest limit you may receive:
