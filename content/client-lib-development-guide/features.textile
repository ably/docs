...
* @(RSC10)@ If a REST request responds with 401 HTTP status code and an Ably code value @40140 <= @code < 40150@ indicating that the token has expired or is invalid, then the Auth class should be responsible for reissuing a token and the request should be reattempted, see "RSC14c":#RSC14c and "RSC14d":#RSC14d
...
** @(RTN15h)@ If a @DISCONNECTED@ message is received from Ably and an @ErrorInfo@ object is present with a 401 @statusCode@ and an Ably @code@ value @40140 <= @code < 40150@, then Ably is closing the connection because the token is expired or invalid and needs to be renewed before reconnecting. If the token is renewable, the error should not emitted, a single attempt to create a new token will be made and a new connection attempt will be initiated using the new token. If the token create fails, the next connection attempt fails for the same reason, or the token was not renewable, the connection will transition to the @FAILED@ state and the @Connection#errorReason@ will be set
