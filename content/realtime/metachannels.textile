...
When creating an outbound webhook Reactor rule it is possible to select @Channel Lifecycle@ as the "source":general/events#sources. This will set all channel lifecycle events as triggers for the webhook, rather than having to create individual message rules for each event type.
