...
Your backend endpoint called by your sync function should return the largest @sequenceID@ in the "outbox":/livesync/connector/database#outbox at the time that the state is read:
...
Change events from your backend are delivered to the Models SDK as messages over Ably "channels":/channels. By default, messages are processed by the Models SDK in the order in which they are received.
