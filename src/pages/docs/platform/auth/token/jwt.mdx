...
Use native [Ably Tokens](/docs/platform/auth/token/ably-tokens) when:
...
- You already have a JWT-based auth system and want to embed Ably tokens within your existing JWTs. See [Embedded Token JWT](/docs/platform/auth/token#embedded).
</Aside>
...
|-------|----------|-------------|
| `x-ably-capability` | Yes | JSON string defining [capabilities](/docs/platform/auth/capabilities) |
| `x-ably-clientId` | No | Sets a trusted [client ID](/docs/platform/auth/identified-clients) |
...
**JWT-only features:** Channel-scoped claims and per-connection rate limits are only available with JWTs. If you use [Ably Tokens](/docs/platform/auth/token/ably-tokens), you will not have access to these features.
</Aside>

## Token lifecycle

Token refresh, TTL limits, and dynamic access control apply to all token types. See [token authentication](/docs/platform/auth/token#refresh) for details.

For a comparison of JWT, TokenRequest, and Ably Token mechanisms, see [choosing a token mechanism](/docs/platform/auth/token#choosing).
