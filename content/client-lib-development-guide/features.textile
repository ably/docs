...
** @(RSA17a)@ Revokes tokens based on provided function arguments. It makes a request to the "REST revokeTokens endpoint":https://ably.com/docs/api/rest-api#revoke-tokens and handles the response.
** @(RSA17b)@ Method signature is @revokeTokens(Array<TokenRevocationTarget>. issuedBefore, allowReauthMargin, revocationCallback)@. This method signature is only for guideline. For each language this function must be adapted to its respective idioms and must be documented. For example for languages that support optional parameters, it makes sense to design this function as it is, marking optional and required parameters.
*** @(RSA17b1)@ @Array<TokenRevocationTarget>@ is an array of @TokenRevocationTarget@s to revoke. This array of token revocation targets must be used to construct the individual @TokenRevocationTarget@s in the form of "@TokenRevocationTarget#type@:@TokenRevocationTarget#value@" for the "targets" array in the request body.
*** @(RSA17b2)@ @issuedBefore@ is an optional timestamp (in milliseconds since the epoch) to revoke tokens issued before this time.
*** @(RSA17b3)@ @allowReauthMargin@ is an optional boolean that permits a token renewal cycle to take place without needing established connections to be dropped. The default for this value is false if not provided.
*** @(RSA17b4)@ @revocationCallback@ is an optional callback function (or lambda) for languages that support this idiom. This callback must carry @TokenRevocationResponse@ if function is successful, it must carry respective error when funciton fails.
** @(RSA17c)@ key to build the request must be retrieved from stored @Auth#AuthOptions#key@
** @(RSA17d)@ If the @targets@ parameter is null or empty, then the function must respond with an error containing @"targets cannot be null or empty"@ message. If @revocationCallback@ is available, it must be trigerred with this error.
** @(RSA17e)@ Optional for languages that cannot enforce nullability in compile time: If any @TokenRevocationTarget@ in @targets@ parameter is invalid that is it either does not contain 'type' or 'value' or both, the function must respond with @"Invalid RevocationTarget in targets array"@ message. If @revocationCallback@ is available, it must be trigerred with this error.
** @(RSA17f)@ If the @targets@ parameter is valid, then the function must create the request and execute it.
** @(RSA17g)@ Function returns a @TokenRevocationResponse@ object that contains the result of the request if the request is successful
** @(RSA17h)@ Function responds with an appropriate error if the request fails either for the reasons of invalid request, or as a result of request failing.
...
* @(TL2)@ @TokenRevocationTarget#type@ is a string that is the target type (for example "clientId").This value cannot be null.
...
* @(TX2)@ @TokenRevocationResponse#revokedTargets@ is a collection of @RevokedTarget@s representing the targets that are revoked.
...
class Auth:
  ...
  revokeTokens([TokenRevocationTarget], Time?,Bool?,TokenRevocationCallback) // RSA17
...
class RevokedTarget:
  target: TokenRevocationTarget // TY2
  issuedBefore: Time // TY3
  appliesAt: Time // TY4

class TokenRevocationResponse:
  revokedTargets: [RevokedTarget]? //TX2

interface TokenRevocationCallback:
  on(TokenRevocationResponse?, Error?) // RSA17g, RSA17h
