...
h2(#endpoint). Ably service endpoints
...
**** @(RAS2b1c)@ No other @zone@ values are valid.
*** @(RAS2b2)@ The value of @environment@ determines the subdomain of the primary host(s). If the @ClientOptions#environment@ option is set to the value @production@, or is unset, then the @environment@ is @production@. Otherwise, the value of @environment@ is the value of the @ClientOptions#environment@ option.
...
** @(RAS2d)@ An attempt to instance the library with an invalid value or combination of values for @ClientOptions#restHost@, @ClientOptions#realtimeHost@, @ClientOptions#environment@ or @ClientOptions#zone@ is rejected with an error.
* @(RAS3)@ Host Fallback
...
*** @(RAS3b3)@ For backwards-compatibility only, if the @ClientOptions#fallbackHostsUseDefault@ is @true@, then the fallback hosts are the production fallbacks @['a.ably-realtime.com', 'b.ably-realtime.com', 'c.ably-realtime.com', 'd.ably-realtime.com', 'e.ably-realtime.com']@. The @ClientOptions#fallbackHostsUseDefault@ option is deprecated and future library releases will ignore any supplied value.
*** @(RAS3b4)@ If the primary host is not modified from the default - that is, if both @zone@ and @environment@ have the value @production@  as defined in @(RSC11)@ - then the fallback hosts are the production fallbacks @['a.ably-realtime.com', 'b.ably-realtime.com', 'c.ably-realtime.com', 'd.ably-realtime.com', 'e.ably-realtime.com']@.
*** @(RAS3b5)@ Otherwise, the set of fallback hosts is determined based on the combination of the @zone@ and @environment@ options as follows.
**** @(RAS3b51)@ For a @zone@ value of @production@, the PD of the fallback hosts is @ably-realtime.com@.
**** @(RAS3b52)@ For a @zone@ value of @dev@, the PD of the fallback hosts is @ably-realtime-dev.com@.
**** @(RAS3b53)@ For any given @environment@ the fallback subdomains are @<environment>-[a-e]-fallback@. For example, for an @environment@ value of @acme@ and a @zone@ of @production@, the fallback hosts are @['acme-a-fallback.ably-realtime.com', 'acme-b-fallback.ably-realtime.com', 'acme-c-fallback.ably-realtime.com', 'acme-d-fallback.ably-realtime.com', 'acme-e-fallback.ably-realtime.com']@.
** @(RAS3c)@ The fallback behavior described below only applies when the set of fallback hosts is not empty. If the set of fallback hosts is empty, failing operations that would have "qualified for a retry against a fallback host (see RSC15d)":#RSC15d, will instead result in an error immediately
...
** @(RSC7c)@ If the @addRequestIds@ client option is enabled, every REST request to Ably should include in a @request_id@ query string parameter a random string obtained by url-safe base64-encoding a sequence of at least 9 bytes obtained from a source of randomness. This request ID must remain the same if a request is retried to a fallback host per @RAS3@. Any log messages associated with the request should include the request ID. If the request fails, the request ID must be included in the @ErrorInfo@ returned to the user.
...
* @(RSC11)@ (Superseded by "RAS2":#RAS2).
* @(RSC12)@ (Superseded by "RAS2":#RAS2).
...
* @(RSC15)@ Host Fallback for REST operations
** @(RSC15e)@ The primary host, via DNS, is automatically routed to the client's closest datacenter. New HTTP requests (except where @RSC15f@ applies and a cached fallback host is in effect) are first attempted against the primary host.
** @(RSC15a)@ In the case of an error necessitating use of an alternative host (see "RSC15d":#RSC15d), try fallback hosts (with a matching Host header as this is necessary when fallbacks are proxied through a CDN) in random order, continuing to try further hosts if "qualifying errors":#RSC15d occur, failing when all have been tried or the configured @httpMaxRetryCount@ has been reached (see "TO3l@":#TO3l5). This ensures that a client library is able to work around routing or other problems for the user's closest datacenter. For example, if a @POST@ request to the primary host fails because that endpoint is unreachable or unserviceable, then the @POST@ request should be retried again against the fallback hosts in attempt to find an alternate healthy datacenter to service the request.
...
** @(RSC19e)@ If the HTTP network request fails for reasons such as a timeout (after the underlying fallback host attempts have failed where applicable, see "RAS3":#RAS3), then the @request@ method should indicate an error in an idiomatic way for the platform
...
** @(RTC1d)@ @realtimeHost@ string, when set, will modify the realtime primary host. See "RAS2":#RAS2 for details.
** @(RTC1e)@ @environment@ string, when set, will modify both the REST and realtime hosts. See "RAS2":#RAS2 for details.
** @(RTC1g)@ @zone@ string, when set, will modify both the REST and realtime hosts. See "RAS2":#RAS2 for details.
...
* @(RTN17)@ Host Fallback for realtime connections
** @(RTN17b)@ The fallback behavior described below only applies when the set of fallback hosts, as defined in "RAS3":#RAS3, is not empty.
** @(RTN17a)@ By default, every connection attempt is first attempted to the primary realtime host which, through DNS, is automatically routed to the client's closest datacenter. The client library must always prefer the default endpoint (closest datacenter), even if a previous connection attempt to that endpoint has failed. (That is, @RSC15f@ does not apply)
** @(RTN17c)@ In the case of an error necessitating use of an alternative host (see "RTN17d":#RTN17d), the @Connection@ manager should first check if an internet connection is available by issuing a @GET@ request to @https://internet-up.ably-realtime.com/is-the-internet-up.txt@. If the request succeeds and the text "yes" is included in the body, then the client library can assume it has a viable internet connection and should then immediately retry the connection against all fallback hosts to find an alternative healthy datacenter. The fallback hosts should be attempted in random order. The connection requests must include a matching Host header as this is necessary when fallbacks are proxied through a CDN. See "RAS3":#RAS3 for details on how custom fallback hosts are specified and used
...
*** @(TO3k2)@ @restHost@ string - for development environments only; allows a non-default Ably REST host to be specified. It is never valid to provide both a @restHost@ and either of an @environment@ or @zone@ value
*** @(TO3k3)@ @realtimeHost@ string - for development environments only; allows a non-default Ably Realtime host to be specified. It is never valid to provide both a @realtimeHost@ and either of an @environment@ or @zone@ value
...
class ClientOptions:
  ...
  environment: String? // RAS2, RAS3, TO3k1
  zone: String? // RAS2, RAS3, TO3k1
