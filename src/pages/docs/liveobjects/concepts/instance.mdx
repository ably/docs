...
<If lang="swift">
<Aside data-type='note'>
Mutations on an `Instance` are asynchronous: they suspend with `try await` until the operation completes. Reads, navigation and case matching on an `Instance` are synchronous and run locally, so they need no `await`. There are no blocking variants. Subscription callbacks fire on ably-cocoa's callback queue, which is the main queue by default, so you can update your UI directly from them without hopping queues.
</Aside>
</If>
...
<If lang="swift">
<Aside data-type="note">
Unlike `PathObject`, the `get(key:)` method on a `LiveMapInstance` returns `nil` if the entry doesn't exist or the object at that location has been deleted. "Not a `LiveMap`" cannot happen, because `get(key:)` only exists on `LiveMapInstance`.
</Aside>
</If>
<If lang="java">
<Aside data-type="note">
...
if case .liveMap(let settings) = try rootObject.get(key: "settings").instance() {
    print(settings.id) // e.g. "map:abc123@1234567890"

    // Chain get() calls for deeper navigation.
    // get(key:) returns nil at any point if the accessed entry does not exist
    if case .liveMap(let theme) = try settings.get(key: "theme"),
       case .primitive(let color) = try theme.get(key: "color"),
       case .string(let colorValue) = try color.value {
        print(colorValue) // e.g. "blue"
    }
}
```
...
<If lang="swift">
The `id` property returns the [object ID](/docs/liveobjects/concepts/objects?lang=swift#object-ids) of the instance, which can be used with the [REST API](/docs/liveobjects/rest-api-usage). A primitive-backed instance has no `id` property, because primitives have no object ID.
</If>
<If lang="java">
...
if case .liveMap(let user) = try rootObject.get(key: "user").instance() {
    ...
    // Get the value of an entry containing a primitive
    if case .primitive(let username) = try user.get(key: "username"),
       case .string(let name) = try username.value {
        print("Username: \(name)") // e.g. "alice"
    }
...
<If lang="swift">
If the entry doesn't exist, `get(key:)` returns `nil`:
</If>
<If lang="java">
...
if case .liveMap(let user) = try rootObject.get(key: "user").instance() {
    // Get an entry that doesn't exist on the instance
    print(String(describing: try user.get(key: "nonexistent"))) // nil
}
```
...
<If lang="swift">
<Aside data-type='important'>
`subscribe()` and `events()` are only available on `LiveMapInstance` and `LiveCounterInstance`; this is enforced at compile time, as `PrimitiveInstance` has no `subscribe` method. To subscribe to a specific entry in a `LiveMap`, regardless of the type of the entry's value, [subscribe](/docs/liveobjects/concepts/path-object?lang=swift#subscribe) through a `PathObject` instead.
</Aside>
</If>
<If lang="java">
<Aside data-type='important'>
...
if case .liveCounter(let visits) = try rootObject.get(key: "visits").instance() {
    try visits.subscribe { event in
        if event.message?.operation.action == .objectDelete {
            print("visits counter was deleted")
    ...
    // Make the LiveCounter stored in visits unreachable.
    // It will eventually be deleted.
    try await rootObject.remove(key: "visits")
}
```
