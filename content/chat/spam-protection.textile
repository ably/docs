...
Large-scale live-streaming conversations can result in members having difficulty keeping up with the flow of the conversation. This may be due to natural volume of messages sent, or caused by members intent on 'spamming' the channel. Slow mode enables you to limit the number of messages sent by each member within a given timeframe.

Members can be restricted either on individual channels, or as a set of channels defined by a common namespace. For example if you set the value of @ably.limits.publish.perAttachment.maxRate.chat:*@ to one of @1@, then members would only be able publish one message per second in any channel defined under the @chat@ namespace. Examples of other possibilities are shown in the table below:
