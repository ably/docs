...
<Aside data-type="usp">
Consistent reaction state across users.

Reaction additions and removals are delivered to all users in the same order, maintaining consistent reaction counts and state across disruptions. [Learn how Ably maintains message ordering](/docs/platform/architecture/connection-recovery#message-identification-with-timeserial).
</Aside>
...
Raw reaction events include a `reaction.userClaim` field containing the server-signed [user claim](/docs/chat/setup#user-claims) of the user who sent the reaction, derived from their [JWT](/docs/chat/setup#set-user-claims).
