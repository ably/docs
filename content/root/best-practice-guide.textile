...
h1(#title). Ably Best Practice Guide
...
You can publish data to named "channels":/realtime/channels-messages within Ably's platform and the platform will make sure that all clients subscribed to these channels will receive the data in realtime.
...
h3. Channels that you attach to, automatically receive messages from Ably even without a subscribe listener
...
h3. Use separate channels to filter messages for different sets of subscribers
...
h3. Being present without subscribing to presence events
...
h3. Handling connection and channel state-change events
...
However, while registering listeners for both connection and channel state change events, bear in mind that certain repeating states might add new listeners each time. For instance, registering a listener for @on(connected)@ adds a new listener every time the client becomes connected, even if this is a reconection after being offline for a while, which is often an unintentional result.

h3. Using the API key on the client side

Ably's Basic Authentication method is the easiest way to authenticate your clients on the Ably platform, but, this will directly expose the API key in the source code. Given that when you use it on the client side, any entity that interacts with the client could view your API key and potentially misuse it. So, if you can run on auth server, we recommend using Token Authentication instead. With this method, your auth server will use the API key to raise a token request from Ably, and the token can then be used by your end clients in the authentication step, eliminating the risk of an exposed API key. Read more about selecting an authentication mechanism.

* Ably's "Basic Authentication":/general/authentication#basic-authentication method is the easiest way to authenticate your clients on the Ably platform, but, this will directly expose the API key in the source code. Hence, when using it on the client side, any entity that interacts with the client could view your API key and potentially misuse it. So, if you can run an auth server, we recommend using "Token Authentication":/general/authentication#token-authentication instead. With this method, your auth server is expected to use the API key to raise a token request from Ably, and the tokent can then be used by your end clients in the authentication step, eliminating the chance of misusing an exposed API key. "Read more":/general/authentication#selecting-auth about selecting an authentication mechanism.

h3. Ensuring tokens are auto-renewable
...
h3. Using the correct client library SDK - Choosing between Realtime, REST, MQTT and others 
...
h3. Subscribing to events on the server-side
...
h3. WebHooks/Reactor Functions vs. Message Queues when to use which?
...
h3. Limiting the effect of a security compromise using the principle of least privilege
...
h3. Embedding client ID in the token to ensure trust
