...
{/* Swift example test harness: to verify compilation, use a function with signature:
func example() {
...
options.authCallback = { params, callback in
    ...
    guard let url = URL(string: "https://your-server.com/api/auth/token") else {
        callback(nil, NSError(domain: "AuthError", code: -1, userInfo: nil))
        return
    }
...
{/* Swift example test harness: to verify compilation, use a function with signature:
func example(ably: ARTRealtime, randomChannelName: String) {
    // --- example code starts here ---
*/}
```swift
// Client code
let acmeChannel = ably.channels.get("org:acme:\(randomChannelName)")
acmeChannel.publish("prompt", data: "What is the weather like today?") // succeeds

let foobarChannel = ably.channels.get("org:foobar:\(randomChannelName)")
...
```swift
// Agent code
let weatherChannel = ably.channels.get("org:acme:weather:\(randomChannelName)")
weatherChannel.subscribe { message in
    print(message)
} // succeeds
weatherChannel.publish("update", data: "It's raining in London") // succeeds

let otherChannel = ably.channels.get("org:acme:other:\(randomChannelName)")
...
```swift
// Client code
let channel = ably.channels.get(randomChannelName)
...
channel.subscribe("prompt") { message in
    ...
    print("Received message from user: \(userId ?? "unknown")")
...
channel.subscribe { message in
    if message.clientId == "weather-agent" {
        print("Weather agent response: \(message.data ?? "nil")")
    }
}
```
{/* --- end example code --- */}
...
```swift
// Agent code
let channel = ably.channels.get("org:acme:\(randomChannelName)")
...
channel.subscribe("prompt") { message in
    ...
    print("Message from user with role: \(role ?? "unknown")")
}
```
{/* --- end example code --- */}
</Code>
...
channel.publish("update", data: "It's raining in London", extras: [
    "headers": [
        "model": "gpt-4"
    ]
] as NSDictionary)
...
channel.subscribe { message in
    if message.clientId == "weather-agent" {
        ...
        print("Response from weather agent using \(model ?? "unknown"): \(message.data ?? "nil")")
    }
}
```
{/* --- end example code --- */}
