...
func toolCalls1(realtime: ARTRealtime, stream: [StreamEvent]) async throws {
...
@MainActor
func toolCalls2(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
// Track responses by ID, each containing tool calls and final response
var responses: [String: ResponseInfo] = [:]
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe(attachCallback: { error in
        ...
        let extras = try? message.extras?.toJSON()
        let headers = extras?["headers"] as? [String: String]
        let responseID = headers?["responseId"]
        ...
        MainActor.assumeIsolated {
            ...
            case "tool_call":
                let toolCallID = headers?["toolCallId"]
                if let toolCallID, let data = message.data as? [String: String] {
                    responses[responseID]?.toolCalls[toolCallID] = ToolCallInfo(
                        name: data["name"] ?? "",
                        args: data["args"] ?? ""
                    )
                }
            case "tool_result":
                let resultToolCallID = headers?["toolCallId"]
                if let resultToolCallID, let data = message.data as? [String: String] {
                    responses[responseID]?.toolCalls[resultToolCallID]?.result = data["result"]
                }
            default:
                break
            }
...
func renderWeatherCard(_ data: [String: Any]) {}

func toolCalls3(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe(attachCallback: { error in
        ...
        // Render component when tool is invoked
        if message.name == "tool_call", let data = message.data as? [String: String], data["name"] == "get_weather" {
            if let argsJSON = data["args"]?.data(using: .utf8),
        ...
        // Update component with results
        if message.name == "tool_result", let data = message.data as? [String: String], data["name"] == "get_weather" {
            if let resultJSON = data["result"]?.data(using: .utf8),
               let result = try? JSONSerialization.jsonObject(with: resultJSON) as? [String: Any] {
                renderWeatherCard(result)
            }
        }
    })
...
func getGeolocationPosition() async -> GeolocationResult {
    GeolocationResult(coords: .init(latitude: 0, longitude: 0))
}

func toolCalls4(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe("tool_call", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        ...
        let extras = try? message.extras?.toJSON()
        let headers = extras?["headers"] as? [String: String]
        guard let responseID = headers?["responseId"] else { return }
        guard let toolCallID = headers?["toolCallId"] else { return }
...
func processResult(_ responseID: String, _ toolCallID: String, _ result: Any) {}

@MainActor
func toolCalls5(channel: ARTRealtimeChannel) async throws {
    // insert example code here
}
*/}
```swift
var pendingToolCalls: [String: (responseID: String, toolCallID: String)] = [:]

try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe("tool_result", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
func toolCalls6(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.publish(
        ...
        data: [
            "name": "process_document",
            "args": ["documentId": "doc_123", "pages": 100] as [String: Any]
        ] as [String: Any],
        extras: [
            "headers": [
                "responseId": "resp_abc123",
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.publish(
        ...
        data: [
            "name": "process_document",
            "result": ["processedPages": 100, "summary": "Document processed successfully"] as [String: Any]
        ] as [String: Any],
        extras: [
            "headers": [
                "responseId": "resp_abc123",
...
struct ToolProgressInfo {
    var name: String
    var status: String
    var percentComplete: Int
}

func renderProgressBar(_ toolCallID: String, _ percentComplete: Int) {}
func renderCompleted(_ toolCallID: String, _ result: Any) {}

@MainActor
func toolCalls7(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
// Track tool execution progress
var toolProgress: [String: ToolProgressInfo] = [:]
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe(attachCallback: { error in
        ...
        MainActor.assumeIsolated {
            switch message.name {
            case "tool_call":
                if let toolCallID, let data = message.data as? [String: Any] {
                    toolProgress[toolCallID] = ToolProgressInfo(
            ...
            case "tool_progress":
                if let toolCallID, toolProgress[toolCallID] != nil, let data = message.data as? [String: Any] {
                    toolProgress[toolCallID]?.status = data["status"] as? String ?? ""
                    toolProgress[toolCallID]?.percentComplete = data["percentComplete"] as? Int ?? 0
                    renderProgressBar(toolCallID, data["percentComplete"] as? Int ?? 0)
                }
            ...
            case "tool_result":
                if let toolCallID {
                    ...
                    if let data = message.data as? [String: Any],
                       let result = data["result"] {
                        renderCompleted(toolCallID, result)
                    }
                }
