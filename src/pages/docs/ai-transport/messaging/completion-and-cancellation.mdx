...
## Why signal completion and cancellation <a id="why"/>
...
Use [operation metadata](/docs/messages/updates-deletes) to signal that a content part or response is complete. Operation metadata is a set of key-value pairs carried on each append or update operation. Subscribers can inspect this metadata to determine the current phase of a message.
...
Await all pending append operations before publishing the response-level completion message. This ensures the completion signal arrives after the last content token.
</Aside>
...
1. The subscriber publishes a cancellation message on the channel with a correlation ID identifying the response to cancel.
...
The subscriber sends a cancellation message with a `correlationId` in the message [extras](/docs/messages#properties) to identify which response to cancel:
...
JsonObject extras = new JsonObject();
JsonObject headers = new JsonObject();
headers.addProperty("correlationId", "resp_abc123");
extras.add("headers", headers);
...
await channel.subscribe('cancel', async (message) => {
  const correlationId = message.extras?.headers?.correlationId;
...
async def on_cancel(message):
    correlation_id = message.extras.get('headers', {}).get('correlationId')

    # Stop generation for the matching response
    stop_generation(correlation_id)
...
channel.subscribe("cancel", message -> {
    ...
    String correlationId = headers != null ? headers.get("correlationId").getAsString() : null;
    ...
    confirmHeaders.addProperty("correlationId", correlationId);
