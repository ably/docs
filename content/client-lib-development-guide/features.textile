...
** @(RSC15f)@ Once/if a given fallback host succeeds, the client should store that successful fallback host for @ClientOptions.fallbackRetryTimeout@. Future HTTP requests during that period should use that host first, except in the case of a connected realtime client, where the connected host should be used per @RTN17e@. If during this period a "qualifying errors":#RSC15d occurs on that host, or  @fallbackRetryTimeout@ has expired, it should be un-stored, and the fallback sequence begun again from scratch, starting with the default primary host (@rest.ably.io@ or @ClientOptions#restHost@), or in the case of @RTN17e@, the connect host again.
