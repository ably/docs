...
Pass in the name of the room to use. If you need to provide further feature configuration, such as enabling occupancy messages, you can pass in an optional [`RoomOptions`](/docs/chat/api/javascript/rooms#get-params) object to the provider.
...
<If lang="react">
By default the `ChatRoomProvider` will automatically call [`release()`](/docs/chat/api/javascript/rooms#release) on the room when it unmounts. Set the [`release`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/interfaces/chat-react.ChatRoomProviderProps.html#release) property to `false` to change this behavior and have the room only [detach](#detach) when the component unmounts. You can manually control this attachment behavior using the [`useRoom`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useRoom.html) hook.
</If>
...
Use the <If lang="javascript">[`attach()`](/docs/chat/api/javascript/room#attach)</If><If lang="swift">[`attach()`](https://sdk.ably.com/builds/ably/ably-chat-swift/main/AblyChat/documentation/ablychat/room/attach%28%29)</If><If lang="kotlin,android">[`attach()`](https://sdk.ably.com/builds/ably/ably-chat-kotlin/main/dokka/chat/com.ably.chat/-room/attach.html)</If> method on a room to attach to it:
</If>

<If lang="react">
The first `ChatRoomProvider` for a given room will automatically call [`attach()`](/docs/chat/api/javascript/room#attach) on the room when it first mounts. Equally, the last `ChatRoomProvider` for a given room will automatically call [`detach()`](/docs/chat/api/javascript/room#detach) on the room when it unmounts. You can also call `attach()` and `detach()` manually using the [`useRoom`](https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useRoom.html) hook.
</If>
...
Rooms maintain their state during temporary disruptions, with all messages received in correct order and [zero message loss](/docs/platform/architecture/connection-recovery).
</Aside>
