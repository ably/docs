...
```swift
// Send a 👍 reaction using the default type
await room.messages.reactions.send(messageSerial: message.serial, params: .init(name: "👍"))

// The reaction can be anything, not just UTF-8 emojis:
await room.messages.reactions.send(messageSerial: message.serial, params: .init(name: ":like:"))
await room.messages.reactions.send(messageSerial: message.serial, params: .init(name: "+1"))

// Send a :love: reaction using the Unique type
await room.messages.reactions.send(messageSerial: message.serial, params: .init(
  reaction: ":love:",
  type: .unique
))
