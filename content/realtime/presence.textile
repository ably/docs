...
A client must be attached to a channel, and must have the relevant permission, in order to enter. Although, the library implicitly calls @attach()@ if the client is not attached at the time @enter()@ is called.
