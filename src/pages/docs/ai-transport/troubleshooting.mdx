...
- Enable the *Message annotations, updates, deletes, and appends* rule on the namespace, using the [dashboard, Control API, or CLI](/docs/ai-transport/getting-started/channel-rules).
...
- Grant every operation the application needs: `subscribe` and `publish` for messages, `history` for loading past conversation, `presence` for agent presence. Check the full list of [capability operations](/docs/auth/capabilities#capability-operations) against the [AI Transport capability shape](/docs/ai-transport/getting-started/authentication).
...
- Enable persistence on the channel namespace and set a [retention period](/docs/ai-transport/features/history#faq-retention) that covers your expected scroll-back window.
- For conversations that need to be retained for longer than the channel allows, persist completed turns to your own store and [hydrate from it when the client reconnects](/docs/ai-transport/features/reconnection-and-recovery#loading-history).
...
- Check your lifecycle handling against the [run lifecycle contract](/docs/ai-transport/concepts/runs#lifecycle).
...
- Trace the signal end to end against the [cancellation flow](/docs/ai-transport/features/cancellation#abort-signal).
...
- Before editing or regenerating, cancel the in-flight run explicitly, following the [recommended edit pattern](/docs/ai-transport/features/branching#edit). AI Transport does not auto-cancel an active run on edit.
...
- Check your ownership assumptions against the [multi-device session model](/docs/ai-transport/features/multi-device).
...
- Confirm how [branch selection works](/docs/ai-transport/features/branching#navigate) before you design the sync.
...
- Decide on the application response: surface a retry control to the user, or auto-retry from the application layer. AI Transport [does not retry the LLM call automatically](/docs/ai-transport/features/reconnection-and-recovery#faq-agent-crash).
...
## When to contact support <a id="escalate"/>
