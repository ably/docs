...
Ably automatically recovers connections and prevents message loss or duplication when clients reconnect within two minutes. [Learn how Ably ensures exactly-once delivery during recovery](/docs/platform/architecture/idempotency#connection-recovery-and-exactly-once-delivery).
</Aside>
...
Ably's SDKs automatically resolve edge network failures within 30 seconds, keeping your users connected even during infrastructure issues. [Learn how Ably's edge network maintains connectivity](/docs/platform/architecture/edge-network).
</Aside>
