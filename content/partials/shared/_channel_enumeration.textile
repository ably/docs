...
- by := _value_ optionally specifies whether to return just channel names (@by=id@) or "ChannelDetails":/api/realtime-sdk/channel-metadata#channel-details (The default has changed: it used to be @by=value@, but for for protocol v3 or newer (the @version@ parameter passed to @rest#request@ is ≥ 3), it is @by=id@. **We strongly recommended to use @by=id@ unless you really need the occupancy for every channel**. For one thing it will be much faster; for another, making very regular @by=value@ enumeration queries can prevent channels from closing down from inactivity.)
...
* If you use @by=value@ (which until recent protocol versions was the default), just enumerating channels can briefly keep them alive, meaning if you do very regular enumeration you can get a situation where channels never close.
