...
** @(RSA17a)@ Revokes tokens based on provided function arguments. It internally creates request for  "REST revokeTokens endpoint":https://ably.com/docs/api/rest-api#revoke-tokens and handles the response.
...
*** @(RSA17b3)@ @allowReauthMargin@ is an optional boolean that permits a token renewal cycle to take place without needing established connections to be dropped. Default for this value is false if not provided.
** @(RSA17c)@ key to build the request must be retrieved from stored @Auth#AuthOptions#key@
** @(RSA17d)@ If the @targets@ parameter is null or empty, then the function must throw an error with a @"targets cannot be null or empty"@ message.
