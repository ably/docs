...
<If lang="java">
The Java SDK additionally offers `exists()` (a best-effort presence check) and `getType()` (returns a `ValueType`, or `null` when nothing resolves at the path). Use them to check what is at a path before reading it through an inferred type.
</If>
...
- A *write* returns a `CompletableFuture` that completes exceptionally with an `AblyException`: code `92007` when the resolved object's type doesn't match the inferred type, or `92005` when the path cannot be resolved to any object at the time the method is called.
- When blocking with `.join()`, the failure surfaces as a `CompletionException` whose cause is the `AblyException`.

See the [Type inference documentation](/docs/liveobjects/typing?lang=java#type-inference) for the full cast contract.
</Aside>
<Aside data-type='note'>
