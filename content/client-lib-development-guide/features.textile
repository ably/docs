...
** @(RTL4h)@ It can emit an @UPDATE@ event (this is the only @ConnectionEvent@ which is not a @ConnectionState@). This is used for changes to connection conditions for which the @ConnectionState@ (e.g. @CONNECTED@) does not change. (The library must never emit a @ConnectionState@ event for a state equal to the previous state).
...
** @(RTE6b)@ Every listener that is registered at the time @emit@ is invoked should be called exactly once. A test should assert that any listener registered and/or removed within an already existing @on@ listener does not result in the newly added listeners being called or the newly removed listeners not being called when an event is emitted, see "https://goo.gl/OVTtjO":https://goo.gl/OVTtjO
...
class AuthOptions: // RSA8e
  authCallback: (() -> io (String | TokenDetails | TokenRequest | JsonObject))? // RSC14c, RSA4, TO3j5, AO2b
