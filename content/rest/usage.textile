...
bc[java]. import io.ably.lib.rest.AblyRest;
...
- <div lang="jsall">callback</div> := is a function of the form: @callback(err, stats, relParams)@
where:
<ul>
...
<div lang="java">
The Ably constructor's @Options@ argument is an instance of @io.ably.lib.types.Options@, constructed either with the default no-argument constructor or with a key string. The following options may be specified.
</div>
...
- <div lang="jsall,ruby">log</div> := Parameters to control the log output of the library. The supplied value must be an object that may contain one or both of the following entries:
