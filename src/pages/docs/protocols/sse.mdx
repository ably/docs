...
Ably's DNS uses a 60-second TTL, allowing traffic to be rerouted away from unhealthy datacenters within minutes when issues are detected. [Learn how Ably's edge network handles routing and failover](/docs/platform/architecture/edge-network#dns-organization-and-latency-based-routing).
</Aside>
