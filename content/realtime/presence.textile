...
See the "presence history documentation":/realtime/history#presence for further details of the supported query parameters.
...
bq(definition).
  default: get(Object query, callback("ErrorInfo":/realtime/types#error-info err, "PresenceMessage[]":#presence-message members))
  ruby:    "Deferrable":/realtime/types#deferrable get(Hash query) -> yields "PresenceMessage[]":#presence-message
  java:    public "PresenceMessage[]":#presence-message get(Param[] query)

Gets an array of members present on the channel as "PresenceMessage":#presence-message objects.

h4. Parameters

- <span lang="default">query</span><span lang="java">Param[] query</span><span lang="ruby">query</span> := <span lang="default">an optional object containing query parameters as specified below.</span><span lang="ruby">an optional set of symbol key and value pairs containing the query parameters as specified below.</span>
...
bq(definition).
  default: history(Object query, callback("ErrorInfo":/realtime/types#error-info err, "PaginatedResult":#paginated-result<"PresenceMessage":#presence-message> resultPage))
  ruby:    "Deferrable":/realtime/types#deferrable history(Hash query) -> yields "PaginatedResult":#paginated-result<"PresenceMessage":#presence-message>
  java:    public "PaginatedResult":#paginated-result<"PresenceMessage":#presence-message> history(Param[] query)

Gets a paginated set of historical presence message events for this channel. If the "channel is configured to persist messages to disk":https://support.ably.io/support/solutions/articles/3000030059-how-long-are-messages-stored-for, then the presence message event history will "typically go back 24 - 72 hours":https://support.ably.io/solution/articles/3000030059-how-long-are-messages-stored-for. If not, presence message events are only retained in memory by the Ably service for two minutes.

h4. Parameters

- <span lang="default">query</span><span lang="java">Param[] query</span> := <span lang="default">an optional object containing the query parameters</span><span lang="ruby">an optional set of key value pairs containing the query parameters</span>, as specified in the "presence history API documentation":/realtime/history#presence.
...
Further details of the supported @query@ params, see "presence history API documentation":/realtime/history#presence-query-params.
