...
When your integration receives messages, you have access to rich data that enables various processing patterns.
...
### Enabling features with headers

Unlike metadata, headers are typically set server-side or by trusted clients and can be used for routing and processing control:

**Command routing:** Use headers like `x-command: 'remind'` to route messages to specific processors without parsing message text, enabling clean separation of command handling from regular chat messages.

**Processing flags:** Headers can indicate whether a message should trigger specific processing (e.g., `x-translate: 'true'`, `x-moderate: 'true'`) based on room settings or user preferences.

**Priority handling:** Mark messages with priority levels (e.g., `x-priority: 'high'`) to ensure time-sensitive messages are processed first in queue-based integrations.
