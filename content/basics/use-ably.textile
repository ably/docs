...
"Integrations":/docs/general/integrations enable data to be sent between Ably channels and external services. "Inbound integrations":#inbound are where data is sent from an external service into Ably. "Outbound integrations":#outbound are where data is sent from Ably to an external service.
...
Outbound integrations enable you to send data from Ably channels to an external streaming or queuing service. Examples include "Amazon Kinesis":/docs/general/firehose/kinesis-rule or "Amazon SQS":/docs/general/firehose/sqs-rule.
