...
Retrieve historical messages sent before subscribing using `historyBeforeSubscribe()`. Messages are returned in order, from the most recent to the oldest. This is useful for providing conversational context when a user joins or rejoins a room, ensuring continuous message history without overlap.
...
<Aside>
The `orderBy` parameter is not available for `historyBeforeSubscribe()` and messages are always returned from the most recent to the oldest.
</Aside>
