...
<If lang="java">
`on(...)` returns a `Subscription`; use `channel.object.off(listener)` or `channel.object.offAll()` to deregister listeners. (`ObjectStateEvent` lives in `io.ably.lib.liveobjects.state`.)
</If>
