...
```text
Client publishes input event
  name: ai-input
  extras.ai.transport: { event-id=E1, run-id=R1, codec-message-id=M1, role=user, parent=M0 }
  extras.ai.codec:     { stream=false }

Agent publishes run-start (invocation-id minted by createRun)
  name: ai-run-start
...
Agent appends text deltas to M2
  name: ai-output  (channel.appendMessage)
  extras.ai.codec:     { stream-id=S1, status=streaming }

Agent closes the stream
  name: ai-output  (channel.updateMessage)
...
A cancel inserts an `ai-cancel` message targeting the `run-id`. The agent's per-Run `AbortSignal` fires, the stream closes with `status=cancelled`, and the agent publishes `ai-run-end` with `run-reason=cancelled`.
...
| Header | Description |
| --- | --- |
| `run-id` | Run correlation. Set on every message in a Run. |
...
| `run-continue` | Literal `'true'` stamped by the client on a continuation `ai-input` (for example, a tool-result follow-up under the same `run-id`). The agent reads it to decide whether to publish `ai-run-start` (first start) or `ai-run-resume` (continuation). |
...
Every header constant is exported from `@ably/ai-transport` as `HEADER_*`:
...
| `HEADER_INPUT_CODEC_MESSAGE_ID` | `'input-codec-message-id'`. Stamped on agent outputs to echo the triggering input's `codec-message-id`, so the client can correlate by an id it owned at send time. |
...
| `HEADER_RUN_CONTINUE` | `'run-continue'` |
...
Every AI Transport message rides one of five Ably message names. The codec event's own `type` is carried in the `codec-type` domain header so the decoder can dispatch.
...
| `ai-run-resume` | Agent | Run lifecycle. The agent publishes this when a continuation Invocation re-activates a Run (tool-result follow-up, suspended-Run resume), instead of a second `ai-run-start`. The client signals continuation by stamping `run-continue: 'true'` on the input event. |
...
3. `channel.updateMessage(...)` to close the stream with `status: 'complete'` (or `'cancelled'`).
...
- `runId`: fresh per send, or reused via `SendOptions.runId` for a continuation (tool result, regenerate).
- `inputEventId`: one per published input event. The last event in the send becomes the trigger.

Both are written to the channel under `extras.ai.transport`. The SDK does not POST to the agent endpoint itself; the developer (or the bundled [Vercel `ChatTransport`](/docs/ai-transport/api/javascript/vercel/chat-transport)) calls `activeRun.toInvocation().toJSON()` to obtain an [`InvocationData`](/docs/ai-transport/concepts/invocations) body (`{ runId, inputEventId, sessionName }`) and POSTs it to the agent.

On the agent side, `session.createRun(invocation)` mints the `invocationId` (one per HTTP request), exposes it as `Run.invocationId`, and stamps it on every event the agent publishes for this invocation. `Run.start()` attaches the channel with rewind and waits for the trigger `event-id` carried in the invocation body to arrive (live or via rewind). The rewind window is configured by `rewindWindow` on `AgentSessionOptions`; the lookup timeout is `inputEventLookupTimeoutMs`. If the lookup lapses, `Run.start()` rejects with [`InputEventNotFound`](/docs/ai-transport/api/errors).

The agent's outputs carry `input-codec-message-id` (the `codec-message-id` of the triggering input) under `extras.ai.transport`. The client uses this to correlate outputs back to the input it owned at send time, so `ActiveRun.started` resolves. The application returns `run.invocationId` on the HTTP response so the caller can observe the agent-minted id directly.
