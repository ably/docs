...
- <span lang="default">force</span><span lang="ruby">:force</span> := _false_ When true, this indicates to "@authorise@":/realtime/authentication#authorise that a new token should be requested, even if the current token is still valid<br>__Type: @Boolean@__

- <span lang="default">key</span><span lang="ruby">:key</span> := Optionally the "API key":https://support.ably.io/solution/articles/3000030054-what-is-an-app-api-key to use can be specified as a full key string; if not, the API key passed into "@ClientOptions@":#client-options when instancing the Realtime or REST library is used<br>__Type: @String@__
...
- <span lang="default">useTokenAuth</span><span lang="ruby">:use_token_auth</span> := _false_ When true, forces "Token authentication":/general/authentication#token-authentication to be used by the libary<br>__Type: @Boolean@__
