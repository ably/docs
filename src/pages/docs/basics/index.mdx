...
Presence powers the [Spaces SDK](/docs/spaces/avatar) for avatar stacks and the [Chat SDK](/docs/chat/rooms/presence) for chat room member notifications. Use presence directly for custom use cases. See [product guidance](/docs/platform/products) for help choosing the right product.
