...
func requestHumanApproval(_ toolCall: ToolCall) async throws {
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe("approval-request", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        ...
        guard let request = message.data else { return }
        let extras = (try? message.extras?.toJSON()) as? [String: Any]
        guard let toolCallID = (extras?["headers"] as? [String: Any])?["toolCallId"] as? String else { return }
        // Display request for human review
        displayApprovalUI(request, toolCallID)
    })
}

func approve(_ toolCallID: String) async throws {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
        channel.publish(
            "approval-response",
            data: [
...
func reject(_ toolCallID: String) async throws {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
        channel.publish(
            "approval-response",
            data: [
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    channel.subscribe("approval-response", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        ...
        MainActor.assumeIsolated {
            guard let response = message.data as? [String: Any],
                  let extras = (try? message.extras?.toJSON()) as? [String: Any],
                  let toolCallID = (extras["headers"] as? [String: Any])?["toolCallId"] as? String,
                  let pending = pendingApprovals[toolCallID] else { return }
