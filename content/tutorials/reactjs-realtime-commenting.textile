...
Test your new React app by changing into the @reactjs-realtime-commenting@ directory and executing @npm start@:
...
Mark the @addComment()@ method as @async@. Make a HTTP request to the Dog API to retrieve the URL of a random dog picture and store the URL in an object property called @avatar@:
...
You'll use Ably's "history feature":/realtime/history to persist all comment messages to disk for later retrieval by clients. This capability must be configured on channels using "channel rules":https://knowledge.ably.com/what-are-channel-rules-and-how-can-i-use-them-in-my-app. In this tutorial you will create a channel rule for all channels in the @persisted@ "namespace":https://knowledge.ably.com/what-is-a-channel-namespace-and-how-can-i-use-them.

# Visit your "account dashboard":https://knowledge.ably.com/how-do-i-access-my-account-dashboard and select the same app you chose in Step 1 when obtaining your API key
