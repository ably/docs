...
```[javascript]
  ...
  channel.subscribe("update", message => {
    data = JSON.stringify(message, undefined, 2);
    console.log(data);
  });
```

An example of the format of the data received is shown here:
...
```[sh]
curl -s -u "{{API_KEY}}" "https://realtime.ably.io/sse?channel=[meta]stats:minute&v=1.1"
```
