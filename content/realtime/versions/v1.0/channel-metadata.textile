...
- channel.occupancy := indicates a change in global "occupancy":#occupancy. Not all "occupancy":#occupancy-events changes are sent; there is a minimum interval on a per-channel basis, so if the "occupancy":#occupancy-events is changing continuously, then only periodic snapshots of occupancy are sent. Further roll-up of events may occur depending on the capacity of the lifecycle channel. Any "occupancy":#occupancy-events changes in which any of the "occupancy":#occupancy-events categories changes from 0 to a non-zero count, or vice versa, are always included

It is worth noting that @channel.occupancy@ events are currently only available to our "enterprise":https://ably.com/pricing#enterprise customers. However, you can still access "occupancy":#occupancy-events data within channel lifecycle events on a free account.
...
You can associate "Integration rules":https://ably.com/integrations with metachannels in the same way as for regular channels; configure these using the "Message":/realtime/messages source and use a @channelFilter@ that matches the metachannel's name. Please note: the *Channel Lifecycle* events are *not* supported with Webhooks.
