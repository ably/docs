...
Using the "Presence API":/presence-occupancy/presence is straightforward. Follow the steps in this tutorial to learn how.
...
blang[ruby].
  ...
  The Ruby realtime library uses "EventMachine":https://github.com/eventmachine/eventmachine to run the client library within an asynchronous event loop. However, typically when using Ruby on servers, most developers use the synchronous "REST client library API":/basics/use-ably. As this tutorial needs realtime access to be present and subscribe to presence messages, it uses the Ruby realtime library.
...
2. If you would like to find out more about how presence works, see the "Realtime presence documentation":/presence-occupancy/presence
