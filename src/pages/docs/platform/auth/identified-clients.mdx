...
<Code>
```javascript
// Server-side: Create JWT with clientId using any JWT library
    ...
    ably_jwt = jwt.encode(
        {
            'iat': now,
            'exp': now + 3600,
            'x-ably-capability': json.dumps({'*': ['publish', 'subscribe', 'presence']}),
            'x-ably-clientId': user_id,  # Set the trusted clientId
        },
        key_secret,
        algorithm='HS256',
...
**Client-side:** Use `authCallback` to fetch the JWT:
...
// The clientId is now set from the JWT - no need to specify it here
```
</Code>
