...
** @(RSA4d)@ If a request by a realtime client to an @authUrl@ results in an HTTP 403 response, then the client library should transition to the @FAILED@ state, with the connection @errorReason@ being set to an @ErrorInfo@ with @code@ @40300@ and some appropriate message.
** @(RSA4e)@ If an @authCallback@ attempt or @authUrl@ request by a realtime client results in an Ably @ErrorInfo@, the client library should transition to the @FAILED@ state, with the connection @errorReason@ being set to the error. (This applies only where the response received over the wire is actually an @ErrorInfo@; any errors generated by the client, such as network errors, must be handled per @RSA4c@).
...
* @(RTL14)@ If an @ERROR ProtocolMessage@ is received for this channel (the channel attribute matches this channel's name), then the channel should immediately transition to the FAILED state, the @errorReason@ should be set and an error should be emitted on the channel.
...
** @(TO3g)@ @queueMessages@ boolean - defaults to true. If false, suppresses the default queuing of messages when connection states that anticipate imminent connection (connecting and disconnected). Instead, publish and presence state changes will fail immediately if not in the connected state
...
class Message:
  ...
  +fromEncoded(JsonObject, ChannelOptions?) // TM3
  +fromEncodedArray(JsonArray, ChannelOptions?) // TM3
...
class PresenceMessage
  +fromEncoded(JsonObject, ChannelOptions?) // TP4
  +fromEncodedArray(JsonArray, ChannelOptions?) // TP4
