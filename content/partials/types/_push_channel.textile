...
Lists push subscriptions on a specified channel name (@channel@) for @clientId@, @deviceId@ or both if @concatFilters@ is set to @true@. This method requires clients to have the "Push Admin capability":/general/push/admin. For more information, take a look at the @GET rest.ably.io/push/channelSubscriptions@ "Rest API":/rest-api.
