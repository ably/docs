...
A protocol adapter inevitably adds some latency. Using the adapter will be a little slower than using Ably client libraries. 
Typically the impact is in the low milliseconds.
...
h2.(#rewind). Channel rewind

You can also use "channel rewind":/storage-history/history#rewind to get the get the most recent message if available:
