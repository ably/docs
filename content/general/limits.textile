...
You can view the limits for your package in the "account dashboard":https://ably.com/accounts/any/package_limits. 

h2(#overview). Overview

Limits are either "time-based":#time or "quantity-based":#quantity. The effect of hitting or exceeding a limit varies depending on its type. 
...
Time-based limits relate to the number of operations that can be made within a month, an hour, or per second. 

For monthly, hourly and local per second time-based limits the resource is blocked for the remainder of that period when a hard limit is hit. For example, if the hourly limit for the number of API requests is hit, no more API requests can be made for the remainder of that hour. Any API requests made in excess of the hard limit will be rejected with an error code.

Time-based limits reset at the following intervals:

* Monthly limits reset the following month. 
* Hourly limits are based on a clock hour, so regardless of whether a limit is hit at 11:01 or 11:59, it will reset at 12:00. 
* Local per second limits are calculated each second and reset the next second. Additional usage each second will be rejected with an error code.

h4(#global-time). Global per second limits

Global per second limits apply to the number of operations that can be made per second just like local per second limits. The difference is that the number of operations for global per second limits are an aggregate of usage across all applications in your account. An example of a global per second limit is the number of messages being published and received per second across all of your applications. 

Global per second limits apply rate suppression to operations that exceed their hard limits. Rate suppression is calculated on a rolling probabilistic basis. For example, the limit on a self-service package for publishing messages into a queue is 100 messages per second. If a queue rule is attempting to publish 200 messages per second into the queue, each message will have a 50% chance of being rejected. The suppression probability is continuously updated based on the publishing rate.

Global per second limits only have a hard limit. Once the hard limit rate has been exceeded then message suppression will occur. As soon as the rate drops below the hard limit threshold, the suppression probability will decrease to zero.

h3(#quantity). Quantity-based limits

Quantity-based limits specify a set amount of resources or operations that can be in use simultaneously. Quantity-based limits are removed when a resource or operation is reduced below the hard limit, or the quota for the limit is increased.

An example of a quantity-based limit is the one for peak connections. When the hard limit is hit then no more connections can be made to Ably until some of the existing ones disconnect and the total is back below the hard limit.
...
If the per second message rate is exceeded, a "global per second limit":#global-time will be applied to message delivery and publishing. The hourly and total message limits are "time-based":#time. If the hard limit is hit for those, message delivery and publishing will be blocked until the following hour or month.
...
If the per second bandwidth rate is exceeded, a "global per second limit":#global-time will be applied to message delivery and publishing. The hourly and total bandwidth limits are "time-based":#time. If the hard limit is hit for those, message delivery and publishing will be blocked until the following hour or month.
...
If the per second token request rate is exceeded, a "global per second limit":#global-time will be applied to token requests. The hourly token request limit is a "time-based limit":#time, meaning no additional token requests will be accepted until the following hour if the hard limit is hit.
...
If the connection rate is exceeded, a "global per second limit":#global-time will be applied to new connection attempts.
...
If the outbound message rate is exceeded for a connection, a "local per second limit":#time will be applied to that connection.
...
If the inbound message rate is exceeded for a connection, a "local per second limit":#time will be applied to that connection.
...
If the channel creation rate is exceeded, a "global per second limit":#global-time will be applied to channel creation.
...
If the message publish rate is exceeded for a channel, a "local per second limit":#time on publishing will be applied to that channel.
...
If the queue publish rate is exceeded, a "global per second limit":#global-time on publishing to queues is applied.
...
If the Firehose message rate is exceeded, a "global per second limit":#global-time on publishing to Firehose is applied.
...
If the function invocation rate is exceeded, a "global per second limit":#global-time on function invocations is applied.
...
If the per second API request rate is exceeded, a "global per second limit":#global-time will be applied to API requests. The hourly API request limit is a "time-based limit":#time, meaning no additional API requests will be accepted until the following hour if the hard limit is hit.
