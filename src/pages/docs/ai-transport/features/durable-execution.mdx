...
Each activity constructs its own `Ably.Realtime` client and its own `AgentSession`. Sharing sessions across activities is not supported. Sharing a Realtime client across activities is supported provided no activity closes a channel that another Run still uses.
...
- An empty pipe on retry publishes no output. The previous attempt's output stays in the conversation. This is by design: an aborted retry does not clear the successful bracket that came before it, only a completed retry supersedes.
