...
An @intervalId@ is a string that identifies an interval. It is a formatted date/time string representing the interval start time in UTC. @intervalIds@ have a format according to their duration, ie:
...
- count := (double) the number of messages;
- data := (double) the aggregate of the size of the payload and other user-specified members of those messages.

h5. MessageTypes

A MessageTypes entry provides a breakdown of MessageCounts by message type.

- all := (MessageCount) the total volume of messages and presence messages;
- messages := (MessageCount) the volume of normal channel messages;
- presence := (MessageCount) the volume of presence messages.

h5. MessageTraffic

A MessageTraffic entry provides a breakdown of message volume, as MessageType elements, by transport type.

- all := (MessageTypes) the total of all transport types;
- realtime := (MessageTypes) the message volume via realtime connections;
- rest := (MessageTypes) the message volume via the REST API;
- push := (MessageTypes) the message volume via WebHooks;
- httpStream := (MessageTypes) the message volume via HTTP streaming (currently unsupported);

h4. Connection and other resource statistics

h5. ResourceCount

A ResourceCount element provides a breakdown of usage of a particular resource.

- opened := (double) the number of resources created or allocated;
- peak := (double) the maximum number of resources concurrently in use;
- mean := (double) the time-averaged mean of the number of resources concurrently in use;
- min := (double) the minimum number of resources concurrently in use;
- refused := (double) the number of resource requests that were declined.

h5. ConnectionTypes

A ConnectionTypes element provides a breakdown of connection numbers as ResourceCount elements.

- all := the aggregate of plain and TLS connections;
- plain := the usage of non-TLS connections;
- tls := the usage of TLS (secure WebSocket or HTTPS) connections.

h5. RequestCount

A RequestCount element contains a breakdown of API and other requests.

- succeeded := (double) the number of successful requests;
- failed := (double) the number of requests that were attempted but failed for a reason other that an access denial, such as invalid request parameters;
- refused := the number of requests that were attempted but were denied, either because the caller did not have permission or because an account limit was reached.

h5. Stats

This is a stats element that makes up stats responses providing an overview of all resource usage.

- all :=  (MessageTypes) the total of all inbound and outbound message traffic. This is the aggregate number that is considered in applying account message limits.
- inbound := (MessageTraffic) inbound messages (ie published by clients and sent inbound to the Ably service) by all transport types;
- outbound := (MessageTraffic) outbound messages (ie delivered by the Ably service to connected and subscribed clients);
- persisted := (MessageTypes) the aggregate volume of messages persisted;
- connections := (ConnectionTypes) the usage of connections;
- channels := (ResourceCount) the number of channels activated and used;
- apiRequests := (RequestCount) the number of requests made to the REST API;
- tokenRequests := (RequestCount) the number of requests made to issue access tokens.
