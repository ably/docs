...
<Aside data-type='note'>
Using an API key directly is safe for server-side Ruby applications. If you're building a client-side application, use [token authentication](/docs/platform/auth/token) instead to keep your API key secure.
</Aside>
