...
** @(RTN15j)@ If a recovery attempt fails for a transient reason, the retry process described in "RTN14d":#RTN14d applies, with the addition that once the connection state has been in the @DISCONNECTED@ state for more than the "default @connectionStateTtl@":#defaults, in addition to the effects described in "RTN15g":#RTN15g, the state will change to @SUSPENDED@ and be emitted with the @reason@, and the @Connection#errorReason@ will be updated. In this state, a new connection attempt will be made periodically as specified within @suspendedRetryTimeout@ of @ClientOptions@.
