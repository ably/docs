...
func example(
    ...
    processAndRespond: @escaping @Sendable (ARTRealtimeChannel, String, String, String) -> Void
) async throws {
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.subscribe("user-input", onAttach: { error in
        if let error {
...
func example(
    channel: ARTRealtimeChannel,
    processAndRespond: @escaping @Sendable (ARTRealtimeChannel, String, String) -> Void
) async throws {
    // insert example code here
}
*/}
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.subscribe("user-input", onAttach: { error in
        ...
        guard let extras = try? message.extras?.toJSON() as? [String: Any],
              let role = extras["userClaim"] as? String else { return }
        // promptId is a user-generated UUID for correlating responses
        guard let data = message.data as? [String: Any],
              let promptID = data["promptId"] as? String,
              let text = data["text"] as? String else { return }
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.publish("user-input", data: ["promptId": promptID, "text": "What is the weather like today?"] as [String: String]) { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
func example(
    channel: ARTRealtimeChannel,
    generateAIResponse: @escaping @Sendable (String) async -> String
) {
    // insert example code here
}
*/}
...
func processAndRespond(channel: ARTRealtimeChannel, prompt: String, promptID: String) async throws {
    ...
    message.extras = ["headers": ["promptId": promptID]] as ARTJsonCompatible
...
func sendPrompt(text: String) async throws -> String {
    ...
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
        channel.publish("user-input", data: ["promptId": promptID, "text": text] as [String: String]) { error in
            if let error {
                continuation.resume(throwing: error)
            } else {
                continuation.resume()
...
// Handle responses
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.subscribe("agent-response", onAttach: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
        ...
        MainActor.assumeIsolated {
            guard let extras = try? message.extras?.toJSON() as? [String: Any],
                  ...
                  let promptID = headers["promptId"] as? String else { return }

            if let originalText = pendingPrompts[promptID] {
                print("Response for \"\(originalText)\": \(message.data ?? "")")
...
func example(
    channel: ARTRealtimeChannel,
    generateTokens: @escaping @Sendable (String) -> any AsyncSequence<String, Never> & Sendable
) {
    // insert example code here
}
*/}
...
func streamResponse(channel: ARTRealtimeChannel, prompt: String, promptID: String) async throws {
    ...
    initialMessage.extras = ["headers": ["promptId": promptID]] as ARTJsonCompatible
    ...
    guard let messageSerial = publishResult.serials.first?.value else {
    ...
    for await token in generateTokens(prompt) {
        ...
        appendMessage.serial = messageSerial
        appendMessage.data = token
        appendMessage.extras = ["headers": ["promptId": promptID]] as ARTJsonCompatible
...
func example(
    channel: ARTRealtimeChannel,
    streamResponse: @escaping @Sendable (ARTRealtimeChannel, String, String) async throws -> Void
) async throws {
    // insert example code here
}
*/}
