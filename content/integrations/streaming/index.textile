...
Note that if you want to consume realtime data from a queue, you should take a look at "Ably Queues":/integrations/queues. They provide a simple and robust way to consume realtime data from your worker servers without having to worry about queueing infrastructure.
...
* "Channel lifecycle":/channels/states#attach – get notified when a channel is created (following the first client attaching to this channel) or discarded (when there are no more clients attached to the channel).
...
* "Amazon Kinesis":/integrations/streaming/kinesis
* "Amazon SQS":/integrations/streaming/sqs
* "AMQP":/integrations/streaming/amqp
* "Apache Kafka":/integrations/streaming/kafka
