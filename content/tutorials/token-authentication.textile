...
Ably supports two types of authentication schemes, "basic authentication":core-features/authentication#basic-authentication using a private API key, which should not be used on user devices, and "token authentication":/core-features/authentication#token-authentication which can be used to keep your private API key secure on your server. Your server (or serverless function) can generate short-lived tokens and pass them to users to enable communication with Ably. These tokens can be limited to specific capabilities on specific channels. Make sure to follow the principle of least privilege, giving clients only the capabilities they need.  Clients should be configured with an @authCallback@ or @authUrl, which the Ably client SDK will use to generate and renew a token whenever needed: when a token doesn't currently exist or when it expires. Don't instantiate an Ably Client with a specific token because it will expire.
...
blang[android].
  ...
  Then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @server.js@ and running the demo @node server.js@
...
blang[java].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @Example.java@ and running the demo:
...
blang[nodejs].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @server.js@ and running the demo @node server.js@
...
blang[ruby].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @server.rb@ and running the demo @bundle exec ruby server.rb@
...
blang[python].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @server.py@, install the required libraries:
...
blang[php].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @ably.php@, install composer dependencies with:
...
6. Learn more about other "Ably features":https://ably.com/features by stepping through our other "Ably tutorials":https://ably.com/tutorials
