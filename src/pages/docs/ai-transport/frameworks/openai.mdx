...
The codec also carries the client-driven half of tool calling: a client executes a tool in the browser and publishes the result, reports a tool failure, or answers a human approval prompt. The agent calls `run.suspend()` to wait for a client, and a continuation resumes the run once the client has answered.
