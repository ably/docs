...
- **Financial market data:** Distributing realtime price updates for stocks, cryptocurrencies, and other instruments to trading platforms and analytics dashboards.
...
- Consume stats live via the [`[meta]stats:minute`](/docs/metadata-stats/stats#app-meta) channel to track delta performance in realtime.
...
<Aside data-type='important'>
  Conflation is unsuitable for scenarios requiring every message, such as [chat applications](/docs/chat) where losing intermediate messages would impact the user experience.
</Aside>
...
- [Chat applications](/docs/chat) with high-activity chat rooms that need to smooth traffic spikes while preserving all messages.
...
**Scenario:**
- 10,000 users in a [chat](/docs/chat) room
...
- Social or [chat applications](/docs/chat) could apply server-side batching with ~100ms interval on `rooms:*` channels.
