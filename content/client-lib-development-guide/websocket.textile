...
Establishment of a "WebSocket":https://www.ably.io/topic/websockets transport requires that the client makes a "WebSocket":https://www.ably.io/topic/websockets or secure "WebSocket":https://www.ably.io/topic/websockets connection to the Ably "WebSocket":https://www.ably.io/topic/websockets host (realtime.ably.io or one of the "WebSocket fallback hosts":/client-lib-development-guide/connection-fallback). The client library must allow the client to specify a non-default host or environment, at least to be able to connect to the sandbox environment at sandbox-realtime.ably.io.

Certain apps may be configured to require TLS connections and will reject non-TLS connections.

The connection URI must be a valid "WebSocket":https://www.ably.io/topic/websockets URI as specified in http://tools.ietf.org/html/rfc6455#section-3 with an empty path component. The URI must include a valid set of authorization params and a valid set of connection params, each as specified below.
...
Alternatively, the service may accept the "WebSocket":https://www.ably.io/topic/websockets connection and complete the handshake, and then send an @ERROR@ "Protocol Message":/client-lib-development-guide/protocol with further error information, followed by a normal close of the "WebSocket":https://www.ably.io/topic/websockets connection. In this case the library must report that error information as the reason for connection failure to the client.
...
Disconnection of the transport, in the case that the client wishes to be able to recover connection state, simply requires the client to perform a normal close on the "WebSocket":https://www.ably.io/topic/websockets.

Closing the connection, resulting in disposal of connection state, is achieved by sending a @CLOSE@ message. The client should then wait a short period for a @CLOSED@ message, however the client can also simply perform a normal close on the "WebSocket":https://www.ably.io/topic/websockets.

Abnormal termination for whatever reason (a transition to the failed connection state) again just requires a normal close of the "WebSocket":https://www.ably.io/topic/websockets.

"WebSocket":https://www.ably.io/topic/websockets close events that are initiated by the remote end are handled as follows.
...
is reported as a transition to the failed state. The reason code is derived from the reported "WebSocket":https://www.ably.io/topic/websockets reason code as follows:
