...
Nodes in the core layer are responsible for all channel message processing and persistence and apply operations such as [delta](/docs/channels/options/deltas) computation, [batching](/docs/messages/batch) and [integration rule](/docs/integrations) invocation. They also aggregate and persist account and app [statistics](/docs/metadata-stats/stats) and enforce [limits](/docs/platform/pricing/limits). Nodes in the core layer communicate cross-regionally to facilitate inter-data-center message transit.
