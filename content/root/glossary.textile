...
See the "batch request documentation":/rest-api/beta#batch for further information.
...
Delta compression is a feature that enables a subscriber to only receive the difference between the payload of a message and its predecessor. It is used when there is a degree of similarity between successive messages.
...
Reactor Firehose is a feature that enables the realtime data published in Ably to be streamed directly into another streaming or queuing service such as "Amazon Kinesis":https://aws.amazon.com/kinesis/.
...
See the "introduction to channels documentation":/core-features/channels for further information.
...
See the "incoming webhook documentation":/general/incoming-webhooks and "outbound webhook documentation":/general/events for further information.

h2(#websocket). WebSocket

WebSocket is a communication protocol. A WebSocket connection is established with Ably when the realtime client library is instanced. This connection is multiplexed, enabling multiple channels to share it. Messages can then be sent between Ably and the client in realtime.
