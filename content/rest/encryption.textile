...
- params := The cipher params that you want to specify. It must at a minimum include a @key@, which should be either a binary (<span lang="java">@byte[]@</span><span lang="javascript">@ArrayBuffer@ or @WordArray@</span><span lang="nodejs">@Buffer@</span><span lang="python">@bytearray@ or @bytes@</span><span lang="ruby">byte array</span><span lang="php">binary string</span><span lang="objc,swift">@NSData@</span>) or a base64-encoded @String@.<span lang="python"> If you are using Python 2 and you wish to pass in a base64-encoded key, make sure you pass it in as a @unicode@ string, not a @str@, or the library will interpret it as a binary.</span>
</div>
...
bq(definition).
  ...
  dotnet:  byte[] GenerateRandomKey(int? keyLength = null, CipherMode? mode = null)
  objc,swift: generateRandomKey(length?: UInt) -> NSData

This call obtains a randomly-generated binary key of the specified key length<span lang="dotnet"> and optional CipherMode</span>. 
...
<span lang="dotnet">- mode := Optional AES @CipherMode@ which is used when the key is generated</span>
...
blang[java,ruby,objc,swift,php,python,dotnet].
  h4. Returns
