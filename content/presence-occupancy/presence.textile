...
Other devices and services can subscribe to presence events in real time using the realtime interface or with "integrations":/docs/general/integrations. You can also request a list of clients or devices on a channel at a particular point in time with the REST interface.
...
<p>The number of clients that can be simultaneously present on a channel is "limited":/pricing/limits. This ensures the rate of presence messages remains supportable, as it is common for all members on a channel to change state at a similar time.</p>
...
<p>In some instances this is the behavior required of your application. In other scenarios you may prefer to only have a server subscribed to presence events and the clients are only present on the channel.</p>
</aside>
