...
You can use the Ably CLI to enter the room from another client by running the following command:
...
Now click the on the Avatar to see the participant list and typing indicators in action.
...
<Code>
  ...
  function ChatWindowExample() {
    return (
      <div className="p-4 h-screen w-1/2 bg-gray-50 rounded-lg border">
        <div className="h-full w-full overflow-hidden border rounded-lg">
          <ChatRoomProvider name="my-first-room">
...
Try them out in the window by using the Ably CLI to simulate typing from another client:
...
<Code>
  ...
  function SidebarExample() {
    const [roomNames, setRoomNames] = useState<string[]>(['general', 'random', 'announcements']);
    ...
    return (
      <div className="p-4 h-screen w-screen bg-gray-50 rounded-lg border">
        <div className="h-full w-full overflow-hidden border rounded-lg">
        <Sidebar
          roomNames={roomNames}
...
The `App` component is a complete, ready-to-use chat application that combines all the components above with sensible defaults.
Great if you need to prototype quickly or see how everything fits together.
