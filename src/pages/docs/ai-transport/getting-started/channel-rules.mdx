...
Channel rules are enabled per [namespace](/docs/channels#namespaces), which is the part of a channel name up to the first colon. Enabling the rule on a namespace applies it to every channel in that namespace. Pick a namespace prefix that keeps your conversations separate from other channels in the same app, such as `conversations:` or `chat:`, and enable the rule on it.

Without the rule, AI Transport [fails on the first token append](/docs/ai-transport/troubleshooting#namespace-not-configured) after a stream starts, with error `93002` `Can only update/delete/append messages on channels with mutableMessages enabled`.
...
- [Troubleshooting: namespace not configured](/docs/ai-transport/troubleshooting#namespace-not-configured): what it looks like at runtime when the rule is missing.
