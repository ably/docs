...
** @(RTN4a)@ Implements @EventEmitter@ and emits @ConnectionEvent@ events for state changes @INITIALIZED@, @CONNECTING@, @CONNECTED@, @DISCONNECTED@, @SUSPENDED@, @CLOSING@, @CLOSED@, @FAILED@
...
** @(RTL4i)@ Optionally, for backwards compatibility with 0.8 libraries, the @Connection@ @EventEmitter@ can provide an overloaded method that supports @on(ConnectionState)@, but must issue a deprecation warning
...
** @(RTL2a)@ Implements @EventEmitter@ and emits @ChannelEvent@ events for state changes @INITIALIZED@, @ATTACHING@, @ATTACHED@, @DETACHING@, @DETACHED@, @SUSPENDED@ and @FAILED@
...
** @(RTP5a)@ If the channel enters the @DETACHED@ or @FAILED@ state then all queued presence messages will fail immediately, and the @PresenceMap@ and "internal PresenceMap (see RTP17)":#RTP17 is cleared. The latter ensures members are not automatically re-entered if the @Channel@ later becomes attached
...
* "v0.8":/client-lib-development-guide/versions/v0.8/features (deprecated in July 2016).<br>"View 0.8 → 0.9 changes":https://github.com/ably/docs/blob/source/content/client-lib-development-guide/versions/features-0-8__0-9.diff
