...
- Finally, many of the advantages of Ably over Pubnub (using [websockets rather than long polling](https://knowledge.ably.com/which-transports-are-supported-1), [continuity guarantees](https://knowledge.ably.com/connection-state-recovery-1), [fallback host support](https://knowledge.ably.com/routing-around-network-and-dns-issues-1) etc.) are unavailable when using Pubnub client libraries. If a native Ably library is available for your platform, we recommend you consider using the Ably client libraries instead or at least make a plan to eventually transition over to Ably native client libraries.
