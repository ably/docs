...
Clients must have the [privileged-headers](/docs/platform/auth/capabilities#capability-operations) capability enabled in their Ably [API key](/docs/platform/auth) or [token](/docs/platform/auth/token) to skip integrations.
</Aside>
