...
blang[python].
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/90d66db
...
blang[python].
  "web.py":http://webpy.org/ is a simple web framework for Ruby. Django is arguably more popular as a web server framework, however for the purposes of this tutorial it is overkill.
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/bbb7f6c
...
In this step we will get the local web server ready to respond to authentication requests by issuing a token request to the requesting client. By adding the following route to your server, it is now ready to service clients wanting to authenticate with Ably.
...
blang[python].
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/62c2e62
...
blang[python].
  Now we need to serve a static home page. Let's replace the code that responds to the @/@ route (look for @class index: def GET(self): ... @) with the following:
...
blang[python].
  "See this step in Github":https://github.com/ably/tutorials/commit/1f3a90e
...
blang[python].
  Now that we have a means to determine if a user is logged in or logged out using the session cookie, we can issue restricted subscribe-only token requests for anonymous users and publish and subscribe token requests for logged in users. Replace the code @class auth: ...@ with the following:
...
blang[python].
  "See this step in Github":https://github.com/ably/tutorials/commit/5d407c2
...
blang[python].
  ...
  And then run the demo locally by adding your "Ably API key":https://support.ably.io/support/solutions/articles/3000030502-setting-up-and-managing-api-keys to @server.py@ and running the demo @python server.py@. Make sure you've installed @web.py@ (@pip install web.py@)
