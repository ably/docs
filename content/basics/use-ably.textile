...
This page will briefly cover some of the key APIs, SDKs, and adapters that Ably provides.

h2(#sdks). Ably SDKs with Realtime and REST Interfaces

Ably supports both Realtime, allowing persistent connections with low latency publishing and subscribing, and REST, communicating using the HTTP protocol and is effectively stateless, interfaces. Ably APIs are consistent across every language offering pub/sub functionality, presence, token and basic authentication, symmetric encryption and access to channel, presence and metrics history.
 ...
 * You're running into your hourly API request limits
 ...
 * Your application server primarily issues tokens for clients and/or publish messages on channels
...
Ably also provides a raw REST API where you don’t need to install any SDKs to use. 

CAVEAT-INPUT

 Reasons to use the raw REST API:
...
The Ably platform also provides protocol adapters, providing open protocol support and interoperability with a huge array of third party client libraries supporting every imaginable platform.

Although Ably SDKs are the recommended method for connecting to Ably, we also provide a number of protocol adapters as an alternative method for connecting to Ably. 
