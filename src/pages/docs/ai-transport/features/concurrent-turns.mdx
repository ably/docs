...
Concurrent turns let multiple request-response cycles run at the same time on one session. Each turn has its own stream, cancel handle, and lifecycle. This is what makes barge-in, multi-user sessions, and multi-agent architectures possible.
...
See [Barge-in](/docs/ai-transport/features/barge-in) for the full pattern.
...
- [Barge-in](/docs/ai-transport/features/barge-in): cancel and immediately send a new message.
