...
The API key string that is your private secret and should never be made public. This API key string is used in all of our SDKs and for authentication with the REST API.

h3(#create). Create an API key

API keys are created in the "Ably dashboard":https://ably.com/dashboard. You can also create an API key programmatically using the "Control API":/control-api.
...
<!-- CAVEAT-INPUT: API keys in code samples -->

Many of the code samples used throughout the documentation use basic authentication to demonstrate features for convenience. It should be stressed that in your own applications, you should never use "basic authentication"/auth/basic on the client-side, as it exposes your Ably API key.
