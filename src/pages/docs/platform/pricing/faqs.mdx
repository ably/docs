...
**Problem**: Using a Realtime library from your servers when you should use the REST library.

Every time the Realtime library is instantiated, a connection is established to Ably and will not be closed unless you explicitly close it. Server-side code typically doesn't need realtime features like subscriptions or presence.

**Solution**: Use the [REST interface](/docs/basics#rest) for server-side operations like:
- Publishing messages
