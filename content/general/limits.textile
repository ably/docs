...
<<<<<<< HEAD
Limits are either "time-based":#time or "quantity-based":#quantity. The effect of reaching or exceeding a limit varies depending on its type. 
=======
...
>>>>>>> parent of e9af3ddc (fix: merge rate limits into time-based limits)
...
<<<<<<< HEAD
For monthly, hourly and local per-second time-based limits the resource is blocked for the remainder of that period when a hard limit is reached. For example, if the hourly limit for the number of API requests is reached, no more API requests can be made for the remainder of that hour. Any API requests made in excess of the hard limit will be rejected with an error code.
=======
...
* Hourly limits are based on a clock hour, so regardless of whether a limit is reached at 11:01 or 11:59, it will reset at 12:00. 
...
Quotas are based on the account "package":https://ably.com/pricing/calculator that you are on. Quotas specify the number of peak channels, peak connections, messages and bandwidth that you expect to consume each month. Monthly quotas can be configured for self-service and business accounts. Quotas for free accounts are fixed. Enterprise account quotas are set in a way that ensures that limits are not reached during normal operations for an account.
...
A channel is considered open when a message is published on the channel using the REST API, or a realtime client attaches to it. The channel will remain active for as long as a client is attached to it, unless all clients explicitly detach from the channel or close their connections.
...
<<<<<<< HEAD
If the per-second bandwidth rate is exceeded, a "global per-second limit":#global-time will be applied to message delivery and publishing. The hourly and total bandwidth limits are "time-based":#time. If the hard limit is reached for those, message delivery and publishing will be blocked until the following hour or month.
=======
...
<<<<<<< HEAD
If the per-second token request rate is exceeded, a "global per-second limit":#global-time will be applied to token requests. The hourly token request limit is a "time-based limit":#time, meaning no additional token requests will be accepted until the following hour if the hard limit is reached.
=======
...
<<<<<<< HEAD
If the per-second API request rate is exceeded, a "global per-second limit":#global-time will be applied to API requests. The hourly API request limit is a "time-based limit":#time, meaning no additional API requests will be accepted until the following hour if the hard limit is reached.
