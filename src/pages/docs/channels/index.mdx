...
### Duplicate messages <a id="duplicate-messages"/>

If your message listener is being called multiple times for every message, one of the following is likely the cause:

#### Multiple SDK instances

You may be mistakenly instantiating the Realtime library more than once in a single page or app, and subscribing to the same channels with both instances. It's very rare you would want multiple library instances in a single web page or app - you can subscribe to hundreds of channels from a single connection, and they are all multiplexed over one WebSocket.

Solution: Ensure you only instantiate the library in one place and keep a single shared reference to it.

#### Multiple identical subscriptions

You may be mistakenly calling `channel.subscribe()` multiple times with the same listener function. This commonly happens when subscribing inside connection state listeners:

<Code>
```realtime_javascript
// WRONG - Creates a new subscription every time the connection reconnects
...
Use `once()` instead of `on()` if you only need to subscribe when first connected:

<Code>
```realtime_javascript
// BETTER - Only subscribes once
...
Or call `subscribe()` at the top level - it doesn't need to be inside a connection state callback:

<Code>
```realtime_javascript
// BEST - Subscribe immediately, SDK handles connection automatically
...
<Aside data-type="note">
If you call `subscribe()` with the same listener function multiple times, duplicates will be registered and the listener will be invoked multiple times for each message.
</Aside>

#### Known issues

There was a bug in ably-java SDK versions 1.2.34 to 1.2.47 where messages were duplicated when using the [rewind channel option](/docs/channels/options/rewind). This has been fixed in version 1.2.48 and later.
