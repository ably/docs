...
last_updated: '2022-07-02T09:35:45+00:00'
languages:
- javascript
- nodejs
level: medium
...
* You can read the "specification of the Ably JWT token":https://ably.com/docs/api/realtime-sdk/authentication#ably-jwt if you want to understand its construction.
...
4. Change to the project branch @jwt-auth-node-tutorial@ where the source code resides.

```[sh]
git checkout jwt-auth-node-tutorial
```
...
3. Copy your API key to the clipboard, from your Ably Dashboard. It will be of the form @<app ID>.<key ID>:<key secret>@. For example: @12AB3a.ab3GOm:c-B4Emryq9aFmOxP@.
...
function login(e) {
  ...
  realtime.connection.once("connected", () => {
    ...
    console.log("Success! You are connected with a JWT token", tokenDetails);
    alert("Success! You are connected with a JWT token");
  });
</script>
```
...
<script type="text/javascript">
    ...
    function login(e) {
        ...
        realtime.connection.once('connected', () => {
            const message = 'Client connected to Ably using JWT token';
            consoleLog(message);
            alert(message);
        })
    }
