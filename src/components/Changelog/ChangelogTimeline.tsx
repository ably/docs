...
const ChangelogTimeline = ({ entries }: { entries: ChangelogEntry[] }) => {
  if (entries.length === 0) {
    return <p className="ui-text-p1 text-neutral-800 dark:text-neutral-500">No updates yet.</p>;
  }

  return (
    <ul className="flex flex-col list-none pl-0 mb-0 divide-y divide-neutral-300 dark:divide-neutral-1100">
      {entries.map((entry) => (
