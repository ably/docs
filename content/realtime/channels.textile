...
Ably provides a "REST API":/general/channel_lifecycle to query your app for metadata about channels, as well as a "realtime API":/general/channel_lifecycle to subscribe to channel lifecycle events. Using the "REST API":/rest-api, you can enumerate all active channels, or obtain the status of an individual channel. Using our Realtime API, you can subscribe to "channel lifecycle":/general/channel_lifecycle events (such as being created or closed etc), or subscribe to periodic "occupancy":/general/channel_lifecycle#occupancy-rest updates for all active channels (such as how many people are subscribed to a channel).
