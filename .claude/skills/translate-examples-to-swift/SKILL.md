...
1. Create the package (use a unique directory per file being translated):
   ...
   Where `{filename}` is the MDX filename without path or extension (e.g., `harness-citations` for `citations.mdx`).
...
The consolidation script merges translation and verification JSONs, validates them, and generates the review HTML:
...
1. Create a Swift test harness in swift-translations/harness-{filename}/
