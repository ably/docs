...
The Ably realtime interface establishes and maintains a connection to the Ably service, using the most efficient transport available, typically "WebSockets":https://ably.com/topic/websockets, allowing you to monitor and manage "connection states":/connect/states. The Ably realtime interface operates and multiplexes all channel traffic over that connection. "Channels":/channels are separate from connections and are used to logically separate messages into different topics. 

CAVEAT-INPUT

  The Ably Realtime SDK opens and maintains a connection to the Ably realtime servers on instantiation, connections can only be established using the realtime interface of an Ably SDK. See "how to use Ably":/basics/use-ably for further information.

h2(#create). Create a connection

The Ably realtime interface opens and maintains a connection to the Ably realtime servers on instantiation, interacted with by using the @Connection@ object. The lifecycle of connections are reported by different "connection states":/connect/states#connection-states to simplify monitoring and managing connections.
...
Metachannels are a namespace of channels beginning with the [meta] qualifier, separating them from regular channels. For connections there is a specific @[meta]connection.lifecycle@ channel that publishes messages about the lifecycle of realtime connections. The connection lifecycle consists of a number of "connection states":/connect/states#available-connection-states that can be observed and interacted with using methods available on the connection object.
