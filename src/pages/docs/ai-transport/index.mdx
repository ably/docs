...
AI Transport solves this by decoupling the conversation from the connection. The session persists independently, so streams resume after disconnection, any device can join, and users can cancel or redirect agents through the same session. [Read more about why AI Transport exists](/docs/ai-transport/why).
