...
Java expresses typing through a fixed set of typed views instead of user-supplied type parameters. The `as*` casts (`asLiveMap()`, `asLiveCounter()`, `asString()` and so on) select a typed view of the path without checking what is actually stored there, and they never throw. A wrong cast only shows up when you use it: reads return `null` or an empty result, and writes complete their returned `CompletableFuture` exceptionally with an `AblyException`. Use `getType()` to check the type first; it returns a `ValueType`, or `null` when nothing resolves at the path.

<Code>
```java
// Select a typed view of the path and update through it
LiveCounterPathObject visits = rootObject.get("visits").asLiveCounter();
visits.increment(1).join();

// Wrong-typed reads return null instead of throwing
...
<If lang="java">
When an entry contains a primitive value or `LiveCounter`, select the matching typed view first and then call its `value()` method, for example `asString().value()` or `asLiveCounter().value()`:
</If>
...
<If lang="java">
If the entry doesn't exist, or the entry does not contain a value matching the typed view, `value()` returns `null`:
</If>
...
// Calling value() through a mismatched typed view also returns null
...
<If lang="java">
The Java SDK additionally offers `exists()` (a best-effort presence check) and `getType()` (returns a `ValueType`, or `null` when nothing resolves at the path). Use them to check what is at a path before reading it with a typed view.
</If>
...
if (username != null) {
    System.out.println(username.asString().value()); // e.g. "alice"
}
