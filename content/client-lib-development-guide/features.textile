...
** @(RSC8c)@ The library sets the @Accept@ and @Content-Type@ type headers, to reflect whether the client is configured to use a binary or JSON based protocol, to @application/x-msgpack@ or @application/json@ respectively.
** @(RSC8d)@ Irrespective of the specified @Accept@ header, the library should attempt where possible to process any response body based on the content type indicated in a @Content-Type@ header.
** @(RSC8e)@ If the library is unable to process the specified content type, then the library processes the response as follows.
*** @(RSC8e1)@ In the case of a @statusCode@ indicating an error response, the error must be propagated back to the caller with as much information as it is possible to obtain from the response, including the @statusCode@ and, where applicable, an error message containing text from the response body to help to diagnose the cause of the error.
*** @(RSC8e2)@ In the case of a @statusCode@ indicating a non-error response, an error must be propagated back to the caller with a @statusCode@ of 400, a @code@ of @40013@ and an error message containing the original @statusCode@ and, where applicable, text from the response body, to help to diagnose the cause of the error.
