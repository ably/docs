...
myObject.get("user").asLiveMap().set("username", LiveMapValue.of("alice")).join();
myObject.get("user").asLiveMap().remove("status").join();
```
</Code>
...
```java
myObject.get("visits").asLiveCounter().increment(5).join();
myObject.get("score").asLiveCounter().decrement(2).join();
```
</Code>
...
LiveMap profile = LiveMap.create(Map.of(
    "name", LiveMapValue.of("Alice"),
    "score", LiveMapValue.of(LiveCounter.create(0)),
    "settings", LiveMapValue.of(settings)));
myObject.set("profile", LiveMapValue.of(profile)).join();
```
</Code>
