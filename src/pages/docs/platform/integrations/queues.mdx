...
2. In the **Select your rule type** section, choose **Queue**.
3. In the **Choose queue** dropdown, select the queue you want to send data to.
4. (Optional) Add custom headers as key:value pairs to include metadata. Click **Another header** to add additional headers.
5. In the [**Source**](/docs/platform/integrations/webhooks#sources) dropdown, select which type of Ably event should trigger the rule.
6. (Optional) Add a [**Channel Filter**](/docs/platform/integrations/webhooks#channel-filter) using a regular expression to apply the rule to specific channels. Leave empty to apply to all.
7. Choose an [**Encoding**](/docs/platform/integrations/webhooks#encoding) format.
8. Click **Create** to finish setting up the rule.

</Tab>
<Tab value="control-api" label="Control API">

Create a queue rule using the Control API by sending a `POST` request to [`/apps/{app_id}/rules`](/docs/api/control-api):
...
curl -X POST https://control.ably.net/v1/apps/{APP_ID}/rules \
  ...
  -d '{
    ...
    "target": {
      "queueId": "your-app-id:your-queue-name",
      "format": "json",
      "enveloped": true
    }
  }'
...
Use the [Ably CLI](/docs/platform/tools/cli) to create a queue rule:
...
<Aside data-type='note'>
The CLI creates a queue rule with default target settings. To customize the target queue and other options, use the dashboard or Control API instead.
</Aside>
...
* The `Message.fromEncodedArray` method decodes the payload into standard [`Message`](/docs/pub-sub/api/javascript/realtime/message) objects.
