...
You will also need to setup Tailwind CSS for styling the application. Ensure you import tailwind in your local @App.CSS@ file and add it to your @vite.config.ts@ file. For installation instructions, see the "Tailwind CSS documentation for Vite":https://tailwindcss.com/docs/guides/vite.

```[sh]
npm install tailwindcss @tailwindcss/vite
```

* Now install the Ably Chat SDK, this will also install the Ably Pub/Sub SDK as a dependency:

```[sh]
npm install @ably/chat
```

h2(#imports). Adding required imports
...
h2(#providers-setup). Step 1: Setting up the Ably and Chat client providers
...
<aside data-type='note'>
<p>The @ChatRoomProvider@ is required when working with any other hooks exposed by the Chat SDK, such as the "useMessages":https://sdk.ably.com/builds/ably/ably-chat-js/main/typedoc/functions/chat-react.useMessages.html hook.</p>
</aside>
...
return (
        <div
            className="flex flex-col w-full h-[600px] item-left border-1 border-blue-500 rounded-lg overflow-hidden mx-auto font-sans">
            <div className="flex-1 p-4 overflow-y-auto space-y-2">
...
* Next, you must update the listener provided to the @useMessages()@ hook to handle the @MessageEvents.Updated@ event:
...
h2(#step-6). Step 7: Show who is typing a message
...
```[sh]
  ably rooms typing start my-first-room --client-id "my-cli"
```

h2(#step-7). Step 8: Display who is present in the room
...
h2(#step-8). Step 9: Send a reaction
...
* Next, add the @ReactionComponent@ component to your main app component:
...
h2(#step-10). Step 10: Disconnection and release
...
h3(#auto-detach). Automatic detachment on unmount
...
If relying on unmount is not enough, or you have opted to disable auto detach by setting `release={false}`, you can manually call `rooms.release(roomId)` to release a room:

```[react]
// App.tsx - Somewhere in your component
...
h3(#connection-close). Closing the realtime connection

It's important to note that no unmount process exists for disconnecting the Ably realtime connection. If you need to do this for some reason (such as when your application is shutting down), you can call `realtimeClient.connection.close()`:
...
* Find out more regarding "online status":/docs/chat/rooms/presence?lang=react.
