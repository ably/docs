...
Without token revocation, in order to restrict potential rogue access, you would make tokens with a short TTL (say 10 minutes) and then simply not issue a new token to a suspect client. Unfortunately, reducing the TTL would increase load on your authentication server, so token revocation can provide a more scalable solution to the issue of rogue clients.

Token revocation has to be enabled for you by Ably. There is a performance cost to enabling this feature. "Contact Ably":https://ably.com/contact if you'd like to enable this feature for a specific application.

h3. Revocable tokens

Tokens are revocable if they are created from an API key that has a @revocable@ attribute set to @true@. 

Tokens created from a key with the @revocable@ attribute set to @true@ are restricted to a TTL of 1 hour; an attempt to create, or present, a token that is revocable, but has an TTL of longer than 1 hr, is rejected with an error.

This functionality applies to client-signed JWTs and Ably-issued tokens, the latter being tokens obtained using the "@requestToken()@ API":/realtime/authentication#request-token).

A JWT may include the following additional claim:
...
```[json]
{
    targets: [/* ... array of target specifier strings ... */],
    issuedBefore: number (timestamp), /* (optional) a Unix timestamp where tokens issued before this time are invalidated */
    allowReauthMargin: bool /* (optional) if true, allows a 30 seconds margin for clients to obtain a new auth token online, default false */
}
```
...
When invoking the revocation API, a client must prove possession of the API key in question. This can be done by using basic authentication, using the API key itself.
...
Note that revocation is not a permanent prohibition of any client, connection, @clientId@ or channel. It invalidates token credentials issued before a certain point in time. Ultimately, the app that issues tokens is the arbiter of whether or not a client is able to continue to interact with the Ably service.

It may be that if you enable token revocation, and attempt to revoke a token already created earlier, then that token cannot be guaranteed to be revoked if it has already been cached.
...
```[sh]
curl -u "${ABLY_KEY}:${ABLY_SECRET}" -X POST https://rest.ably.io/keys/${ABLY_KEY}/revokeTokens -d @data1.json \
     --header "Accept: application/json" \
     --header "Content-Type: application/json"
```

Where @data.json@ is as follows:
...
The Ably JWT must be signed with the secret part of your "Ably API key":https://faqs.ably.com/what-is-an-app-api-key, using one of the following signature algorithms (as defined in "JWA":https://tools.ietf.org/html/rfc7518):
