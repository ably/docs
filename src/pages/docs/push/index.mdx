...
If you can't publish push notifications on channels, check these common issues:

#### Channel rules

Push notifications are disabled by default on all channels. You must explicitly enable push notifications for each channel or namespace:

1. Go to your Ably dashboard → App settings → Channel rules.
...
Verify your push payload in the message `extras` field is valid:
