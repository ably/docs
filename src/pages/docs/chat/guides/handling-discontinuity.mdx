...
If you are using [Pub/Sub](/docs/pub-sub) directly, see the [Pub/Sub discontinuity guide](/docs/pub-sub/guides/handling-discontinuity) instead. Pub/Sub uses the `resumed` flag rather than the Chat `onDiscontinuity()` handler.
</Aside>
