...
const MDXWrapper: React.FC<MDXWrapperProps> = ({ children, pageContext, location }) => {
  ...
  const { json_ld: jsonLd } = frontmatter;
  ...
  const structuredData: StructuredData | undefined = useMemo(() => {
    // A page can supply hand-authored schema via the `json_ld` frontmatter field
    // (generated by the docs-page-schema-jsonld skill). When present it takes
    // precedence over the auto-generated language schema below.
    if (jsonLd !== undefined && jsonLd !== null) {
      if (typeof jsonLd === 'object' && !Array.isArray(jsonLd)) {
        return jsonLd as StructuredData;
      }
      // Present but malformed: warn and fall through to the generated schema.
      console.warn(
        `Ignoring invalid json_ld frontmatter on ${location.pathname}: expected an object, received ${
          Array.isArray(jsonLd) ? 'array' : typeof jsonLd
        }.`,
      );
    }
  ...
  }, [jsonLd, activePage.languages, title, description, canonical, location.pathname, canonicalUrl]);
