...
const StyledApiReferenceH6: FC<HtmlAttributes<'h6'>> = ({ children, ...attribs }) => {
  return (
    <h6
      {...attribs}
      className={`inline-block font-mono items-center text-p2 w-full py-4 px-8 my-12 rounded-sm bg-api-reference-attribute-highlight border border-api-reference-attribute-border ${
        attribs?.className ?? ''
      }`}
    >
      {children}
