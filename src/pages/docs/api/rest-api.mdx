...
| Parameter | Description |
|-----------|-------------|
| `limit` | _100_ optionally specifies the maximum number of results to return. A limit greater than 1000 is unsupported<br/>**Type: `integer`** |
| `prefix` | optionally limits the query to only those channels whose name starts with the given prefix<br/>**Type: `string`** |
...
<Code>
```bash
curl -u "{'{{API_KEY}}'}"{""} -X POST https://main.realtime.ably.net/keys/{'{{API_KEY_NAME}}'}/revokeTokens \
...
|-----------|---------|------|-------------|
| channels | - | `string` | a list of channel names to retrieve the presence states for. Each name in the list should be separated by commas unless you specify an alternative `separator` |
| separator | _,_ | `string` | optionally, the character to use as a separator for the list of channel names in the `channels` parameter |
