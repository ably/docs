...
| `userClaim` | A server-signed [user claim](/docs/chat/setup#user-claims) attached to this message, derived from the publishing user's [JWT](/docs/chat/setup#set-user-claims). Unlike `headers` and `metadata`, this value is read-only and cannot be set by clients. | `String` or `undefined` |
...
Ably guarantees messages arrive to all subscribers in a [consistent order](/docs/platform/architecture/message-ordering), within each site. Unique serials are associated to all messages so that a consistent global order can be enforced.
</Aside>
