...
** @(RSA17c)@ If the @TokenRevocationRequest#targets@ attribute is null (as per @TF3@), then the function must throw an error with a @"targets are not provided"@ message.
** @(RSA17d)@ If the @TokenRevocationRequest#targets@ attribute or invalid (as per @TF3@), then the function must throw an error with a @"targets cannot be empty"@ message.
** @(RSA17e)@ If the @TokenRevocationRequest#key@ is provided and @TokenRevocationRequest#targets@ valid, then the function must create the request and execute it.
** @(RSA17f)@ Function returns a @TokenRevocationResponse@ object that contains the result of the request if request is successful.
** @(RSA17g)@ Function throws an appropriate error/exception if the request fails either for the reasons of invalid request, or as  a result of request failing.
** @(RSA17h)@ Tests in this section must exist in client library.
*** @(RSA17h1)@ Given a null @TokenRevocationRequest#key@ attribute, verify that the function throws an error with a @"Key is not provided"@ message.
*** @(RSA17h2)@ Given a null @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets are not provided"@ message.
*** @(RSA17h3)@ Given an empty @TokenRevocationRequest#targets@ attribute, verify that the function throws an error with a @"targets cannot be empty"@ message.
*** @(RSA17h4)@ Given a non-null @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function creates the request and executes it.
*** @(RSA17h5)@ Given a non-null but invalid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that client library throws an API error.
*** @(RSA17h6)@ Given a valid @TokenRevocationRequest#key@ attribute and a valid @TokenRevocationRequest#targets@ attribute, verify that the function executes the request and returns a @TokenRevocationResponse@ with targets matching with the original targets in request.
*** @(RSA17h7)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the future for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
*** @(RSA17h8)@ Given valid required attributes in @TokenRevocationRequest@ and a timestamp in the past > 1 hour for @TokenRevocationRequest#issuedBefore@ attribute, verify that the request is rejected. 
...
* @(TF3)@ @TokenRevocationRequest#targets@ is an array of @TokenRevocationTarget@s to revoke. For request to be valid, it must exist and contain at least one target. A test must exist to ensure there is at least one target in the request before it is sent.
...
* @(TL2)@ @TokenRevocationTarget#key@ is a string that is the target key.This value cannot be null.
* @(TL3)@ @TokenRevocationTarget#value is a string that is the target value. This value cannot be empty or null. 
