---
title: WebRTC file transfers with Ably
section: tutorials
index: 106
---
...
The idea behind file transfers using webRTC is to convert files into buffers or DataUrl, transmit them in chunks, then compile them back to the file it was on the receiving end.

In this tutorial, we will build a WebRTC file transfer with data channels, which lets us share files multiple people in separate chats.

This is very similar to our initial lesson on data channels.

<a href="/images/tutorials/file-transfer.gif" target="_blank">
  <img src="/images/tutorials/file-transfer.gif" style="width: 50%" alt="Demo image">
</a>
...
Open up a new file and name it @index.html@. Add the following code to this file:
...
# Required the Ably JavaScript library
# Required the simple peer JavaScript library.
# Required a JavaScript file named @ably-datachannel.js@ (which we will create soon).
# Required a JavaScript file named @connection-helper.js@ (which we will create soon).
...
In step 3 above, we required a library called @connection-helper.js@. This library helps to manage the simple-peer connections, so as to keep our code organized, as we will be having multiple instances of the connection.

Open a new file called @connection-helper.js@ and paste:
...
# @initiator@: this is a Boolean parameter that states either this is the peer initiating the connection or not.
...
# @_onData@: this method is called when a new message has been sent to the other peer. Here, it calls another method called @recieveMessage@, which we will later define in the @ably-datachannel.js@ file.
...
# @messageLis@: this is an object which will contain keys of each client you are currently chatting with, or that has sent a message, with an array of what the messages are.
...
First, we need to make an @enter@ subscription, so we will be notified every time a new member joins our channel. We do so by using @AblyRealtime.presence@. In the callback, we get a list of all current members and add it to our member’s list. After this, we call on the render members method.
...
<a href="/images/tutorials/users-online.png" target="_blank">
  <img src="/images/tutorials/users-online.png" style="width: 50%" alt="Demo image">
</a>

<a href="/images/tutorials/users-offline.png" target="_blank">
  <img src="/images/tutorials/users-offline.png" style="width: 50%" alt="Demo image">
</a>

h2.
  default: Step 6 – Sending files

We have been able to identify who is online and ready to receive instant files using WebRTC data channels, now we move to the part where we send files. Add the following to your @ably-datachannel.js@ file.
...
The @prepareChat@ method receives a single argument, i.e, the @client-id@, which is the client you want to send files to. We then check to be sure that the user is not trying to send files to himself. Although it isn’t possible for the user to do this, we do this as an extra layer of security. At this point, if the user does not have an active connection with the other peer, it means he is the one initiating the chat/file transfer, so we create a new connection with that @client_id@, passing the initiator status to be true. After this, we call the @render@ method to render the files that might have been sent by the other peer.
...
Receiving the files consist of storing the chunks in an array, then joining the chunks together when all parts have been received. Let’s take a look at the @recieveMessage@ function below:
...
In the @recieveMessage@ method, we receive the file here with the id of the user who sent it, then create a new key with the name of the file we want to receive in the object that belongs to the sender. Next, we set the properties of @competed@ to false, the @downloadLink@ to empty. The size of the string we are expecting in total is received as @stringSize@, then we defined a chunks array where the chunks would be stored. 
...
Next, let us update the @recieveMessage@ method to be able to play the notification
...
In this tutorial, we have seen how to use WebRTC data channels in conjunction with Ably to create realtime file share. Do you have any thoughts on making this lesson better? "Let us know.":https://www.ably.io/contact
