...
# Follow "these instructions for setting up an API key":https://knowledge.ably.com/setting-up-and-managing-api-keys if you haven't done so before.
# In your "application dashboard":https://knowledge.ably.com/how-do-i-access-my-app-dashboard, go to the &quot;Settings&quot; tab and scroll down to the &quot;Channel rules&quot; section.
...
# Hit &quot;Save&quot; and you're done. Further details about setting up channel rules "can be found here":https://knowledge.ably.com/what-are-channel-rules-and-how-can-i-use-them-in-my-app.
...
Now we'll implement the @enterLobby@ function. It has one parameter, @onGamePending@, which is a callback function provided in the @lobby.vue@ file that we implemented earlier. See the "Ably documentation on Presence":https://ably.com/documentation/realtime/presence for full details on how presence works. In the code below, we subscribe to the lobby's @presence@ channel. The documentation for the @presence.subscribe()@ method "explains this in more detail":https://ably.com/documentation/realtime/presence#subscribe-listener.
...
export function enterGame(gameId, onGameStateUpdated) {
  ...
  gameChannel.history({ limit: 1000 }, (error, page) => {
    ...
    // See https://ably.com/documentation/realtime/history#paginated-result for details.
...
And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @server/index.js@, and building and running the client and server as described at the start of the tutorial.
...
5. Learn more about other "Ably features":https://ably.com/features by stepping through our other "Ably tutorials":https://ably.com/tutorials
