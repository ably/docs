...
- [`createClientSession()`](/docs/ai-transport/api/javascript/core/client-session): creates the client-side session that subscribes to the conversation.
- [`session.createView()`](/docs/ai-transport/api/javascript/core/client-session): returns a view of the conversation that updates as tokens are appended.
- [`view.send()`](/docs/ai-transport/api/javascript/core/client-session): publishes a user message and starts a run.
