...
In this tutorial, we will see how to use the "Ably Realtime client library":/realtime to build a realtime news feed with React. Ably enables realtime data sharing using Pub/Sub messaging architecture via a concept called "channels":https://www.ably.io/channels.

For the purpose of this tutorial, we'll not discuss storage of messages or other server-side mechanisms.

"React":https://reactjs.org":https://reactjs.org is a JavaScript library for building user interfaces. It is component-based and declarative which makes it easy to build complex UI with it.
...
h2(#update-css). Step 3 - Update CSS for the tutorial
...
In our newfeed app, we would like to display the profile information of each user who is publishing a new update. Since we are not including any database and such for this tutorial, we'll simply assign a random avatar and random name to each user who is using an instance of the application.
...
We have passed a few "client options":https://www.ably.io/documentation/realtime/usage#client-options when instantiating Ably: 
* @key@ is the API key 
...
"Presence":https://www.ably.io/documentation/realtime/presence#title enables clients to be aware of other clients that are currently “present” on a channel, i.e, connected or online.
...
h2(#retrieve-online-users). Step 6 - Getting online users using Ably's Channel Occupancy API

Ably's offers some special channels called Metachannels to share information about the channels themselves in a given app. These channels always start with the @[meta]@ qualifier, separating them from the regular channels used for realtime data trasnfer.

Among other things, the metadata that is of interest to us is the channel occupancy event updates which are published on the metachannel idetified by @[meta]channel.lifecycle@. We will use the channel occupancy here to get active connections. Please note that channel occupancy event updates are an enterprise only feature. You can still use the "Channel Status API":/rest/channel-status to poll Ably for meta info about the channels in an app.
