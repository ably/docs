...
jump_to:
 Section:
   ...
   - 2. Step 2 - Install Ably#install-ably
   - 3. Step 3 - Publish encrypted messages to a channel#publish-messages
   - 4. Step 4 - Subscribe and decrypt messages#subscribe-messages
   - 5. Live Demo#live-demo
   - 6. Download tutorial source code#download-tutorial
...
blang[java].
  To start using Ably in your Java application, you need to include the Ably Client library. We recommend that you include the latest client library via Gradle in your project's @gradle.build@ file.
  ...
  Now that you have your API key, let's define a constant @API_KEY@ as belows:
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/encryption-java-step2
...
blang[javascript].
  ...
  **What have we setup in @publisher.html@ so far?**
  ...
  # We added Ably library and configured it with our API key (from Step 1)

  "See this step in Github":https://github.com/ably/tutorials/commit/encryption-javascript-step2

blang[nodejs].
  The Ably Realtime and REST library is available as an "NPM module":https://www.npmjs.com/package/ably. Simply add it to your @package.json@ file
  ...
  **What is going on in the code block above?**
  ...
  # @Ably@: instance of Ably
  # @ApiKey@: your Ably API key from Step #1

  Then we define a @Realtime@ instance, which we'll use to obtain a publisher and a subscriber, in the next step.
...
We will use the encryption API to publish encrypted messages as well as receive and decrypt the encrypted messages.

blang[java].

  First, let's define a few constants in @Constants.java@ class:
  ...
  Now, let's have a look at how we would publish encrypted messages to a channel in the @Publisher.java@ class
  ...
  **What is going on in the code block above?**
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/encryption-java-step3
...
blang[javascript].
  ...
  **What is going on in the code block above?**
  ...
  "See this step in Github":https://github.com/ably/tutorials/commit/commit/encryption-javascript-step3

blang[nodejs].

  Let's first have a look at how we would publish encrypted messages to a channel.

  ```[nodejs]
  Ably.Realtime.Crypto.generateRandomKey(function(err, key) {
      var channelOpts = { cipher: { key: key } };
      var channel = realtime.channels.get('encrypted:messages', channelOpts);
  ...
  **What is going on in the code block above?**
  ...
  # We create @channelOpts@ variable that holds the cipher key that we intend to use for the encryption. Note that the @cipher key@ should not be a pass-phrase, but a cryptographic key - generated from a secure random source, 128 or 256 bits long, binary or base64-encoded. If you wish to encrypt messages with a pass-phrase (for example, one entered by a user), you should use a "key derivation function":https://en.wikipedia.org/wiki/Key_derivation_function to transform that into a key. Here we are using an Ably utility (@Ably.Realtime.Crypto.generateRandomKey()@) to generate a random key.
  # We then define a variable (@channel@) for publishing messages. Notice how we pass the @channelOpts@ variable to @realtime.channels.get()@. This configures @channel@ to encrypt the messages with the cipher key, before they are published.
  # In the next step we'll see how easy it is to retrieve and decrypt this encrypted message

  "See this step in Github":https://github.com/ably/tutorials/commit/commit/encryption-nodejs-step3

h2(#subscribe-messages). Step 4 - Subscribe and decrypt messages

blang[java].

  Now that we have published an encrypted message on a channel, we can retrieve it using a similar @ChannelOptions@ object in @Subscriber.java@ class.
  ...
  **What is going on in the code block above?**
  ...
  We simply subscribe to the @CHANNEL_NAME@ channel, and print the (decrypted) message to the the console.

  Although, here we're using the same @Channel@ object to publish and subscribe encrypted messages, in a real world application, your publisher and subscriber will most likely reside in different components or modules or even applications.
  But the important point to note here is that in either case we obtain the @Channel@ instance the same way i.e. by passing the @ChannelOptions@ object.

  "See this step in Github":https://github.com/ably/tutorials/commit/encryption-java-step4
...
blang[javascript].
  ...
  **What is going on in the code block above?**
  ...
  # All we're doing here is calling the @channelSub.subscribe()@ and passing it a callback function that appends each received message to the textbox's value.
  # Notice, how easily we implemented enabled the decryption mechanism simply by using a @channelOpts@ containing our cipher key.

  "See this step in Github":https://github.com/ably/tutorials/commit/encryption-javascript-step4

blang[nodejs].

  Now that we have published an encrypted message on a channel, we can retrieve it using a similar @ChannelOptions@ object.
  ...
  **What is going on in the code block above?**
  ...
  We simply subscribe to the @channel@, and log the (decrypted) message to the the console.

  Note: For brevity we're using the same channel instance to publish and subscribe here, but in real life you may have the publisher and subscriber channels initiated in different modules or even applications.
...
<a href="#" id="new-browser" target="_blank">Open this demo in a new browser window</a> to see publishing & subscribing of encrypted messages in action.  Alternatively, scan the QR code to see the published message decrypted and delivered in real time.
...
blang[java].
  ...
  bc[sh]. git checkout encryption-java
  ...
  Don't forget to replace your @Constants#API_KEY@ field with "Ably API key":https://support.ably.io/support/solutions/articles/3000030502-setting-up-and-managing-api-keys.
...
blang[javascript].
  ...
  bc[sh]. git checkout encryption-javascript
...
blang[nodejs].
  ...
  bc[sh]. git checkout encryption-nodejs

  Add your "Ably API key":https://support.ably.io/support/solutions/articles/3000030502-setting-up-and-managing-api-keys to @example.js@. To run the demo locally, switch to project's root directory and use this command in your terminal:
