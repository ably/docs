...
The provider also renders an ably-js `<ChannelProvider>` for the session's channel, so ably-js's channel hooks (`usePresence`, `usePresenceListener`, `useChannel`) work for any descendant without wrapping the subtree in your own `<ChannelProvider>`, which is what [agent presence in React](/docs/ai-transport/features/agent-presence#react) relies on. Pass the same `channelName` you gave the provider.
