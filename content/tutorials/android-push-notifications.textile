...
* "@push-admin@":/general/push/admin: A client with this permission has full access to the "Push Admin API":https://ably.com/documentation/general/push/admin and can manage registrations and subscriptions for all devices.
...
Next, navigate to the Ably app's *Settings* tab, go down to the *Channel rules* section and add a new channel rule for a namespace by checking the *Push notifications enabled* option. In this tutorial, we'll be eventually publishing some Push Notifications via the @push:test_push_channel@ channel, so we'll use the "channel namespace":https://knowledge.ably.com/what-is-a-channel-namespace-and-how-can-i-use-them @push@, as seen below.
