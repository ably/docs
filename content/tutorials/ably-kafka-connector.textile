...
"Kafka":https://en.wikipedia.org/wiki/Apache_Kafka is an incredibly powerful tool, allowing for distributed event storage and stream processing. It excels when it comes to acting as a traditional message broker with an impressive throughput and reliability, and in use-cases such as stream processing.

h2(#what-is-connector). What is the Ably Kafka Connector

Whilst Kafka is amazing at storing and processing streams of data, it doesn't inherently have methods of data distribution. 
...
If you need an Ably Account, you can "sign up":https://ably.com/signup a free one on the Ably website. Once you have an account and are signed in, you can go to one of your "Ably Apps":https://ably.com/accounts/any/apps/any, navigate to the "API key tab":https://ably.com/accounts/any/apps/any/app_keys, and create or copy an existing API key from there which has the publish permission.
...
With the above running, we now have the ability to send messages from Kafka into an Ably Channel. From here, we can distribute these messages to as many users as desired. This can be incredibly powerful for many use-cases, such as vehicle tracking, "ticket booking systems":https://ably.com/blog/realtime-ticket-booking-solution-kafka-fastapi-ably, and generally any scenario where you'd be interested in distributing data to end users.
