...
You can "configure events":https://faqs.ably.com/how-to-set-up-a-reactor-rule from the "Integrations tab in your app":https://faqs.ably.com/how-to-set-up-a-reactor-rule on a per app basis which can apply to one or more channels in that app. Reactor Events can be filtered by channel naming using a regular expression, for example @^click_.*_mouse$@. This would match the string @click_@ followed by a string followed by @_mouse@, for example, @click_left_mouse@.
...
Ably currently supports the following sources for all rule types, in both "single":#single-request and "batch":#batch-request mode:

* "Messages":/realtime/messages#messages - If the source @channel.message@ is selected, messages trigger function calls as soon as they are published on a channel.
* "Presence":/realtime/presence - If the source @channel.presence@ is selected, presence events trigger function calls when clients enter, update their data, or leave channels.
* "Channel lifecycle":/realtime/channels#channel-lifecycle – If the source @channel.lifecycle@ is selected, you receive notifications for channel lifecycle events, such as when a channel is created (following the first client attaching to this channel) or discarded (when there are no more clients attached to the channel).
* "Occupancy":/realtime/inband-occupancy#occupancy-events - If the source @channel.occupancy@ is selected, you receive notifications for occupancy events, which relate to the number and type of occupants in the channel.

For @channel.lifecycle@ and @channel.occupancy@ metadata in particular, for scalability reasons it is recommended that your rules use these sources, rather than creating @channel.message@ rules on the corresponding "metachannels":/realtime/metachannels.
