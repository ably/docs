...
<Code>
```shell
npm install @ably/ai-transport ably ai @ai-sdk/react @ai-sdk/anthropic \
...
## Start the Temporal dev server <a id="start-temporal"/>

Run the Temporal dev server locally. `--db-filename` persists state across restarts so a workflow you inspect in the Web UI survives a machine reboot.
...
The Web UI is at `http://localhost:8233`. Every workflow in this app is one HTTP POST, so a fresh conversation turn appears as a fresh workflow. Retries and activity attempts are visible under the workflow.

## Define shared types <a id="shared-types"/>
...
## Define the tool <a id="define-tool"/>
...
## Define the activities <a id="define-activities"/>
...
async function runInference(run, stepId: string): Promise<InferenceOutcome> {
...
## Run the worker <a id="run-worker"/>
...
Run the worker with `npx tsx workflow/worker.ts`.
