...
If you're ready to enable a custom endpoint to take advantage of these features, then please "see our guide":#setting-up-a-custom-endpoint. Otherwise, read on to find out more.
...
We provide you with custom endpoints for your cluster, for example: @yourcompany.realtime.ably.net@ and @yourcompany.rest.ably.net@.  Additionally, we can support CNAMEs using your own custom root domain.
...
h2(#setting-up-a-custom-endpoint). Setting up a custom endpoint
...
- @realtime.ably.net@ - this endpoint is used for all WebSocket and other realtime protocol connections from the Ably client libraries.
- @rest.ably.net@ - this endpoint is used for all REST-based requests and is optimized for all standard HTTP-based request types including all "REST requests":/docs/api/rest-api, "authentication requests":/docs/auth, and Comet and JSONP fallback when WebSockets cannot be used.
...
For "example,":#modify @example.rest.ably.net@ and @example.realtime.ably.net@.

These use Ably's existing wildcard certificate, which supports @*.ably.net@ and @*.ably-realtime.com@.
...
* a routing policy:
  * @endpoint=main@ routes to `main.realtime.ably.net` (or `realtime.ably.net`)
...
- *Realtime*: @https://[ENDPOINT].realtime.ably.net/time@ (e.g. https://endpoint-example-realtime.ably.net/time)
- *REST*: @https://[ENDPOINT]-rest.ably.net/time@ (e.g. https://endpoint-example-rest.ably.net/time)
