...
intro: "Your users get notified when a long-running agent finishes, even after they close the app. AI Transport keeps the conversation in the session, and Ably Push delivers the alert to the device that originally subscribed."
redirect_from:
...
Push notifications come from the Ably Push API rather than the AI Transport SDK, and they deliver notifications to devices that are no longer connected to the session.
</Aside>
...
3. The agent completes the task and publishes the result to the session.
...
Because AI Transport persists messages in the session, the full response is available when the user returns. You do not need to store results separately.
