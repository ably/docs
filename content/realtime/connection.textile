...
When a connection failure occurs this <span lang="default,ruby">attribute</span><span lang="java">member</span><span lang="jsall">property</span> contains the <span lang="jsall">"@ErrorInfo@":/realtime/types#error-info</span><span lang="java,ruby">"@AblyException@":/realtime/types#ably-exception</span>.
...
Explicitly calling @connect@ is unnecessary unless the "@ClientOption@":#client-option <span lang="default">@autoConnect@</span><span lang="ruby">@auto_connect@</span> is disabled. Unless already @connected@ or @connecting@, this method causes the connection to open, entering the @connecting@ state.
...
Register the given listener <span lang="ruby">block</span><span lang="swift,objc">function</span> for the specified "ConnectionState":#connection-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span> changes on the @Connection@. The listener is passed a "ConnectionStateChange":#connection-state-change object that contains the current state, previous state, and an optional reason for the state change.</span>
...
Register the given listener <span lang="ruby">block</span><span lang="swift,objc">function</span> for all "ConnectionState":#connection-state changes on the @Connection@. The listener is passed a "ConnectionStateChange":#connection-state-change object that contains the current state, previous state, and an optional reason for the state change.</span>

h4. Parameters

- <div lang="jsall">String state</div> := the connection state to subscribe to
- <div lang="java">"ConnectionState":#connection-state state</div> := the connection state to subscribe to
- <div lang="ruby">"ConnectionState":#connection-state state</div> := the connection state as a Symbol such as @:connected@ or @ConnectionState@ object to subscribe to
- <div lang="swift,objc">"ARTRealtimeConnectionState":#connection-state state</div> := the connection state to subscribe to

- <div lang="jsall">listener</div> := is a function of the form @function(stateChange)@ to be notified for matching state changes
- <div lang="java">"ConnectionStateListener":#connection-state-listener listener</div> := listener to be notified for matching state changes
...
Register the given listener <span lang="ruby">block</span><span lang="swift,objc">function</span> for a single occurence of the specified "ConnectionState":#connection-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span> change on the @Connection@. Once the listener has been called, it is removed as a registered listener and will not be called again. The listener is passed a "ConnectionStateChange":#connection-state-change object that contains the current state, previous state, and an optional reason for the state change.
...
Register the given listener <span lang="ruby">block</span> for a single occurence of any "ConnectionState":#connection-state change on the @Connection@. Once the listener has been called, it is removed as a registered listener and will not be called again. The listener is passed a "ConnectionStateChange":#connection-state-change object that contains the current state, previous state, and an optional reason for the state change.
...
- <div lang="java">"ConnectionStateListener":#connection-state-listener listener</div> := listener to be notified for a single occurence of a matching state change
...
Remove the given listener <span lang="ruby">block</span> for the "ConnectionState":#connection-state <span lang="default">@state@</span><span lang="ruby">@state(s)@</span><span lang="java">@state@</span>.
...
Remove the given listener <span lang="ruby">block</span> for all "ConnectionState":#connection-state states.

h4. Parameters

- <div lang="jsall">String state</div> := the connection state to unsubscribe from
- <div lang="java">"ConnectionState":#connection-state state</div> := the connection state to unsubscribe from
- <div lang="ruby">"ConnectionState":#connection-state state</div> := the connection state as a Symbol such as @:connected@ or @ConnectionState@ object to unsubscribe from
- <div lang="swift,objc">event</div> := the connection state to unsubscribe from

- <div lang="jsall">listener</div> := is the listener function to be removed
- <div lang="java">"ConnectionStateListener":#connection-state-listener listener</div> := is the listener to be removed
...
bq(definition).
  ...
  ruby:    "Deferrable":/realtime/types#deferrable ping -> yields @Integer milliseconds@
  java:    public void ping("CompletionListener":#completion-listener listener)
  objc,swift: ping(cb: (ARTErrorInfo?) -> Void)
...
blang[ruby].
  ...
  On successfully echoing a heartbeat from Ably, the registered success blocks for the "@Deferrable@":/realtime/types#deferrable and any block provided to this method yields an Integer representing the time in milliseconds for the heartbeat ping request to be echoed.
...
h3(#connection-state-change).
  ...
  default: ARTConnectionStateChange
