...
If you have enabled the "**Persist Last** channel namespace rule,":/general/channel-rules-namespaces/ on the channel in question, you can attach with @rewind=1@ to retrieve the last message. Note that only the last message can be stored long-term (up to a year), and persisting the last message does not work for presence messages.
