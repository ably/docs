...
Device registrations don't last forever: users upgrade phones, uninstall apps, revoke notification permissions, or log out and in with different accounts. Devices need updates when:
...
Example to update a device registration from the server-side:
```javascript
...
From the client-side, first deactivate the device, and then activate it again with the new details.
...
Capabilities can be set for:
- [API keys](/docs/auth/basic): set in the Ably dashboard.
- [Ably tokens](/docs/auth/token#tokens): set in the `capability` field when requesting a token.
- [JWT tokens](/docs/auth/token#jwt): set in the `x-ably-capability` claim as a JSON-encoded string.
