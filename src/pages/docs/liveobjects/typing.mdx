...
On a [`PathObject`](/docs/liveobjects/concepts/path-object?lang=java), the `as*` casts select a typed view of the path without checking what is actually stored there, and they **never throw**. A wrong cast only shows up when you use it:
