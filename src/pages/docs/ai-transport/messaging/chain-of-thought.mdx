...
func example(realtime: ARTRealtime, stream: any AsyncSequence<(type: String, text: String, responseId: String), Never> & Sendable) async {
...
for await event in stream {
    if event.type == "reasoning" {
        ...
        channel.publish(
            "reasoning",
            data: event.text,
            extras: ["headers": ["responseId": event.responseId]] as ARTJsonCompatible
        )
    } else if event.type == "message" {
        // Publish model output messages
        channel.publish(
...
channel.subscribe { message in
    ...
    let headers = extras?["headers"] as? NSDictionary
...
for await event in stream {
    if event.type == "start" {
        ...
        reasoningChannel.publish("reasoning", data: event.text)
    } else if event.type == "message" {
        // Publish model output to main channel
        conversationChannel.publish(
            "message",
