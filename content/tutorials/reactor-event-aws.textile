...
For this tutorial, we'll be using *"Reactor Events":https://www.ably.io/reactor* in response to incoming messages in order to *automatically invoke your own AWS Lambda functions*. We're going to build a simple pizza ordering system with a virtual pizza assistant chat bot. When a new customer hits your website, they'll be assigned a random ID, their messages will be published to a private channel created just for that customer, and a Reactor Event will call your AWS Lambda functions to invoke the pizza chat bot, who will take their order and then, via an HTTPS request to the Ably REST API, post a response back to a similar channel that the web application will use to listen for responses from the chat bot.

The chat bot will be built using the "AWS Lex":https://aws.amazon.com/lex/ service, which provides specialized functionality for managing natural language conversations between users and software. Though there are a number of different chat bot services we could integrate with, by using the AWS Lex service, you'll be able to use the same account and credential management area for managing access to both the Lambda and Lex services, and potentially take advantage of any other integrations between Lex and Lambda, seeing as they both belong to the same service provider.
...
<a href="/images/tutorials/reactor-event-aws-project-animation.gif" target="_blank">
  <img src="/images/tutorials/reactor-event-aws-project-animation.gif" style="width: 100%" alt="Animated image of the browser app in action">
</a>
...
h3. (c) IAM user for use by your Ably Reactor event
...
h3. (a) Python and the AWS CLI

Make sure you have Python installed. A Python 3.6.x installation is used in this tutorial. You can "get Python here":https://www.python.org/downloads/, and this will also give you the *pip* installer, which you'll need to install the AWS CLI. If you're familiar with Node.js, pip is a package manager like npm, but for Python.

Open your command line shell of choice. This would likely be Power Shell or Bash (WSL) on Windows, or a Bash terminal on Mac and Linux. Check that Python and pip are available by typing `python -V` to print out the installed version of Python, and `pip -V` to check that pip is also installed.

As per the instructions on "this page":http://docs.aws.amazon.com/cli/latest/userguide/installing.html, install the AWS CLI by typing `pip install awscli`. This will download and install the AWS CLI as a global Python package that you can access from the command line. Once you've done this, type `aws configure`. You'll be prompted to add the credentials you retrieved earlier. You should also choose *us-east-1* for the region (AWS Lex isn't available in all regions), and *json* for the output format.
...
This will create and configure your chat bot so that it can take a simple pizza order via a text-based conversation with a user. You'll use your Lambda function to route user messages from your Ably channel to Lex, and to post the bot's response back to the user on the same channel.
...
We're going to need an API key for use by your browser application, and for the AWS Lambda function to allow it to post a response back to Ably. For our purposes, only one API key is required, but you may wish to create a separate API key for the Lambda function if you need it to have a different set of permissions than those assigned for use by the browser app.
...
The next thing is to set up a Reactor event, which we need in order to relay channel messages from the customer to our AWS Lambda function for processing. We'll actually be using two channels - one outbound, and one inbound. We'll do this to ensure that only the customer's communications are sent to AWS, and so that the browser app need only process messages from the chat bot. Channels are lightweight constructs that are created on demand and discarded when no longer in use, so using two channels in this way is not a problem.
...
# The JSON option means that your channel uses standard JSON packets for messaging, rather than custom-formatted data, as might be the case if your channel were being used for voice or other binary data.
...
Also, because channels are lightweight, it's ok to make as many channels as you need. If two different customers are trying to order a pizza at the same time, we don't want their messages getting mixed in together, so we'll be appending a random customer identification value to the inbound and outbound channel names, with the effect being that every customer is allocated their own unique pair of inbound and outbound channels. The customer ID will also need to be included in outbound messages so that your AWS Lambda function can figure out where to send responses.
...
Let's refactor the message publishing code into its own function. We'll also need to modify what we're sending so that it includes the customer ID so that the Lambda function can reply on the correct channel:
...
The Lex bot we built isn't exactly robust (or even complete, for that matter). If you'd like to improve it, spend some time investigating "the Lex documentation and API":https://aws.amazon.com/documentation/lex/ to come up with some creative approaches of your own.

Ably Reactor events are just one of the features that the Reactor engine offers. Why not "investigate some of the others":https://www.ably.io/reactor and see what else is possible?
