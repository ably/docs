...
func example(channel: ARTRealtimeChannel, pendingApprovals: inout [String: (toolCall: ToolCall, Void)]) {
    // --- example code starts here ---
*/}
```swift
let channel = realtime.channels.get("{{RANDOM_CHANNEL_NAME}}")
var pendingApprovals: [String: (toolCall: ToolCall, Void)] = [:]
...
func example(channel: ARTRealtimeChannel, displayApprovalUI: @escaping (Any?, String?) -> Void) {
    // --- example code starts here ---
*/}
```swift
let channel = realtime.channels.get("{{RANDOM_CHANNEL_NAME}}")
...
channel.subscribe("approval-request") { message in
    ...
    let headers = (message.extras as? NSDictionary)?["headers"] as? NSDictionary
...
func example(
    ...
    pendingApprovals: [String: ToolCall],
...
*/}
```swift
var pendingApprovals: [String: ToolCall] = [:]
...
channel.subscribe("approval-response") { message in
    ...
    guard let toolCallId = toolCallId,
          let pending = pendingApprovals[toolCallId] else {
        return
    }
...
func canApprove(approverRole: String?, requiredRole: String) -> Bool {
    guard let approverRole = approverRole else { return false }
...
channel.subscribe("approval-response") { message in
    ...
    // Get the trusted role from the JWT claim
    let approverRole = (message.extras as? NSDictionary)?["userClaim"] as? String
