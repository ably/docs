...
blang[javascript].
  Initialize the @Subscriber@ with a "@ClientOptions@":/api/rest-sdk/index#client-options object. You can also optionally configure event handlers for "location updates":#subscriber-subscribe and "asset state changes":#subscriber-subscribe-asset-state-changes during subscriber initialization.
