...
describe('anthropic-human-in-the-loop', () => {
  let agentClient: Ably.Realtime;
  let approverClient: Ably.Realtime;
  let channelName: string;

  beforeAll(() => {
    channelName = `ai:test-anthropic-hitl-${Date.now()}`;
  ...
  it('agent publishes approval request when tool use is triggered', async () => {
    ...
    const requestReceived = waitForMessage(
      approverChannel,
      (m) => m.name === 'approval-request',
      30000,
    );

    // Set up auto-approve so the agent can complete (otherwise it hangs waiting)
    ...
    expect(message.name).toBe('approval-request');
    const data = message.data as { tool: string; arguments: { title: string } };
  ...
  it('agent rejects when approval is denied', async () => {
    ...
    await expect(
      runAgent(agentChannel, "Publish the blog post called 'Rejected Post'"),
    ).rejects.toThrow('Tool call rejected');
