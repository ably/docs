...
<If lang="javascript">
### Path-based API <a id="path-based-api"/>

LiveObjects provides a [path-based API](/docs/liveobjects/concepts/path-object) for accessing and manipulating objects. Instead of working with explicit object instances, you work with paths that resolve to objects dynamically at runtime. Subscriptions observe a location rather than a specific object instance, automatically tracking changes even when objects are replaced.
</If>
