One or more channel namespaces, or channel name prefixes, may be "configured for an app in your dashboard":https://knowledge.ably.com/how-do-i-access-my-app-dashboard-1. When a channel is created whose name is prefixed with one of the namespaces, the channel assumes certain configured attributes associated with that namespace. For example, a channel namespace named "@private@" would match channels named "@private@", "@private:chat@", "@private:chat:mike@".
...
Namespaces are defined and configured via the "application dashboard settings":https://knowledge.ably.com/what-are-channel-rules-and-how-can-i-use-them-in-my-app-1. The namespace attributes that can be configured are:
