...
* @objects-subscribe@ - grants clients read access to LiveObjects, allowing them to get the root object and subscribe to updates.
* @objects-publish@ - grants clients write access to LiveObjects, allowing them to perform mutation operations on objects.

To use LiveObjects, an API key must have at least the @objects-subscribe@ capability. If you want clients to have read-only access, you can create API keys with this capability only, preventing them from calling mutation methods on LiveObjects.

For the purposes of this guide, make sure your API key includes both @objects-subscribe@ and @objects-publish@ "capabilities":/docs/auth/capabilities to allow full read and write access.
...
Next, you need to "attach to the channel":/docs/channels/states. Attaching to a channel also starts a LiveObjects synchronization sequence where the server state on the channel is sent to the client.
...
Use it to get the root object, which is the entry point for accessing and persisting LiveObjects on a channel. It is a "LiveMap":/docs/liveobjects/map instance that always exists on a channel and acts as the top-level node in your LiveObjects state tree. You can get the root object using the @getRoot@ function of LiveObjects:
...
<p>
  Objects that are not descendants of the root object are "unreachable" and will eventually be garbage collected. Read more in the "Live Objects lifecycle events section":/docs/liveobjects/lifecycle#objects-lifecycle.
</p>
</aside>
