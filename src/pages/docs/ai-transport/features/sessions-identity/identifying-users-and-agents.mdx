...
Agents run as trusted code in server environments, so they can directly specify metadata in `message.extras.headers`. This is useful for communicating agent characteristics, such as which model the agent uses, the agent's role in a multi-agent system, or version information.
