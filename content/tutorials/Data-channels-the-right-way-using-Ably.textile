---
title: Data channels the right way using Ably
section: tutorials
index: 105
---
...
In this tutorial, we will build a WebRTC chat with data channel, which lets us chat with multiple people in separate chats.

<a href="/images/tutorials/data-channels.gif" target="_blank">
  <img src="/images/tutorials/data-channels.gif" style="width: 50%" alt="Demo image">
</a>
...
Open up a new file file and name it @index.html@. Add the following code to this file:
...
# Declared a @div@ with the id of @chat@ which holds the input text to type messages as well as the send message @button@.
...
Lastly, we have our @renderMembers@ method, which loops through the @membersList@, and append them as list items to the @ul@ tag with the id of @memberList@. In this method, there is also a check for the @readStatus@ of the conversation, to check if you have an unread message with that user or not.
...
h2.
  default: Step 6 - Sending and receiving messages

We have been able to identify who is online and ready to receive instant messages using WebRTC data channels, now we move to the part where we send and receive messages. Add the following to your @ably-datachannel.js@ file.
...
The chat method receives a single argument, i.e, the @client-id@, which is the client you want to have a chat with. We then check to be sure that the user is not trying to call himself. Although it isn’t possible for the user to call himself, we do this as an extra layer of security. At this point, if the user does not have an active connection with the other peer, it means he is the one initiating the chat, so we create a new connection with that @client_id@, passing the initiator status to be true. After this, we call the @render@ method to render the chat messages.
...
Remember the @receiveMessage@ method we called in our @connection-helper.js@ file we declared earlier? Here it is. we receive the message here with the id of the user who sent it, then push it to the message list of that user. Then we call our render method.

Finally, we defined the much loved @render@ method, which loops through all our messages and sets the read status to false.
...
h2.
  default: Step 7 - Adding audio notifications
...
h2.
  default: Step 7 - Conclusion

In this tutorial, we have seen how to use WebRTC data channels in conjunction with Ably to create realtime chats. Do you have any thoughts on making this lesson better? "Let us know":https://www.ably.io/contact
