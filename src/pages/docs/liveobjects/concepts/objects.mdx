...
// Create and assign a LiveMap with initial entries
LiveMap initialSettings = LiveMap.create(Map.of(
    "theme", LiveMapValue.of("dark"),
    "notifications", LiveMapValue.of(true)));
myObject.set("settings", LiveMapValue.of(initialSettings)).join();
...
<If lang="java">
In the Java SDK the class names `LiveMap` and `LiveCounter` refer to **creation value types** (immutable blueprints), not live handles — live access goes through [`PathObject`](/docs/liveobjects/concepts/path-object)s and [`Instance`](/docs/liveobjects/concepts/instance)s.
</If>
...
myObject.set("app-version", LiveMapValue.of("1.0.0")).join();
...
myObject.get("settings").asLiveMap().set("notifications", LiveMapValue.of(true)).join();
```
</Code>
...
LiveMap profile = LiveMap.create(Map.of(
    ...
    "activity", LiveMapValue.of(LiveCounter.create(0))));
...
myObject.at("profile.activity").asLiveCounter().increment(5).join();
myObject.at("profile.preferences").asLiveMap().set("theme", LiveMapValue.of("light")).join();
```
</Code>
...
Instance oldCounter = myObject.get("visits").instance();
if (oldCounter != null) {
...
// Replace the counter - the old instance becomes unreachable
myObject.set("visits", LiveMapValue.of(LiveCounter.create(0))).join();
```
</Code>
</If>
