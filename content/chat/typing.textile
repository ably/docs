...
To implement a typing indicator you would make use of the presence functionality. Each chat participant that joins a channel would need to first subscribe to that channel and join the presence set of that channel. Whenever a member enters or leaves the presence set of a channel, or updates their member data, an event is emitted to all presence subscribers on that channel. Any client that uses chat participant statuses must "identify":/auth/identified-clients by having a @ClientId@ to be present. The client also requires the @presence@ "capability":/auth/capabilities to enter the chat participant presence set, and the @subscribe@ capability to subscribe to chat participant events.
