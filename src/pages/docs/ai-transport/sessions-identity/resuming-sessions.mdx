...
func example(
    loadChannelName: () -> String,
    loadLastProcessedTimestamp: () -> Date?,
...
// Load persisted session state
let channelName = loadChannelName()
let lastProcessedTimestamp = loadLastProcessedTimestamp()
...
if let lastProcessedTimestamp = lastProcessedTimestamp {
...
do {
    try channel.history(query) { page, error in
        ...
        // Process pages using recursive function to handle pagination
    ...
    print("Error setting up history query: \(error)")
}
```
{/* --- end example code --- */}
</Code>
