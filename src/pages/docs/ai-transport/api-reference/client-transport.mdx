...
intro: "Client transport API reference: options, methods, events, the View interface, and the ActiveTurn type."
---
...
| api | required | `string` | URL of the API endpoint to send user messages to. The Vercel-pre-bound `createClientTransport` makes this optional and defaults it to `"/api/chat"`. |
