...
Ably's platform guarantees [global message ordering](/docs/platform/architecture/message-ordering) on every channel, which is the foundation that enables LiveObjects to resolve concurrent updates deterministically using last-write-wins semantics.
</Aside>
