...
**Account ID**: Displays your unique Ably account identifier, which can be useful for support requests and API integrations.

**Authentication settings**: Configure [two-factor authentication (2FA)](/docs/platform/account/2fa) requirements for all account users. When enabled, users who haven't set up 2FA will be prompted to do so when accessing the account. A setup guide is available for configuring 2FA.

**Change account owner**: As the account owner, you are the only person who can access the account settings page and make changes to packages and billing. Ownership can be transferred by contacting [Ably support](https://ably.com/support).

**Delete profile**: Provides the option to request account deletion. This requires contacting [Ably support](https://ably.com/support) to process the deletion.
...
**Account owner**: Displays the primary owner of the account with full permissions.

**Invite new users**: You can invite team members by email to collaborate on your Ably account.

**Role management**: Assign different [roles](/docs/platform/account/users#roles) to team members, including admin, developer, and billing roles. This allows for granular control over what each team member can access.

**Invitation status**: Track pending invitations, including expired invitations that can be resent.

**Remove users**: Remove team members who no longer need access to the account.
...
**Current package**: Shows your active subscription tier (Free, Standard, Pro, or Enterprise) with its included features and capacity limits.
...
**Billing details form**: Enter company name, billing email address, full address, telephone, timezone, and VAT number for invoicing purposes.

**Upgrade options**: Buttons to upgrade from your current plan to higher tiers.
...
**Payment method information**: Details about your saved payment methods.

**Invoice history**: A list of all past invoices once you have an upgraded account. For free accounts, this section shows a message that invoices will appear when the account is upgraded.

**Change account owner**: A reminder that only the account owner can access billing pages and make package changes.
...
**Limits summary**: A quick overview showing your monthly allocation versus actual usage for messages, data transferred, concurrent connections, and concurrent channels.

### Detailed account limits <a id="detailed-limits"/>

**Messages & bandwidth**: Monthly limits (6M messages, 11.5 GiB for Free tier), hourly limits, and rate per second limits.
