---
title: "Channels append"
meta_description: "Append data to a message on an Ably Pub/Sub channel using the CLI."
meta_keywords: "ably cli, cli, channels, append, message, pub/sub, ably channels append"
---

Use the `ably channels append` command to append data to a message on an Ably Pub/Sub channel.

## Synopsis

<Code>
```shell
ably channels append <channel-name> <message-serial> <message> [flags]
```
</Code>

## Arguments

### `channel-name` <a id="channel-name"/> <RequiredBadge />

The name of the channel containing the message.

### `message-serial` <a id="message-serial"/> <RequiredBadge />

The serial of the message to append data to.

### `message` <a id="message"/> <RequiredBadge />

The data to append. This can be plain text or JSON.

## Flags

### `--description` <a id="description"/>

A description for the appended data.

### `--encoding | -e` <a id="encoding"/>

The encoding of the message, such as `json/utf-8`.

### `--name | -n` <a id="name"/>

The event name for the appended message.

### `--client-id` <a id="client-id"/>

A client ID to use when appending the message.

## Global flags

* [`--json`](/docs/cli#json) or [`--pretty-json`](/docs/cli#pretty-json) — JSON output. Mutually exclusive flags.
* [`--verbose`](/docs/cli#verbose) — Enable verbose logging. Can be combined with `--json` or `--pretty-json`.

## Examples

Append data to a message:

<Code>
```shell
ably channels append my-channel "01234567890:0" '{"extra": "data"}'
```
</Code>

Append with an event name:

<Code>
```shell
ably channels append my-channel "01234567890:0" "additional info" --name update
```
</Code>

Append with a description:

<Code>
```shell
ably channels append my-channel "01234567890:0" '{"note": "correction"}' --description "Added correction"
```
</Code>

Append plain text data to a message:

<Code>
```shell
ably channels append my-channel "01234567890:0" "Appended plain text"
```
</Code>

Append data and output the result in JSON format:

<Code>
```shell
ably channels append my-channel "01234567890:0" '{"data":"appended"}' --json
```
</Code>

Append data and output the result in formatted JSON:

<Code>
```shell
ably channels append my-channel "01234567890:0" '{"data":"appended"}' --pretty-json
```
</Code>

## See also

* [Channels](/docs/cli/channels) — Explore all `ably channels` commands.
* [CLI reference](/docs/cli) — Full list of available commands.
