...
The following is an example of a batch publish request (using the [`batchPublish()`](/docs/api/rest-sdk#batchPublish) method on SDKs that support it and the [`request()`](/docs/api/rest-sdk#request) method on others):
