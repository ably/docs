...
* @(RTC8)@ Authentication upgrades are only possible when the client is @CONNECTED@ or @CONNECTING@. If @auth.authorise@ is called with @AuthOptions#force@ set to @true@ and the state is @CONNECTED@ or @CONNECTING@, the client will obtain a new token, disconnect the current transport and resume the connection (see "@RTN15c@":#RTN15c for details on resume). A test should exist that performs an upgrade of capabilities without any message loss during the upgrade process. A test should also for an auth failure which should result in the connection entering the @FAILED@ state
