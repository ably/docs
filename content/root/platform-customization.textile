...
- @rest.ably.io@ - this endpoint is used for all REST-based requests and is optimized for all standard HTTP-based request types including all "REST requests":/docs/api/rest-api, "authentication requests":/auth, and Comet and JSONP fallback when WebSockets cannot be used.
