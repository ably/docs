...
Although <If lang="javascript,nodejs">[`attach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#attach)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`attach()`](/docs/api/realtime-sdk/channels#attach)</If> can be called explicitly, it is more common for a client to [subscribe](/docs/pub-sub#subscribe) directly to a channel, which will automatically initiate the attach. This is also known as an implicit attachment.
...
Normally, errors in attaching to a channel are communicated through the <If lang="javascript,nodejs">[`attach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#attach)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`attach()`](/docs/api/realtime-sdk/channels#attach)</If> callback. For implicit attaches, where a client only calls `subscribe()`, there is no callback.In these instances, listen for [channel state changes](#listen) to monitor errors.
...
A client can detach from a channel so that it no longer receives any messages published to it. Detaching is different to unsubscribing from a channel because <If lang="javascript,nodejs">[`unsubscribe()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#unsubscribe)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`unsubscribe()`](/docs/api/realtime-sdk/channels#unsubscribe)</If> is a client-side operation. The Ably platform does not know that a client has unsubscribed and will continue to stream messages to that client until <If lang="javascript,nodejs">[`detach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#detach)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`detach()`](/docs/api/realtime-sdk/channels#detach)</If> is called.
...
Register a listener to monitor the current channel state. This can be a listener for the first occurrence, using <If lang="javascript,nodejs">[`once()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#once)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`once()`](/docs/api/realtime-sdk/channels#once)</If>, or for every change using <If lang="javascript,nodejs">[`on()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`on()`](/docs/api/realtime-sdk/channels#on)</If>.

Use the <If lang="javascript,nodejs">[`on()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`on()`](/docs/api/realtime-sdk/channels#on)</If> method to register a listener for a specific channel state:
...
You can also use the <If lang="javascript,nodejs">[`on()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`on()`](/docs/api/realtime-sdk/channels#on)</If> method to register a listener for all channel state changes:
...
Listeners are passed a <If lang="javascript,nodejs">[`ChannelStateChange`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`ChannelStateChange`](/docs/api/realtime-sdk/channels#channel-state-change)</If> object in the first argument. This object has the following properties:
...
Use the <If lang="javascript,nodejs">[`off()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#off)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`off()`](/docs/api/realtime-sdk/channels#off)</If> method to remove listeners:
...
If you receive such an event, you'll know there may be messages you've missed on the channel, and if necessary you can use <If lang="javascript,nodejs">[history](/docs/pub-sub/api/javascript/realtime/realtime-channel#history)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[history](/docs/api/realtime-sdk/channels#history)</If> to retrieve them.
...
Whilst fatal errors won't get better on their own, they are fixable. For example, if a channel goes into the `FAILED` state due to the client not having the right capabilities to attach to it, that client could call <If lang="javascript,nodejs">[`authorize()`](/docs/pub-sub/api/javascript/realtime/auth#authorize)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`authorize()`](/docs/api/realtime-sdk/authentication#authorize)</If> to obtain a new token which does have the right capabilities, then call <If lang="javascript,nodejs">[`attach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#attach)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`attach()`](/docs/api/realtime-sdk/channels#attach)</If> on the channel. The library will not automatically reattach in the `FAILED` state, however explicit calls to <If lang="javascript,nodejs">[`attach()`](/docs/pub-sub/api/javascript/realtime/realtime-channel#attach)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`attach()`](/docs/api/realtime-sdk/channels#attach)</If> will make the client try again.
...
For every channel `ATTACHED` and `UPDATE` event, the <If lang="javascript,nodejs">[`ChannelStateChange`](/docs/pub-sub/api/javascript/realtime/realtime-channel#on)</If><If lang="csharp,flutter,go,java,objc,python,ruby,swift">[`ChannelStateChange`](/docs/api/realtime-sdk/types#channel-state-change)</If> object contains a `resumed` attribute. When `true`, there has been no loss of continuity from the last time the channel was attached. When `false`, there has been a loss of continuity.
