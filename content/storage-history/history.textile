...
Two minutes of message history is available to retrieve by default. This can be extended by configuring the "storage options":/message-storage/storage for an app.
...
* History can return up to 1000 messages in a single call. Rewind returns at most 100 messages.
...
The following query parameters can be included in the @options@ object when making a call to @history()@. 
