...
Client-side applications must use [JWT authentication](/docs/platform/auth/token/jwt). API keys should never be exposed in client-side code because they don't expire and cannot be scoped to specific users.
</Aside>

Chat requires an authenticated client with a `clientId` to identify users. The recommended approach is:

1. **Client-side apps** (browsers, iOS, Android): Use [JWT authentication](/docs/platform/auth/token/jwt) with `authCallback` to fetch JWTs from your server
...
For more guidance, see the [capabilities documentation](/docs/platform/auth/capabilities) and [Chat authentication](/docs/chat/authentication) for room-scoped examples.
...
See [Token authentication](/docs/platform/auth/token#choosing) for detailed guidance on when to use JWT vs Ably Tokens.
