...
| Browser crash | All progress lost, task must be restarted | New session hydrates with complete history, including in-progress response |
...
| Multi-user session | Agent must stream to a connection per-user | Agent publishes to single channel, thousands of subscribed users receive the response |
...
Different models and frameworks use different events to signal streaming state, for example start events, stop events, tool calls, and content deltas. When you publish a message to an Ably channel, set the [message name](/docs/messages#properties) to the event type your client expects. This allows your frontend to handle each event type appropriately without parsing message content.
