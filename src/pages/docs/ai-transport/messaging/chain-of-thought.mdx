...
{/*
Swift test harness:
func example(realtime: ARTRealtime, stream: [(type: String, text: String, responseID: String)]) async throws {
        ...
        MainActor.assumeIsolated {
            ...
            case "reasoning":
                if let data = message.data as? String {
                    responses[responseID]?.reasoning.append(data)
                }
            default:
                break
            }
            ...
            // Display the reasoning and response for this turn
            print("Response \(responseID):", responses[responseID] as Any)
        }
    })
}
```
...
for event in stream {
    if event.type == "start" {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
            reasoningChannel.publish("reasoning", data: event.text, callback: { error in
                if let error {
                    continuation.resume(throwing: error)
                } else {
                    continuation.resume()
...
// Subscribe to all messages on the main channel
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
    conversationChannel.subscribe(attachCallback: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
func onClickViewReasoning(responseID: String) async throws {
    ...
    // Subscribe to reasoning messages
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
        reasoningChannel.subscribe(attachCallback: { error in
            if let error {
                continuation.resume(throwing: error)
            } else {
                continuation.resume()
