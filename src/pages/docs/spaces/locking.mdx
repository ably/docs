...
Ably's SDKs automatically [resolve edge network failures](/docs/platform/architecture/edge-network) within 30 seconds, so brief connectivity issues don't cause members to lose their locks unexpectedly.
</Aside>
