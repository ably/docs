...
Having two options to register your devices gives you flexibility of implementation as per your use case. 

In case of direct registration, you rely on the device to register itself and attach to the channel. This means, you'll have less amount of code to be handled on your server side. 

However, this also means that you trust your devices to handle this securely and efficiently, which can of course be solved by using the token authentication strategy. 

Even though direct registration is a simpler option, if you wish to own control of all of your devices on your server side (even though our Push Admin API already lets you do this) then registration via server is the way to go. 
