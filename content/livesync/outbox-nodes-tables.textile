...
<aside data-type='note'>
  <p>The Database Connector will always keep at least one outbox record in the table, specifically the one with the latest @sequence_id@. This record is still marked as processed via a tombstone and will be deleted when subsequent records are written to the outbox. This approach ensures that the backend endpoint called by your "sync function":/livesync/frontend#create-sync can always obtain the @sequence_id@ at the time that the state is read from the database.</p>
</aside>

h3(#why-outbox). Why use an outbox pattern?

Clients using the "Models SDK":/livesync/frontend observe changes to the database state by consuming a stream of ordered database change events. An update to the data in the database must happen atomically, with the corresponding change event broadcast to all clients, so that they see a consistent view of the data.
