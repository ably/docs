...
You need the @channel-metadata@ capability for the channel for which you want to receive occupancy events. You can set this for your API key using the "Ably dashboard":https://ably.com/dashboard, or you can set it using a capability added to a "token request":/auth/token#token-request-process. See the "capability documentation":/auth/capabilities#capability-operations for further details.
