...
blang[nodejs].
  The Ably Realtime and REST library is available as an "NPM module":https://www.npmjs.com/package/ably.
  ...
  Now, let's create a @publisher.js@ with following content:
...
blang[nodejs].
  ...
  In order to encrypt our messages we'll need a cipher key (@SECRET@). Let's add one to our @constants.js@:
...
blang[nodejs].
  ...
  **What is going on in the code block above?**
  ...
  Execute the following to start the subscription
  ```[sh]
    npm run app:subscribe
  ```

  Execute the following to publish a message
  ```[sh]
    npm run app:publish
  ```
