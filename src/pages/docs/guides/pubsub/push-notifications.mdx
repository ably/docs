...
```javascript
// On device: Get platform token and send it to your server
...
await ablyClient.push.admin.deviceRegistrations.save({
  id: generateDeviceId(clientId, platform),
  platform: platform,
  formFactor: 'phone',
  clientId: clientId,
  push: {
...
* **push-admin:** Full access to push admin API - register any device, subscribe any device to channels, send direct notifications. Only for 
servers.
