...
| Type | Origin | Handle |
| --- | --- | --- |
| Own Run | This client sent the HTTP POST that created the Run. | An `ActiveRun` returned from `view.send`, with `runId`, `inputCodecMessageId`, `cancel()`, and `toInvocation()`. |
