...
### Subscribe to channel push <a id="step-3-subscribe-channel"/>

Add the following functions to subscribe and unsubscribe the device from channel-based push:
...
async function subscribeToChannel() {
  ...
  try {
    ...
    setStatus('Subscribed to push on channel: ' + CHANNEL_NAME);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    setStatus('Failed to subscribe: ' + message);
  }
}
...
async function unsubscribeFromChannel() {
  ...
  try {
    ...
    setStatus('Unsubscribed from push on channel: ' + CHANNEL_NAME);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    setStatus('Failed to unsubscribe: ' + message);
  }
}
```
</Code>

To test push notifications using the Ably CLI, send a push directly to the client ID:
...
return (
  <main style={{ fontFamily: 'Arial, sans-serif', padding: '20px', maxWidth: '900px', margin: '0 auto' }}>
    ...
    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
      <button onClick={activatePush}>Activate Push</button>
      <button onClick={deactivatePush}>Deactivate Push</button>
      ...
      <button onClick={sendPushToDevice}>Send to Device</button>
      <button onClick={sendPushToClient}>Send to Client</button>
      <button onClick={sendPushToChannel}>Send to Channel</button>
...
To test channel push, click **Subscribe to Channel**, then publish a message with a `push` `extras` field using the Ably CLI:
...
Add the following functions inside `PushPage` to send push notifications programmatically using the Push Admin API:
