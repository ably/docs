...
<Code>
```javascript
const pending = session.view.getMessagesWithIds().find(({ message }) =>
...
if (pending) {
  ...
  const run = await session.view.send(
    ...
    { runId: session.view.runOf(pending.codecMessageId).runId },
  );
...
await run.addEvents([
  {
    kind: 'event',
    codecMessageId: pendingAssistantCodecMessageId,
    events: [
      {
