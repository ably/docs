...
Webhooks are "rate-limited":/docs/pricing/limits#integration and best suited for low to medium traffic. If you need to process more than 25 events per second, consider using "Message Queues":/docs/integrations/queues or "Firehose":/docs/integrations/streaming instead.
...
h3(#vs). Single vs Batched requests
...
h4(#single). Single request details
...
h4(#batched). Batch request details
...
# "Configure a webhook":#configure with the URL set to the RequestBin endpoint, and ensure you have chosen to "batch":/docs/integrations/outbound-streaming/overview#batched-events messages and are using a key to sign each webhook request
