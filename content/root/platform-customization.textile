...
Ably client libraries connect to the @main.realtime.ably.net@ endpoint by default. This endpoint is used by both WebSocket and other realtime connections from the Ably client libraries, as well as all REST-based requests. It is optimized for all standard HTTP-based request types, including all "REST requests":/docs/api/rest-api, "authentication requests":/docs/auth, and Comet and JSONP fallback when WebSockets cannot be used.
...
These use Ably's existing wildcard certificate, which supports @*.realtime.ably.net@ and different fallbacks to the following range of subdomains @*.[a-e]-fallback.main.cluster.ably-realtime.com@.
...
- *REST*: @https://[ENDPOINT].realtime.ably.net/time@ (e.g. https://endpoint-example.realtime.ably.net/time)
