...
* "For experienced developers":#for-experienced-developers - Recommendations that Ably has compiled for developers to help you avoid common issues.

h2(#ably-101). Ably 101

Ably Realtime is a cloud-based platform that enables you to easily integrate realtime functionality into your applications. Just plug and play!
...
**Important:** Do not use token authentication on the server, as this will result in unnecessary overhead - the server will periodically require token requests from Ably to authenticate. Simply use basic authentication server-side.
...
Depending on your use case, you may only need to use the REST interface on your server. For example, the server may only need to authenticate with Ably, and then act as an auth server for clients. You might then use the realtime interface on your client, so it can both subscribe and publish messages.

The PHP and Python SDKs only have a REST interface. While the Ruby SDK implements both realtime and REST interfaces, there is also a Ruby SDK available that implements only the REST interface. The full list of available SDKs can be found on the "SDK download":https://ably.com/download page.
...
All Ably client libraries "use TLS by default":https://knowledge.ably.com/are-messages-sent-to-and-received-from-ably-securely-using-tls. This ensures that data is transmitted to and from Ably securely. However, messages are not encrypted _within_ the Ably system. Using the "encryption feature":/realtime/encryption of our client libraries, you can ensure that message payloads are opaque: they can never be decrypted by Ably but only by other clients that share your secret key.
