...
* **Server-side batching:** Reduce costs and network overhead by grouping messages before delivery. For example, [batching messages](#server-side-batching) over a 500ms interval can reduce the number of outbound messages by up to 8x in high-traffic streams.
...
How you authenticate is also key. To balance security and experience, you want short-lived tokens that can be easily revoked if a users is misbehaving or needs their permissions changed, but also avoids the overhead of issuing new tokens every 2 minutes. **In production apps, you should not use API keys for client-side authentication**. You can use them server-side, but as they are long-lived they are much more difficult to revoke in a hurry.
...
Ably's flexible [integration capabilities](/docs/integrations) make it easy to connect your chat system with AI services. Through Ably's integrations, you can automatically route messages to:
