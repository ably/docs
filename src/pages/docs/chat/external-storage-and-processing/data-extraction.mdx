...
2. [Streaming](#streaming) sends messages to external systems like [Kafka](/docs/platform/integrations/streaming/kafka), [Kinesis](/docs/platform/integrations/streaming/kinesis), [SQS](/docs/platform/integrations/streaming/sqs), [AMQP](/docs/platform/integrations/streaming/amqp), or [Pulsar](/docs/platform/integrations/streaming/pulsar).
3. [Queues](#queues) route messages to [Ably-managed queues](/docs/platform/integrations/queues) for consumption by your services.
...
When configuring an integration rule in your Ably dashboard, set the channel filter to a regular expression that matches the room names you want to target. For example, `^support:.*` matches all rooms starting with `support:`. Set the event type to `channel.message` to forward chat messages and exclude presence events. Enable enveloped messages to receive full message metadata including `serial`, `version`, and `headers`.
...
- **Targeted integrations**: Apply different processing rules to different types of rooms (e.g., AI moderation for public game chat, analytics for support chats)
- **Simplified filtering**: Use simple regex patterns like `^gamechat:.*` to match all rooms in a namespace
- **Cost optimization**: Only forward messages that need processing, reducing integration costs
- **Organizational clarity**: Room names clearly indicate their purpose and context
...
* `action` indicates the message type: `message.created`, `message.updated`, `message.deleted`, or `message.summary` for reactions.
...
For Lambda, AWS automatically retries failed invocations up to two times with delays of 1 minute, then 2 minutes. For generic HTTP webhooks, Ably retries for a limited window. Use the [`[meta]log` channel](/docs/metadata-stats/metadata/subscribe#log) to detect failures (see [monitoring section](#monitoring-performance)).
...
You are responsible for managing and maintaining the streaming system. Messages will be lost if the system is unavailable, so reliability depends on your infrastructure. This approach has higher operational overhead compared to webhooks or Ably Queues.
...
You can use the statistics entries to monitor specific integration methods such as webhooks, queues, and streaming independently. Use the `requestId` from error events to correlate with your own system logs.
