...
| --- | --- | --- |
| current | The latest occupancy data cached from realtime events. Returns `null` if no occupancy events have been received yet since room attachment. Requires `enableEvents` to be `true` in the [room's occupancy options](/docs/chat/api/kotlin/rooms#get). | <Table id='OccupancyData'/> or null |
...
This is a suspend function. It returns the current occupancy data, or throws an [`ErrorInfo`](/docs/chat/api/kotlin/chat-client#errorinfo) on failure.
...
val subscription = room.occupancy.subscribe { event ->
    val (connections, presenceMembers) = event.occupancy
