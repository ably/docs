...
Ably delivers messages to persistently connected subscribers on a channel in the order they were published when using the Realtime client library. Each message sent on a realtime connection has a unique incrementing serial number, enabling this ordering guarantee.
