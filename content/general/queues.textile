...
Once your "Ably Queue":/docs/general/queues is provisioned, and your "Ably Queue rules":#setup are configured, there are a number of ways we recommend customers can debug the configured rules and queues:
...
**Note:** the @messages@ attribute is an @Array@ so that future envelope options may allow messages to be bundled into a single envelope ("Webhooks":/docs/general/webhooks can batch messages). However, with the current queue rule design, an envelope will only ever contain one message.
...
**Note:** the @presence@ attribute is an @Array@ so that future envelope options may allow presence messages to be bundled into a single envelope ("Webhooks":/docs/general/webhooks can batch messages). However, with the current queue rule design, an envelope will only ever contain one presence message.
