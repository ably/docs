...
* @(G4)@ All REST requests and "WebSocket":https://www.ably.io/concepts/websockets connections to Ably must include the current API version @0.8@. Should any new API version with breaking changes be released, the client library will continue to use the API version explicitly requested
...
* @(RTN1)@ @Connection@ connects to the Ably service using a "websocket":https://www.ably.io/concepts/websockets connection. The "ably-js library":https://github.ably.io/ably/ably-js supports additional transports such as Comet and XHR streaming; however non-browser client libraries typically use only a "websocket transport":https://www.ably.io/concepts/websockets
* @(RTN2)@ The default host used for realtime "websocket connections":https://www.ably.io/concepts/websockets is @realtime.ably.io@, and the following query string params should be used when opening a new connection:
...
* @(RTN6)@ A @Connection@ is successful and considered @CONNECTED@ once the "websocket":https://www.ably.io/concepts/websockets connection is open and the initial @CONNECTED@ @ProtocolMessage@ has been received
