...
Ably currently supports the following sources for all rule types:

* "Messages":/realtime/messages#messages - messages trigger function calls as soon as they are published on a channel.
* "Presence":/realtime/presence - presence events trigger function calls when clients enter, update their data, or leave channels.
...
* "Occupancy":/realtime/inband-occupancy#occupancy-events - occupancy events relate to the number and type of occupants in the channel.
...
- "Source":#sources := Choose which of @Message@, @Presence@, @Channel Lifecycle@, or @Occupancy@ events on channels should activate this Event Rule.
