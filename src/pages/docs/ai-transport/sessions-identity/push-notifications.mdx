...
func pushNotifications1(ably: ARTRealtime) async throws {
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.publish("request", data: [
        "prompt": "Analyse last quarter's sales data and generate a report",
        "notifyOnComplete": true
...
@MainActor
func pushNotifications2(channel: ARTRealtimeChannel) {
    // insert example code here
}
*/}
```swift
// Agent code
var notifyUserOnComplete = false

channel.subscribe("request") { message in
    MainActor.assumeIsolated {
        let data = message.data as? [String: Any]
        notifyUserOnComplete = data?["notifyOnComplete"] as? Bool == true
        // Begin processing the task...
    }
}
```
...
func pushNotifications3() {
    // insert example code here
}

Stubs: getUserSettings(_:) -> UserSettings, UserSettings.pushNotificationsEnabled
*/}
```swift
// Agent code
func shouldNotifyUser(_ userId: String) async -> Bool {
    // Check the user's notification preference from your application's settings
...
func pushNotifications4() {
    // insert example code here
}

Stubs: sendPushNotification(_:_:_:)
*/}
```swift
// Agent code
func onTaskComplete(channel: ARTRealtimeChannel, userId: String, result: Any) async throws {
    // Always publish results to the channel for history and connected clients
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
        channel.publish("result", data: result) { error in
            if let error {
                continuation.resume(throwing: error)
            } else {
                continuation.resume()
    ...
    // If user is offline, also send a push notification
    let clientIDs = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[String], Error>) in
        channel.presence.get { members, error in
            if let error {
                continuation.resume(throwing: error)
            } else {
                continuation.resume(returning: members!.compactMap { $0.clientId })
            }
        }
    }
    let userIsOnline = clientIDs.contains(userId)
...
func pushNotifications5() {
    // insert example code here
}
*/}
```swift
// Agent code
func shouldSendPush(channel: ARTRealtimeChannel, userId: String) async throws -> Bool {
    let clientIDs = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[String], Error>) in
        channel.presence.get { members, error in
            if let error {
                continuation.resume(throwing: error)
    ...
    let userDevices = clientIDs.filter { $0 == userId }
...
func pushNotifications6(ably: ARTRealtime) {
    // insert example code here
}
*/}
```swift
...
ably.push.admin.publish(recipient, data: data, callback: nil)
```
</Code>
...
func pushNotifications7(ably: ARTRealtime, channel: ARTRealtimeChannel, userId: String) {
    // insert example code here
}
*/}
```swift
// Agent code
let recipient: [String: Any] = ["clientId": userId]
...
func pushNotifications8() {
    // insert example code here
}

Stubs: navigateToSession(_:_:)
*/}
```swift
// Client code - handling a push notification tap
func onNotificationTap(notification: [String: Any]) {
...
func pushNotifications9(ably: ARTRealtime, channel: ARTRealtimeChannel, userId: String, sessionId: String, result: Any) {
    // insert example code here
}

Stubs: generateSummary(_:)
*/}
```swift
// Agent code
