...
<MethodSignature>{`ChatClient(realtimeClient: AblyRealtime, clientOptions: ChatClientOptions = buildChatClientOptions()): ChatClient`}</MethodSignature>
...
<MethodSignature>{`ChatClient(realtimeClient: AblyRealtime, init: MutableChatClientOptions.() -> Unit): ChatClient`}</MethodSignature>
