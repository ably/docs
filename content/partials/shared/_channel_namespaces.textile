One or more channel namespace rules may be "configured for an app in your dashboard":https://support.ably.com/solution/articles/3000030053-how-do-i-access-my-app-dashboard. These are rules which apply to a channel based on its 'namespace'. The namespace is the first colon-delimited segment of its name (from the start up to and including the last character before the @:@). If the channel name contains no colon, the namespace is the entire channel name.
