...
intro: "Your users see the conversation when they come back to it. AI Transport loads history from the session itself, with gapless continuity into the live stream."
---
...
- A client without `history` capability cannot load anything beyond the live subscription window. Capability scoping is part of [authentication](/docs/ai-transport/getting-started/authentication).
- A regenerated [branch](/docs/ai-transport/features/branching) shows up in history with its `forkOf` header. The view's branch selection determines which sibling renders.
...
Persist completed runs to your own store as they end, then [hydrate from the database](/docs/ai-transport/features/database-hydration) and reconcile that stored history with the live session on reload. The session handles live and in-progress activity.
...
- [Database hydration](/docs/ai-transport/features/database-hydration): retain conversations beyond channel retention by reconciling a stored seed with the live session.
