...
This pattern enables bandwidth optimization because you don't stream detailed telemetry for every vehicle until the user actually needs it. You can implement this by [attaching](/docs/channels/states#attach) to a channel when the user drills down and [detaching](/docs/channels/states#detach) when they navigate away, this would avoid unnecessary outbound messages. On the publisher side, you can use [occupancy](/docs/presence-occupancy/occupancy) to check whether any subscribers are present before publishing detailed data, avoiding unnecessary messages when no one is viewing.
...
#### Message conflation: Latest-value batching <a id="message-conflation-for-latest-value-scenarios"/>
...
function generateViewerToken() {
  const header = {
    typ: 'JWT',
    alg: 'HS256',
    kid: 'your-api-key-name' // The name of your Ably API key, in the format appId.keyId
  };
  ...
  return jwt.sign(claims, 'your-api-key-secret', { header });
}
