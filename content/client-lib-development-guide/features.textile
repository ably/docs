...
** @(RSA13)@ @Auth#clientIdValidated@ boolean is @true@ when the @Auth#clientId@ has been validated through the inspection of a @clientId@ attribute of a signed @TokenRequest@, @TokenDetails@ or a @ProtocolMessage#connectionDetails@ object. When @clientIdValidated@ is @true@ and @Auth#clientId@ is @null@, the client is anonymous and cannot assume a @clientId@ in any operations. However, when @clientIdValidated@ is @false@, a @null@ value @Auth#clientId@ does not necessarily mean that the client is anonymous as the client ID has simply not yet been confirmed by Ably following authentication or establishing a realtime connection
