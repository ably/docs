...
This call back tells us we have deactivated push, there is no reason for us to do that at the moment so we reactivate push. Now we need to handle what happens when we have registered by adding the following:
