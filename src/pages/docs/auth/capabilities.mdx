...
JWTs can contain channel-scoped user claims for trusted metadata that other clients can read. See [JWT authentication — channel-scoped claims](/docs/auth/jwt#channel-claims) for details and examples.

## Per-connection publish rate limits <a id="jwt-limits"/>

JWTs can specify per-connection publish rate limits to restrict how many messages a client can send. See [JWT authentication — rate limits](/docs/auth/jwt#rate-limits) for details and examples.
