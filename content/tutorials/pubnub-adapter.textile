...
Full usage notes for the adapter are available separately in a support article: "Using the Ably Pubnub Protocol Adapter":https://knowledge.ably.com/using-the-ably-pubnub-protocol-adaptor. Please note that this tutorial assumes that you already have Pubnub client libraries set up with Pubnub, and just want to convert it to use Ably.

To take advantage of "Ably features not supported by the PubNub client libraries":https://www.ably.io/compare/ably-vs-pubnub such as "connection state recovery":https://knowledge.ably.com/connection-state-recovery you’ll need to fully migrate over to Ably. But in this tutorial, we'll see how you can get started using the PubNub protocol adapter.
...
blang[ruby].
  ...
  And then run the demo locally by adding your "Ably API key":https://knowledge.ably.com/setting-up-and-managing-api-keys to @example.rb@ and running the demo with @bundle exec ruby example.rb@

h2. Next steps

# "Review the many reasons to migrate from Pubnub to Ably":https://www.ably.io/compare/ably-vs-pubnub
# Full usage notes for the Pubnub adapter are available at: "Using the Ably Pubnub Protocol Adapter":https://knowledge.ably.com/using-the-ably-pubnub-protocol-adaptor.
# Consider gradually transitioning to the Ably client libraries, for platforms which have them. They will be faster and more efficient: faster because native Ably libraries can use "websockets as a transport":https://knowledge.ably.com/which-transports-are-supported (Pubnub can only do "long polling":https://www.ably.io/topic/long-polling), and more efficient because using native Ably libraries avoids the overhead of protocol translation.
