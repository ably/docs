...
describe('anthropic-human-in-the-loop', () => {
  ...
  beforeAll(() => {
    channelName = `test-anthropic-hitl-${Date.now()}`;
  ...
  it('publishes an approval request when tool use is triggered', async () => {
    ...
    // Verify arguments (Anthropic returns input as an object, not a string)
    expect(request.arguments).toBeTruthy();
