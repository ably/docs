...
<Aside data-type='usp'>
Ordered space events

Every space event is assigned a unique serial number, ensuring all members observe state changes in the [same order](/docs/platform/architecture/message-ordering). This keeps space membership, locations, and lock states consistent across all connected clients.
</Aside>
