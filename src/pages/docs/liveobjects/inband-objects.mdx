...
// Or enable notification mode
ChannelOptions notificationOptions = new ChannelOptions();
notificationOptions.params = Map.of("objects", "notification");
Channel notificationChannel = realtime.channels.get("my-channel", notificationOptions);
...
## Limitations <a id="inband-objects-limitations"/>
...
<Aside data-type='important'>
  If your use case requires storing more than 64KB in a single object, you should use the <If lang="java">[LiveObjects plugin](/docs/liveobjects/quickstart/java)</If><If lang="javascript">[LiveObjects plugin](/docs/liveobjects/quickstart/javascript)</If><If lang="swift">[LiveObjects plugin](/docs/liveobjects/quickstart/swift?lang=swift)</If> instead of inband objects. The LiveObjects plugin supports partial object synchronization, which can efficiently handle objects of any size by streaming them in chunks.
</Aside>
...
<If lang="javascript,nodejs">[Subscribe](/docs/pub-sub/api/javascript/realtime/realtime-channel#subscribe)</If><If lang="swift">[Subscribe](/docs/api/realtime-sdk/channels?lang=swift#subscribe)</If><If lang="java">[Subscribe](/docs/api/realtime-sdk/channels#subscribe)</If> to `[meta]objects` messages like you would any other message on the channel. For convenience, use a message name filter to only receive messages with the name `[meta]objects` in your listener:
...
// Subscribe to [meta]objects messages
channel.subscribe("[meta]objects", message ->
    System.out.println("Received inband objects message: " + message.data));
```
</Code>
...
### Enable objects mode <a id="objects-enable"/>

Enable objects mode using the `objects` [channel parameter](/docs/channels/options#objects) with a value of `'objects'`:
...
// Subscribe to [meta]objects messages in objects mode
channel.subscribe("[meta]objects", message ->
    System.out.println("Received object: " + message.data));
```
</Code>


### Objects message format <a id="inband-objects-message-format"/>
...
| Property | Description |
| -------- | ----------- |
| `link`   | A relative URL path to fetch the current state of the channel object on the channel via the [REST API](/docs/liveobjects/rest-api-usage#fetch-channel-object). This can be used directly with the Ably REST client's <If lang="javascript,nodejs">[request](/docs/pub-sub/api/javascript/rest/rest-client#request)</If><If lang="swift">[request](/docs/api/rest-sdk?lang=swift#request)</If><If lang="java">[request](/docs/api/rest-sdk#request)</If> method. |
...
// Subscribe to [meta]objects messages in notification mode
channel.subscribe("[meta]objects", message -> {
    String link = ((JsonObject) message.data).get("link").getAsString();
    System.out.println("Objects operation received, fetching state from: " + link);

    // Fetch the current state using the REST API
    try {
        HttpPaginatedResponse response = ablyRest.request("GET", link, null, null, null);
        if (response.items().length > 0) {
            System.out.println("Current state: " + response.items()[0]);
        }
    } catch (AblyException e) {
        System.err.println("Failed to fetch state: " + e);
    }
});
```
</Code>
