...
When the "Message annotations, updates, deletes and appends" rule is enabled, Ably persists messages irrespective of whether or not persistence has also been explicitly enabled. This increases usage since [Ably charges for persisting messages](https://faqs.ably.com/how-does-ably-count-messages).
</Aside>
...
The `ai:` namespace is just a naming convention used in this guide. There's nothing special about it - you can use any namespace pattern you like, as long as your channel name matches the configured rule.
</Aside>
...
{/*
Swift test harness:
func exampleCitations1(realtime: ARTRealtime) async throws {
...
let msgSerial: String = try await withCheckedThrowingContinuation { continuation in
    channel.publish("response", data: response) { result, error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume(returning: result!.serials[0].value!)
        }
    }
}

// Add citations by annotating the response message
let citation1 = ARTOutboundAnnotation(
    id: nil,
    type: "citations:multiple.v1",
    clientId: nil,
    name: "science.nasa.gov",
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.annotations.publish(forMessageSerial: msgSerial, annotation: citation1) { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
let citation2 = ARTOutboundAnnotation(
    id: nil,
    type: "citations:multiple.v1",
    clientId: nil,
    name: "en.wikipedia.org",
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    channel.annotations.publish(forMessageSerial: msgSerial, annotation: citation2) { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
{/*
Swift test harness:
func exampleCitations2(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
{/*
Swift test harness:
func exampleCitations3(realtime: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
channel.annotations.subscribe { annotation in
    if annotation.action == .create,
       annotation.type == "citations:multiple.v1" {
        let data = annotation.data as! NSDictionary
