...
```[objc]
ARTMessage *message = [[ARTMessage alloc] initWithName:@"example" data:@"realtime data"];
message.extras = @{
    @"push": @{
        @"notification": @{
...
[[realtime.channels get:@"pushenabled:foo"] publish:@[message] callback:^(ARTErrorInfo *_Nullable error) {
    // Check error.
}];
```
```[swift]
