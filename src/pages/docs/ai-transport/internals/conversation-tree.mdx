...
Branch selection is message-anchored: the branch decision lives on a `codec-message-id`, not on a `runId`. The View exposes `branchSelection(codecMessageId)`, whose returned handle carries a `select(index)` verb, so a UI renders navigation arrows next to a bubble without knowing whether it is an input or a run.
