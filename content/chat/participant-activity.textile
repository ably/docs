...
Subscribe chat participant clients to the presence set to receive realtime updates on other participant's statuses and update the UI accordingly. For example, displaying online status icons.

When a chat participant calls the @enter@ method, realtime notifications are dispatched to all subscribers of the associated presence channel. This functionality enables subscribers to promptly update the visual online status of the user, ensuring that everyone in the channel is informed of the current online presence.
...
When a chat participant calls the @update@ method, realtime notifications are dispatched to all subscribers of the associated presence channel. This functionality enables subscribers to promptly update the status of the chat participant, ensuring that everyone in the channel is informed of the current statuses.
...
When a chat participant calls the @leave@ method, realtime notifications are dispatched to all subscribers for this associated presence channel. This functionality enables subscribers to promptly update the status of the chat participant as offline.
...
To avoid a potentially high volume of unnecessary messages, for example, having all messages, status updates, and typing indicators in one channel. You could have separate channels for specific purposes. An example of this is to have one channel for a specific chat ("general") while having another channel for the chat participants' status ("status-general") updates. Your application could then periodically retrieve the chat participants' online status updates from your backend (the exclusive subscriber). When issuing API keys to clients, assign the clients the following "capabilities":/auth/capabilities :
...
h2(#remain-present-for). Chat participant timeout
