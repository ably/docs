...
```[swift]
let presenceSubscription = await room.presence.subscribe(events: [.enter, .leave, .update])
...
blang[javascript,swift].
  ...
  ```[swift]
  // Subscribe to only 'enter' events:
  let presenceSubscription = await room.presence.subscribe(event: .enter)

  // Subscribe to 'update' and 'leave' events:
  let presenceSubscription = await room.presence.subscribe(events: [.leave, .update])
  ```
