...
* Send messages over Ably using the channel instances provided by the hooks.
* Get notifications of user presence updates on channels.
...
Ably >= 1.2.44 includes React hooks functionality out of the box. You don't need to install any additional packages.
...
Alternatively, you can create your options and pass them to the context provider:
...
```[react]
const { presenceData, updateStatus } = usePresence("your-channel-name", "initial status");
