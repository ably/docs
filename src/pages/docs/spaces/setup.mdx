...
Client-side applications must use [token authentication](/docs/platform/auth/token). API keys should never be exposed in client-side code because they don't expire and cannot be scoped to specific users.
</Aside>

Spaces requires an authenticated client with a `clientId` to identify users. The recommended approach is:

1. **Client-side apps** (browsers, mobile apps): Use [JWT authentication](/docs/platform/auth/token/jwt) with `authCallback` to fetch JWTs from your server
