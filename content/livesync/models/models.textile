...
Keeping your frontend applications up-to-date, a model is a single instance of a live, observable data model backed by your database. A model uses a "sync function":#sync to initialize with the latest data and a "merge function":#merge to update the it's state upon receiving change events from your backend database.
...
The sync function should call your backend endpoint to return the largest @sequenceID@ present in the outbox when reading the state:
