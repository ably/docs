...
Each batch publish request can contain a single @BatchPublishRequest@ object, or an array of @BatchPublishRequest@ objects. Each @BatchPublishRequest@ object contains a single channel name or an array of channel names in the @channels@ property and a single message or array of messages in the @messages@ property.

This means that in a single request N number of messages can be published to N number of channels.

*Note*: If the same channel name appears in multiple @BatchPublishRequest@ objects within a single request, it only counts as one channel towards the 100 channel limit per batch request.

*Note*: The total size of all messages in a @messages@ array of a @BatchPublishRequest@ object must be less than the "message limit":/general/limits#message-size for each channel. If the total message size exceeds the limit for a single channel then the messages can be split into multiple @BatchPublishRequest@ objects instead.

h4(#request-single). Single @BatchPublishRequest@ object

The following is an example of a single @BatchPublishRequest@ object publishing a single message to 2 channels:
...
h4(request-multiple). Multiple @BatchPublishRequest@ objects

The following is an example of an array of @BatchPublishRequest@ objects. The first publishes a single message to two channels and the second publishes two messages to a single channel:
...
The examples for each possible outcome will use the following @BatchPublishRequest@ object as the request data:
...
The batch "presence":/realtime/presence endpoint enables you to retrieve the presence state for multiple channels with a single request. The presence state contains details about the clients in the presence set, such as their @clientId@, member data and "presence action":/api/realtime-sdk/types#presence-action.
