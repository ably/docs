...
The @channel lifecycle API@ is a set of related features that enable an application to obtain information about the existence, and lifecycle, of "channels":/realtime/channels.

The primary feature is the channel lifecycle metachannel; this is a channel, in principle just like any other channel, except that it carries events with metadata, and exists in the metachannel namespace so it does not conflict with any application channels. "Messages":/realtime/messages in the lifecycle channel indicate activation and deactivation of channels (both globally and regionally) and, when enabled for the app, periodic "occupancy":#occupancy updates for active channels.

APIs are also provided in the "REST API":/rest-api to:

- "enumerate":#enumeration-rest the active channels in an app
- get the "status":#metadata-rest of a given channel
