...
<If lang="javascript">
<Aside data-type='note'>
Calling `channel.object.get()` implicitly attaches to the channel if not already attached and waits for the LiveObjects state to synchronize.
</Aside>
</If>

<If lang="javascript">
## Path-based Access <a id="path-based-access"/>

LiveObjects provides a [path-based API](/docs/liveobjects/concepts/path-object) for accessing and manipulating objects. Instead of working with explicit object instances, you work with paths that resolve to objects dynamically at runtime:
...
// Or use the at() method for deeply nested paths
...
This path-based approach makes your code more resilient to structural changes and simplifies common operations. See the [PathObject documentation](/docs/liveobjects/concepts/path-object) for more details.
</If>
...
<If lang="javascript">
With the path-based API, objects are automatically reachable when you assign them to a path. You don't need to worry about explicitly making objects reachable:

<Code>
```javascript
// The counter is automatically reachable when assigned
await myObject.set('visits', LiveCounter.create(0));

// The map and its nested structures are all automatically reachable
