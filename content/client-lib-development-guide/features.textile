...
** @(RSA8e)@ Method signature is @revokeToken(TokenRevocationRequest)@. Token revocation details
...
* @(TF3)@ @TokenRevocationRequest#targets@ is an array of @TokenRevocationTarget@s to revoke. For request to be valid, it must contain at least one target. Valid targets are
...
* @(TL2)@ @TokenRevocationTarget#key@ is a string containing the target key. Target key can be "clientId","revocationKey" or "channel".
