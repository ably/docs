...
Ably supports two types of authentication schemes, basic authentication using a private API key, which should not be used on user devices, and "token authentication":/core-features/authentication#token-authentication which can be used on your server to keep your private API key secure. Your server (or serverless function) can generate short-lived tokens and pass them to users when they need to communicate with Ably. These tokens can be limited to specific capabilities on specific channels. Make sure to follow the principle of least privilege, giving clients only the capabilities they need.  

Clients should be configured with an @authCallback@ or @authUrl@, which the Ably client SDK will use to generate and renew a token whenever needed: when a token doesn't currently exist or when it has expired.
