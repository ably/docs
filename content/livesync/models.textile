...
The @sequenceId@ is automatically maintained within the "outbox table":/livesync/outbox-node-tables#outbox-table. Your sync function should call a backend API that returns both the current state of the model and the largest sequenceId from the outbox table. These two values (the @sequenceId@ and model state) should be queried from the database within the same database transaction. The database transaction must use the @read committed@ transaction isolation level (or greater).
