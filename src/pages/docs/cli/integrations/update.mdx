...
<Code>
```shell
ably integrations update <integration-id> [flags]
```
</Code>
...
Update the [source type](/docs/platform/integrations) for the integration rule. Valid options are `channel.message`, `channel.presence`, `channel.lifecycle`, or `presence.message`.
