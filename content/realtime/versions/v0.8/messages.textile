...
A set of "channel options":/api/realtime-sdk/message#channel-options may also be passed to configure a channel for encryption. Find out more about "symmetric message encryption":/realtime/encryption.
...
A client can register for messages on a channel by implementing "MessageListener":/realtime/types#message-listener and calling the @subscribe(MessageListener listener)@ or @subscribe(String name, MessageListener listener)@ method. The listener is passed an array of one or more "@Message@":/realtime/types#message objects when messages are received.
</div><div lang="ruby">
...
A client can subscribe to all messages on a channel by passing a lambda expression to the @Subscribe@ method. The lambda is passed a "@Message@":/realtime/types#message object for each message are received.
</div>
...
View the "Realtime channel and message objects":/api/realtime-sdk/message for the associated Client Library SDK API reference.
