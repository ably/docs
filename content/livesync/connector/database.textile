...
The Database Connector automatically observes new records written to the outbox table and publishes them as messages over "channels":/channels.
...
The Database Connector will publish the change event, written to the outbox, to the channel in the exact order in which the underlying change to the data is committed to the database.
