...
In the above code when producing data, we're specifying how we'll include a key in the messages via the proprties @key.separator@ and @parse.key@. The separator is defined to be a colon (:), so the messages both have a key of @my_group@.
