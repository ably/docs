...
blang[kotlin].
  ...
  ```[kotlin]
  dependencies {
    implementation("com.ably.chat:chat-kotlin:0.1.0")
  }
  ```
...
```[javascript]
import { ChatClientOptions, LogLevel } from '@ably/chat';

const realtimeClient = new Ably.Realtime({ key: '{{API_KEY}}', clientId: '<clientId>'});
const chatClientOptions: ChatClientOptions = { logLevel: LogLevel.Error }
const chatClient = new ChatClient(realtimeClient, chatClientOptions);
```
...
val chatOptions = ChatClientOptions(
