---
title: Pubnub Protocol Adaptor Tutorial
section: tutorials
index: 15
---

The Pubnub protocol adaptor allows you to interact with Ably using the Pubnub protocol.  You can use this to gradually migrate across from Pubnub to Ably. Rather than having to rewrite all your apps to use Ably client libraries at once, you can use the protocol adaptor, then migrate across to Ably client libraries one by one at your leisure, to take advantage of "Ably features not supported by the Pubnub client libraries":https://www.ably.io/features, such as "the ability to use websockets":https://support.ably.io/solution/articles/3000044831-which-transports-are-supported%2D (Pubnub client libraries support long-polling only). Alternatively, you can use a Pubnub client library on some platform for which no native Ably client library is available yet, such as Arduinos.

Full usage notes for the adaptor are available at: "Using the Ably Pubnub Protocol Adaptor":https://support.ably.io/support/solutions/articles/3000055107-using-the-ably-pubnub-protocol-adaptor.
...
If we don't have an example in the language you'll be using, check the "Pubnub documentation for that library":https://pubnub.com/docs to find the equivalent options to the ones in the javascript example (for realtime libraries) or the ruby or nodejs example (for REST libraries) above.

Note that we do not yet support Pubnub v4 client libraries. (Also note that some non-v4 client libraries use the v4 protocol for some things, but usually have an option to disable it: for example, if using the Pubnub android java library 3.7.8 to 3.7.10, you can call the (undocumented) method "setV2(false)" on the pubnub client immediately after initializing it, or downgrade to 3.7.7).

h2.
  default: Step 3 - check you're not using any unsupported features

The Pubnub adaptor does not yet support PAM, wherenow, channel groups, backfill subscribes, or stream filters. If you use any of these, "let us know":https://www.ably.io/contact and we can advise on Ably-supported ways to achieve the same goal. (For example, instead of PAM we have our "token authentication system":https://www.ably.io/documentation/general/authentication).

Also note that since the adaptor has to translate every Pubnub operation to the equivalent Ably operation, some Pubnub operations can be quite expensive, in terms of the time it takes to do them and the number of API requests they count against your package quota. For example, if you do a 'global herenow' request (which returns every presence member in every active channel in your app), behind the scenes the adaptor will do a request for a list of active channels, followed by a presence request for each of those channels, for a total of n+1 requests for n active channels.

h2.
  default: Step 4 - interoperability between Pubnub and Ably client libraries

Behind the scenes, the adaptor uses the normal Ably service, so you can use Pubnub and Ably client libraries side by side, and they'll interoperate. For example, here's some code that subscribes to the same channel from both Pubnub and Ably client libraries, then publishes using both.
...
* Pubnub does not have a concept of a message name. So A publish using the Pubnub adaptor will leave the Ably message name blank, and message name is not visible to Pubnub adaptor subscribers. Pubnub message content is mapped to Ably message data (the payload).
...
* Pubnub considers the same @UUID@ to be the same (in terms of membership of the presence set); Ably considers the same @clientid@ on different connections to be different. (If using the adaptor to get the state for a @clientId@, if there are multiple members in the set with the same @clientId@, the adaptor will just pick one).
...
* In Pubnub's model, message continuity is available for '5-20 minutes' and up to 100 messages. To provide compatibility with this, the Pubnub adaptor will stay connected and attached to your channels for 5 minutes after the last subscribe long poll it receives. After 5 minutes, the adaptor will disconnect from Ably, and any subsequent subscribe poll will be treated as a new connection (and will get messages from that point in time onwards).
...
blang[javascript].
  The complete source code of a working example of Ably and Pubnub client libraries communicating with each other "is available on Github":https://github.com/ably/tutorials/commits/pubnub-adaptor-javascript.
  ...
  bc[sh]. git checkout pubnub-adaptor-javascript
...
blang[nodejs].
  The complete source code of a working example of Ably and Pubnub client libraries communicating with each other "is available on Github":https://github.com/ably/tutorials/commits/pubnub-adaptor-nodejs.
  ...
  bc[sh]. git checkout pubnub-adaptor-nodejs
...
blang[ruby].
  The complete source code of a working example of Ably and Pubnub client libraries communicating with each other "is available on Github":https://github.com/ably/tutorials/commits/pubnub-adaptor-ruby.
  ...
  bc[sh]. git checkout pubnub-adaptor-ruby
...
# Full usage notes for the Pubnub adaptor are available at: "Using the Ably Pubnub Protocol Adaptor":https://support.ably.io/support/solutions/articles/3000055086-using-the-ably-pusher-protocol-adaptor/.
