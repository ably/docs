...
- by := _value_ optionally specifies whether to return just channel names (@by=id@) or "ChannelDetails":/api/realtime-sdk/channel-metadata#channel-details (@by=value@)
...
Client libraries currently do not provide a dedicated API to enumerate channels, but make this available using the "request":/api/rest-sdk#request method. When using this, you can simply iterate through the "PaginatedResults":/api/rest-sdk/types#paginated-result to enumerate through the results.
