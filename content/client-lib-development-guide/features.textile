...
** @(RSA17e)@ If the @targets@ parameter is valid, then the function must create the request and execute it.
** @(RSA17f)@ Function returns a @TokenRevocationResponse@ object that contains the result of the request if the request is successful.
** @(RSA17g)@ Function responds with an appropriate error if the request fails either for the reasons of invalid request, or as a result of request failing.
