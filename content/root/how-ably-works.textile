...
All messages received by Ably are immediately stored in RAM in three separate physical locations for redudancy. They are then persisted as follows:
...
<a href="/assets/diagrams/global-routing.png" target="_blank">
  <img src="/assets/diagrams/global-routing.png" style="width: 100%" alt="Global routing diagram">
</a>
...
* Connection state recovery provides certainty and is thus binary in its behaviour - it either succeeds and all operations continue as if the connection was never disconnected, or it fails by detaching the channel or emitting an error on the channel so that the developer is made aware of the state loss and is able to respond accordingly
...
<a href="/assets/diagrams/redundancy.png" target="_blank">
  <img src="/assets/diagrams/redundancy.png" style="width: 100%" alt="Ably redundancy diagram">
</a>
...
- "Proactive health checked DNS":https://support.ably.io/support/solutions/articles/3000044636-routing-around-network-and-dns-issues := our DNS TTL is kept very low allowing us to route traffic away from unhealthy data centres in 60s once our monitoring systems detect an issue
...
- "Host fallback":https://support.ably.io/solution/articles/3000044636-routing-around-network-and-dns-issues#fallback := clients that are unable to connect to their closest data centre using the primary @ably.io@ domain will automatically fallback to an alternative data centre using the secondary domain
...
- "Data replicas":https://support.ably.io/support/solutions/articles/3000029531-100-uptime-guarantee := all data is stored in at least three data centres across at least two regions ensuring data availability through any imaginable failure
- "Multiple availability zones":https://support.ably.io/solution/articles/3000029525-where-are-ably-s-servers-located-around-the-world := in every region our our data is replicated between servers in at least two independent data centres ensuring outages in one data centre cannot cause data loss
- "Multiple regions":https://support.ably.io/solution/articles/3000029525-where-are-ably-s-servers-located-around-the-world := ensuring that data is always stored in at least two regions protects against complete region outage or network partioning
