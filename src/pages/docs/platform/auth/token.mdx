...
### Ably JWT (recommended) <a id="choosing-jwt"/>

**Best for:** Most applications, including web, mobile, IoT, and serverless.

Your server creates a JWT signed with your Ably API key secret. The client uses this JWT directly without contacting Ably first. Any JWT library can create these tokens—no Ably SDK required on the server.

**Benefits:**
- No Ably SDK required on the server—use any JWT library
- No client round-trip to Ably (client uses JWT directly)
- Channel-scoped user claims (`ably.channel.*` for trusted metadata)
- Per-connection publish rate limits (`ably.limits.publish.*`)
- Works with MQTT clients that need token authentication
- Ideal for serverless/edge functions (stateless token generation)

**Considerations:**
- Server clock must be accurate (use NTP)
- Token expiry handling is managed by Ably SDKs automatically when using `authUrl` or `authCallback`

### TokenRequest <a id="choosing-tokenrequest"/>

**Best for:** Edge cases where JWT is not suitable.

Your server creates a signed `TokenRequest` using the Ably SDK. The client exchanges this with Ably for an actual token.

**Choose TokenRequest only when:**
- Your capability list is very large, making a JWT token impractically large
- You need to keep capabilities or `clientId` confidential (JWT claims are readable by anyone with the token)

**Trade-offs:**
- Requires Ably SDK on your server
- Client makes a round-trip to Ably to exchange TokenRequest for token
- Cannot use channel-scoped claims or per-connection rate limits

### Embedded Token JWT <a id="choosing-embedded"/>

**Best for:** Deep integration with existing JWT-based authentication systems.

Wrap an Ably token inside your existing JWT structure using the `x-ably-token` header claim. This lets you combine your application's JWT claims with Ably authentication.

**Choose Embedded Token JWT when:**
- You have existing JWT infrastructure with custom claims you want to preserve
- You want a single JWT for both your app authentication and Ably
- You need to pass both Ably capabilities and app-specific claims together

**Trade-offs:**
- Most complex to implement
- Requires requesting an Ably token first, then embedding it
- Outer JWT expiry must not exceed inner token expiry
...
If you use TokenRequest or Embedded JWT, you will **not** have access to these features.
</Aside>

### JWT-only features <a id="jwt-only-features"/>

These capabilities are **only available with JWT authentication**:

- **Channel-scoped user claims** (`ably.channel.<channelName>.*`): Embed trusted metadata about users that other clients can read. Useful for roles, permissions, or user attributes scoped to specific channels.

- **Per-connection publish rate limits** (`ably.limits.publish.perAttachment.maxRate`): Restrict how many messages a specific connection can publish per second on specific channels or namespaces.

See [capabilities](/docs/platform/auth/capabilities) for details on implementing these features.

## Common architectural scenarios <a id="scenarios"/>
...
For provider-specific integration examples (Auth0, Firebase, Cognito), see [JWT integration](/docs/platform/auth/jwt-integration).
...
### Scenario 6: Rate-limited publishing <a id="scenario-ratelimit"/>
...
You can specify an authentication callback function when you create the Ably client. Inside `authCallback`, you make a network request to your server to obtain a JWT. **Returning a JWT string is recommended** as it provides the best combination of security and features.
...
realtime, err := ably.NewRealtime(
  ably.WithAuthCallback(func(ctx context.Context, params ably.TokenParams) (ably.Tokener, error) {
    // Fetch JWT from your auth server
    req, _ := http.NewRequestWithContext(ctx, "GET", "/api/ably-token", nil)
    ...
    // Return JWT string directly
    return ably.TokenString(string(jwt)), nil
  }),
)
```
...
final clientOptions = ably.ClientOptions(
  authCallback: (ably.TokenParams tokenParams) async {
    ...
    // Return JWT string directly
    return ably.TokenDetails.fromMap({'token': response.body});
  },
);
...
options.authCallback = new Auth.TokenCallback() {
    ...
    public Object getTokenRequest(Auth.TokenParams params) throws AblyException {
        try {
            // Fetch JWT from your auth server
            URL url = new URL("/api/ably-token");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestProperty("Authorization", "Bearer " + userAuthToken);
            ...
            throw AblyException.fromErrorInfo(
                new ErrorInfo("Auth failed", 40000, 401)
            );
        }
    }
};
...
options.authCallback = { tokenParams, callback in
    guard let url = URL(string: "/api/ably-token") else {
        callback(nil, NSError(domain: "AuthError", code: -1))
        return
    }
    ...
    URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        guard let data = data, let jwt = String(data: data, encoding: .utf8) else {
            callback(nil, NSError(domain: "AuthError", code: -1))
            return
        }
...
val options = ClientOptions().apply {
    authCallback = Auth.TokenCallback { params ->
        ...
        try {
            ...
            throw AblyException.fromErrorInfo(
                ErrorInfo("Auth failed: ${e.message}", 40000, 401)
            )
        } finally {
...
rest = AblyRest(auth_callback=get_ably_jwt)
```
...
The [`tokenParams`](/docs/api/realtime-sdk/authentication#token-params) argument in `authCallback` is available for convenience, allowing you to see the capabilities, `clientId`, and other details requested by the client. However, `tokenParams` should not be trusted or used to generate tokens on the server side. Your auth endpoint should authenticate clients separately using cookies, headers, or request body.
