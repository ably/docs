...
During large-scale attacks, AWS can leverage its global network infrastructure to absorb and diffuse attack traffic, preventing it from reaching Ably's resources. AWS Shield includes access to the AWS DDoS Response Team (DRT), providing expert assistance during attacks.
