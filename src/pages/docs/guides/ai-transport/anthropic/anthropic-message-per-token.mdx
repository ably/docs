...
Create a new file `publisher.mjs` (JavaScript) or `publisher.py` (Python) with the following contents:
...
def process_event(event):
    ...
    if event.type == 'message_start':
        ...
        # Publish start event
        channel.publish('start', extras={'headers': {'responseId': response_id}})
        ...
        # Publish stop event when response completes
        channel.publish('stop', extras={'headers': {'responseId': response_id}})
```
</Code>
...
Create a new file `subscriber.mjs` (JavaScript) or `subscriber.py` (Python) with the following contents:
