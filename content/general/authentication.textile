...
See "understanding capabilities and token security":#understanding-capabilities below.
...
h3(#understanding-capabilities). Capabilities and Token Security

"API keys":https://support.ably.io/solution/articles/3000030054-what-is-an-app-api-key, like tokens, have a set of capabilities assigned to them that specify which operations on what channels a client can perform operations such as subscribe or publish to a channel. However, unlike tokens, API keys are intended to be secret and not to be shared with remote clients, and tend to be long-lived. API keys and their capabilities are "configured using the dashboard":https://support.ably.io/support/solutions/articles/3000030502-setting-up-and-managing-api-keys, that is they cannot be added or removed programatically.

Tokens on the other hand are designed to be configured and issued programatically for the purpose of sharing with clients, primarily because:

* All tokens are short lived, typically for one hour, and as such, they significantly reduce the risks associated with them should they be compromised;
* Tokens can be revoked (details coming soon, "get in touch":https://www.ably.io/contact if you require this functionality now);
* A @clientId@ can be configured for the token thus "identifying the user":/realtime/authentication#identified-clients and ensuring all operations by that client are trusted to be that @clientId@ and restricted from emulating any other @clientId@;
* A set of capabilities (privileges) can be assigned to tokens that explicitly specify what operations a client can perform on which channels
...
<li>the wildcard <code>*</code> can be used for resource or operation fields</li>
</ul>
</notextile>
...
See "understanding capabilities and token security":#understanding-capabilities above to get a more thorough overview of how capabilities can be used to secure your application along with working examples.
