...
bq(definition#publish-data).
  ...
  go:       (c &ast;RestChannel) Publish(name string, data interface{}) (error "ErrorInfo":#error-info)
...
bq(definition#publish-msg-array).
  ...
  go:       (c &ast;RestChannel) PublishAll(messages []&ast;proto.Message) (error "ErrorInfo":#error-info)

Publish several messages on this channel. <span lang="jsall,java,objc,swift">A <span lang="default">callback</span><span lang="java">listener</span> may optionally be passed in to this call to be notified of success <span lang="default">or failure</span><span lang="ruby"></span> of the operation.</span> It is worth noting that idempotency is "not fully supported":https://support.ably.io/solution/articles/3000088357-how-can-i-publish-multiple-messages-in-a-single-rest-request-with-idempotency when sending serveral messages at once with this method.
...
blang[go].
  ...
  The function returns "@ErrorInfo@":#error-info if an error has occured, otherwise it returns @nil@.
