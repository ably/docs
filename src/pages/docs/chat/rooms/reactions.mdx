...
Use the `unsubscribe()` function returned in the `subscribe()` response to remove a room reaction listener<If lang="jetpack">. Jetpack Compose automatically handles lifecycle and cleanup when using `LaunchedEffect`</If>:
</If>
