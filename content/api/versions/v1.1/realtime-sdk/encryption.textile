...
You will rarely need to call this yourself, since the client library will handle it for you if you specify @cipher@ params when initializing a channel (as in the "getting started example":/realtime/encryption#getting-started)<span lang="jsall"> or when setting channel options with @channel.setOptions()@</span>.
