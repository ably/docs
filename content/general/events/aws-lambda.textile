...
minimize. View enveloped @presence@ payload example
  ```[json]
  {
    "source": "channel.message",
    "appId": "aBCdEf",
...
- source := the source for the event, namely "@channel.presence@" or "@channel.message@"
- appId := the Ably app this message came from
- channel := the Ably channel where the event occured
- site := the Ably datacenter which sent the message
- timestamp := a timestamp represented as milliseconds since the epoch for the presence event
...
The payload will contain the "data":/rest/messages#data of the @Message@ event.
...
The payload will contain the "data":/rest/presence#data of the @Presence@ event.
