...
*** @(RSA4a1)@ if the server responds with a token error (401 HTTP status code and an Ably error value @40140 <= code < 40150@), then the client library should indicate an error, not retry the request and in the case of the realtime library, transition the connection to the @FAILED@ state
*** @(RSA4a2)@ a message at @info@ log level should be logged indicating that no means has been provided to renew the supplied token, with a link to a "solution article":https://help.ably.io/a_suitable_article with configuration advice relating to token authentication. 
