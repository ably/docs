...
* Whilst the API we provide now is planned for our GA (general availability) release, *the API is subject to change*. However, we will endeavour to only make changes to the API where necessary before we transition the service to GA.
...
<%= partial 'general/push/_activate_subscribe_intro' %>
