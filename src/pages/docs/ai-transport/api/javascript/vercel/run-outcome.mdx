...
The helper applies three rules in order:
...
2. If `pipeResult.reason` is `'complete'`, await `finishReason` and return `'suspend'` for `'tool-calls'`. Any other Vercel finish reason returns `'complete'`.
3. If `finishReason` rejects, classify the rejection. Abort-shaped errors return `'cancelled'`. Anything else (for example `NoOutputGeneratedError`) returns `'error'`.
