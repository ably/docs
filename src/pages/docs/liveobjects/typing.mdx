...
The Java SDK has no user-supplied type parameters; static typing comes from a fixed typed hierarchy instead.

On the path layer, the `as*` casts (`asLiveMap()`, `asLiveCounter()`, `asString()`, `asNumber()`, `asBoolean()`, `asBinary()`, `asJsonObject()`, `asJsonArray()`) select a typed view of the same path without validating it and **never throw** — a wrong cast surfaces on use: reads return `null`/empty, and writes fail with an `AblyException` (error code `92007`). On the instance layer, the `as*` casts **throw** an `IllegalStateException` on mismatch — check `getType()` first when the type isn't known. The `ValueType` enum lists the nine value categories (`STRING`, `NUMBER`, `BOOLEAN`, `BINARY`, `JSON_OBJECT`, `JSON_ARRAY`, `LIVE_MAP`, `LIVE_COUNTER`, `UNKNOWN`).

<Code>
```java
LiveMapPathObject myObject = channel.object.get().join();

// Path layer: never-throwing casts, typed reads
...
// Instance layer: throwing casts - discriminate with getType() first
Instance instance = myObject.get("score").instance();
if (instance != null) {
...
Learn more about the typed hierarchy on the [PathObject](/docs/liveobjects/concepts/path-object#typing) and [Instance](/docs/liveobjects/concepts/instance#typing) concept pages.
</If>
