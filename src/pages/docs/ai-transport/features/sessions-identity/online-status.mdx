...
Both devices are now present in the session with the same `clientId` but different `connectionId` values. When either device leaves or disconnects, the other device remains in the presence set.
...
For example, a user client can explicitly leave presence when the leave a session:
