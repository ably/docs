...
| You have no server to control access | Yes | No  | If you do not have your own server to perform authentication and provide "tokens":#tokens to users, you'll need to use "Basic Authentication":#basic-authentication. |
| You require fine-grained access control | No | Yes | If you need to provide "privileges":#capabilities-explained on a user-by-user basis, you'd be better using "Token Authentication":#token-authentication. If you only need a few access control groups, "Basic Authentication":#basic-authentication is reasonable. |
