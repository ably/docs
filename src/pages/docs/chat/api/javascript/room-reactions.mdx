...
| `metadata` | *Optional* Metadata associated with the reaction. | [`Metadata`](#metadata) |
| `headers` | *Optional* Headers associated with the reaction. | [`Headers`](#headers) |

#### Headers

Headers enable attaching extra info to a reaction. Do not use headers for authoritative information. There is no server-side validation. When reading the headers, treat them like user input.

`Headers: Record<string, number | string | boolean | null | undefined>`

#### Metadata

Metadata enables attaching extra info to a reaction. Do not use metadata for authoritative information. There is no server-side validation. When reading the metadata, treat it like user input.
...
| `metadata` | *Optional* Metadata associated with the reaction. | [`RoomReactionMetadata`](#metadata) |
| `headers` | *Optional* Headers associated with the reaction. | [`RoomReactionHeaders`](#headers) |

### Returns

Returns a [`Subscription`](#subscription) object that allows you to control the subscription.
...
| Property | Description | Type |
| -------- | ----------- | ---- |
| `unsubscribe` | A method to unsubscribe from room reaction events. | `() => Promise<void>` |
