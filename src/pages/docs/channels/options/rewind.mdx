...
<Aside data-type="note">
If you have enabled the [persist last message](/docs/storage-history/storage#persist-last-message) rule on a channel, you can attach with `rewind=1` to retrieve the last message. The last message is stored for the duration of the configured [retention period](/docs/storage-history/storage#persist-last-message). Persist last message doesn't apply to presence or object messages.
</Aside>
