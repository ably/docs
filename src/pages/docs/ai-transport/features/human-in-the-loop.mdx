...
On the client, detect pending approval requests and present them to the user. A statically-declared tool arrives as a `tool-${name}` part, not `dynamic-tool`, so match both representations and read the name with the AI SDK's `getToolName`:
