...
{/*
Swift test harness:
func identifyingUsersAndAgents7(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    weatherChannel.subscribe(attachCallback: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
    otherChannel.subscribe(attachCallback: { error in
        if let error {
            continuation.resume(throwing: error)
        } else {
            continuation.resume()
...
{/*
Swift test harness:
func identifyingUsersAndAgents12(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
    ...
    channel.subscribe(attachCallback: { error in
        ...
        if message.clientId == "weather-agent" {
            if let data = message.data {
                print("Weather agent response: \(data)")
            }
        }
    })
}
...
{/*
Swift test harness:
func identifyingUsersAndAgents13(ably: ARTRealtime) async throws {
    // insert example code here
}
*/}
```swift
