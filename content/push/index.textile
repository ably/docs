...
** The process begins with your app (client) or server, which publishes a push notification -- this could be any backend service that needs to send notifications to users' devices.
** Your app or server sends the notification data to Ably.
* Handle push notification:
** After receiving the push notification from your app or server, Ably processes and packages the notification with any additional data payload. This payload may include data such as message content, metadata, or any other relevant information needed for the notification.
