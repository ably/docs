...
redirect_from:
  ...
  - /docs/platform/account/app/stats
...
### Account statistics via API <a id="account-api"/>

You can retrieve account statistics using the [Control API](/docs/platform/account/control-api) by making a call to the `accounts` endpoint using your `accountId` and `accessToken`. For example, to retrieve the last two minutes of account statistics:
...
### Account statistics from the dashboard <a id="account-dashboard"/>
...
### App statistics via Control API <a id="app-control"/>

You can retrieve app statistics using the [Control API](/docs/platform/account/control-api) by making a call to the `apps` endpoint using your `APP_ID` and `ACCESS_TOKEN`:
...
The following is an example of querying app-level statistics using the Ably SDK <If lang="javascript,nodejs">[`stats` method](/docs/pub-sub/api/javascript/rest/rest-client#stats)</If><If lang="csharp,flutter,go,java,objc,php,python,ruby,swift">[`stats` method](/docs/api/rest-sdk/statistics)</If>.
...
### Subscribe to app statistics <a id="app-meta"/>
...
### App statistics from the dashboard <a id="app-dashboard"/>
...
## Statistics response <a id="payload"/>
...
## Statistics entry type <a id="metrics"/>
...
### Outbound messages <a id="outbound"/>

Outbound message metrics include those messages that are sent outbound from Ably to either clients that subscribe or request messages (such as history requests), to [integrations](/docs/platform/integrations), or to [push notification](/docs/push) targets.
...
### Persisted messages <a id="persisted"/>
...
### Account-only metrics <a id="account-only"/>
