...
Rewind by a period of time can only get messages at most 2 minutes old, as it is not possible to retrieve messages from "persisted history":/storage-history/storage using a time-based parameter. If you specify @rewind=3m@, you will only get the last 2 minutes of history upon attaching.

Rewind by a certain number of messages is restricted by the channel's persistence period. If "persisted history":/storage-history/storage, isn't enabled for the channel then this will be 2 minutes. If you have persistence enabled, this will be 24 hours for free accounts, and 72 hours for paid accounts.
