...
intro: "Combine automated AI moderation with human oversight to handle complex content decisions that require human judgment. All chat messages are sent to Hive's moderation dashboard where your team can review and take action on flagged content."
...
5. Set the "Endpoint URL" field to `https://main.realtime.ably.net/chat/v4/moderation/hive/delete?clientId=<base64Id>` (if you have a [dedicated cluster](https://ably.com/docs/platform/account/enterprise-customization#setting-up-a-custom-environment) the domain should be your custom REST domain)
   * The `clientId` will show up as the one responsible for the message deletion (`message.version.clientId`) when it is published.
6. Under "Customize API Request" add the `Authorization` header with the value `Basic <your Base64 encoded Ably API key>`. If you wish to use a long-lived JWT, set the value to `Bearer <your JWT>`.
