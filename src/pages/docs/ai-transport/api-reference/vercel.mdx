...
intro: "Vercel AI SDK integration reference: UIMessageCodec, ChatTransport adapter, and pre-bound transport factories."
---
...
React hook that reads a `ChatTransport` and its underlying `ClientTransport` from the nearest `ChatTransportProvider`. The hook is a context reader; the transports are created by the provider.
...
The returned `ChatTransportHandle` exposes `{ transport, chatTransport, transportError, chatTransportError }`. Construction of the transport happens once inside `ChatTransportProvider`. Configure it through the provider's props (`channelName`, `api`, `clientId`, and the rest of `VercelClientTransportOptions`).
