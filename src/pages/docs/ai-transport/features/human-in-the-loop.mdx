...
The pattern builds on [tool calling](/docs/ai-transport/features/tool-calling). The agent defines a tool that requires human approval. When the LLM invokes that tool, the run suspends with a pending tool call. The client presents the approval request to the user. When the user approves or rejects, the client publishes a `tool-approval-response` input, which resumes the run.

The flow:

1. The agent streams a response that includes a tool call requiring approval.
2. The run suspends. The pending tool call is published to the channel.
3. Any connected client renders the pending approval.
4. The user approves or rejects. The client publishes a `tool-approval-response` input addressed to the pending message.
5. The run resumes. The agent receives the approval result and proceeds.
...
When the LLM decides an action needs approval, it invokes `requestApproval`. The run suspends with the tool call pending.
...
if (pendingNode && pendingApproval) {
  ...
  const runId = view.runOf(pendingNode.id).runId;
