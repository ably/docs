...
* If you wish to migrate from PubNub/Pusher to Ably, you can use the relevant "protocol adapter":/adapters to "easily":https://knowledge.ably.com/can-you-help-ease-the-pain-of-migration-from-another-data-stream-network-1 switch to Ably right away and gradually migrate to using native Ably client SDKs.
...
You should almost always instantiate a single Ably client library instance per device, web page or server. Don't forget that the Client Library SDK can "multiplex":https://knowledge.ably.com/do-you-support-multiplexing-and-channel-groups-1 many channels over a single connection. This maximizes throughput, minimizes bandwidth consumption and reduces power & battery drain.
