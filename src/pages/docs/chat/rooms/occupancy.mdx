...
Use the `unsubscribe()` function returned in the `subscribe()` response to remove a room occupancy listener<If lang="jetpack">. Jetpack Compose automatically handles lifecycle and cleanup when using `collectAsOccupancy()`</If>:
</If>
