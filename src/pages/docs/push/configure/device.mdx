...
Activating a device for push notifications is typically done directly on the device itself. Use the <If lang="javascript,nodejs">[`push.activate()`](/docs/pub-sub/api/javascript/realtime/push#activate)</If><If lang="csharp,flutter,java,kotlin,objc,swift">[`push.activate()`](/docs/api/realtime-sdk/push#activate)</If> method to activate push notifications from a device. The following steps describe the direct activation process:
...
Note that once activated, the device remains registered even after the application is closed until the <If lang="javascript,nodejs">[`push.deactivate()`](/docs/pub-sub/api/javascript/realtime/push#deactivate)</If><If lang="csharp,flutter,java,kotlin,objc,swift">[`push.deactivate()`](/docs/api/realtime-sdk/push#deactivate)</If> method is called. Calling activate again has no effect if the device is already activated.
...
const ably = getAblyRealtime({ plugins: { Push } });
...
#### Update the token when the platform rotates it <a id="update-token"/>

The push platform can rotate the device's token at any time. On React Native your application receives the rotation, so forward the new token to Ably with [`push.updateToken()`](/docs/pub-sub/api/javascript/realtime/push) to keep the device registration deliverable:

<Code>
```realtime_javascript
messaging().onTokenRefresh(async (token) => {
  await ably.push.updateToken({ transportType: 'fcm', token });
});
```
</Code>
...
* Use the Ably [dashboard](https://ably.com/accounts) or <If lang="javascript,nodejs">[API](/docs/pub-sub/api/javascript/realtime/push-admin)</If><If lang="csharp,flutter,java,kotlin,objc,swift">[API](/docs/api/realtime-sdk/push-admin)</If> to send a test push notification to a registered device.
...
<Aside data-type='note'>
Utilize the <If lang="javascript,nodejs">[push notifications admin API](/docs/pub-sub/api/javascript/realtime/push-admin)</If><If lang="csharp,flutter,java,kotlin,objc,swift">[push notifications admin API](/docs/api/realtime-sdk/push-admin)</If> on your server to register devices.
</Aside>
...
#### Test your push notification activation <a id="test-activation-net"/>
...
When the <If lang="javascript,nodejs">[`push.activate()`](/docs/pub-sub/api/javascript/realtime/push#activate) and [`push.deactivate()`](/docs/pub-sub/api/javascript/realtime/push#deactivate)</If><If lang="csharp,flutter,java,kotlin,objc,swift">[`push.activate()`](/docs/api/realtime-sdk/push#activate) and [`push.deactivate()`](/docs/api/realtime-sdk/push#deactivate)</If> methods are called, the device registers with FCM or APNs. In addition, whenever there is an update to the push token (registration token for FCM or device token for APNs), the Ably SDK attempts to update this token on Ably's servers.
