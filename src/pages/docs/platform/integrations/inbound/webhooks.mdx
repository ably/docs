...
Webhook messages are subject to rate [limits](/docs/platform/pricing/limits#integration), similar to REST publishes.
</Aside>
...
A successful request returns a `201` status. Failures return with an [`ErrorInfo`](/docs/pub-sub/api/javascript/rest/rest-client#errorinfo) response.
