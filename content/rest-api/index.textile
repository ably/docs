...
Batch mode enables you to submit multiple REST API requests as an array of requests in a single API call. The REST API then processes all requests in the array in parallel.

h3(#batch-publish). Batch publish

h6. POST rest.ably.io/messages

You can publish messages to channels in parallel, by passing either a single @BatchSpec@ object or an array of @BatchSpec@ objects in the request body.
