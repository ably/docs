...
9. Create the @/auth@ route in this server. This route is responsible for creating a JWT and sending this token back to the client. Normally we would validate the User before issuing the JWT token but for simplicity we'll just use their input as the `clientId`.
