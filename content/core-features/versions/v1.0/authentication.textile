...
Every application has one or more private API keys that you set up and "configure via the dashboard":https://knowledge.ably.com/setting-up-and-managing-api-keys-1. These keys can be used to authenticate with Ably. It is recommended that private API keys are never shared with untrusted parties, and as such, should only be used by your trusted secure servers when authenticating with Ably.
...
In order to define which capabilities an API key will have, simply select the appropriate capability boxes when "setting up your API key":https://knowledge.ably.com/setting-up-and-managing-api-keys-1. The structure to define channels and namespaces is structurally the same as defined in the above "wildcards":#wildcards section.
...
The Ably JWT must be signed with the secret part of your "Ably API key":https://knowledge.ably.com/what-is-an-app-api-key-1, using one of the following signature algorithms (as defined in "JWA":https://tools.ietf.org/html/rfc7518):
