...
| method | Either `get`, `post`, `put`, `patch` or `delete` | string |
| path | The path to query | string |
| params | (optional) any querystring parameters needed | `Dictionary<string, string>` |
| body | (optional; for `post`, `put` and `patch` methods) the body of the request, as anything that can be serialized into JSON, as a JToken. | `Token` |
| headers | (optional) any headers needed. If provided, these will be mixed in with the default library headers | `Dictionary<string, string>` |
...
<If lang="javascript,nodejs,go,java,objc,php,python,ruby,swift">

#### <If lang="javascript,nodejs,go,objc,php,swift">Properties</If><If lang="java">Members</If><If lang="ruby">Attributes</If><If lang="python">Keyword arguments</If>
...
| <If lang="flutter,java,objc,php,python,ruby,swift">push</If><If lang="csharp,go">Push</If> | Detailed stats on push notifications, see [our Push documentation](/push) for more details | `PushStats` |
...
##### <If lang="javascript,nodejs,java,objc,swift,ruby,php,python,go">first</If><If lang="csharp">First</If>
