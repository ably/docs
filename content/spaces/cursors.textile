...
window.addEventListener('mousemove', ({ clientX, clientY }) => {
  await space.cursors.set({ position: { x: clientX, y: clientY }, data: { color: 'red' } });
});
```
...
The @outboundBatchInterval@ is the interval at which a batch of cursor positions are published, in milliseconds, for each client. This is multiplied by the number of members in a space, minus 1.
...
// Create a new space
const space = await spaces.get('slideshowPresentation');
...
window.addEventListener('mousemove', ({ clientX, clientY }) => {
  await space.cursors.set({ position: { x: clientX, y: clientY } });
});
```
