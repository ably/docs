...
"Batch mode":/rest/batch enables you to submit multiple API requests in a single API call. The API processes all the requests in parallel.
...
The maximum size of each request body must be less than 2MiB and the total size of all messages in a @messages@ array must be less than the "message size limit":https://faqs.ably.com/what-is-the-maximum-message-size (64kiB for paid accounts, 16kiB for free accounts).
...
When the request is successful, the API returns an array of objects with @channel@ and @messageId@ properties. For example, the following is returned if the sample request above completes successfully:
...
When unsuccessful or partially unsuccessful, the response includes an "ErrorInfo":/rest/types#error-info object.
...
When successful, the operation returns an array of objects with @channel@ and @presence@ properties.
...
Each object in the @presence@ array for each channel describes the presence state of one of that channel's members:
