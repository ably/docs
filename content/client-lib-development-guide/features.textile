...
** Presence state changes to @ENTER@ when acknowledgement of the @ENTER@ is received from Ably
...
** Additionally a test should exist that registers a number of members each with different @clientId@s on a presence channel, then udpates their data, and then a @Presence#get@ should be used to verify that all members are present with the correct updated data as expected
