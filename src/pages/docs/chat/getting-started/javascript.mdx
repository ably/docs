...
Create an `index.ts` file in your project and add the following function to instantiate a realtime client with the Pub/Sub SDK and then pass that client into the Chat SDK constructor. Provide an API key and a [`clientId`](/docs/platform/auth/identified-clients) to identify the client. In production, use [token authentication](/docs/platform/auth/token) so that your API keys are not exposed publicly.
...
* Understand [token authentication](/docs/platform/auth/token?lang=javascript) before going to production.
