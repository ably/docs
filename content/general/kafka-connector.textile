...
jump_to:
  Help with:
    - Data from Ably to Kafka#data-ably-kafka
    - How does Ably Kafka Connector work?#how
    - Mapping options#mapping-options
    - Publishing messages with a schema#schema
...
The Ably Kafka Connector provides a ready-made integration between "Kafka":https://kafka.apache.org/ and Ably. It allows for realtime event distribution from Kafka to web, mobile, and IoT clients, over Ably's feature-rich, multi-protocol pub/sub channels. 

**Note:** This connector is verified by Confluent as "Gold":https://www.confluent.io/hub/ably/kafka-connect-ably, following the guidelines set forth by Confluent's "Verified Integrations Program":https://www.confluent.io/verified-integrations-program/.

You can use the Ably Kafka Connector to send data from one or more "Kafka topics":https://developer.confluent.io/learn-kafka/apache-kafka/topics/ into Ably "channels":/realtime/channels. The following diagram illustrates a typical deployment:

<a href="/images/diagrams/ably-kafka-connector-overview.png" target="_blank">
  <img src="/images/diagrams/ably-kafka-connector-overview.png" style="width: 100%"
  title="Click to enlarge" alt="Ably Kafka Connector overview">
</a>

h2(#data-ably-kafka). Data from Ably to Kafka

If you need to send data from Ably to Kafka, use a "Kafka rule":/general/firehose/kafka-rule.
...
It is important to note that you can also use static mappings in conjunction with pattern-based mapping. For example, you could define the channel as @channel_#{topic}@, but @message.name@ as @single_message@. In this instance, the channel will be mapped by pattern and the message name will be mapped statically, as shown in the following:
...
View the "Ably Kafka Connector tutorial":/tutorials/ably-kafka-connector for an example of how to use the Ably Kafka Connector.

h2(#see-also). See also

* "Firehose overview":/general/firehose
* "Kafka rules":/general/firehose/kafka-rule
* "Ably Kafka Connector tutorial":/tutorials/ably-kafka-connector
* "Ably Kafka Connector GitHub repo":https://github.com/ably/kafka-connect-ably
